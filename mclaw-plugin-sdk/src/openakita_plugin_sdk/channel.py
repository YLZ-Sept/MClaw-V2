...
This mirrors ``mclaw.channels.base.ChannelAdapter`` so plugin authors can
...
class ChannelAdapter(ABC):
    ...
    async def send_text(self, chat_id: str, text: str, **kwargs: Any) -> str:
        ...
        """
        try:
            from mclaw.channels.types import OutgoingMessage

            return await self.send_message(OutgoingMessage.text(chat_id, text, **kwargs))
        except ImportError:
            raise NotImplementedError(
                "Default send_text requires the full Mclaw runtime. "
