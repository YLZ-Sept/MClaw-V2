...
Mclaw 提供 **14 个钩子**，覆盖插件的完整生命周期：初始化、关闭、消息收发、会话管理、记忆检索、工具调用（含前后拦截）、Prompt 构建、定时任务、配置变更和错误处理。

Mclaw provides **14 hooks** covering the full plugin lifecycle: init, shutdown, message I/O, sessions, memory retrieval, tool execution (with before/after interception), prompt assembly, scheduled tasks, config changes, and error handling.
...
```python
from mclaw_plugin_sdk.hooks import HOOK_SIGNATURES
