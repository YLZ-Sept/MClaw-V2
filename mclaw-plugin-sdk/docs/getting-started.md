...
从零开始创建一个 Mclaw 插件：安装 SDK、脚手架生成、编写代码、测试验证、部署运行。

Build your first Mclaw plugin from scratch: install the SDK, scaffold, code, test, and deploy.
...
- Python **3.11+**
- Mclaw **≥ 1.5.0**（仅运行时需要，开发和测试不需要完整安装）
- Mclaw **≥ 1.5.0** (only needed at runtime; development and testing work without the full app)

## 安装 SDK / Install the SDK

```bash
# 从源码安装（开发模式）/ Install from source (editable mode)
pip install -e ./mclaw-plugin-sdk
...
```python
import mclaw_plugin_sdk
print(mclaw_plugin_sdk.__version__)  # 0.1.0
```
...
使用 `MockPluginAPI` 在没有 Mclaw 运行时的情况下测试插件：

Test your plugin without a running Mclaw instance using `MockPluginAPI`:
...
2. Start Mclaw (CLI or API server mode)
...
> `project_root` 是 Mclaw 的工作目录（通常是运行 `mclaw` 命令的目录）。
>
> `project_root` is the Mclaw working directory (typically where `mclaw` is run).
