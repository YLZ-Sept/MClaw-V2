...
Mclaw 可以发现并安装来自其他 AI 工具生态的插件包：**OpenClaw**、**Claude**、**Cursor** 和 **Codex**。

Mclaw can discover and install plugin bundles from four AI tool ecosystems: **OpenClaw**, **Claude**, **Cursor**, and **Codex**.

---

## 工作原理 / How It Works

`BundleMapper`（位于 `mclaw.plugins.bundles`）检测插件包的格式，并将其映射为 Mclaw 原生的 `plugin.json` 清单。

The `BundleMapper` in `mclaw.plugins.bundles` detects the format of a plugin package and maps it to Mclaw's native `plugin.json` manifest.
...
|----------------|-------------------|
| `SKILL.md` 文件 | 直接加载为 Mclaw Skill（格式原生兼容）/ Loaded directly as skills |
...
mclaw plugin install https://github.com/user/openclaw-obsidian-plugin

# 从本地路径安装 / From local path
mclaw plugin install /path/to/claude-skill-bundle
...
要让你的 Mclaw 插件被其他生态发现：

To make your Mclaw plugin discoverable by other ecosystems:
...
- Python 原生能力（通道、记忆、LLM 提供商）需要 Mclaw 原生 `plugin.json` / Python-native capabilities require native `plugin.json`
- 钩子和 API 路由是 Mclaw 特有的，不映射到其他生态 / Hooks and API routes are Mclaw-specific
- 权限模型是 Mclaw 特有的 / The permission model is Mclaw-specific

---

## 相关文档 / Related

- [plugin-json.md](plugin-json.md) — Mclaw 原生清单格式 / Native manifest format
- [getting-started.md](getting-started.md) — 创建 Mclaw 原生插件 / Create native plugins
