...
```python
from mclaw_plugin_sdk import PluginAPI
```
...
```python
from mclaw_plugin_sdk.tools import tool_definition
...
```python
from mclaw_plugin_sdk.channel import ChannelAdapter
...
```python
from mclaw_plugin_sdk.llm import LLMProvider, ProviderRegistry, ProviderRegistryInfo
...
```python
from mclaw_plugin_sdk import LLMSelectionPolicy
...
- `inherit`: 使用 Mclaw 的正常路由和故障切换；不能指定 `endpoint`。
- `prefer`: 优先使用指定端点，不健康或能力不匹配时允许安全降级。
- `require`: 只使用指定端点；能力不匹配时直接报错。

- `inherit`: Use Mclaw's normal routing and failover; no `endpoint` may be supplied.
...
前端 bridge 的具体协议（`bridge:theme-change` / `bridge:locale-change` / `window.Mclaw.api(...)` 等）见参考插件 `plugins/tongyi-image/ui/dist/index.html` 与 `plugins/seedance-video/ui/dist/index.html`，以及归档参考副本 `plugins-archive/_shared/web-uikit/`。

For the bridge protocol (`bridge:theme-change`, `bridge:locale-change`,
`window.Mclaw.api(...)`, etc.) refer to the two first-class plugins
...
|-------------|--------------|----------------|
| `PluginBase` | `mclaw_plugin_sdk` | 插件入口基类 / Plugin entry class |
| `PluginManifest` | `mclaw_plugin_sdk` | 清单数据类 / Manifest dataclass |
| `tool_definition()` | `mclaw_plugin_sdk.tools` | 工具定义构建器 / Tool definition builder |
| `ToolHandler` | `mclaw_plugin_sdk.tools` | 工具处理基类 / Tool handler base |
| `ChannelAdapter` | `mclaw_plugin_sdk.channel` | 通道适配器基类 / Channel adapter base |
| `LLMProvider` | `mclaw_plugin_sdk.llm` | LLM 提供商基类 / LLM provider base |
| `MemoryBackendProtocol` | `mclaw_plugin_sdk.protocols` | 记忆后端协议 / Memory backend protocol |
| `RetrievalSource` | `mclaw_plugin_sdk.protocols` | 检索源协议 / Retrieval source protocol |
| `SearchBackend` | `mclaw_plugin_sdk.protocols` | 搜索后端协议 / Search backend protocol |
| `MockPluginAPI` | `mclaw_plugin_sdk.testing` | 测试用模拟 / Test mock |
| `HOOK_NAMES` | `mclaw_plugin_sdk.hooks` | 有效钩子名集合 / Valid hook names |
| `HOOK_SIGNATURES` | `mclaw_plugin_sdk.hooks` | 钩子回调签名参考 / Hook callback signature reference |
| `UnifiedMessage` | `mclaw_plugin_sdk.types` | 统一消息类型 / Unified message type |
