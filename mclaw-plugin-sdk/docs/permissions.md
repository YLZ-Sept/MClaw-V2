...
Mclaw 插件使用**三级权限模型**控制对系统资源的访问。插件必须在 `plugin.json` 的 `permissions` 数组中声明所需权限。

Mclaw plugins use a **three-tier permission model** to control access to system resources. Plugins must declare required permissions in the `permissions` array of `plugin.json`.
