...
Build plugins for [Mclaw](https://github.com/mclaw/mclaw) without installing the full runtime.

## Install

```bash
pip install mclaw-plugin-sdk
```

For development from source (from the [main repository](https://github.com/mclaw/mclaw)):

```bash
git clone https://github.com/mclaw/mclaw.git
pip install -e mclaw/mclaw-plugin-sdk
```
...
```bash
python -m mclaw_plugin_sdk.scaffold --id my-tool --type tool --dir ./plugins
```
...
```python
from mclaw_plugin_sdk import PluginBase, PluginAPI
from mclaw_plugin_sdk.decorators import tool, hook, auto_register
...
bundle (theme/locale bridge, `oa-*` CSS, `MclawIcons`, `MclawI18n`,
`MclawMarkdown`) is preserved as a copy-paste reference at
...
```python
from mclaw_plugin_sdk.testing import MockPluginAPI, assert_plugin_loads
...
```python
from mclaw_plugin_sdk import PluginBase, PluginAPI, tool_definition
from mclaw_plugin_sdk.decorators import tool, hook, auto_register
from mclaw_plugin_sdk.scaffold import scaffold_plugin
from mclaw_plugin_sdk.testing import MockPluginAPI, assert_plugin_loads
from mclaw_plugin_sdk.hooks import HOOK_NAMES, HOOK_SIGNATURES
from mclaw_plugin_sdk.channel import ChannelAdapter, ChannelPluginMixin
from mclaw_plugin_sdk.llm import LLMProvider, ProviderRegistry
from mclaw_plugin_sdk.protocols import MemoryBackendProtocol, RetrievalSource, SearchBackend
from mclaw_plugin_sdk.config import config_schema, config_property
from mclaw_plugin_sdk.types import UnifiedMessage, OutgoingMessage, ToolCall
```
