...
- **IM 收口**：lifespan IM drain 各阶段并行 + per-stage `wait_for`；为 wework_ws / qqbot
  WS adapter 增加 force-close 路径（2s 硬截止）；新增 `mclaw stop` CLI（含绕过 dev 代理）。
...
- **背景**：Mclaw 在 system prompt 里要求 LLM 给涉及外部事实的句段标注
  ...
  "我已成功保存"）—— Mclaw 此前缺少对应的 belt，verify=completed 路
  径上的这类幻觉可能直接到用户面前。
- **前端（A 方案）—— badge 渲染**：
...
- **后端（C 方案）—— 工具失败一致性 belt**：
  - `src/mclaw/core/reasoning_engine.py` 新增 `_check_tool_failure_
...
- **bug**：`PolicyConfigV2.expand_placeholders` 旧实现用 `if p == "${CWD}":`
  ...
  无任何 warn。
- **修复**：新建 `src/mclaw/core/policy_v2/path_placeholders.py` 单一职责模块，
...
- **UI 不再卡转圈**：`_assets/{bootstrap,styles,icons,i18n}` 4 件齐全，
  `apiCall` 优先 `Mclaw.api`（5s 超时）回退 `directFetch`，render 后
  `Mclaw?.ready()` 通知宿主收起 spinner。
- **UI 风格对齐**：`Ic` 双路径优先 `MclawIcons`、`PromptGuide` / 列表
...
- **删除** `mclaw_plugin_sdk.contrib` 整个子包（28 个模块 + `tts/` / `asr/`
...
#### 12 个 0-消费者模块 → `mclaw-plugin-sdk/staging/contrib/`
...
- **`web/` 整体下沉** —— `git mv mclaw-plugin-sdk/src/mclaw_plugin_sdk/web/`
  ...
  注释也一并清理。
- **host 不再 mount `/api/plugins/_sdk/*`** —— `src/mclaw/api/server.py`
  原 L400–L442 的 `_sdk_web_dir` 解析 + `StaticFiles` mount 段全部删除,
  改为一段 NOTE 说明历史与新规则。
- **`plugin_deps.py` 路由退役** —— 因 `contrib/dep_gate.py` 已下沉到 staging,
  对应的 host 端 REST 路由 `src/mclaw/api/routes/plugin_deps.py` 与
  `server.py` 中的 import / `include_router` 一并删除。dep-gate 特性彻底退役;
  `seedance-video` 中的 `MclawDepGate` 调用因有 defensive check,在 JS
  缺失时安全 no-op,无前端报错。
- **插件 UI 自包含化** —— `plugins/tongyi-image/ui/dist/_assets/` inline
  ...
  实际 11×MclawI18n / 6×MclawIcons / 22×oa-section 用法)。
...
`mclaw_plugin_sdk.skill_loader`(SKILL.md frontmatter 解析,314 行)
没有任何 plugins/ 真消费者(host 自有 `src/mclaw/skills/loader.py`
完全独立),与 contrib 同期"扩展定位"残留。

- `git mv mclaw-plugin-sdk/src/mclaw_plugin_sdk/skill_loader.py
  mclaw-plugin-sdk/staging/skill_loader.py`
- `git mv mclaw-plugin-sdk/tests/test_skill_loader.py
  mclaw-plugin-sdk/staging/tests/test_skill_loader.py`
- 新增 `mclaw-plugin-sdk/staging/README.md` 说明 staging/ 整体性质,
  与原 `staging/contrib/README.md` 一致:**0 消费者、不打包、不进 CI**。

#### 杂项

- 删除老 wheel `mclaw-plugin-sdk/dist/mclaw_plugin_sdk-0.3.0-py3-none-any.whl`
  (130KB,内含早期 contrib + web/,误用风险)。
- `pyproject.toml` description 从 `"... and Plugin 2.0 UI support"` 回退为
  `"SDK for building Mclaw plugins — protocols, base classes, testing helpers"`。
...
- 删除 `mclaw-plugin-sdk/docs/contrib.md` / `ai-media-scaffold-guide.md` /
  `dependency-gate.md` / `plugin-ui.md` / `plugin-i18n-and-uikit-migration.md`
  (对应模块/特性都已下沉或退役)。
- `mclaw-plugin-sdk/docs/README.md` / `getting-started.md` /
  `api-reference.md` 改写为「最小壳 + UI 自包含」口径,删除 Plugin 2.0 UI
  专节链接。
- `mclaw-plugin-sdk/README.md` / 顶层 `__init__.py` 删除 contrib 段落,
  明确 SDK 只做"最小壳子"。
...
- `src/mclaw/plugins/compat.py` — `PLUGIN_API_VERSION = "1.0.0"` →
...
- 新增 `mclaw_plugin_sdk.contrib.tts`（`qwen3-tts-flash` /
  `cosyvoice` / `openai-tts` / `edge-tts` + `select_provider("auto", ...)`
  自动凭证降级到 `edge` / `stub-silent`）。
- 新增 `mclaw_plugin_sdk.contrib.asr`（DashScope `paraformer-v2` /
...
- **`seedance-video` 缺失 `prompt_optimizer.py` 导致
  ...
  `mclaw_plugin_sdk.contrib.prompt_optimizer.PromptOptimizer` 是另一
...
### Added — SDK `mclaw-plugin-sdk/contrib/` 6 件套（Sprint 8）
...
- `mclaw-plugin-sdk/docs/contrib.md` — `cost_tracker` / `checkpoint` 标 🅿️ waiting-for-consumer
...
- Security policy
- **Unified LLM Client Architecture** (`src/mclaw/llm/`)
...
- **Message Converters** (`src/mclaw/llm/converters/`)
...
[Unreleased]: https://github.com/mclaw/mclaw/compare/v1.2.0...HEAD
[1.2.0]: https://github.com/mclaw/mclaw/compare/v1.1.0...v1.2.0
[1.1.0]: https://github.com/mclaw/mclaw/compare/v1.0.2...v1.1.0
[0.5.9]: https://github.com/mclaw/mclaw/compare/v0.5.0...v0.5.9
[0.5.0]: https://github.com/mclaw/mclaw/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/mclaw/mclaw/compare/v0.3.0...v0.4.0
[0.3.0]: https://github.com/mclaw/mclaw/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/mclaw/mclaw/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/mclaw/mclaw/releases/tag/v0.1.0
