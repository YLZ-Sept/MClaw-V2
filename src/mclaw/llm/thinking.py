...
def normalize_thinking_depth(depth: object) -> str | None:
    """Normalize Mclaw's user-facing thinking depth aliases."""
...
def minimax_thinking_depth(depth: object) -> str | None:
    """Map Mclaw thinking depth to MiniMax's documented low/medium/high enum."""
    normalized = normalize_thinking_depth(depth)
    if not normalized:
        return None
    if normalized == "max":
...
def reasoning_effort_for_depth(
    *,
    ...
    """Map Mclaw thinking depth to a provider-safe reasoning_effort."""
    normalized = normalize_thinking_depth(depth)
    if not normalized:
        return None
