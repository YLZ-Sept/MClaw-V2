"""Public agent runtime APIs for Mclaw."""
...
# imported before ``mclaw.agent``. See the notes beside ``__getattr__`` below.
if TYPE_CHECKING:
...
#
# Importing ``mclaw.core._reasoning_runtime`` directly before
# ``mclaw.agent`` enters ``sys.modules`` would otherwise trigger:
#
#   _reasoning_runtime -> ``from mclaw.agent.errors import UserCancelledError``
#     -> runs THIS ``mclaw/agent/__init__`` for the first time
#     -> eager ``from .reasoning import Checkpoint, ...``
#     -> agent.reasoning ``from mclaw.core._reasoning_runtime import Checkpoint``
...
def __getattr__(name: str):  # PEP 562 module-level lazy attribute access
    ...
    if name in _LAZY_CONFIRMATION_EXPORTS:
        from mclaw.core import confirmation_state as _confirmation_state
