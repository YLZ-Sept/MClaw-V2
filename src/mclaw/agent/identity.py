...
* detect upstream upgrades to the ``.example`` shipped templates and
  ...
  CLI / API to confirm;
Compiled prompt assembly belongs to ``mclaw.prompt``. This class owns only
...
def _apply_agent_name_placeholder(text: str, agent_voice: str | None = None) -> str:
    ...
    if not resolved:
        resolved = getattr(settings, "agent_name", "") or "Mclaw"
...
def _resolve_bundled_identity_template(rel_name: str) -> Path | None:
    ...
    1. Repo checkout: walk up from this file's directory looking for a sibling
       ``identity/`` (next to ``pyproject.toml`` or ``src/mclaw``). Used
       during ``pip install -e .`` and ``python -m mclaw`` from source.
    2. Wheel / Tauri install: look inside the installed ``mclaw`` package
...
Mclaw 是运行平台或上游项目名称；当前 Agent 的自称以身份文件为准，\
不要因为平台名称把自己介绍为 Mclaw。
    ...
    def update_memory(self, section: str, content: str) -> bool:
        ...
        try:
            ...
            if new_memory != memory:
                from mclaw.memory.types import MEMORY_MD_MAX_CHARS, truncate_memory_md
