...
* When ``include_chain=True`` (the default), rows are written via
  :mod:`mclaw.core.policy_v2.audit_chain.ChainedJsonlWriter`
  so each row gains ``prev_hash`` + ``row_hash`` and post-hoc
  edits become detectable. The async batch writer in
  :mod:`mclaw.core.policy_v2.audit_writer` is preferred when
...
``from mclaw.core.policy_v2.*`` imports here are deliberate.
"""
...
class AuditLogger:
    ...
    def log_event(self, event_type: str, data: dict[str, Any]) -> None:
        ...
        if self._include_chain:
            ...
            try:
                from mclaw.core.policy_v2.audit_chain import get_writer
