...
- ``requires.mclaw``  — minimum system version  (``>=1.27.0``)
...
def check_compatibility(manifest: PluginManifest) -> CompatResult:
    ...
    _check_mclaw(manifest.id, requires.get("mclaw", ""), result)
...
def _check_mclaw(plugin_id: str, spec: str, result: CompatResult) -> None:
    if not spec:
        return
    if not spec.startswith(">="):
        result.warnings.append(f"Unrecognised mclaw spec '{spec}' (expected >=X.Y.Z)")
        return

    min_ver = _parse_version(spec[2:])
    if min_ver is None:
        result.warnings.append(f"Cannot parse mclaw version in '{spec}'")
        return

    current = _get_system_version()
    if current < min_ver:
        msg = (
            f"Plugin '{plugin_id}' requires mclaw {spec}, "
...
def _check_sdk(plugin_id: str, spec: str, result: CompatResult) -> None:
    ...
    try:
        from mclaw_plugin_sdk.version import SDK_VERSION
    ...
    if sdk is None:
        result.warnings.append(
            f"Plugin '{plugin_id}' needs mclaw-plugin-sdk {spec} but it is not "
            f'installed. Fix it with `pip install "mclaw[plugins]"` '
            f"(or `pip install -e ./mclaw-plugin-sdk` in monorepo dev). "
            f"Plugins that import mclaw_plugin_sdk classes will fail to "
            f"load until this is resolved."
        )
    elif sdk < req:
        result.warnings.append(
            f"Plugin '{plugin_id}' recommends mclaw-plugin-sdk {spec}, "
            f"installed is {'.'.join(str(x) for x in sdk)}. "
            f"Upgrade with `pip install -U mclaw-plugin-sdk`."
        )
