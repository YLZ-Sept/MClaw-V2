"""Monorepo / bundled-tree fallback loader for ``mclaw-plugin-sdk``.

Why this exists
---------------
Plugin entry modules import ``mclaw-plugin-sdk`` directly (for
...
plugin fails with::
    ...
    ModuleNotFoundError: No module named 'mclaw-plugin-sdk'
...
``mclaw-plugin-sdk/src/mclaw_plugin_sdk`` source tree and prepends
...
- **No imports of mclaw.*** — plugins/manager imports this at boot, so
...
_SDK_PACKAGE_NAME = "mclaw-plugin-sdk"
_SDK_SOURCE_DIRNAME = "mclaw-plugin-sdk"
_INJECTED_PATH: Path | None = None


def ensure_plugin_sdk_on_path() -> Path | None:
    """Make ``mclaw-plugin-sdk`` importable, returning the package path.
    ...
    3. Walk parent directories of this file looking for
       ``<repo>/mclaw-plugin-sdk/src/mclaw_plugin_sdk/__init__.py``.
    ...
    for parent in here.parents:
        ...
        if candidate_pkg.is_file():
            ...
            logger.info(
                "mclaw-plugin-sdk not installed; injected monorepo source from %s",
                candidate_src,
            )
            return _INJECTED_PATH
