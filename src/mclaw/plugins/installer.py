...
def _pip_subprocess_env(python_executable: str) -> dict[str, str]:
    ...
    """
    from mclaw.runtime_manager import apply_runtime_pip_environment
...
def _validate_tool_classes_completeness(
    ...
    Args:
        manifest: A :class:`~mclaw.plugins.manifest.PluginManifest`
...
def _download_to_file(url: str, dest: Path) -> None:
    req = Request(url, headers={"User-Agent": "Mclaw-PluginInstaller/1.0"})
    try:
...
def _resolve_pip_runner() -> tuple[str, list[str]]:
    ...
    ``mclaw-server.exe`` which intercepts Click args and refuses
...
def deps_appear_installed(plugin_dir: Path, manifest_requires: dict) -> bool:
    ...
    try:
        for child in deps_dir.iterdir():
            ...
            # ``pip --target`` writes e.g. ``mclaw-plugin-sdk-0.7.0.dist-info``.
...
def _finalize_install(plugin_dir: Path, *, remove_on_failure: bool = True) -> str:
    ...
    if not install_pip_deps(plugin_dir, manifest.requires):
        ...
        raise PluginInstallError(
            ...
            "Mclaw does not migrate plugin dependency directories in place; "
            "the isolated plugin directory was removed and reinstall/rebuild is required. "
            "See mclaw.plugins.installer logs for the pip error excerpt."
        )
    return manifest.id
...
def install_from_git(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="mclaw-git-") as tmp:
        tmp_path = Path(tmp)
...
def install_from_url(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="mclaw-plugin-") as tmp:
        tmp_path = Path(tmp)
