...
the path actually loaded by :class:`mclaw.plugins.manager.PluginManager`).
This module provides:

* :func:`compute_drift` -- pure-function, per-file diff used by both the
  startup warning and the ``mclaw plugins reseed`` CLI.
    ...
    *,
    ...
    for idx, entry in enumerate(source_newer):
        ...
        logger.warning(
            ...
            "stale code. Run `mclaw plugins reseed --apply` to sync.",
            runtime_path,
            source_path,
            delta,
        )
    if len(source_newer) > _MAX_WARN_DETAILS:
        logger.warning(
            ...
            "run `mclaw plugins reseed` for the full diff.",
    ...
    if source_only:
        ...
        for pid, n in sorted(per_plugin.items()):
            logger.info(
                ...
                "seeded to %s/. Run `mclaw plugins reseed --apply` to seed.",
                pid,
                n,
                source_root.name,
                runtime_root.name,
