...
class PluginManager:
    ...
    def __init__(
        ...
        # Make ``mclaw-plugin-sdk`` importable for plugin code before
    ...
    @staticmethod
    def _check_mclaw_version(manifest: PluginManifest) -> bool:
        ...
        for e in result.errors:
            if record_health_event(
                ...
                e,
                severity="error",
                suggestion="插件与当前 Mclaw/API 版本不兼容，请升级插件或禁用它。",
            ):
                logger.error(e)
        if not result.ok:
            if record_health_event(
        ...
        for plugin_dir, manifest in sorted_plugins:
            ...
            try:
                ...
                if (
                    ...
                    and getattr(e, "name", "") == "mclaw-plugin-sdk"
                ):
                    msg = (
                        "mclaw-plugin-sdk is not installed. "
                        'Run `pip install "mclaw[plugins]"` (production) or '
                        "`pip install -e ./mclaw-plugin-sdk` (monorepo dev), "
    ...
    def _format_missing_module_error(
        ...
        module_dir = Path.home() / ".mclaw" / "modules" / manifest.id / "site-packages"
        pip_specs: list[str] = []
        try:
            ...
            try:
                ...
                if pip_specs and not deps_appear_installed(plugin_dir, manifest.requires):
                    logger.warning(
                        ...
                        "the wheels under ~/.mclaw/modules/%s/site-packages/.",
                        manifest.id,
                        pip_specs,
                        plugin_dir / "deps",
                        manifest.id,
    ...
    def _load_python_plugin(
        ...
        ``runtime_env.inject_module_paths`` takes for ``~/.mclaw/modules``.
        ...
        module_name = f"mclaw_plugin_{manifest.id.replace('-', '_')}"
        ...
        # which is not a pattern Mclaw plugins use.
    ...
    def _maybe_warn_on_source_drift(self) -> None:
        ...
        :func:`mclaw.plugins.reseed.warn_on_drift` for the details.
        """
        try:
