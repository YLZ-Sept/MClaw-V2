...
from mclaw.llm.types import Message, Tool
...
def _sdk_symbol(name: str, fallback: Any) -> Any:
    ...
    try:
        from mclaw_plugin_sdk import llm as sdk_llm
...
class PluginLLMFacade:
    ...
    def _brain(self) -> Any:
        ...
        if brain is None:
            raise _unavailable_error("Mclaw host Brain is not available")
        return brain
    ...
    def _client(self) -> Any:
        ...
        if client is None or not callable(getattr(client, "chat", None)):
            raise _unavailable_error("Mclaw host LLM client is not available")
        return client
    ...
    def list_models(
        ...
        *,
        ...
        if not callable(list_models):
            raise _unavailable_error("Mclaw host cannot list configured models")
    ...
    async def complete(
        ...
        *,
        ...
        if endpoint_name and endpoint_name not in {item.endpoint for item in self.list_models()}:
            raise _selection_error(f"Endpoint {endpoint_name!r} is not configured in Mclaw")
