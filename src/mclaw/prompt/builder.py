...
def _load_prompt_asset(relative_path: str) -> str:
    """Load a required prompt bundled under ``mclaw/prompts``."""
    parts = PurePosixPath(relative_path).parts
    if not parts or any(part in {"", ".", ".."} for part in parts):
        raise ValueError(f"Invalid prompt asset path: {relative_path!r}")

    try:
        prompt_path = resources.files("mclaw").joinpath("prompts", *parts)
...
def build_system_prompt(
    ...
    if prompt_mode in (PromptMode.FULL, PromptMode.MINIMAL) and session_context:
        try:
            from mclaw.agent.working_facts import format_working_facts
...
_DEFAULT_AGENT_VOICE = "Mclaw"
_DEFAULT_BASE_PROMPT_SELF_INTRO = "你是 Mclaw，一个帮助用户完成各类任务的 AI 助手。"
...
def _build_identity_section(
    ...
    "Mclaw" 这一个自称上。空字符串时回退到 ``_DEFAULT_AGENT_VOICE``。
    """
    parts = []
    ...
    if resolved_voice != _DEFAULT_AGENT_VOICE:
        parts.append(
            f"当前 Agent 的自称是「{resolved_voice}」。Mclaw 仅指运行平台或上游"
...
- `run_shell`、`pip install`、打开带窗口的程序、浏览器自动化等：**全部发生在当前 Mclaw 进程所在的主机及其图形会话/无头环境中**。
- **默认不等于**用户发消息时所用的设备：IM/手机、另一台电脑、飞书/钉钉客户端所在环境与此**不是同一执行域**；图形窗口**不会**自动出现在用户屏幕上，软件也**不会**自动装到用户个人电脑上。
- 若用户要的是「在我这台电脑上看到窗口 / 本机安装 / 游戏内 overlay」等**用户侧可观测效果**：须通过 **可交付产物**（如脚本、`deliver_artifacts`）、**用户在本机可复制执行的命令/步骤**，或说明需要 **本地运行的 Mclaw / 远程桌面到同一台机器** 等产品能力；**禁止**仅因宿主侧命令退出码为 0 就声称用户已在其设备上看到效果。
...
def _build_arch_section(
    ...
    if is_sub_agent:
        return (
            f"## 系统概况\n\n"
            f"你是 Mclaw 多 Agent 系统中的**子 Agent**{model_part}。\n"
    ...
    lines = ["## 系统概况\n"]
    lines.append(f"你运行在 Mclaw 多 Agent 系统中{model_part}。核心架构：")
...
def _detect_deploy_mode() -> str:
    ...
    try:
        dist = importlib.metadata.distribution("mclaw")
    ...
    # 检查是否通过 pip 安装
    try:
        importlib.metadata.version("mclaw")
        return "pip install"
    except Exception:
        pass
...
def _build_memory_section(
    ...
    from mclaw.memory.types import MEMORY_MD_MAX_CHARS as _MD_MAX
...
def _get_core_memory(memory_manager: Optional["MemoryManager"], max_chars: int = 600) -> str:
    ...
    """
    from mclaw.memory.types import truncate_memory_md
