...
_COMPILE_PROMPTS: dict[str, dict] = {
    ...
    "agent_behavior": {
        ...
        "user": (
            "将以下 AGENT.md 编译为不超过 450 tokens 的行为增量。只保留 Mclaw "
...
_OWNED_SECTION_MARKERS: dict[str, tuple[tuple[str, ...], ...]] = {
    ...
    # Mclaw-specific traits that are not already enforced by the platform.
    "agent_behavior": (
...
_STATIC_FALLBACKS: dict[str, str] = {
    "identity_core": """\
## 身份核心
- 你是 {{agent_name}}，由 Mclaw 项目驱动的全能自进化 AI 助手。
