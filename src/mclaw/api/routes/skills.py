...
async def _load_external_skills_for_organize() -> tuple[list, set[str] | None]:
    ...
    from mclaw.skills.allowlist_io import read_allowlist
    from mclaw.skills.categories import CategoryRegistry
    from mclaw.skills.category_store import CategoryStore
...
async def _auto_translate_new_skills(request: Request, install_url: str) -> None:
    ...
    try:
        ...
        from mclaw.skills.i18n import auto_translate_skill
...
async def _build_skills_list_response(request: Request) -> dict:
    ...
    for skill in all_skills:
        ...
        runtime_state = {}
        try:
            from mclaw.skills.runtime_registry import read_skill_runtime_registry
...
async def install_skill(request: Request):
    """安装技能（远程模式替代 Tauri mclaw_install_skill 命令）。
    ...
    """
    from mclaw.skills.allowlist_io import upsert_skill_ids
    ...
    from mclaw.setup_center.bridge import SkillInstallError
    from mclaw.setup_center.bridge import install_skill as _install_skill
    ...
    try:
        from mclaw.setup_center.bridge import _resolve_skills_dir
        ...
        if candidates:
            from mclaw.skills.parser import SkillParser
...
async def uninstall_skill(request: Request):
    ...
    """
    from mclaw.skills.allowlist_io import remove_skill_ids
    ...
    try:
        from mclaw.setup_center.bridge import uninstall_skill as _uninstall_skill
...
async def update_skill_content(skill_name: str, request: Request):
    ...
    """
    from mclaw.skills.parser import skill_parser
    ...
    try:
        from mclaw.skills.usage_events import get_skill_usage_log
...
async def search_marketplace(q: str = "agent"):
    ...
    from mclaw.llm.providers.proxy_utils import (
        get_httpx_transport,
        get_proxy_config,
    )
...
try:
    from mclaw.skills.events import register_on_change
