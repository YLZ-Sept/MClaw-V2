...
def validate_identity_file(name: str, content: str) -> dict[str, list[str]]:
    ...
    if name == "POLICIES.yaml":
        ...
        from mclaw.memory.types import MEMORY_MD_MAX_CHARS
...
async def compile_identity(request: Request, mode: str = "rules"):
    ...
    if mode == "llm":
        ...
        if brain:
            from mclaw.prompt.compiler import PromptCompiler
    ...
    from mclaw.prompt.compiler import get_compiled_content
...
async def compile_status():
    ...
    from mclaw.prompt.compiler import check_compiled_outdated, get_compiled_content
