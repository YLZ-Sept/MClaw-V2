...
def mark_memory_repair_completed_restart_required() -> None:
    ...
    :class:`mclaw.storage.degraded.DegradedRegistry` so the unified
    ...
    try:
        from mclaw.storage.degraded import registry as _registry
...
async def health(request: Request):
    ...
    from mclaw import __git_hash__, get_version_string
    ...
    from mclaw.storage.degraded import registry as _degraded_registry
...
# is alive. That's useful for "is Mclaw on at all?" but it tells external
...
async def _check_policy_engine() -> dict[str, Any] | None:
    ...
    try:
        from mclaw.core.policy_v2.global_engine import get_engine_v2
...
async def logs_health_summary():
    ...
    from mclaw.core.log_health import get_log_health_registry
...
async def deprecated_redirect_stats() -> dict[str, Any]:
    ...
    ``src/mclaw/api/routes/_orgs_v2_deprecated_redirects.py`` shim can
    ...
    """
    from mclaw.api.routes._orgs_v2_deprecated_redirects import (
...
async def clear_session_caches_endpoint(request: Request, conversation_id: str | None = None):
    ...
    summaries.
    """
    from mclaw.core.session_caches import clear_session_caches
...
async def domain_rules(conversation_id: str = ""):
    ...
    from mclaw.agent.domain_allowlist import get_domain_allowlist
...
def _probe_command_version(command: str, *args: str) -> dict[str, str | bool]:
    ...
    from mclaw.runtime_manager import resolve_toolchain_command
...
async def diagnostics():
    ...
    from mclaw import __version__ as backend_version
    from mclaw.runtime_manager import (
    ...
    try:
        from mclaw.setup_center import bridge  # noqa: F401
        ...
        checks.append(
            {
                ...
                "status": "pass",
                "code": "CORE_OK",
                "evidence": [f"mclaw {backend_version}"],
                "autoFix": False,
                "fixHint": None,
            }
        )
    ...
    for mod_name in ("mclaw", "pydantic", "pydantic_core", "certifi"):
        try:
...
async def health_loop(request: Request):
    ...
    from mclaw.core.engine_bridge import is_dual_loop
