...
from mclaw.core.confirmation_state import ConfirmationDecision, get_confirmation_store
from mclaw.core.context_stats import (
    context_snapshot_from_dict,
    get_context_snapshot,
    merge_context_snapshot_into_usage,
)
from mclaw.core.engine_bridge import engine_stream, is_dual_loop, to_engine
...
def _chat_endpoint_names() -> set[str]:
    ...
    """
    try:
        from mclaw.api.routes.config import _get_endpoint_manager
            ...
            if session_manager and conversation_id:
                try:
                    ...
                    if session:
                        ...
                        try:
                            from mclaw.core.feature_flags import (
                                is_enabled as _ff_enabled,
                            )
                            from mclaw.core.risk_intent import (
...
def _cleanup_chat_runtime_state(request: Request, conversation_id: str) -> None:
    ...
    try:
        ...
        try:
            from mclaw.main import _orchestrator as global_orchestrator
...
def _extract_source_used(event: dict) -> dict | None:
    """Extract Mclaw source provenance from tool results.
...
def _strip_org_structure_marker(event: dict) -> dict:
    """Hide the Mclaw org marker from frontend tool-result rendering."""
...
def _resolve_agent(agent: object):
    ...
    from mclaw.agent.core import Agent
...
def _resolve_profile(agent_profile_id: str | None):
    ...
    from mclaw.agents.profile import AgentProfile, get_profile_store
...
async def _stream_chat(
    ...
    from mclaw.agent.sse_replay import format_sse_frame
    from mclaw.agent.sse_replay import (
    ...
    try:
        ...
        from mclaw.core.policy_v2 import DeferredApprovalRequired
        ...
        if _agent_task and not _agent_task.done() and not _bg_save_scheduled:
            ...
            try:
                ...
                pass
            try:
                import mclaw.main as _main_mod
...
def _enrich_org_content_with_attachments(content: str, attachments: list | None) -> str:
    ...
    from mclaw.channels.gateway import MessageGateway
    ...
    for att in attachments:
        ...
        if not local_path:
            ...
            if url:
                try:
                    from mclaw.api.routes.upload import resolve_upload_path
...
async def _stream_org_command_chat(
    ...
    *,
    ...
    try:
        ...
        from mclaw.orgs.command_models import (
            OrgCommandError,
            OrgCommandRequest,
            OrgCommandSource,
            OrgCommandSurface,
...
async def chat(request: Request, body: ChatRequest):
    ...
    (canonical definitions in mclaw.events.StreamEventType):
    ...
    # before falling back to 409.  Header X-Mclaw-DoubleTexting can
    ...
    _dt_header = request.headers.get("x-mclaw-doubletexting")
    ...
    # 数字员工：LLM 意图分析路由到最合适的 Agent（异步）
    _de_id = getattr(body, "digital_employee_id", None)
    logger.info(f"[Chat API] digital_employee_id={_de_id!r} agent_profile_id={body.agent_profile_id!r}")
    if _de_id and not body.agent_profile_id:
        from mclaw.digital_employee.manager import get_digital_employee_manager
        try:
            dem = get_digital_employee_manager()
            routed_id, reason = await dem.route_async(_de_id, body.message or "")
            logger.info(f"[Chat API] 路由结果: routed_id={routed_id!r} reason={reason}")
            if routed_id:
...
async def chat_resume(
    ...
    * Does NOT modify the SSE session except by reading its current_seq.
    ...
    """
    from mclaw.agent.sse_replay import format_sse_frame
    from mclaw.agent.sse_replay import get_registry as _get_sse_registry
