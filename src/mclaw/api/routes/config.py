...
from mclaw.api.runtime_response import runtime_operation_response
from mclaw.utils.env_config import parse_env_content
...
def _endpoints_config_path() -> Path:
    ...
    """
    try:
        from mclaw.llm.config import get_default_config_path
...
class SecurityConfirmRequest(BaseModel):
    ...
    def normalized_decision(self) -> str:
        from mclaw.core.security_confirm_channel import require_security_confirm_decision
...
def _normalize_security_profile(profile: str) -> str:
    ...
    from mclaw.core.policy_v2.defaults import FACTORY_DEFAULT_PROFILE
...
def _write_profile_event(profile: str, *, previous: str | None = None) -> None:
    ...
    """
    try:
        from mclaw.agent.audit import AuditLogger
...
def _apply_security_profile_defaults(sec: dict[str, Any], profile: str) -> None:
    ...
    """
    from mclaw.core.policy_v2.defaults import PROFILE_BUNDLES, profile_bundle
...
def _runtime_env_key_map() -> dict[str, str]:
    ...
    from mclaw.config import _PERSISTABLE_KEYS
...
def _sync_runtime_agent_settings(request: Request, changed_fields: set[str]) -> str | None:
    ...
    try:
        from mclaw.agent.persona import apply_persona_runtime
        from mclaw.config import settings
...
async def classify_config(body: ConfigClassifyRequest):
    ...
    from mclaw.config_lifecycle import classify_config_changes, effective_config_values
...
async def write_env(body: EnvUpdateRequest, request: Request):
    ...
    from mclaw.config import runtime_state, settings
    ...
    from mclaw.utils.env_config import commit_env_config
    ...
    from mclaw.config_lifecycle import classify_config_changes
    from mclaw.runtime_config_coordinator import (
        RuntimeApplyResult,
...
def _get_endpoint_manager():
    ...
    from mclaw.llm.endpoint_manager import EndpointManager
...
async def save_endpoint(body: SaveEndpointRequest, request: Request):
    ...
    """
    from mclaw.llm.endpoint_manager import ConflictError
    ...
    try:
        from mclaw.llm.endpoint_validation import (
...
def _context_length_payload(endpoint: dict) -> dict[str, Any]:
    from mclaw.config import settings
    from mclaw.llm.types import DEFAULT_CONTEXT_WINDOW
...
def _trigger_reload(request: Request) -> dict[str, Any]:
    ...
    from mclaw.runtime_config_coordinator import get_runtime_config_coordinator
...
async def restart_service(request: Request):
    ...
    """
    from mclaw import config as cfg
    from mclaw.runtime_config_coordinator import get_runtime_config_coordinator
...
async def read_skill_external_allowlist():
    ...
    """
    from mclaw.agent.security_actions import list_skill_external_allowlist
...
async def write_skills_config(body: SkillsWriteRequest, request: Request):
    ...
    """
    from mclaw.agent.security_actions import set_skill_external_allowlist
...
def _hot_patch_agent_tools(request: Request, *, enable: bool) -> None:
    ...
    if agent is None:
        return
    try:
        from mclaw.tools.definitions.agent import AGENT_TOOLS
        from mclaw.tools.definitions.org_setup import ORG_SETUP_TOOLS
        from mclaw.tools.handlers.agent import create_handler as create_agent_handler
        from mclaw.tools.handlers.org_setup import create_handler as create_org_setup_handler
...
async def write_agent_mode(body: AgentModeRequest, request: Request):
    ...
    if not old:
        try:
            from mclaw.main import _init_orchestrator

            await _init_orchestrator()
            from mclaw.main import _orchestrator
        ...
        try:
            from mclaw.agents.presets import ensure_presets_on_mode_enable
...
async def write_tool_loading(body: ToolLoadingRequest, request: Request):
    """更新工具常驻加载配置。立即生效并持久化。"""
    from mclaw.config import runtime_state
...
async def list_providers_api():
    ...
    """
    try:
        from mclaw.llm.registries import list_providers
...
async def list_models_api(body: ListModelsRequest):
    """拉取 LLM 端点的模型列表（远程模式替代 Tauri mclaw_list_models 命令）。

    直接复用 bridge.list_models 的逻辑，在后端进程内异步调用，无需 subprocess。
    """
    try:
        from mclaw.setup_center.bridge import (
        ...
        if "errno 2" in raw or "no such file" in raw:
            ...
            try:
                from mclaw.llm.providers.proxy_utils import format_proxy_hint
...
async def preview_security_config(body: dict | None = None):
    ...
    from mclaw.core.policy_v2 import (
        PolicyContext,
        SessionRole,
        ToolCallEvent,
    )
    from mclaw.core.policy_v2.enums import ConfirmationMode
    from mclaw.core.policy_v2.global_engine import make_preview_engine

    proposed_security = (body or {}).get("security") if isinstance(body, dict) else None

    try:
        if proposed_security is not None:
            from mclaw.core.policy_v2.loader import load_policies_from_dict
...
async def read_security_approval_matrix():
    from mclaw.core.policy_v2 import ApprovalClass, ConfirmationMode, SessionRole
    from mclaw.core.policy_v2.matrix import lookup as lookup_matrix
...
async def read_security_commands():
    ...
    """
    from mclaw.core.policy_v2.defaults import default_blocked_commands
...
async def read_permission_mode():
    ...
    """
    try:
        from mclaw.core.policy_v2 import read_permission_mode_label
...
async def read_security_audit():
    ...
    """
    try:
        from mclaw.agent.audit import get_audit_logger
        from mclaw.core.policy_v2.audit_chain import (
...
async def list_checkpoints():
    ...
    try:
        from mclaw.core.checkpoint import get_checkpoint_manager
...
async def security_confirm(body: SecurityConfirmRequest, request: Request):
    ...
    try:
        from mclaw.core.security_confirmation import resolve_security_confirmation
...
async def security_confirm_batch(body: SecurityConfirmBatchRequest):
    ...
    try:
        from mclaw.agent.ui_confirm_bus import get_ui_confirm_bus
...
async def reset_death_switch():
    ...
    try:
        from mclaw.agent.security_actions import (
...
async def write_security_confirmation(body: _ConfirmationUpdate, request: Request):
    ...
    if body.default_on_timeout is not None:
        try:
            from mclaw.core.security_confirm_channel import (
...
async def read_self_protection():
    ...
    try:
        from mclaw.core.policy_v2 import get_death_switch_tracker
...
async def read_user_allowlist():
    ...
    try:
        from mclaw.agent.security_actions import list_security_allowlist
...
async def add_allowlist_entry(body: dict):
    ...
    try:
        from mclaw.agent.security_actions import add_security_allowlist_entry
...
async def delete_allowlist_entry(entry_type: str, index: int):
    ...
    try:
        from mclaw.agent.security_actions import remove_security_allowlist_entry
