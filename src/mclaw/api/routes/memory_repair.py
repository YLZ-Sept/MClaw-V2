...
from mclaw.memory.storage import MemoryStorage
from mclaw.memory.telemetry import emit_memory_health_event
...
def _verify_desktop_token(request: Request) -> None:
    ...
    if not expected:
        ...
        return
    supplied = request.headers.get("X-Mclaw-Desktop-Token", "")
...
def _recover_method() -> str:
    ...
    if os.environ.get("LOCALAPPDATA"):
        candidates.append(
            Path(os.environ["LOCALAPPDATA"]) / "Programs" / "Mclaw" / "sqlite3.exe"
        )
    if os.environ.get("PROGRAMFILES"):
        candidates.append(Path(os.environ["PROGRAMFILES"]) / "Mclaw" / "sqlite3.exe")
...
def _sqlite3_executable() -> str | None:
    ...
    for env_name in ("LOCALAPPDATA", "PROGRAMFILES"):
        ...
        if not root:
            continue
        candidate = Path(root) / "Mclaw" / "sqlite3.exe"
        if candidate.exists():
...
async def repair_status(request: Request):
    ...
    return {
        ...
        "backups": _list_candidates("mclaw.db.bak.*"),
        "snapshots": _list_candidates("mclaw.db.snapshot.*"),
...
async def run_recover(payload: RepairTokenRequest, request: Request):
    def work():
        db = _db_path()
        stamp = time.strftime("%Y%m%d_%H%M%S")
        recovered = _memory_dir() / f"mclaw.recovered.{stamp}.db"
        sqlite3_exe = _sqlite3_executable()
        if sqlite3_exe:
            sql_path = _memory_dir() / f"mclaw.recovered.{stamp}.sql"
...
# convention: both ``X-Mclaw-Desktop-Token`` (Tauri-injected) AND a
...
async def _quiesce_subsystem(request: Request, subsystem: str) -> None:
    ...
    try:
        if subsystem == "token_tracking":
            from mclaw.core.token_tracking import shutdown_token_tracking
