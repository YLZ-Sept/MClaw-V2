...
def _collect_system_info() -> dict:
    ...
    try:
        ...
        info["mclaw_version"] = get_version_string()
    except Exception:
        info["mclaw_version"] = "unknown"
    ...
    try:
        from mclaw.llm.config import get_default_config_path, load_endpoints_config
    ...
    try:
        from mclaw.channels.status import collect_effective_im_status
        from mclaw.config import settings

        gateway = None
        try:
            from mclaw.main import get_message_gateway
...
def _resolve_global_logs_dir() -> Path:
    """Return the global logs directory (Tauri-managed, under mclaw_home).

    Respects custom root via MCLAW_ROOT env var or settings.mclaw_home."""
    try:
        from mclaw.config import settings

        return settings.mclaw_home / "logs"
    except Exception:
        import os

        root = os.environ.get("MCLAW_ROOT", "").strip()
        return Path(root) / "logs" if root else Path.home() / ".mclaw" / "logs"


def _resolve_mclaw_home_dir() -> Path:
    """Return the Mclaw home directory shared with the desktop shell."""
    try:
        from mclaw.config import settings

        return settings.mclaw_home
    except Exception:
        import os

        root = os.environ.get("MCLAW_ROOT", "").strip()
        return Path(root) if root else Path.home() / ".mclaw"
...
def _add_windows_crash_artifacts(zf: zipfile.ZipFile) -> None:
    ...
    try:
        _add_dir_recent(
            zf,
            _resolve_mclaw_home_dir() / "crashdumps",
            "crashdumps",
            days=30,
...
def _add_wer_reports(zf: zipfile.ZipFile) -> None:
    """Add recent WER reports (and any captured minidumps) for Mclaw.
        ...
        for report_dir in candidates:
            ...
            try:
                ...
                if not report_wer.is_file():
                    continue
                matched = "mclaw" in report_dir.name.lower()
                if not matched:
                    try:
                        sample = report_wer.read_bytes()[: 64 * 1024]
                        matched = "mclaw" in sample.decode("utf-8", "ignore").lower()
                    except Exception:
                        matched = False
                if not matched:
                    continue
...
def _add_webview2_crashpad(zf: zipfile.ZipFile) -> None:
    ...
    if not local_appdata:
        return
    reports = (
        Path(local_appdata) / "com.mclaw.setupcenter" / "EBWebView" / "Crashpad" / "reports"
    )
    _add_dir_recent(
        zf,
        reports,
...
def _collect_windows_event_log() -> bytes:
    """Collect recent Application Error/WER events mentioning Mclaw."""
    import subprocess
    ...
    ps_cmd = (
        ...
        "Where-Object { $_.Message -match 'mclaw' } | Select-Object -First 30; "
...
def _collect_sanitized_config() -> dict:
    ...
    if runtime_path.exists():
        try:
            from mclaw.utils.redaction import redact_value
...
def _collect_endpoint_summary() -> dict:
    ...
    try:
        from mclaw.llm.config import get_default_config_path, read_workspace_env_values
...
async def download_feedback_package(report_id: str):
    ...
    return FileResponse(
        ...
        filename=f"mclaw-feedback-{safe_id}.zip",
    )
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        if upload_logs:
            try:
                ...
                main_log = logs_dir / "mclaw.log"
                error_log = logs_dir / "error.log"

            log_data = _tail_file(main_log, LOG_TAIL_BYTES)
            if log_data:
                zf.writestr("logs/mclaw.log", log_data)
            err_data = _tail_file(error_log, LOG_TAIL_BYTES)
            if err_data:
                zf.writestr("logs/error.log", err_data)
            serve_data = _tail_file(logs_dir / "mclaw-serve.log", LOG_TAIL_BYTES)
            if serve_data:
                zf.writestr("logs/mclaw-serve.log", serve_data)
            ...
            runtime_dir = _resolve_mclaw_home_dir() / "runtime"
