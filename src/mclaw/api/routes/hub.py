...
def _invalidate_imported_profile_runtime(request: Request, profile_id: str) -> None:
    try:
        from mclaw.prompt.builder import clear_prompt_section_cache
...
async def _propagate_store_skill_install(request: Request, skill_dir: Path) -> None:
    ...
    from mclaw.skills.events import SkillEvent
    ...
    try:
        from mclaw.api.routes.skills import _propagate
...
async def export_agent(req: ExportRequest):
    ...
    from mclaw.agents.packager import AgentPackager, PackageError
...
async def import_agent(
    ...
    from mclaw.agents.packager import AgentInstaller, PackageError
...
# ---------------------------------------------------------------------------
# Hub proxy routes — forward requests to the Mclaw Platform
# ---------------------------------------------------------------------------


def _get_hub_client():
    from mclaw.hub import AgentHubClient

    return AgentHubClient()


def _get_skill_client():
    from mclaw.hub import SkillStoreClient
