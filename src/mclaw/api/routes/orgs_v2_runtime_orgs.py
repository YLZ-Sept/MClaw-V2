...
:mod:`mclaw.api.routes.orgs_v2_runtime` -- thin wiring only
(D-4 LOCKED). Behavioural oracle: ``src/mclaw/api/routes/orgs.py``
...
from mclaw.api.schemas.orgs_v2 import OrgCreate
from mclaw.api.schemas.orgs_v2.orgs import OrgPatch
...
def create_org(request: Request, body: OrgCreate) -> dict[str, Any]:
    from mclaw.orgs import OrgNameConflictError
...
def get_avatar_presets() -> list[dict[str, Any]]:
    from mclaw.orgs._runtime_templates import list_avatar_presets
...
def list_plugin_workbench_templates(request: Request) -> list[dict[str, Any]]:
    from mclaw.orgs._runtime_templates import build_workbench_templates
