...
``~/.mclaw/`` (or ``MCLAW_ROOT``).  The Python backend always runs
...
def _state_file_path() -> Path:
    return _mclaw_home() / "state.json"


def _workspaces_dir() -> Path:
    return _mclaw_home() / "workspaces"
...
def _ensure_scaffold(ws_dir: Path) -> None:
    ...
    if not env_path.exists():
        env_path.write_text(
            "# Mclaw workspace environment (managed by Setup Center)\n"
            "#\n"
...
def _copy_template_files(ws_dir: Path) -> None:
    ...
    pkg_root = Path(__file__).resolve().parent.parent.parent  # mclaw package
...
def _find_repo_root() -> Path | None:
    ...
    for _ in range(10):
        if (p / "identity").is_dir() and (p / "src" / "mclaw").is_dir():
            return p
