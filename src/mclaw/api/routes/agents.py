...
from mclaw.agents.identity_files import PROFILE_IDENTITY_FILENAMES
from mclaw.api.agent_events import (
...
from mclaw.memory.json_utils import coerce_text
...
def _invalidate_bot_agent_sessions(bot_cfg: dict) -> None:
    ...
    """
    try:
        from mclaw.main import _bot_channel_name, get_message_gateway
...
async def _apply_persisted_bot_change(
    ...
    *,
    ...
    from mclaw.main import get_im_bot_runtime_error
...
def _unmask_credentials(submitted: dict, stored: dict) -> dict:
    ...
    from mclaw.utils.redaction import REDACTION
...
def _validate_bot_credentials(bot_type: str, credentials: dict) -> None:
    from mclaw.channels.status import missing_bot_credentials
...
async def create_bot(body: BotCreateRequest, request: Request):
    ...
    from mclaw.agents.profile import get_profile_store
    from mclaw.runtime_config_coordinator import profile_reference_change_lock
...
async def list_agent_tool_categories():
    ...
    from mclaw.orgs.tool_categories import list_agent_system_tool_categories
...
async def create_agent_profile(body: ProfileCreateRequest):
    ...
    from mclaw.agents.profile import AgentProfile, AgentType, SkillsMode, get_profile_store
...
async def reset_agent_profile(profile_id: str, request: Request):
    ...
    from mclaw.agents.presets import get_preset_by_id
    from mclaw.agents.profile import get_profile_store
    ...
    if existing is None:
        ...
        from mclaw.agents.profile import AgentProfile
...
async def init_profile_identity(profile_id: str, request: Request):
    ...
    from mclaw.agents.identity_resolver import ProfileIdentityResolver
    from mclaw.config import settings
...
async def get_profile_memory_stats(profile_id: str):
    ...
    db_path = memory_dir / "mclaw.db"

    if not db_path.exists():
        return {"exists": False, "semantic_count": 0, "db_size_bytes": 0}

    from mclaw.storage.safe_sqlite import SQLiteUnavailable, safe_open_async_ctx
    ...
    try:
        ...
        try:
            from mclaw.storage.degraded import registry as _degraded
