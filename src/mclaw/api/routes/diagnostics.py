...
(:mod:`mclaw.core.conversation_metrics`) and the in-flight turn registry
(:mod:`mclaw.api.routes.turn_registry`).  Admin uses these to verify
...
from mclaw.core.conversation_metrics import snapshot as conversation_metrics_snapshot
...
async def get_conversation_metrics() -> dict:
    ...
    Counter names (see ``mclaw.core.conversation_metrics``):
