"""
FastAPI HTTP API server for Mclaw.

集成在 `mclaw serve` 中，提供：
- Chat (SSE streaming)
- Models list
- Health check
- Skills management
...
import mclaw._ensure_utf8  # noqa: F401  # Windows UTF-8 编码保护
...
# :func:`mclaw.api.host_resolution.resolve_api_host` at startup and passed
...
def _do_force_exit(grace_s: int) -> None:
    ...
    ``mclaw.api.server.os._exit`` and intercept the call here.
    """
    logger.error(
...
def _arm_force_exit_watchdog_sync(app: FastAPI) -> None:
    ...
    try:
        ...
        timer.name = "mclaw-force-exit-watchdog"
        timer.daemon = True
        timer.start()
...
def _arm_force_exit_watchdog_async(app: FastAPI) -> None:
    ...
    try:
        ...
        task = loop.create_task(_force_exit(), name="mclaw-force-exit-watchdog")
...
def _find_web_dist() -> Path | None:
    ...
    2. mclaw/web/ (pip wheel install & PyInstaller bundle)
    """
...
def _find_docs_dist() -> Path | None:
    ...
    Search order:
    1. mclaw/docs_dist/ (pip wheel install)
...
def _schedule_startup_llm_health_check(app_state: Any) -> asyncio.Task[None] | None:
    ...
    task = asyncio.create_task(
        ...
        name="mclaw-startup-llm-health-check",
    )
...
def create_app(
    ...
    from mclaw import get_version_string
    ...
    from mclaw.runtime_config_coordinator import RuntimeConfigCoordinator
    ...
    from mclaw.orgs._default_agent_builder import DefaultAgentBuilder
    from mclaw.orgs._runtime_agent_pipeline import (
        AgentCache,
        AgentPipelineExecutor,
        ProfileResolver,
    )
    from mclaw.orgs._runtime_templates import ensure_builtin_templates
    from mclaw.orgs.manager import OrgManager
    from mclaw.orgs.runtime import OrgRuntime, _InMemoryEventBus
    from mclaw.orgs.store import set_default_org_manager
    ...
    from mclaw.orgs.command_service import OrgCommandService, set_command_service
    ...
    # :mod:`mclaw.runtime.supervisor_factory`.
    from mclaw.runtime.supervisor_factory import get_or_create_checkpointer
    ...
    from mclaw.orgs.scoped_subsystems import (
        OrgScopedBlackboard,
    ...
    #
    # ROADMAP — Legacy shim removal target: Mclaw 2.1.0 minor.
    ...
    # bootstrap.js and the shared ui-kit out of the mclaw-plugin-sdk
    ...
    from mclaw import get_version_string as _get_ver
    ...
    async def _import_pending_feedback():
        ...
        try:
            ...
            home = settings.mclaw_home
        except Exception:
            home = Path.home() / ".mclaw"
    ...
    async def _cleanup_expired_resume_state():
        ...
        try:
            from mclaw.core.cancel_cleanup import cleanup_expired_working_messages
    ...
    async def _wire_pending_approvals_sse():
        ...
        """
        try:
            from mclaw.agent.pending_approval_notifications import (
                build_pending_approval_event_hook,
                notify_pending_approval_im,
            )
            from mclaw.agent.pending_approvals import get_pending_approvals_store
            from mclaw.api.routes.websocket import fire_event

            api_loop = asyncio.get_running_loop()

            async def _notify_owner(payload: dict) -> None:
                try:
                    from mclaw.scheduler import get_active_scheduler
        ...
        # 影响 server 启动。
        try:
            from mclaw.core.policy_v2.hot_reload import start_hot_reloader
    ...
    async def shutdown(request: Request):
        """Gracefully shut down the Mclaw service process.
    ...
    async def _start_inbox_service():
        try:
            from mclaw.config import settings
            from mclaw.inbox import get_inbox_service
    ...
    async def _shutdown_org_runtime():
        ...
        # loop itself.
        try:
            from mclaw.runtime.supervisor_factory import (
        ...
        if hasattr(app.state, "org_runtime") and app.state.org_runtime:
            try:
                from mclaw.core.engine_bridge import to_engine
    ...
    async def _shutdown_policy_hot_reloader():
        try:
            from mclaw.core.policy_v2.hot_reload import stop_hot_reloader
    ...
    async def _shutdown_memory_storage():
        try:
            from mclaw.memory.storage import checkpoint_and_close_all_storages
    ...
    async def _start_async_audit_writer():
        ...
        """
        try:
            from mclaw.agent.audit import DEFAULT_AUDIT_PATH
            from mclaw.core.policy_v2.audit_writer import (
    ...
    async def _check_frontend_bundle_freshness() -> None:
        try:
            from mclaw import __version__ as backend_version
            from mclaw.api.routes.build_info import (
    ...
    async def _shutdown_plugin_aiosqlite_workers() -> None:
        ...
        try:
            from mclaw.api.routes import token_stats as _token_stats
    ...
    async def _arm_shutdown_diagnostics() -> None:
        ...
        try:
            from mclaw.api._shutdown_diagnostics import arm_shutdown_diagnostics

            try:
                from mclaw.config import settings as _settings2
...
async def start_api_server(
    ...
    try:
        from mclaw.config import settings as _serve_settings
    ...
    from mclaw.core.engine_bridge import set_api_loop, set_engine_loop
...
def _refresh_node_tool_host(app: FastAPI) -> None:
    ...
    if agent is None:
        ...
        return
    try:
        from mclaw.orgs._runtime_agent_host import build_node_tool_host
...
def update_runtime_refs(
    ...
    *,
    ...
    """
    app = getattr(api_task, "_mclaw_api_app", None) if api_task is not None else None
    if app is None:
        return False
    if agent is not None:
        _attach_agent_to_app(app, agent)
