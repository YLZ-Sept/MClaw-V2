...
from mclaw.optional_assets import load_optional_asset_feature
...
def _mclaw_root() -> Path:
    configured = os.environ.get("MCLAW_ROOT", "").strip()
    return Path(configured).expanduser() if configured else Path.home() / ".mclaw"


def _request_store_path() -> Path:
    return _mclaw_root() / "data" / "optional_feature_requests.json"
...
def _managed_driver_dir() -> Path:
    return _mclaw_root() / "modules" / "browser" / "playwright-driver" / _playwright_version()
...
def _optional_asset_cache() -> Path:
    return _mclaw_root() / "cache" / "optional-assets"
...
def _download_resumable(
    ...
    *,
    ...
    for url in sources:
        ...
        headers = {"User-Agent": "Mclaw optional-feature"}
        if offset:
...
def install_playwright_runtime(progress: Any | None = None) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="mclaw-playwright-") as temp_name:
        temp = Path(temp_name)
...
def cache_chromium_downloads(progress: Any | None = None) -> Path:
    from mclaw.optional_assets import resolve_optional_asset_mirror
...
async def install_browser_automation(request_id: str) -> dict:
    ...
    async with lock:
        ...
        try:
            ...
            from mclaw.tools.browser.manager import (
