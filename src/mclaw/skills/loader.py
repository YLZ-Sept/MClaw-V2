...
class SkillLoader:
    ...
    def _cheap_skill_id_candidates(skill_dir: Path) -> set[str]:
        ...
        # ``mclaw/skills@<dir>``. Preset profiles and default allowlists use
        # that key, so matching it here avoids reading SKILL.md just to learn it.
        candidates.add(f"mclaw/skills@{skill_dir.name}")
    ...
    def _load_i18n(self, skill_dir: Path, metadata: SkillMetadata) -> None:
        ...
        优先 agents/openai.yaml 的 i18n 字段，回退 .mclaw-i18n.json。
        """
    ...
    def run_script(
        ...
        if script_path.suffix == ".py":
            ...
            from mclaw.runtime_manager import get_python_executable
