...
# mclaw.api.server`` after the upstream merge reordered the agents imports).
from mclaw.agent.capabilities import (
...
_MARKETPLACE_HOSTS = {"github.com/mclaw/mclaw", "mclaw.com", "skill.mclaw.ai"}
...
class SkillEntry:
    ...
    # metadata.mclaw structured fields
    ...
    # 技能来源 URL（来自 .mclaw-source 文件，用于区分同名技能）
    ...
    # 国际化（由 agents/openai.yaml i18n 字段注入，兼容旧的 .mclaw-i18n.json）
    ...
    def from_parsed_skill(
        ...
        *,
        ...
        if skill.path:
            ...
            source_file = Path(skill.path).parent / ".mclaw-source"
            try:
