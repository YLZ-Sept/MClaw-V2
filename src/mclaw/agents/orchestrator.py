...
from mclaw.agents.task_queue import TaskQueue

if TYPE_CHECKING:
    from mclaw.channels import MessageGateway
...
_BUDGET_GUIDE = (
    "\n\n提示：可以在 Mclaw「配置 → 高级配置 → 长任务与上下文保护 → 任务预算」"
...
class AgentOrchestrator:
    ...
    def _ensure_deps(self) -> None:
        ...
        try:
            ...
            if self._fallback is None:
                from mclaw.agents.fallback import FallbackResolver

                self._fallback = FallbackResolver(self._profile_store)

            if self._log_dir is None:
                from mclaw.config import settings as _s
    ...
    async def _dispatch(
        ...
        try:
            ...
            try:
                from mclaw.tracing.tracer import get_tracer
    ...
    async def _run_with_progress_timeout(
        ...
        *,
        ...
        if isolated_browser and hasattr(agent, "browser_manager"):
            from mclaw.tools.browser import BrowserUseRunner, PlaywrightTools
    ...
    async def _call_agent(
        ...
        *,
        ...
        async with agent._execution_lock:
            ...
            _mode = "agent"
            try:
                from mclaw.config import settings as _cfg
            ...
            try:
                ...
                if is_sub_agent:
                    try:
                        from mclaw.agent.output_guard import (
...
def _cleanup_sub_agent_resources(agent: Any, session: Any) -> None:
    ...
    try:
        from mclaw.tools.handlers.todo_state import cleanup_session
