...
def create_backup(
    ...
    *,
    ...
    zip_name = f"mclaw-backup-{workspace_id}-{ts}.zip"
...
def _rotate_backups(backup_dir: Path, workspace_id: str, max_backups: int) -> None:
    ...
    pattern = re.compile(rf".mclaw-backup-{re.escape(workspace_id)}-\d{{8}}_\d{{6}}\.zip$")
    backups = sorted(
...
def list_backups(backup_path: str) -> list[dict[str, Any]]:
    ...
    pattern = re.compile(r".mclaw-backup-.+-\d{8}_\d{6}\.zip$")
