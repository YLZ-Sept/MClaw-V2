...
class MemoryManager:
    ...
    def __init__(
        ...
        self._session_turns_var: contextvars.ContextVar[list[ConversationTurn] | None] = (
            contextvars.ContextVar(f"mclaw_memory_session_turns_{id(self)}", default=None)
        )
        ...
        self._session_cited_memories_var: contextvars.ContextVar[list[dict] | None] = (
            contextvars.ContextVar(f"mclaw_memory_cited_{id(self)}", default=None)
        )
        ...
        db_path = self.data_dir / "mclaw.db"
        try:
...
""".format(timestamp=datetime.now().strftime("%Y-%m-%d %H:%M"))
    ...
    def contribute_to_compaction(self, **_kwargs):
        ...
        from mclaw.runtime.context.continuity import CompactionContribution
