...
- `python -m mclaw.setup_center.bridge list-providers`
- `python -m mclaw.setup_center.bridge list-models --api-type ... --base-url ... [--provider-slug ...]`
- `python -m mclaw.setup_center.bridge list-skills --workspace-dir ...`
...
def list_providers() -> None:
    from mclaw.llm.registries import list_providers as _list_providers
...
async def _list_models_openai(api_key: str, base_url: str, provider_slug: str | None) -> list[dict]:
    ...
    from mclaw.llm.capabilities import infer_capabilities
    ...
    from mclaw.llm.types import normalize_base_url
    ...
    from mclaw.llm.providers.proxy_utils import get_httpx_client_kwargs
...
async def health_check_endpoint(workspace_dir: str, endpoint_name: str | None) -> None:
    ...
    from mclaw.llm.client import LLMClient
...
async def feishu_onboard_start(domain: str) -> None:
    ...
    from mclaw.setup.feishu_onboard import FeishuOnboard
...
async def feishu_validate(app_id: str, app_secret: str, domain: str) -> None:
    """验证飞书凭证有效性"""
    from mclaw.setup.feishu_onboard import validate_credentials
...
async def wecom_onboard_start() -> None:
    ...
    from mclaw.setup.wecom_onboard import WecomOnboard
...
async def qqbot_onboard_start() -> None:
    ...
    from mclaw.setup.qqbot_onboard import QQBotOnboard
...
async def qqbot_validate(app_id: str, app_secret: str) -> None:
    """验证 QQ 机器人凭证有效性"""
    from mclaw.setup.qqbot_onboard import validate_credentials
...
async def wechat_onboard_start() -> None:
    ...
    from mclaw.setup.wechat_onboard import WeChatOnboard
...
def list_skills(workspace_dir: str) -> None:
    from mclaw.skills.loader import SkillLoader
    ...
    for s in skills:
        ...
        if skill_path:
            try:
                origin_file = Path(skill_path) / ".mclaw-source"
...
def _try_platform_skill_download(skill_id: str, dest_dir: Path) -> bool:
    """Try downloading a cached skill ZIP from the Mclaw platform.
    ...
    headers = {"User-Agent": "Mclaw-SetupCenter"}
...
def _download_github_zip(repo_owner: str, repo_name: str, dest_dir: Path) -> None:
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        ...
        tmp_extract = Path(tempfile.mkdtemp(prefix="mclaw_zip_"))
        try:
...
def _parse_github_url(url: str) -> tuple[str, str, str | None] | None:
    ...
    """
    from mclaw.skills.source_url import parse_github_source
...
def _download_gitee_zip(repo_owner: str, repo_name: str, dest_dir: Path) -> None:
    ...
    with zipfile.ZipFile(io.BytesIO(data)) as zf:
        _validate_zip_members(zf)
        tmp_extract = Path(tempfile.mkdtemp(prefix="mclaw_gitee_"))
        try:
            zf.extractall(tmp_extract)
            children = list(tmp_extract.iterdir())
            src = children[0] if len(children) == 1 and children[0].is_dir() else tmp_extract
...
def _read_skill_source(d: Path) -> str:
    """读取技能目录的安装来源标记。"""
    try:
        return (d / ".mclaw-source").read_text(encoding="utf-8").strip()
    except Exception:
        return ""
...
def _quarantine_broken_skill_dir(d: Path) -> Path:
    ...
    quarantine_root = d.parent / ".mclaw-broken"
...
def _extract_skill_signal(raw: str) -> str:
    ...
    支持三种粘贴方式：
      ...
      2. 单行 CLI：``mclaw install-skill owner/repo`` —— 先用
...
def install_skill(workspace_dir: str, url: str, *, category: str | None = None) -> None:
    ...
    if category:
        try:
            from mclaw.skills.categories import is_valid_category_name
    ...
    if url.startswith("github:"):
        ...
        if _try_platform_skill_download(platform_skill_id, target):
            try:
                origin_file = target / ".mclaw-source"
    ...
    marketplace = [
        {
            ...
            "author": "mclaw",
            "url": "github:mclaw/skills/web-search",
            "stars": 42,
            "tags": ["搜索", "网络"],
        },
        {
            ...
            "author": "mclaw",
            "url": "github:mclaw/skills/code-interpreter",
            "stars": 38,
            "tags": ["代码", "数据分析"],
        },
        {
            ...
            "author": "mclaw",
            "url": "github:mclaw/skills/browser-use",
            "stars": 25,
            "tags": ["浏览器", "自动化"],
        },
        {
            ...
            "author": "mclaw",
            "url": "github:mclaw/skills/image-gen",
            "stars": 19,
            "tags": ["图片", "生成"],
        },
    ]
...
def main(argv: list[str] | None = None) -> None:
    ...
    p = argparse.ArgumentParser(prog="mclaw.setup_center.bridge")
...
if __name__ == "__main__":
    from mclaw.runtime_env import IS_FROZEN, ensure_ssl_certs, inject_module_paths
