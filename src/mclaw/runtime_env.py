...
PyInstaller 打包后 sys.executable 指向 mclaw-server.exe 而非 Python 解释器，
...
def _get_mclaw_root() -> Path:
    """获取 Mclaw 根目录路径 (避免循环导入 config)。

    优先使用 MCLAW_ROOT 环境变量，默认 ~/.mclaw。
    """
    import os

    env_root = os.environ.get("MCLAW_ROOT", "").strip()
    if env_root:
...
def _get_bundled_internal_python() -> str | None:
    ...
    构建时 mclaw.spec 会将 sys.executable 和 pip 一起复制到 _internal/，
...
def get_python_executable() -> str | None:
    ...
    **只使用项目自带或项目自行安装的 Python，不使用用户系统 Python。**
    ...
    logger.warning(
        "未找到项目自带的 Python 解释器。"
        "已搜索: 工作区 venv → ~/.mclaw/venv → "
        "打包内置 Python。"
        "请重新安装 Mclaw，确保安装包资源完整。"
    )
    return None
...
def get_runtime_root() -> Path:
    """Return the dual-venv runtime root under ~/.mclaw/runtime."""
    return _get_mclaw_root() / "runtime"
...
def get_toolchain_root() -> Path:
    return _get_mclaw_root() / "toolchains"
...
def get_readonly_seed_roots() -> list[Path]:
    ...
    Seed roots are never written by Mclaw. They are only used as verified
...
def sanitize_runtime_environment(
    ...
    *,
    ...
    """Return an Mclaw-managed subprocess environment.
...
def apply_subprocess_secret_scrub(env: dict[str, str]) -> dict[str, str]:
    """Remove Mclaw/provider secrets unless explicitly force-injected."""
    merged: dict[str, str] = {}
...
def build_user_subprocess_environment(
    ...
    *,
    ...
    This keeps normal shell/project context while applying Mclaw's minimal
    ...
    locale, proxy, Git/SSH config), while Mclaw still scrubs runtime
...
def apply_managed_node_environment(env: dict[str, str]) -> dict[str, str]:
    """Prefer Mclaw-managed Node/npm/corepack for agent/tool subprocesses."""
    merged = dict(env)
    node = get_managed_node_seed()
    node_bin = get_managed_node_bin_dir()
...
def get_channel_deps_dir() -> Path:
    ...
    路径: ~/.mclaw/modules/channel-deps/site-packages
    该目录会被 inject_module_paths() 自动扫描并注入到 sys.path。
    """
    return _get_mclaw_root() / "modules" / "channel-deps" / "site-packages"
...
def _sanitize_sys_path() -> None:
    ...
    mclaw_root = str(_get_mclaw_root())
...
def inject_module_paths() -> None:
    ...
    2. 扫描 ~/.mclaw/modules/*/site-packages — 兜底机制
    ...
    # 来源 3：扫描 ~/.mclaw/modules/*/site-packages（兜底）
    # 跳过已内置到 core 包的模块，避免外部旧版本与内置版本冲突
    _BUILTIN_MODULE_IDS = {"browser"}
    modules_base = _get_mclaw_root() / "modules"
...
def inject_module_paths_runtime() -> int:
    ...
    # 扫描 ~/.mclaw/modules/*/site-packages
    modules_base = _get_mclaw_root() / "modules"
    if modules_base.exists():
        for module_dir in modules_base.iterdir():
            if not module_dir.is_dir():
