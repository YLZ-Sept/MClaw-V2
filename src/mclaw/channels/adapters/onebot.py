...
- WebSocket 正向连接 (forward): Mclaw 作为客户端连接 OneBot 实现的 WS 服务器
- WebSocket 反向连接 (reverse): Mclaw 作为 WS 服务端，OneBot 实现主动连入（推荐）
- 文本/图片/语音/文件收发
"""
...
class OneBotAdapter(ChannelAdapter):
    ...
    支持两种连接模式:
    - reverse (默认): Mclaw 作为 WS 服务端，NapCat/Lagrange 配置 Websocket 客户端连入
    - forward: Mclaw 主动连接 NapCat/Lagrange 的 Websocket 服务器
    """
