...
class MessageGateway:
    ...
    def __init__(
        ...
        from mclaw.runtime.cancel_token import CancellationToken as _CT
        ...
        # mclaw.sessions（避免 fork-style 重写后的循环依赖，ADR-0001）。
        try:
            from mclaw.runtime.session_bridge import register_session_org_lookup
    ...
    async def _handle_feishu_command(self, cmd: str, message: "UnifiedMessage") -> str | None:
        ...
        if sub == "start":
            if adapter and hasattr(adapter, "get_status_info"):
                ...
                lines = [
                    f"Mclaw Feishu Adapter v{info['version']}",
    ...
    def _ensure_ffmpeg(self) -> None:
        ...
        try:
            ...
            from mclaw.tools._import_helper import import_or_hint
    ...
    def _resolve_install_error_for_adapter(self, adapter_name: str) -> str | None:
        ...
        if not self._channel_install_errors:
            return None
        try:
            from mclaw.channels.deps import CHANNEL_DEPS
        except Exception:
            return None
    ...
    async def _try_dispatch_v2(
        ...
        try:
            ...
            session_key = self._get_session_key(message)
            from mclaw.runtime.session_bridge import get_org_id_for_session

            org_id = get_org_id_for_session(session_key)
            if not org_id or org_id not in canary_orgs:
                return False

            from mclaw.runtime.cancel_token import CancellationToken
            from mclaw.runtime.channel_routing import dispatch_inbound_message_to_v2
            from mclaw.runtime.im_stream_bridge import ImStreamBridge
    ...
    def _get_org_manager(self):
        ...
        """
        try:
            from mclaw.orgs.store import get_default_org_manager
        ...
        try:
            from mclaw.orgs.command_service import get_command_service
