...
# (smoke-F0/F6) -- importing ``mclaw.agent.confirmation`` at module-top
# triggers ``mclaw.agent.__init__`` which immediately re-enters this very
# module via ``agent.core -> mclaw.core._agent_runtime``.

# Import Identity from the canonical home (not the ``core.identity`` re-export
# shim) so a cold ``import mclaw.agent.identity`` entry point cannot deadlock
# on a partially-initialised shim (shim -> agent.identity -> agent.__init__ ->
# agent.core -> _agent_runtime -> shim). See ADR-0003.
from mclaw.agent.errors import UserCancelledError
from mclaw.agent.identity import Identity
from mclaw.agent.ralph import RalphLoop, Task, TaskResult
from mclaw.agent.skill_manager import SkillManager
from mclaw.agent.user_profile import get_profile_manager
...
# (smoke-F0/F6) -- its module top-loads ``mclaw.agents.lock_manager`` which
...
# cycle whenever this module is loaded BEFORE ``mclaw.agent``.
...
- 不要给建议
- 不要输出最终答案
- 不要编造能力：不得虚构「工具在用户本机执行」等事实；但若任务涉及**用户本机才可观测的效果**（本机 GUI、本机安装、游戏内 overlay 等），必须在 `constraints` 或 `risks_or_ambiguities` 中**如实**写出「默认仅在 Mclaw 宿主执行、与用户聊天设备可能不同域」等部署边界——这与「假设能力限制」不同，是事实约束
- 只输出 YAML 格式的结构化任务定义
- 保持简洁，每项不超过一句话
...
class Agent:
    ...
    # 现在改为使用 `mclaw.core.im_context` 中的 contextvars（协程隔离）。
    ...
    def __init__(
        ...
        # ==================== 人格系统 + 活人感 + 表情包 ====================
        from mclaw.agent.persona import PersonaManager
        from mclaw.agent.trait_miner import TraitMiner
    ...
    async def _initialize_unlocked(
        ...
        if not settings.hub_device_id:
            from mclaw.hub.device import get_or_create_device_id
    ...
    def notify_pools_skills_changed() -> None:
        ...
        """
        try:
            from mclaw.main import _desktop_pool, _orchestrator
    ...
    def _prepare_prompt_identity_dir(self) -> Path:
        ...
        runtime_root = Path(
            getattr(self, "_prompt_identity_runtime_root", None) or settings.mclaw_home
        )
...
- **重要**：这两个 marker 是系统注入的、由真实工具凭证支撑的回放摘要；不要在你自己的回复中模仿这种格式编造执行结果"""
    ...
    async def _mine_traits_in_background(self, message: str, session_id: str) -> None:
        ...
        try:
            ...
            from mclaw.agent.persona import persist_trait_to_memory
    ...
    async def _prepare_session_context(
        ...
        *,
        ...
        if session and hasattr(session, "context"):
            try:
                from mclaw.agent.working_facts import extract_working_facts, merge_working_facts
        ...
        if session is not None and not topic_changed and hasattr(session, "context"):
            try:
                from mclaw.runtime.context.continuity import content_digest
            ...
            for fdata in pending_files:
                if has_pdf_cap and fdata.get("type") == "document":
                    ...
                    if local_path and Path(local_path).exists():
                        try:
                            from mclaw.channels.media.handler import MediaHandler
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                from mclaw.agent.confirmation import (
    ...
    async def _preempt_or_queue_prev_task(
        ...
        *,
        ...
        策略来自 :func:`mclaw.api.routes.double_texting.resolve_policy`（按 channel；
            ...
            if success:
                ...
                if settings.desktop_notify_enabled and not getattr(
                    ...
                    if channel in ("cli", "desktop"):
                        from mclaw.agent.desktop_notify import notify_task_completed_async
