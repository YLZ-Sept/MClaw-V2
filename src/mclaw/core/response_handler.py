...
# ==================== 内部 trace marker 常量 ====================
#
# Mclaw 通过 `build_tool_trace_summary()` 注入到历史回放上下文里的内部
...
_SYSTEM_REQUEST_PREFIXES_TUPLE: tuple[str, ...] = (
    ...
    # 判命中正文里出现的「文件 / 链接 / 写一份 /mclaw-promotion-plan.md」
...
_ARTIFACT_INTENT_KEYWORDS: tuple[str, ...] = (
    ...
    "做一份",
    "写一份",
    # 强信号：带量词的"整理/准备/拟一份"——用户场景"帮我整理一份Mclaw
...
class ResponseHandler:
    ...
    async def verify_task_completion(
        ...
        plan_fail_reason = ""
        try:
            from mclaw.agent.validators import (
                ValidationContext,
                ValidationResult,
...
工具在 **Mclaw 宿主**执行，与用户发消息的设备/IM 客户端**默认不同域**。宿主上命令成功 ≠ 用户本机已出现窗口或已安装软件。
...
NEXT: 建议的下一步"""
        ...
        try:
            response = await self._brain.think_lightweight(
                ...
                system=(
                    "你是任务完成度判断助手。Mclaw 工具在宿主环境执行，与用户聊天设备通常不是同一台机器；"
