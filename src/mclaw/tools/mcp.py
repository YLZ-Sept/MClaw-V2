...
def _try_import_mcp() -> bool:
    ...
    try:
        ...
        MCP_SDK_AVAILABLE = False
        logger.warning(
            "MCP SDK not installed. Mclaw will install it into the isolated channel-deps runtime."
        )
        return False
...
class MCPClient:
    ...
    async def _connect_runtime(self, server_name: str) -> MCPConnectResult:
        ...
        if not MCP_SDK_AVAILABLE:
            if not ensure_mcp_sdk():
                msg = (
                    "MCP SDK 未安装且自动安装失败。\n"
                    "Mclaw 会把 MCP SDK 安装到隔离的 channel-deps 运行时，"
                    "不会要求你污染宿主 Python。\n"
                    "请前往「设置中心 → Python 环境」点击「一键修复」，修复后重启 Mclaw。"
                )
                logger.error(msg)
        ...
        # stdio 模式预检查命令是否存在
        # ``python -m mclaw.*`` 会在 _connect_stdio 中被适配为当前运行环境，
        # 避免误用系统 Python 导致内置模块不可导入。
        if config.transport == "stdio" and config.command:
            if not self._adapt_mclaw_module_command(config) and not self._resolve_command(
                config
            ):
    ...
    def _resolve_command(config: MCPServerConfig) -> str | None:
        ...
        # 3) Mclaw-managed Node/npm/npx should satisfy built-in MCP servers
    ...
    @staticmethod
    def _adapt_mclaw_module_command(
        config: MCPServerConfig,
    ) -> tuple[str, list[str]] | None:
        """将 ``python -m mclaw.*`` 适配为当前 Mclaw 运行环境。
        ...
        if not (
            ...
            and config.args[0] == "-m"
            and config.args[1].startswith("mclaw.")
        ):
            return None
    ...
    async def _connect_stdio(self, server_name: str, config: MCPServerConfig) -> MCPConnectResult:
        ...
        adapted = self._adapt_mclaw_module_command(config)
        if adapted:
        ...
        from mclaw.runtime_manager import build_user_subprocess_environment
            ...
            if not ensure_mcp_sdk():
                return MCPCallResult(
                    ...
                    error=(
                        ...
                        "Mclaw 会安装到隔离的 channel-deps 运行时。"
                    ),
                )
