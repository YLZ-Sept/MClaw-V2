...
提供 Chrome 安装检测、Mclaw 专用 profile 管理、Cookie 同步等工具函数。
...
def get_mclaw_chrome_profile() -> str:
    """
    获取 Mclaw 专用的 Chrome profile 目录。
    ...
    profile_dir = base_dir / "Mclaw" / "ChromeProfile"
...
def sync_chrome_cookies(src_user_data: str, dst_profile: str) -> bool:
    """
    同步用户 Chrome 的 cookies 到 Mclaw profile。
    只复制关键文件以保持登录状态。
    """
...
def detect_chrome_devtools_mcp() -> dict:
    ...
    # Mclaw-managed Node first, then host PATH (with macOS login-shell fallback).
