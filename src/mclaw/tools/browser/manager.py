...
def _managed_browsers_dir() -> Path:
    ...
    base = Path(root).expanduser() if root else Path.home() / ".mclaw"
...
async def _download_managed_chromium(
    ...
    *,
    ...
    from mclaw.optional_features import configure_playwright_driver
...
class BrowserManager:
    ...
    async def _try_user_chrome(self, headless: bool, *, use_oa_profile: bool) -> bool:
        ...
        if use_oa_profile:
            from .chrome_finder import get_mclaw_chrome_profile, sync_chrome_cookies

            user_data = get_mclaw_chrome_profile()
