...
# Treats the Mclaw data plane and any vendored Python install as no-go
# zones — the LLM should never need to recursively scan these. Previously a
# misrouted grep on `~/.mclaw` could lock up the worker for minutes
...
class FileTool:
    ...
    async def grep(
        ...
        *,
        ...
        - Hard-rejects scanning the Mclaw runtime/workspaces directory or
