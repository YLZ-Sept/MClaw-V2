...
class SkillsHandler:
    ...
    def _get_skill_info(self, params: dict) -> str:
        ...
        if skill.allowed_tools:
            try:
                from mclaw.core.policy_v2 import get_skill_allowlist_manager
        ...
        if "{{" in body:
            ...
            from mclaw.skills.arguments import substitute
        ...
        if skill.arguments:
            from mclaw.skills.arguments import format_argument_schema
...
修改已生效。"""
    ...
    def _manage_skill_enabled(self, params: dict) -> str:
        """批量启用/禁用外部技能"""
        from mclaw.skills.allowlist_io import overwrite_allowlist, read_allowlist
    ...
    async def _execute_skill(self, params: dict) -> str:
        ...
        hook_runner = None
        if skill.hooks:
            from mclaw.skills.skill_hooks import create_hook_runner
