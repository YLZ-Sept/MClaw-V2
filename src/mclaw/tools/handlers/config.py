...
class ConfigHandler:
    ...
    async def _add_endpoint(self, params: dict) -> str:
        ...
        from mclaw.llm.types import normalize_context_window
    ...
    async def _probe_endpoint_before_enable(self, endpoint: dict, api_key: str) -> dict:
        ...
        from mclaw.llm.providers.base import LLMProvider
        from mclaw.llm.types import normalize_base_url
    ...
    def _ext_status(self) -> str:
        ...
        lines.append("---")
        lines.append("*安装后无需修改 Mclaw 配置，系统启动时自动检测 PATH。*")
        return "\n".join(lines)

    def _ext_credits(self) -> str:
        lines = ["## 致谢 — 外部扩展模块\n"]
        lines.append("Mclaw 的工具调用和浏览器访问能力得益于以下开源项目：\n")
