...
将 Mclaw 的核心能力暴露为 MCP 服务器，
允许其他 AI Agent（如 Claude Desktop、Cursor 等）通过 MCP 协议调用。

暴露的工具:
- mclaw_chat: 与 Mclaw 对话
- mclaw_memory_search: 搜索记忆
- mclaw_schedule_task: 创建定时任务
- mclaw_list_skills: 列出可用技能
- mclaw_execute_skill: 执行技能

启动方式:
    python -m mclaw.mcp_server [--port 8765]
"""
...
MCP_SERVER_NAME = "mclaw"
MCP_SERVER_VERSION = "1.0.0"

EXPOSED_TOOLS = [
    {
        "name": "mclaw_chat",
        "description": "Send a message to Mclaw and get a response",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "mclaw_memory_search",
        "description": "Search Mclaw's memory for relevant information",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "mclaw_list_skills",
        "description": "List available Mclaw skills",
        "inputSchema": {
            "type": "object",
            "properties": {},
        },
...
class MCPServer:
    """Lightweight MCP server that exposes Mclaw capabilities via stdio."""
    ...
    async def _execute_tool(self, tool_name: str, arguments: dict) -> str:
        ...
        if tool_name == "mclaw_chat":
            ...
            # process over stdio). Each ``mclaw_chat`` invocation is
            ...
            try:
                ...
                mcp_ctx = build_policy_context(
                    session_id="mcp_tool_mclaw_chat",
                    channel="mcp",
        ...
        elif tool_name == "mclaw_memory_search":
        ...
        elif tool_name == "mclaw_list_skills":
    ...
    async def run_stdio(self):
        ...
        logger.info("Mclaw MCP Server starting on stdio...")
