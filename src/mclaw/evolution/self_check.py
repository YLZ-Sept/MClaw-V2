...
class SelfChecker:
    ...
    def __init__(
        ...
        self.test_dir = test_dir or (
            settings.project_root / "src" / "mclaw" / "testing" / "cases"
        )
        ...
        if "shell" in test.id:
            ...
            if isinstance(test.input, dict):
                if test.input.get("action") == "write_read":
                    test_file = str(Path(tempfile.gettempdir()) / "mclaw_test.txt")
            ...
            report.tool_error_patterns.append(
                {
                    ...
                    "count": 1,
                    "logger": "mclaw.evolution.self_check",
                    "message": reason,
...
"""
    ...
    def _load_selfcheck_system_prompt() -> str:
        ...
        try:
            bundled_path = resources.files("mclaw").joinpath(
        ...
        message = (
            ...
            f"{override_path} and bundled resource mclaw/prompts/selfcheck/system.md"
        )
                ...
                results.append(
                    {
                        ...
                        "module": "mclaw.evolution.self_check",
                        "error_type": "core",
                        "analysis": item.strip(),
                        "severity": "low",
                        "can_fix": False,
            ...
            try:
                ...
                agent._selfcheck_fix_policy = {
                    ...
                    "write_roots": [
                        ...
                        str((project_root / "src" / "mclaw" / "tools").resolve()),
                        str((project_root / "src" / "mclaw" / "channels").resolve()),
                    ],
...
2. **禁止** 修改 Akita 核心系统代码（`src/mclaw/core/`、`src/mclaw/llm/`、`src/mclaw/memory/`、`src/mclaw/scheduler/`、`src/mclaw/storage/`、`src/mclaw/agents/` 等）
    ...
    async def _fix_timeout_error(self, sample) -> bool:
        ...
        try:
            ...
            if platform.system() == "Windows":
                ...
                # Linux/Mac 下清理僵尸进程
                await self.shell.run("pkill -9 -f 'mclaw.*timeout' || true")
