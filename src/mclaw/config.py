...
class Settings(BaseSettings):
    ...
    api_token: str = Field(
        ...
        description=(
            ...
            "或 X-Mclaw-Token 头里携带它，作为 web_access 密码之外的二次校验。"
    ...
    uvicorn_graceful_shutdown_timeout_s: float = Field(
        ...
        description=(
            ...
            "等代理 / 浏览器 / mclaw stop CLI 自身的 keep-alive 连接 "
    ...
    # 在 .env / mclaw.toml 里这样写就能关掉本批新行为之一：
    ...
    # Agent 配置
    agent_name: str = Field(default="Mclaw", description="Agent 名称")
    ...
    # === 搜索源（Provider）配置 ===
    # 调度规则见 src/mclaw/tools/web_search/runtime.py：
    ...
    log_file_prefix: str = Field(default="mclaw", description="日志文件前缀")
    ...
    bug_report_endpoint: str = Field(
        default="https://feedback-mclaw.fzstack.com",
    ...
    inbox_broadcast_url: str = Field(
        default="https://dl-mclaw.fzstack.com/inbox/broadcast.json",
        description="站内信 L0 公开广播 JSON URL。",
    )
    inbox_api_url: str = Field(
        default="https://mclaw-admin-api.fzstack.com",
        description="Mclaw Platform L1 客户端 API base URL。",
    )
    ...
    updater_policy_endpoint: str = Field(
        default="https://mclaw-admin-api.fzstack.com/updater",
        description="在线升级策略层 endpoint base URL，setup-center/updater 可按需使用。",
    )

    # === Mclaw Platform (Agent Hub / Skill Store) ===
    hub_enabled: bool = Field(
        default=False,
        description="启用 Mclaw Platform 连接（Agent Hub / Skill Store）。关闭时不注册远程市场工具。",
    )
    ...
    # Controls whether the v2 runtime under src/mclaw/runtime/
    ...
    runtime_v2_enabled: bool = Field(
        default=True,
        description=(
            "Master gate for the v2 runtime facade (src/mclaw/runtime/) "
    ...
    @property
    def mclaw_home(self) -> Path:
        """用户数据根目录，优先使用 MCLAW_ROOT 环境变量，默认 ~/.mclaw"""
        import os
    ...
    def user_workspace_path(self) -> Path:
        ...
        如果 project_root 位于 mclaw_home/workspaces/ 下（生产模式），
        直接使用 project_root 作为工作区路径；否则（开发模式）回退到 default。
        """
        ws_dir = self.mclaw_home / "workspaces"
        try:
    ...
    def skills_path(self) -> Path:
        """用户技能安装目录 (~/.mclaw/workspaces/default/skills)
    ...
    def mcp_builtin_path(self) -> Path:
        ...
        若不存在则回退到 wheel 打包位置 site-packages/mclaw/builtin_mcps/。
        """
