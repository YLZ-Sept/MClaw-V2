...
import mclaw._ensure_utf8  # noqa: F401  # isort: skip
...
# ``python -m mclaw.main`` executes this file as ``__main__``. Register that
...
if __name__ == "__main__" and getattr(_executed_main, "__dict__", None) is globals():
    sys.modules["mclaw.main"] = _executed_main
...
app = typer.Typer(
    name="mclaw",
    help="Mclaw - 全能自进化AI助手",
    add_completion=False,
)

# Sub-app: ``mclaw plugins ...`` (see ``cli/plugins_cmd.py``).
...
async def _init_orchestrator():
    ...
    if _orchestrator is not None:
        return
    from mclaw.agents.orchestrator import AgentOrchestrator
...
def _ensure_channel_deps(progress_fn=None) -> dict:
    ...
    """
    from mclaw.runtime_channel_deps import ensure_channel_dependencies
...
def _set_im_bot_runtime_state(
    ...
    from mclaw.channels.runtime_status import set_bot_runtime_state

    set_bot_runtime_state(channel_name, status, error, progress)

    try:
        from mclaw.api.routes.websocket import broadcast_event
...
def get_im_bot_runtime_state(channel_name: str) -> dict:
    ...
    from mclaw.channels.runtime_status import resolve_bot_runtime_state
...
async def remove_im_bot(bot_cfg: dict) -> bool:
    ...
    if _message_gateway is None:
        return False
    channel_name = _bot_channel_name(bot_cfg)
    from mclaw.channels.runtime_status import clear_bot_runtime_state
...
async def init_core_services(agent_or_master):
    ...
    if _desktop_pool is None:
        from mclaw.agents.factory import AgentFactory, AgentInstancePool
...
def main(
    ...
    cwd: str | None = typer.Option(
        None,
        "--cwd",
        help="会话工作目录（默认使用启动 Mclaw 时的当前目录）",
    ),
):
    """
    Mclaw - 全能自进化AI助手
    ...
    if version:
        ...
        console.print(f"Mclaw v{__version__}")
        raise typer.Exit(0)
    ...
    if ctx.invoked_subcommand is None:
        ...
        if not has_endpoint:
            ...
            console.print(
                "请设置 ANTHROPIC_API_KEY，或运行 'mclaw init' 配置 data/llm_endpoints.json"
            )
            raise typer.Exit(1)

        # C14 / R4-8: 交互模式需要 TTY 来驱动 prompt_toolkit + Rich 的
        # security_confirm 提示。stdin 为管道时（``cat ... | mclaw`` /
        ...
        if cli_class.is_unattended:
            ...
            console.print(
                ...
                '  • [bold]mclaw run "<task>"[/bold] - 单次任务执行（unattended）\n'
                "  • [bold]mclaw serve[/bold] - 启动 API 服务并通过 /api/chat 调用"
            )
            raise typer.Exit(1)
...
def run(
    ...
    async def _run():
        ...
        # C14 / R4-8: ``mclaw run`` 是一次性非交互入口 — 即使 stdin
        ...
        # 桌面通知
        from mclaw.agent.desktop_notify import notify_task_completed
...
def stop(
    ...
    Sprint 14 / v31 Phase A 配套 CLI：用 ``mclaw stop`` 取代手敲
...
def prompt_debug(
    ...
    async def _debug():
        ...
        if compiled:
            ...
            # 使用全文版本
            from mclaw.agent.identity import Identity
...
def serve(
    ...
    # 用 Path.cwd() 作 fallback 仅在 CLI 用户从其它目录跑 `mclaw serve`
    ...
    def _write_heartbeat():
        ...
        try:
            ...
            from mclaw import __git_hash__, __version__
    ...
    async def _serve():
        ...
        logger.info(f"Mclaw {_version_str} starting...")
        try:
        ...
        try:
            ...
            from mclaw.api.host_resolution import resolve_api_host
            from mclaw.api.server import API_PORT, start_api_server
            ...
            try:
                from mclaw.api.server import update_runtime_refs
        ...
        if dev:
            ...
            async def _file_watcher():
                try:
                    ...
                    src_dir = Path(__file__).resolve().parent  # src/mclaw/
    ...
    while first_run or cfg._restart_requested:
        ...
        if cfg._restart_requested:
            ...
            try:
                from mclaw.runtime_env import inject_module_paths_runtime
            ...
            try:
                ...
                from mclaw.api.host_resolution import resolve_api_host
                from mclaw.api.server import API_PORT, wait_for_port_free
...
def plugin_scaffold(
    ...
    manifest: dict = {
        ...
        "type": "python",
        "entry": "plugin.py",
        "description": f"Mclaw plugin: {name}",
...
<head>
  ...
  <script type="module">
    // Replace with: import {{ PluginBridge }} from "@mclaw/plugin-ui-sdk";
...
"""
    ...
    console.print(f"  Next: copy to data/plugins/{name}/ and restart Mclaw")
...
def run_mcp_module(
    ...
    """
    if not module_path.startswith("mclaw."):
        print(f"Error: only mclaw.* modules allowed, got: {module_path}", file=sys.stderr)
        raise typer.Exit(1)
...
def reset_password(
    ...
    if backend_running:
        console.print(
            "[yellow]⚠ 检测到 mclaw backend 仍在运行。请手动重启 backend "
            "（关闭 Setup Center / kill 进程后重新启动），新的 setup 流程才会生效。[/yellow]"
        )
    else:
        console.print("[dim]下次启动 mclaw 时会进入 setup 流程。[/dim]")
