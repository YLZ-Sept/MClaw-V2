...
:data:`mclaw.tools.handlers.default_handler_registry` the v1 chat path
...
from mclaw.runtime.execution_context import (
...
# ``src/mclaw/orgs/tool_handler.py`` therefore landed INSIDE the source
...
# resolves into the Mclaw source tree (or the VCS dir), anchored on the
...
def _guarded_source_roots() -> tuple[Path, ...]:
    ...
    Anchored on ``mclaw.__file__`` so the source tree is protected even
    when the process CWD differs from the repo root.
    """
    try:
        import mclaw

        pkg_dir = Path(mclaw.__file__).resolve().parent  # .../src/mclaw
        ...
        for root in roots:
            try:
                if resolved == root or resolved.is_relative_to(root):
                    return (
                        f"[拒绝写入：{tool_name} 的目标路径 '{raw}' 落在 Mclaw 工程源码"
...
def _flatten_external_tools(entries: Iterable[str] | None) -> set[str]:
    ...
    Mirrors :func:`mclaw.orgs._runtime_tool_categories.expand_tool_categories`
...
def resolve_node_tools(
    *,
    ...
    from mclaw.tools.definitions import get_tool_definition
...
async def execute_node_tool(
    ...
    * ``is_error=True`` -- the handler raised or no handler was mapped.
    ...
    from mclaw.llm.converters.tools import PARSE_ERROR_KEY
    ...
    # Mclaw source tree (the "stray tool_handler.py" pollution incident).
    ...
    from mclaw.tools.tool_hints import ToolConfigError
    ...
    idempotency_param = (
        str(tool_definition.get("x-mclaw-idempotency-param") or "").strip()
        if tool_definition
        else ""
    )
    ...
    try:
        if tool_host is not None:
            ...
            from mclaw.tools.handlers import default_handler_registry
