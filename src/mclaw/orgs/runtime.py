...
subsystems. The v1 ``src/mclaw/orgs/runtime.py`` is 6 355
    ...
    **Composes** (DI via ``__init__``) the 6 reused Protocols
    ...
    def __init__(
        ...
        *,
        ...
        # managers are reachable via ``mclaw.orgs``
    ...
    async def _stream_event_tap(self, event_name: str, payload: dict[str, Any]) -> None:
        ...
        lazily because ``mclaw.runtime`` pulls a chunk of the
    ...
    async def _maybe_render_root_pdf(
        ...
        *,
        ...
        rendered = await render_markdown_to_pdf(
            ...
            out_path=pdf_path,
            title="任务交付报告",
            meta=f"由根节点 {node_id} 汇总交付 · Mclaw 组织编排",
        )
        if not rendered:
            return
        done.add(command_id)
