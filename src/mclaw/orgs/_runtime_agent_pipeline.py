...
contract; concrete factories live in ``mclaw.agents``
and get wired by the runtime composition root (P9.6i).

ADR-0012 (no-shim): zero ``mclaw.orgs`` imports; v2
...
dispatch_depth_var: ContextVar[int] = ContextVar("mclaw_orgs_v2_dispatch_depth", default=0)
...
current_command_id_var: ContextVar[str] = ContextVar("mclaw_orgs_v2_command_id", default="")
...
current_chain_id_var: ContextVar[str] = ContextVar("mclaw_orgs_v2_chain_id", default="")
...
class AgentSpec:
    ...
    ``mclaw.agents`` at the type level. The builder
...
class AgentBuilderProtocol(Protocol):
    ...
    Implementations in ``mclaw.agents.factory`` /
    ``mclaw.agent`` wire concrete Agent / Brain
