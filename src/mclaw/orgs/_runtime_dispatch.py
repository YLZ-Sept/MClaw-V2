...
# dispatch module is imported very early (before ``mclaw.config.settings``
...
def _resolve_delegation_log_dir() -> Path | None:
    """Locate ``data/delegation_logs/`` without forcing ``mclaw.config``
    import at module load time."""

    try:
        from mclaw.config import settings  # local import: avoid cycle
