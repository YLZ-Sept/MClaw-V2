...
:data:`mclaw.tools.handlers.default_handler_registry`, expecting
...
* The handler registry still comes from the main Agent, but org node file
  ...
  :mod:`mclaw.orgs._runtime_node_tools`. They therefore do not inherit the
        ...
        if isinstance(agent_tools, list):
            for defn in agent_tools:
                if isinstance(defn, dict) and defn.get("name") == tool_name:
                    ...
                    execution = defn.get("x-mclaw-execution")
                    if isinstance(execution, dict):
                        resolved["x-mclaw-execution"] = dict(execution)
                    idempotency_param = defn.get("x-mclaw-idempotency-param")
                    if isinstance(idempotency_param, str) and idempotency_param.strip():
                        resolved["x-mclaw-idempotency-param"] = idempotency_param.strip()
                    media_contract = defn.get("x-mclaw-media-contract")
                    if isinstance(media_contract, dict):
                        resolved["x-mclaw-media-contract"] = dict(media_contract)
                    return resolved
