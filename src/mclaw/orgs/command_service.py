...
Replaces v1 ``mclaw.orgs.command_service.OrgCommandService``
...
:class:`mclaw.orgs.node_scheduler.CommandDispatcher`
...
3. **Sprint-9 supervisor HTTP takeover**: every command now runs
   through :class:`mclaw.runtime.supervisor.Supervisor` built
   by :func:`mclaw.runtime.supervisor_factory.build_supervisor_for_command`.
   The Sprint-5 wall-clock ``_watchdog_loop`` is gone -- stall
   detection is now LLM-evaluated by the supervisor's
   :class:`~mclaw.runtime.stall_detector.StallDetector` on
   :class:`~mclaw.runtime.ledger.ProgressLedger` signals.
   Cancellation is cooperative through the supervisor's
   :class:`~mclaw.runtime.cancel_token.CancellationToken`.
...
if TYPE_CHECKING:  # pragma: no cover -- import-cycle break
    from mclaw.runtime.supervisor import Supervisor
...
class ChannelGatewayProtocol(Protocol):
    ...
    Replaces v1 ``from mclaw.main import get_message_gateway``
...
class OrgCommandService:
    ...
    def _build_supervisor(
        ...
        *,
        ...
        :func:`mclaw.runtime.supervisor_factory.build_supervisor_for_command`
        ...
        if executor is None and self._supervisor_factory is None:
            ...
            from mclaw.runtime.checkpoint import MemoryCheckpointer
            from mclaw.runtime.supervisor import DelegationResult
    ...
    def _build_supervisor_llm_client(self, org_id: str) -> Any | None:
        ...
        wrap the process-shared :func:`mclaw.llm.client.get_default_client`
        in :class:`~mclaw.runtime.llm_supervisor_client.GatewaySupervisorLLMClient`,
        ...
        try:
            ...
            from mclaw.config import settings
            from mclaw.llm.client import get_default_client
            from mclaw.runtime.llm_supervisor_client import (
    ...
    def _build_node_directory(self, org_id: str) -> list[Any] | None:
        ...
        :class:`~mclaw.orgs.org_models.OrgNode` to a
        :class:`~mclaw.runtime.llm_supervisor_brain.NodeDescriptor`
        ...
        """
        try:
            from mclaw.runtime.llm_supervisor_brain import NodeDescriptor
    ...
    def _require_org_running(self, org_id: str):  # noqa: ANN202 -- duck-typed
        ...
        # v1 imports OrgStatus from mclaw.orgs.models; v2 stays
    ...
    def _schedule_run(
        ...
        *,
        ...
        task = loop.create_task(_run(), name=f"mclaw-orgs-supervisor-{command_id}")
    ...
    async def _run_supervisor_with_hard_ceiling(
        ...
        run_task: asyncio.Task[Any] = loop.create_task(
            supervisor.run(),
            name=f"mclaw-orgs-supervisor-run-{command_id}",
        )
        ...
        if 0.0 < soft < float(ceiling):
            watchdog = loop.create_task(
                ...
                name=f"mclaw-orgs-supervisor-soft-watchdog-{command_id}",
            )
        try:
            try:
                ...
                try:
                    from mclaw.runtime.supervisor import (
                        FinalOutcome,
                        SupervisorOutcome,
                    )
    ...
    def _is_bare_cancel_outcome(outcome: Any) -> bool:
        ...
        *that* shape into a best-effort soft landing -- a run that reached a real
        ...
        """
        try:
            from mclaw.runtime.supervisor import FinalOutcome
    ...
    def _build_soft_landing_outcome(
        ...
        """
        from mclaw.runtime.supervisor import FinalOutcome, SupervisorOutcome
    ...
    def _hard_ceiling_seconds(self, org_id: str | None = None) -> int:
        ...
        Lazy import keeps the ``mclaw.orgs`` <-> ``mclaw.config``
    ...
    async def start_reconcile_loop(self) -> None:
        ...
        self._reconcile_task = loop.create_task(
            ...
            name="mclaw-orgs-reconcile-loop",
        )
    ...
    def _cancel_drain_budget_s() -> int:
        ...
        ``mclaw.orgs`` <-> ``mclaw.config`` cycle loose and a
