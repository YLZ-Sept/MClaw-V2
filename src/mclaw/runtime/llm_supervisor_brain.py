...
--------------------
The dual-ledger :class:`~mclaw.runtime.supervisor.Supervisor` skeleton
(outer/inner loop, :class:`~mclaw.runtime.stall_detector.StallDetector`,
...
``mclaw.agent.supervisor_brain`` (``Degenerate`` / ``PassThrough``),
...
* ``mclaw.runtime.supervisor.SupervisorBrain`` -- the *orchestration*
  protocol the :class:`Supervisor` actually depends on:
  ``extract_facts`` / ``draft_plan`` / ``emit_progress_ledger``.
* ``mclaw.agent.brain`` -- a *gateway* protocol (``think_lightweight`` /
...
from mclaw.runtime.supervisor import DelegationResult, SupervisorBrain
...
# The PROGRESS_LEDGER schema is reordered to match Mclaw's strict
