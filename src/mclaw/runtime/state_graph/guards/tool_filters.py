...
if TYPE_CHECKING:
    from mclaw.agent.permission import Ruleset as PermissionRuleset
...
def get_mode_ruleset(mode: str) -> PermissionRuleset:
    ...
    from mclaw.agent.permission import (
        ASK_MODE_RULESET,
...
def filter_tools_by_mode(tools: list[dict], mode: str) -> list[dict]:
    ...
    if mode == "agent" or not tools:
        return tools
    from mclaw.agent.permission import disabled as permission_disabled
