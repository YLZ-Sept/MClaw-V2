...
---------------------------------------------------------------
The orchestration brain (:class:`~mclaw.runtime.llm_supervisor_brain.LLMSupervisorBrain`)
depends only on the narrow :class:`~mclaw.runtime.llm_supervisor_brain.SupervisorLLMClient`
...
from mclaw.llm.types import Message
...
class GatewaySupervisorLLMClient:
    """Adapt the gateway :class:`~mclaw.llm.client.LLMClient` to the narrow
    ``SupervisorLLMClient`` ``complete`` seam.

    Args:
        client: the gateway LLM client (typically
            :func:`mclaw.llm.client.get_default_client`).
