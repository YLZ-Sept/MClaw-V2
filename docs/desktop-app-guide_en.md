# Mclaw Desktop Terminal - User Guide & Feature Overview

> Mclaw Desktop Terminal — an all-in-one desktop client integrating AI chat, IM channel monitoring, skill management, system status, and step-by-step configuration.
...
Mclaw Desktop Terminal is built with **Tauri 2 + React 18**, connecting to the `mclaw serve` backend via HTTP SSE. It supports two operating modes:

- **Local mode**: The desktop app starts the local backend service automatically (requires completing the configuration wizard)
- **Remote mode**: Connect to a running `mclaw serve` instance (ideal for development, debugging, or server deployment)
...
2. **Start Service**: Click "Start Service" in the topbar, or run `mclaw serve` in terminal
...
Skills are extensible capability modules for Mclaw, following the Agent Skills standard.
...
1. **Remote debugging**: Run `mclaw serve` in terminal, then click "Connect" in the desktop app. Python code changes take effect immediately (no recompilation needed)
...
*This document is based on Mclaw v1.10.2, last updated: 2026-02-12*
