...
> 本文是 Mclaw Security Architecture v2 重构（plan: `security_architecture_v2_31fbf920.plan.md`）的 **Commit 0 调研落档**。
>
...
Mclaw 当前的安全/权限决策代码分散在 4 处，互相**并行运行**而不是串联：

1. **`src/mclaw/core/policy.py`**（1783 行，含 7 种 confirmation 模式逻辑）— `PolicyEngine`：zone 矩阵 + shell pattern + checkpoint + sandbox + ui_confirm + audit + death_switch + user_allowlist
2. **`src/mclaw/core/permission.py`**（OpenCode 风格）— `Ruleset` + `PLAN_MODE_RULESET` + `ASK_MODE_RULESET` + `COORDINATOR_MODE_RULESET` + `disabled()` + `EDIT_TOOLS` + `READ_TOOLS`
3. **`src/mclaw/core/agent.py:RiskGate`** — trusted-path skip + trust-mode skip + historical session authorization + `classify_risk_intent`（pre-LLM 层）
4. **`src/mclaw/core/reasoning_engine.py`** + **`tool_executor.py`** — 双重检查 `policy_engine.assert_tool_allowed` + `check_permission`
...
**文件**：[`src/mclaw/core/tool_executor.py:804-846`](../src/mclaw/core/tool_executor.py)
...
**文件**：[`src/mclaw/tools/handlers/mode.py:18-46`](../src/mclaw/tools/handlers/mode.py)
...
**问题**：[`src/mclaw/sessions/session.py`](../src/mclaw/sessions/session.py) 的 `Session` dataclass **没有** `mode` 字段。`hasattr(session, "mode")` 永远是 False，工具静默失败。
...
**文件**：[`src/mclaw/core/reasoning_engine.py:4390-4398, 4780-4813`](../src/mclaw/core/reasoning_engine.py)
...
**现象**：IM 前缀的会话遇到 confirm 时，reasoning_engine 直接报错"需要桌面确认"结束，**永远不 yield `security_confirm` 事件**，导致 [`gateway._handle_im_security_confirm`](../src/mclaw/channels/gateway.py) 永远收不到事件 → IM 卡片确认链路实际不工作。
...
**文件**：[`src/mclaw/core/trusted_paths.py`](../src/mclaw/core/trusted_paths.py)
...
**文件**：[`src/mclaw/api/routes/config.py:write_security_config`](../src/mclaw/api/routes/config.py)
...
> 这是 ApprovalClassifier 第 1 步"工具自身 metadata"的权威源数据。Commit 2 在 [`agent.py:_init_handlers`](../src/mclaw/core/agent.py) 集中处把这些值通过 `tool_classes={...}` 注入。
>
...
新工具属于现有 handler（如 filesystem）→ 在 [`core/agent.py:_init_handlers`](../src/mclaw/core/agent.py) 找到对应 `register(...)` 调用，加 `tool_classes={...}`：
...
如 `write_file` 写 workspace 内/外是不同风险。修改 [`policy_v2/classifier.py:_refine_with_params`](../src/mclaw/core/policy_v2/classifier.py) 加分支：
...
**注意**：[`src/mclaw/channels/policy.py`](../src/mclaw/channels/policy.py)（IM 群组 ACL，含 `GroupPolicyConfig`）是**完全不同的文件**，**不动**。Commit 8 在删 `core/policy.py` 主体时严禁误删。
...
[`agent.py:126`](../src/mclaw/core/agent.py) + [`api/routes/chat.py:22`](../src/mclaw/api/routes/chat.py) import 其 `consume_session_trust` / `is_trusted_workspace_path` / `grant_session_trust`。Commit 8 修内部 bug + 加"过期真删除"逻辑，接口不变。

### 5.5 `core/risk_intent.py` 不删（保留独立模块）

[`agent.py:116, 807`](../src/mclaw/core/agent.py) + [`chat.py:220-224`](../src/mclaw/api/routes/chat.py) import 其 `RiskIntentResult`, `RiskLevel`, `TargetKind`, `classify_risk_intent`, `derive_authorized_intent`。Commit 5 让 RiskGate 调用 `evaluate_message_intent` 而不是这些函数，但 risk_intent.py 本身保留（仍是分类源）。

---

## 6. 外部依赖图（哪些文件 import 了即将重构的模块）

### 6.1 `from mclaw.core.policy import` / `from .policy import`

| 文件 | 行号 | 符号 |
|---|---|---|
| [`api/routes/chat.py`](../src/mclaw/api/routes/chat.py) | 399-401 | `get_policy_engine` → cleanup_session |
| [`api/routes/config.py`](../src/mclaw/api/routes/config.py) | 1466, 1478, 1516, 1528, 1559, 1605, 1618-1639, 1708-1710, 1789, 1819-1821, 1869 | **`reset_policy_engine`, `_default_forbidden_paths`, `_default_protected_paths`, `_DEFAULT_BLOCKED_COMMANDS`, `get_policy_engine`** |
| [`channels/adapters/feishu.py`](../src/mclaw/channels/adapters/feishu.py) | 1090-1092 | `get_policy_engine` → resolve_ui_confirm |
| [`channels/adapters/telegram.py`](../src/mclaw/channels/adapters/telegram.py) | 698-700 | 同上 |
| [`channels/gateway.py`](../src/mclaw/channels/gateway.py) | 4696-4703 | `get_policy_engine` (IM streaming) |
| [`cli/stream_renderer.py`](../src/mclaw/cli/stream_renderer.py) | 303-306 | `get_policy_engine` (CLI confirm) |
| [`core/agent.py`](../src/mclaw/core/agent.py) | 861-863, 2412-2414, 5709-5711 | `get_policy_engine` |
| [`core/audit_logger.py`](../src/mclaw/core/audit_logger.py) | 111-113 | **`get_policy_engine` 在 init 时调用**（启动顺序关键）|
| [`core/checkpoint.py`](../src/mclaw/core/checkpoint.py) | 248-250 | 同上 |
| [`core/permission.py`](../src/mclaw/core/permission.py) | 295-297 | `get_policy_engine` |
| [`core/reasoning_engine.py`](../src/mclaw/core/reasoning_engine.py) | 4380-4383, 4738-4742 | `PolicyDecision`, `PolicyResult`, `get_policy_engine`, `assert_tool_allowed`, `wait_for_ui_resolution` |
| [`core/security_actions.py`](../src/mclaw/core/security_actions.py) | 11-13, 18-27, 38-42, 53-55 | `get_policy_engine` |
| [`core/tool_executor.py`](../src/mclaw/core/tool_executor.py) | 805-810 | `get_policy_engine`, `mark_confirmed` |
| [`tools/handlers/skills.py`](../src/mclaw/tools/handlers/skills.py) | 289-291, 820-822, 908-910, 986-988 | `get_policy_engine` (skill tool allowlists) |

### 6.2 `from mclaw.core.permission import`

| 文件 | 行号 | 符号 |
|---|---|---|
| [`core/reasoning_engine.py`](../src/mclaw/core/reasoning_engine.py) | 293-307 | Ruleset 相关 + mode ruleset helpers |
| [`core/tool_executor.py`](../src/mclaw/core/tool_executor.py) | 24, 1176-1181 | `PermissionDecision`, `check_permission` |
...
### 6.3 `from mclaw.core.security_actions import`

| 文件 | 行号 | 符号 |
|---|---|---|
| [`api/routes/config.py`](../src/mclaw/api/routes/config.py) | 1085, 1102, 1723-1727, 1884-1920 | allowlist helpers, death-switch wiring |
| [`api/routes/chat.py`](../src/mclaw/api/routes/chat.py) | 21, 263-268 | `execute_controlled_action`, `maybe_broadcast_death_switch_reset`, `maybe_refresh_skills` |

### 6.4 `from mclaw.core.trusted_paths import`

| 文件 | 行号 | 符号 |
|---|---|---|
| [`core/agent.py`](../src/mclaw/core/agent.py) | 126 | `consume_session_trust`, `is_trusted_workspace_path` |
| [`api/routes/chat.py`](../src/mclaw/api/routes/chat.py) | 22, 162-165 | `grant_session_trust` |

### 6.5 `from mclaw.core.risk_intent import`

| 文件 | 行号 | 符号 |
|---|---|---|
| [`core/agent.py`](../src/mclaw/core/agent.py) | 116, 807 | `RiskIntentResult`, `RiskLevel`, `TargetKind`, `classify_risk_intent`, `AuthorizedIntent` (lazy) |
| [`api/routes/chat.py`](../src/mclaw/api/routes/chat.py) | 220-224 | `derive_authorized_intent` (feature flag) |

### 6.6 `orgs/runtime.py` patches `execute_tool_with_policy`

[`orgs/runtime.py`](../src/mclaw/orgs/runtime.py) 对 [`tool_executor.execute_tool_with_policy`](../src/mclaw/core/tool_executor.py) 做了 monkey-patch（org 委派路径）。Commit 4 必须保持 `execute_tool_with_policy` 的**函数签名 + 异常类型**完全不变，否则 org 委派挂掉。
...
> [`tools/tool_hints.py`](../src/mclaw/tools/tool_hints.py)），用于把 handler 抛出的
...
**v2 新 schema 见 plan §7**。迁移规则（在 [`policy_v2/loader.py`](../src/mclaw/core/policy_v2/loader.py) 实现）：
...
|---|---|---|
| Telegram | [`channels/adapters/telegram.py`](../src/mclaw/channels/adapters/telegram.py) | 部分（`OWNER_USER_ID` env） |
| Feishu | [`channels/adapters/feishu.py`](../src/mclaw/channels/adapters/feishu.py) | 部分 |
| DingTalk | [`channels/adapters/dingtalk.py`](../src/mclaw/channels/adapters/dingtalk.py) | 缺失 |
| WeWork (WS) | [`channels/adapters/wework_ws.py`](../src/mclaw/channels/adapters/wework_ws.py) | 缺失 |
| WeChat | [`channels/adapters/wechat.py`](../src/mclaw/channels/adapters/wechat.py) | 缺失 |
| QQ Official | [`channels/adapters/qq_official.py`](../src/mclaw/channels/adapters/qq_official.py) | 缺失 |
| OneBot | [`channels/adapters/onebot.py`](../src/mclaw/channels/adapters/onebot.py) | 缺失 |
...
[`api/routes/im.py`](../src/mclaw/api/routes/im.py) 的 `_GROUP_POLICY_PATH = data/sessions/group_policy.json` 是**独立的 IM 群组级 ACL**（"哪些群能用哪个 mode"），与 `owner_only` 是**AND 关系**：
...
**所有 handler 注册在 [`core/agent.py:2215-2331`](../src/mclaw/core/agent.py) 的 `_init_handlers()` 一处**，30+ 个 `registry.register("name", create_xxx_handler(self))` 调用集中：
...
1. [`tools/handlers/__init__.py`](../src/mclaw/tools/handlers/__init__.py) `SystemHandlerRegistry.register()` 加 `tool_classes: dict[str, ApprovalClass]` 可选参数
2. [`core/agent.py:_init_handlers`](../src/mclaw/core/agent.py) 集中处的 30+ 个 register 调用补 `tool_classes={...}`（按 §4 表填）
...
| `.mclaw/system_tasks.lock` | **新增**（C15）| SYSTEM_TASKS.yaml hash 校验 | §17.2 防篡改 |
...
AI coding agent（含我自己）在 Mclaw 后续迭代中会频繁加内置工具。如果新工具不在 ApprovalClass 体系里：
...
**位置**：`src/mclaw/tools/handlers/__init__.py:register()` 内（修改）
...
**位置**：所有 `src/mclaw/tools/handlers/*.py`（30+ 文件）顶部统一加 6 行注释块。
...
```mdc
---
description: 新增/修改 Mclaw 内置工具时的 ApprovalClass 规范
globs:
  - "src/mclaw/tools/handlers/**/*.py"
  - "src/mclaw/core/agent.py"
  - "src/mclaw/core/policy_v2/classifier.py"
alwaysApply: false
---
...
最小改动（推荐方案 B）：
在 src/mclaw/core/agent.py 的 _init_handlers 里找到对应 register()，
给 tool_classes={} 加一行：
...
| pre-commit hook 跑 completeness test | Mclaw 默认无 pre-commit，避免增加新依赖；CI red 已足够 |
...
| `src/mclaw/tools/handlers/__init__.py` | 改 `register()` 加 WARN 逻辑 | +20 |
| `src/mclaw/core/policy_v2/classifier.py` | 加 `classify_with_source()` 公开方法（返回 source 字段）| +15 |
| `src/mclaw/tools/handlers/*.py` | 30+ 文件统一加 6 行 docstring 块（脚本批量）| +180（30×6）|
...
- **测试结果**：N/A（仅文档）
- **手测验证**：N/A
- **下一步**：C1 创建 `src/mclaw/core/policy_v2/` 目录结构

#### C1 实施记录

- **完成日期**：2026-05-13
- **实际修改文件**（7 个新文件，零 v1 改动）：
  - `src/mclaw/core/policy_v2/__init__.py` (+71)
  - `src/mclaw/core/policy_v2/enums.py` (+118)
  - `src/mclaw/core/policy_v2/exceptions.py` (+98)
  - `src/mclaw/core/policy_v2/models.py` (+114)
  - `src/mclaw/core/policy_v2/context.py` (+185)
  - `src/mclaw/core/policy_v2/matrix.py` (+170)
...
- **测试结果**：
  ...
  - `ruff check src/mclaw/core/policy_v2/ tests/unit/test_policy_v2_skeleton.py`：clean
  - 手动 import smoke：`from mclaw.core import policy` + 5 个私有/公共符号导入正常
...
- **实际修改文件**（4 个，1 新增 + 3 改动）：
  - `src/mclaw/core/policy_v2/classifier.py` (新增, +280)
  - `src/mclaw/core/policy_v2/enums.py` (+50：`strictness()` + `most_strict()` + `_STRICTNESS_ORDER`)
  - `src/mclaw/core/policy_v2/__init__.py` (+5：导出 `ApprovalClassifier` / `strictness` / `most_strict`)
  - `src/mclaw/tools/handlers/__init__.py` (+85 -2：`register(tool_classes=)` 参数 + `_collect_tool_classes` + `get_tool_class` + `_tool_classes` 字典 + `unregister/unmap_tool` 同步清理)
...
**2. 架构**：
- 无循环依赖（实测 `import mclaw.tools.handlers` 不触发 `policy_v2` 加载，lazy import 在 `register()` 内）
...
- **classifier cache 不自动随 registry mutation 失效**。Mclaw 启动时一次注册，运行时不变 → 实际无影响。plugin 动态注册时必须显式调 `classifier.invalidate(tool)`。C10 plugin 接入时设计自动同步机制（registry 触发 hook 通知 classifier）。
...
- **实际修改/新增文件**（9 个，5 新增 + 4 改动）：
  - `src/mclaw/core/policy_v2/zones.py`（新增, +71）：`is_inside_workspace` 从 `classifier.py` 提升为公共 API；新增 `candidate_path_fields` / `all_paths_inside_workspace` 复用给 engine + classifier
  - `src/mclaw/core/policy_v2/shell_risk.py`（新增, +205）：`ShellRiskLevel` enum + 迁移 v1 的 CRITICAL/HIGH/MEDIUM patterns + DEFAULT_BLOCKED_COMMANDS + `classify_shell_command` 纯函数（支持 user-supplied extra/excluded）
  - `src/mclaw/core/policy_v2/engine.py`（新增, +475）：`PolicyEngineV2` 类、双入口 (`evaluate_tool_call` / `evaluate_message_intent`)、12 步决策链、fail-safe try/except、threading.RLock、stats 计数器、audit_hook 钩子、C5/C6/C8/C12 stub 私有方法
  - `src/mclaw/core/policy_v2/models.py`（+15）：`PolicyDecisionV2` 加 `shell_risk_level` / `needs_sandbox` / `needs_checkpoint` 三字段（R2-5）；`to_audit_dict` 同步
  - `src/mclaw/core/policy_v2/classifier.py`（+82 -32）：新增 `ClassificationResult` dataclass + `classify_full()` 富信息入口；`_refine_with_params_full` 接入 shell_risk + zones 公共 API；`_is_inside_workspace` 改为 zones 的 backward-compat alias（不破 C2 测试）
  - `src/mclaw/core/policy_v2/__init__.py`（+15）：导出 `PolicyEngineV2` / `ClassificationResult` / `ShellRiskLevel` / `classify_shell_command` / `DEFAULT_BLOCKED_COMMANDS` / `is_inside_workspace` 等
...
新增文件：
- `src/mclaw/core/policy_v2/schema.py`（249 行）：13 个 Pydantic v2 模型
...
- `src/mclaw/core/policy_v2/migration.py`（299 行）：v1→v2 纯函数迁移
...
- `src/mclaw/core/policy_v2/loader.py`（173 行）：YAML I/O + pipeline 编排
...
修改文件：
- `src/mclaw/core/policy_v2/__init__.py`：新增 schema / loader / migration 导出，
...
| 文件 | 变更 | 行数 ± |
|---|---|---|
| `src/mclaw/core/policy_v2/context.py` | + `ReplayAuthorization` / `TrustedPathOverride` frozen dataclass + `_coerce_replay_auths` / `_coerce_trusted_paths` + `__post_init__` enum 归一 + `user_message` 字段 | +130 |
| `src/mclaw/core/policy_v2/classifier.py` | + `shell_risk_config` 构造参数 + `_shell_risk_enabled()` / `_classify_shell_with_customs()` | +30 |
| `src/mclaw/core/policy_v2/engine.py` | + `config: PolicyConfigV2` 构造参数 + `_apply_class_override` / `_collect_immune_paths` + 实装 `_check_replay_authorization` / `_check_trusted_path` / `_handle_unattended` 5 策略 + `_infer_operation_from_tool` + `build_engine_from_config` 工厂 | +250 |
| `src/mclaw/core/policy_v2/__init__.py` | export `ReplayAuthorization` / `TrustedPathOverride` / `build_engine_from_config` | +6 |
...
C6 把 Mclaw 的 **决策路径** 从 v1 PolicyEngine 切到 PolicyEngineV2，
**UI 状态机**（`store_ui_pending` / `wait_for_ui_resolution` / `readonly_mode` 等）
仍留 v1 实例（待 C9 SecurityView 重建一并迁移）。

| 文件 | 改动 | LOC |
|---|---|---|
| `src/mclaw/core/policy_v2/global_engine.py` | 新增：单例 + 延迟加载 + 线程安全 + rebuild API | +175 |
| `src/mclaw/core/policy_v2/adapter.py` | 新增：v2→v1 PolicyResult 翻译 + DEFER 降级 + fail-closed + ContextVar 优先 | +330 |
| `src/mclaw/core/policy_v2/__init__.py` | 导出新增 6 个符号 | +15 |
| `src/mclaw/core/policy_v2/classifier.py` | 新增 5 exact + 2 prefix heuristic（web_/news_ 等高频缺类工具） | +25 |
| `src/mclaw/core/policy_v2/engine.py` | 修复 `_path_under` 不识别 `/**` glob 锚定符的 C5 隐藏 bug | +35 |
| `src/mclaw/core/permission.py` | Step 2 `pe.assert_tool_allowed` → `evaluate_via_v2_to_v1_result` | ±10 |
| `src/mclaw/core/reasoning_engine.py` | 2 处 ReAct 决策切 v2（保留 `_pe.store_ui_pending` 等 UI helper） | ±10 |
...
**修复**（`src/mclaw/core/policy.py` `reset_policy_engine()`）：
...
**修复**（`src/mclaw/core/policy_v2/global_engine.py`）：
...
`mclaw run "..."` 一次性任务、`evolution/self_check.py` 自我修复循环）
...
- **#7 删除 src/mclaw/core/policy.py** 同样需要 C9 完成 `prepare_ui_confirm`
...
| `tests/unit/test_policy_engine_v2.py` | 边界 path test 改用 `/private_test_lab/ssh` / `D:/TestLab/Mclaw` 等合成路径，避免与 builtin `/etc/**` 冲突 |
...
- **#7 删除 `src/mclaw/core/policy.py`**：当前仍是 SSE 等待中枢
...
- `ruff check src/mclaw/core/`：基线干净
...
3. **`policy_v2/death_switch.py`** — `DeathSwitchTracker`（**module 级 singleton**）
   ...
   - `set_broadcast_hook(callable)` **解耦 v2→api 反向耦合**：v1 直接 `from mclaw.api.routes.websocket import broadcast_event`；v2 用 hook 注入（启动时由 api/routes/websocket 调一次）
...
4. **`config.py` × 8 处 callsite 迁移**：
   ...
   - 1 处 `from mclaw.core.policy import _default_forbidden_paths, _default_protected_paths` → `from mclaw.core.policy_v2.defaults import default_forbidden_paths, default_protected_paths`
   - 1 处 `from mclaw.core.policy import _DEFAULT_BLOCKED_COMMANDS` → `from mclaw.core.policy_v2.defaults import default_blocked_commands`
...
3. **`api/routes/config.py` 端点迁移**：
   - GET `/api/config/permission-mode`：去掉 `from mclaw.core.policy import get_policy_engine` + `getattr(pe, "_frontend_mode", "yolo")`；改用 `read_permission_mode_label()` 直读 v2。
...
C8b-6a 完成。**生产代码 0 处 `from mclaw.core.policy import` 或 `from .policy import`**（除 `policy_v2/adapter.py` 内 2 处延迟 import 等待 6b 一并删）。可进入 **C8b-6b（删 `policy.py` 整文件 + adapter.py 清理 + v1-only 测试文件删除）** —— 详见「C8b 粒度化执行计划 §F · C8b-6b」。
...
#### 2.1 `src/mclaw/core/policy.py`（整文件，~1607 LOC）
...
#### 2.2 `src/mclaw/core/policy_v2/adapter.py`（删 3 helper + 2 延迟 import）
...
- D3：全树（src/ + tests/ + scripts/）877 文件 0 处 `from mclaw.core.policy import` / `from ..core.policy import` / `from .policy import`（剔除 `channels/` 子树的合法 channels.policy）。
...
3. **audit 脚本字符串字面量自我误报**：`scripts/c8b6b_audit.py` 内自己包含字符串 `"from mclaw.core.policy import"`（用作匹配模式），D3 扫描时把自己当成残余 import。**经验**：自审计脚本在扫描时跳过自身（`if py.resolve() == Path(__file__).resolve(): continue`）。
...
- 测试代码 0 处 `from mclaw.core.policy import`；
...
**Skill 层（lookup wire）**
- `src/mclaw/skills/parser.py`：`SkillMetadata` 新增 `approval_class` 字段；新增 `_parse_approval_class()` 静态方法支持 canonical (`approval_class:`) + alias (`risk_class:`) + deprecation WARN + 非法值降级为 None。
- `src/mclaw/skills/registry.py`：`SkillEntry` 新增 `approval_class` 字段；新增 `get_exposed_tool_name()` 方法集中化"系统技能 / 外部技能"的 LLM-facing 工具名规则；新增 `SkillRegistry.get_tool_class()` lookup（系统技能按 `tool_name`、外部技能按 `skill_<safe-id>`）。

**Plugin 层（lookup wire + mutates_params manifest）**
- `src/mclaw/plugins/manifest.py`：`PluginManifest` 新增 `tool_classes: dict[str, str]` + `mutates_params: list[str]` 字段，配套 `_normalize_tool_classes` / `_normalize_mutates_params` 验证器（lowercase 归一、非法值过滤）。
- `src/mclaw/plugins/manager.py`：新增 `PluginManager.get_tool_class()` 多源取严 lookup + `plugin_allows_param_mutation()` 闸门 helper。

**MCP 层（lookup wire）**
- `src/mclaw/tools/mcp.py`：`MCPTool` 新增 `annotations: dict` 字段；`_discover_capabilities` 解析 MCP SDK 对象兼容 `BaseModel.model_dump` / `dict`；新增 `MCPClient._format_tool_name()` 单一规则源 + `MCPClient.get_tool_class()` lookup（识别 `approval_class` / `risk_class` 显式声明 + `destructiveHint`/`openWorldHint`/`readOnlyHint` MCP 协议 hints）。

**全局引擎 wire**
- `src/mclaw/core/policy_v2/global_engine.py`：`rebuild_engine_v2()` 新增 `skill_lookup` / `mcp_lookup` / `plugin_lookup` 3 个 kwarg；`_skill_lookup` / `_mcp_lookup` / `_plugin_lookup` 模块级缓存（保证 UI Save Settings 触发的 hot-reload 不会让 4 类来源退化到启发式分类——C7 二轮 audit 教训扩展到 4 类来源）。
- `src/mclaw/core/agent.py`：`_initialize` 末段新增 `rebuild_engine_v2(skill_lookup=..., mcp_lookup=..., plugin_lookup=...)` 调用；`_load_plugins` 末段新增 `tool_executor._plugin_manager = self._plugin_manager` wire。

**mutates_params 强制审计（R2-12 主载体）**
- `src/mclaw/core/policy_v2/param_mutation_audit.py`（**新增**, 235 LOC）：`ParamMutationAuditor` 类负责 `snapshot()` (deep-copy) → `evaluate()` (diff + allow/deny 决策) → `write()` (jsonl append + threading.Lock)；`_diff_recursive()` 递归 diff 算法（dict / list / 标量，emit `add` / `remove` / `modify` 操作）；`get_default_auditor()` 进程级单例 + `set_default_auditor()` 测试覆盖入口。
- `src/mclaw/core/tool_executor.py`：`_dispatch_hook` 对 `on_before_tool_use` 走专门 `_dispatch_before_tool_use_hook` 路径（snapshot → dispatch → diff → 收集 callback 的 `__plugin_id__` 候选 → 任一候选授权则保留 / 否则 `tool_input.clear() + update(snapshot)` 原地还原 → write jsonl）；新增 `_plugin_manager: Any = None` slot。
...
| `src/mclaw/tools/handlers/__init__.py` | 改 register() 加 WARN (D2) | +18 |
| `src/mclaw/tools/handlers/*.py` × 34 | 顶部 docstring 加 6-line checklist (D3) | +204 (6×34) |
...
| **L2 register WARN** | Mclaw 启动 | `SystemHandlerRegistry.register()` 在 `_collect_tool_classes` 后扫 `tool_names - self._tool_classes` → `logger.warning` | `test_register_warns_when_tool_lacks_explicit_approval_class` + 对照组 |
...
- **Ruff**：`ruff check src/mclaw/tools/handlers/__init__.py
...
| 文件 | 操作 | LOC |
|---|---|---|
| `src/mclaw/sessions/session.py` | 加 2 个一等字段 + to_dict/from_dict back-compat | +12 |
| `src/mclaw/core/policy_v2/context.py` | `from_session` 优先读一等字段，metadata 兜底 | +6 |
| `src/mclaw/core/pending_approvals.py` | 新建：`PendingApproval` + `PendingApprovalsStore` + 月度 archive + RLock + atomic write + event hook | +480 |
| `src/mclaw/core/tool_executor.py` | `_defer_unattended_confirm`（85 LOC）+ `_emit_tool_intent_previews` + `_sanitize_preview_params`（170 LOC）+ execute_batch 顶部 emit + `_run_one` 无 lying 重写 | +260 |
| `src/mclaw/core/agent.py` | `chat_with_tools` + `execute_task` 检测 `_deferred_approval_id` 抛 `DeferredApprovalRequired` | +30 |
| `src/mclaw/scheduler/task.py` | `TaskStatus.AWAITING_APPROVAL` + 转换表 + `mark_awaiting_approval()` | +35 |
| `src/mclaw/scheduler/executor.py` | ContextVar 安装 unattended PolicyContext + 提升 task.metadata.replay_authorizations + 捕获 `DeferredApprovalRequired` 回 `[awaiting_approval]` marker | +75 |
| `src/mclaw/scheduler/scheduler.py` | 检测 `[awaiting_approval]` marker → `task.mark_awaiting_approval()` + bypass 失败计数 | +25 |
| `src/mclaw/api/routes/pending_approvals.py` | 新建：4 个 route + `_resume_task` (写 30s ReplayAuthorization + 回拨 SCHEDULED) + `_fail_task` | +290 |
| `src/mclaw/api/server.py` | 注册 `pending_approvals.router` + startup hook 把 `broadcast_event` 注入 store | +35 |
| `src/mclaw/core/policy_v2/global_engine.py` | `reset_policy_v2_layer(scope='all')` 加签 + `_emit_reload_event` helper + 失败 path | +60 |
| `src/mclaw/api/routes/config.py` | 7 个 callsite 把 `scope=...` 传给 `reset_policy_v2_layer` | +7 |
...
- **R4-8**：stdin isatty 检测 —— `main.py::main` callback 在进入交互前先 `classify_entry("cli")`，非 TTY 拒绝并指引用户改用 `mclaw run` / `serve`；`mclaw run` 命令本身也显式 `is_unattended=True` PolicyContext。
...
交互式 CLI 需要终端。请改用以下任一非交互入口：
  • mclaw run "<task>" - 单次任务执行（unattended）
  • mclaw serve - 启动 API 服务并通过 /api/chat 调用
```
...
5. **CLI run 单独 force_unattended**：即使 TTY，`mclaw run "<task>"` 也不是交互模式 —— 一次性命令没有人在 prompt 后接 Enter。
...
2. **D6：MCP server `mclaw_chat` 未走 classifier**：MCP 通过 stdio（无 TTY、无 SSE）被 Claude Desktop/Cursor 调用，是真正的 headless stdin 入口。旧代码直接 `agent.chat(message)` 让 fallback ctx `is_unattended=False` 生效 → CONFIRM 类工具会挂死等永远不会到来的用户响应。修复：`classify_entry("mcp", force_unattended=True)` + `build_policy_context(...)` + `set/reset_current_context`（try/finally 对称）。
...
4. **D1：`build_policy_context` 缺 `unattended_strategy` 参数**：旧实现 caller 只能传 `is_unattended=True`，`strategy` 走全局默认兜底。这意味着 classifier 给的 `default_strategy="ask_owner"` 在 `mclaw run` / MCP 路径走的是"等价但绕路"通道。修复：参数化 `unattended_strategy: str = ""`，classifier 输出直接喂给 `build_policy_context`；session metadata 优先级保持不变（C12 既有契约不破）。
...
6. **D12：`execute_task` docstring 过期**：从 C7 一路到 C12 的 note 没跟上 C14，仍写"本路径不安装 PolicyContext ContextVar … 由 C12 补齐"。修复：重写为"调用方负责"，明确列出当前 4 个 SoT 注入点（`mclaw run` / scheduler / MCP / evolution-TBD）。
...
| ``.mclaw/system_tasks.lock`` | sha256 of YAML, regenerated only via setup-center / CLI action（agent 自己**不**写） |
...
**方案**：
- 新增 [`scheduler/locks.py`](file:src/mclaw/scheduler/locks.py)：
...
- 修改 [`scheduler/scheduler.py`](file:src/mclaw/scheduler/scheduler.py) `_execute_task`：
...
- 新增 [`scheduler.start()`](file:src/mclaw/scheduler/scheduler.py) 三步 startup：
...
- 修改 [`core/tool_executor.py`](file:src/mclaw/core/tool_executor.py) 的 `_defer_unattended_confirm` 调用点：
...
**方案**：
- 新增 [`core/sse_replay.py`](file:src/mclaw/core/sse_replay.py)：
...
- 修改 [`api/routes/chat.py`](file:src/mclaw/api/routes/chat.py) `_stream_chat`：
...
- 修改 [`core/ui_confirm_bus.py`](file:src/mclaw/core/ui_confirm_bus.py)：
...
- 修改 [`api/server.py`](file:src/mclaw/api/server.py)：startup 时把
  `UIConfirmBus.set_broadcast_hook` 接到现有的 `websocket.fire_event` 通
  道——和 PendingApprovalsStore 走同一条路。
- 修改 [`api/routes/sessions.py`](file:src/mclaw/api/routes/sessions.py)：新增
...
**方案**：[`api/routes/health.py`](file:src/mclaw/api/routes/health.py)
...
**方案**：[`orgs/event_store.py`](file:src/mclaw/orgs/event_store.py)
...
**方案**：[`core/policy_v2/audit_chain.py`](file:src/mclaw/core/policy_v2/audit_chain.py)
...
**方案**：[`core/policy_v2/param_mutation_audit.py`](file:src/mclaw/core/policy_v2/param_mutation_audit.py)
...
- 新文件 [`core/policy_v2/hot_reload.py`](file:src/mclaw/core/policy_v2/hot_reload.py)：``PolicyHotReloader``
...
- 复用 [`global_engine.rebuild_engine_v2`](file:src/mclaw/core/policy_v2/global_engine.py) 的 LKG 路径：校验失败保留
...
- ``schema.HotReloadConfig`` 默认 ``enabled=False``（参考 4 个邻近项目
  没有一个把"文件即改即生效"作为默认）。
- [`api/server.py`](file:src/mclaw/api/server.py) 启动 hook 调 ``start_hot_reloader``，关闭 hook 调
...
- 新文件 [`core/policy_v2/env_overrides.py`](file:src/mclaw/core/policy_v2/env_overrides.py)：
  ``(env_name, cfg_path, coerce, redact)`` 注册表 4 条 + 显式 coerce 函数
  （bool 拒绝 ambiguous、unattended 强制 enum 校验、path 拒绝空串）。
- ``OPENAKITA_POLICY_FILE`` 在 [`global_engine._resolve_yaml_path`](file:src/mclaw/core/policy_v2/global_engine.py) 优先
...
- 报告挂到 ``MigrationReport.env_overrides``，[`global_engine._audit_env_
  overrides`](file:src/mclaw/core/policy_v2/global_engine.py) 把 ``applied`` / ``skipped_errors`` 写 audit 链
...
- [`main.py`](file:src/mclaw/main.py) 顶层 typer callback 加 ``--auto-confirm`` 标志 →
...
[`hot_reload._do_reload`](file:src/mclaw/core/policy_v2/hot_reload.py)
...
[`audit_logger._global_audit`](file:src/mclaw/core/audit_logger.py)
...
**修法**：[`rebuild_engine_v2`](file:src/mclaw/core/policy_v2/global_engine.py)
...
(file:src/mclaw/core/policy_v2/global_engine.py)`` 被
...
2. ``AsyncBatchAuditWriter``（``src/mclaw/core/policy_v2/audit_writer.py``
   新模块）：
...
```
grep start_global_audit_writer src/mclaw/
  src/mclaw/core/policy_v2/audit_writer.py   ← 自己定义
  # 没有第二个文件
```
