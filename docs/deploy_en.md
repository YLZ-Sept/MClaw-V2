# Mclaw Deployment Guide (English)
...
# 2. Install Mclaw (core)
pip install mclaw

# 3. Install optional features
pip install mclaw[feishu]     # + Feishu (Lark) support
pip install mclaw[windows]    # + Windows desktop automation
pip install mclaw[all]       # Install all optional features (Windows-only deps are auto-skipped on non-Windows)
...
```bash
curl -fsSL https://raw.githubusercontent.com/mclaw/mclaw/main/scripts/quickstart.sh | bash
```

**Windows (PowerShell):**

```powershell
irm https://raw.githubusercontent.com/mclaw/mclaw/main/scripts/quickstart.ps1 | iex
```

For extras / mirrors, download and run with parameters (recommended):

```bash
curl -fsSL -o quickstart.sh https://raw.githubusercontent.com/mclaw/mclaw/main/scripts/quickstart.sh
bash quickstart.sh --extras all --index-url https://pypi.tuna.tsinghua.edu.cn/simple
```

```powershell
irm https://raw.githubusercontent.com/mclaw/mclaw/main/scripts/quickstart.ps1 -OutFile quickstart.ps1
.\quickstart.ps1 -Extras all -IndexUrl https://pypi.tuna.tsinghua.edu.cn/simple
```

> The script installs into `~/.mclaw/app` and uses an isolated venv at `~/.mclaw/venv` by default
> (Windows: `%USERPROFILE%\.mclaw\...`), to avoid polluting system Python.
...
This is the **core configuration file** of Mclaw, supporting multi-endpoint, automatic failover, and capability-based routing.

#### Configuration Methods

**Method A: Interactive Wizard (Recommended)**
```bash
python -m mclaw.llm.setup.cli
```
...
Mclaw supports 5 major IM platforms, all enabled via `.env`:

| Platform | Status | Protocol | Extra Dependency |
|----------|--------|----------|-----------------|
| Telegram | ✅ Stable | Bot API | Built-in |
| Feishu (Lark) | ✅ Stable | WebSocket | `pip install mclaw[feishu]` |
| WeCom (WeWork) | ✅ Stable | HTTP API | None |
| DingTalk | ✅ Stable | HTTP API | None |
| QQ Official Bot | ✅ Stable | QQ Open Platform API | `pip install mclaw[qqbot]` |
| OneBot | ✅ Stable | OneBot WS | Requires OneBot server + `pip install mclaw[onebot]` |
...
> Running `mclaw init` will automatically create these files.
...
```bash
mclaw           # Interactive CLI (with IM channels running simultaneously)
python -m mclaw # Same
```
...
```bash
mclaw serve     # IM-only service, no CLI interaction
```

### Single Task

```bash
mclaw run "Analyze the code structure of the current directory"
```
...
mclaw status            # Show Agent status
mclaw selfcheck         # Run self-check
mclaw compile           # Compile identity files (reduces token usage)
mclaw prompt-debug      # Show prompt debug info
mclaw --version         # Show version
```
...
# Verify
mclaw --version
python -c "import mclaw; print(mclaw.__version__)"
```
...
Create service file `/etc/systemd/system/mclaw.service`:

```ini
[Unit]
Description=Mclaw AI Agent Service
After=network.target

[Service]
Type=simple
User=your-user
WorkingDirectory=/path/to/mclaw
Environment="PATH=/path/to/mclaw/venv/bin"
ExecStart=/path/to/mclaw/venv/bin/mclaw serve
Restart=always
RestartSec=10
...
sudo systemctl enable mclaw
sudo systemctl start mclaw
sudo systemctl status mclaw

# View logs
journalctl -u mclaw -f
```
...
nohup mclaw serve > logs/serve.log 2>&1 &
echo $! > mclaw.pid
```
...
pip install mclaw -i https://pypi.tuna.tsinghua.edu.cn/simple
...
```bash
# Stop service
sudo systemctl stop mclaw
sudo systemctl disable mclaw
sudo rm /etc/systemd/system/mclaw.service
...
- Issues: Submit a [GitHub Issue](https://github.com/mclaw/mclaw/issues)
