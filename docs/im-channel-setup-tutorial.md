...
> 本教程详细介绍如何为 Mclaw 配置各 IM 通道（Telegram、飞书、钉钉、企业微信、QQ 官方机器人、OneBot），包含平台端完整的申请流程和配置步骤。
...
- [平台概览](#平台概览)
- [三种配置方式](#三种配置方式)（Mclaw Desktop / CLI 向导 / 手动 .env）
...
| 飞书 | ✅ 稳定 | WebSocket 长连接 | ❌ 不需要 | `pip install mclaw[feishu]` | ⭐⭐ 简单 |
| 钉钉 | ✅ 稳定 | Stream 模式 (WebSocket) | ❌ 不需要 | `pip install mclaw[dingtalk]` | ⭐⭐ 简单 |
| 企业微信 | ✅ 稳定 | HTTP 回调（智能机器人） | ⚠️ 需要公网 IP | `pip install mclaw[wework]` | ⭐⭐⭐ 中等 |
| QQ 官方机器人 | ✅ 稳定 | QQ 开放平台 API (WebSocket) | ❌ 不需要 | `pip install mclaw[qqbot]` | ⭐⭐ 简单 |
| OneBot | ✅ 稳定 | OneBot v11 (WebSocket) | ❌ 不需要 | `pip install mclaw[onebot]` | ⭐⭐⭐ 中等 |
...
> **提示**：如果你使用 **Mclaw Desktop** 桌面程序安装，依赖会自动处理，无需手动执行 pip 命令。以下仅适用于手动 pip 部署的用户。
...
# 或按需安装
pip install mclaw[feishu]      # 飞书
pip install mclaw[dingtalk]    # 钉钉
pip install mclaw[wework]      # 企业微信
pip install mclaw[qqbot]       # QQ 官方机器人
pip install mclaw[onebot]      # OneBot（通用协议）

# 组合安装
pip install mclaw[feishu,dingtalk,qqbot]
```
...
Mclaw 提供了三种方式来配置 IM 通道，你可以选择最适合自己的方式：

### 方式一：桌面终端程序（Mclaw Desktop）— 推荐新手

Mclaw Desktop 是 Mclaw 提供的可视化桌面安装程序（基于 Tauri），提供图形化界面完成所有配置，无需手动编辑文件。

<!-- 📸 配图：Mclaw Desktop 主界面全貌截图 -->
> **[配图位]** Mclaw Desktop 桌面程序主界面
...
**使用方式**：
1. 启动 Mclaw Desktop 桌面程序
2. 在左侧导航栏找到 **「IM 通道」** 配置步骤
3. 按平台分组，填入对应的凭证信息（如 Token、App ID 等）
4. 开关 `*_ENABLED` 启用对应通道
5. 保存后自动写入 `.env`，可一键重启服务生效

<!-- 📸 配图：Mclaw Desktop 中 IM 通道配置的步骤页面截图 -->
> **[配图位]** Mclaw Desktop — IM 通道配置步骤页面

<!-- 📸 配图：Mclaw Desktop 中 Telegram 配置表单的截图 -->
> **[配图位]** Mclaw Desktop — Telegram 配置表单示例

<!-- 📸 配图：Mclaw Desktop 状态页面中 IM 通道健康检查的截图 -->
> **[配图位]** Mclaw Desktop — 状态页面的 IM 通道健康检查

> **提示**：Mclaw Desktop 的状态页面会显示每个通道的实时连接状态（🟢 在线 / 🔴 离线 / ⚪ 未启用），可以随时检查配置是否正确。
...
> **提示**：三种方式最终都是写入同一个 `.env` 文件，可以随时切换使用。例如先用 Mclaw Desktop 完成基本配置，再手动编辑 `.env` 微调参数。
...
2. BotFather 会要求你为机器人设置一个**显示名称**（name），例如 `My Mclaw Bot`
3. 接着要求设置一个**用户名**（username），必须以 `bot` 结尾，例如 `my_mclaw_bot`
...
在平台端获取 Bot Token 后，通过以下任一方式配置 Mclaw：

#### 方式 A：Mclaw Desktop 桌面程序（推荐）
...
<!-- 📸 配图：Mclaw Desktop 中 Telegram 配置表单的截图，标注 Token 和 Proxy 输入框 -->
> **[配图位]** Mclaw Desktop — Telegram 配置表单，填入 Bot Token 和代理地址

<!-- 📸 配图：Mclaw Desktop 状态页面显示 Telegram 通道在线状态 -->
> **[配图位]** Mclaw Desktop — 保存后状态页面显示 Telegram 在线 🟢
...
先安装飞书依赖（Mclaw Desktop 用户可跳过，依赖会自动安装）：

```bash
pip install mclaw[feishu]
```
...
<!-- 📸 配图：Mclaw Desktop 中飞书配置表单的截图，标注 App ID 和 App Secret 输入框 -->
> **[配图位]** Mclaw Desktop — 飞书配置表单

<!-- 📸 配图：Mclaw Desktop 状态页面显示飞书通道在线状态 -->
> **[配图位]** Mclaw Desktop — 保存后状态页面显示飞书在线 🟢
...
1. 启动 Mclaw，观察日志输出，应出现：
   ```
   Feishu adapter: WebSocket started in background
   ```
2. 打开飞书，在搜索栏中搜索你创建的应用名称（如 `Mclaw Bot`）
3. 点击进入对话，发送一条消息
4. 观察日志和机器人回复
...
先安装钉钉依赖（Mclaw Desktop 用户可跳过，依赖会自动安装）：

```bash
pip install mclaw[dingtalk]
```
...
<!-- 📸 配图：Mclaw Desktop 中钉钉配置表单的截图 -->
> **[配图位]** Mclaw Desktop — 钉钉配置表单

<!-- 📸 配图：Mclaw Desktop 状态页面显示钉钉通道在线状态 -->
> **[配图位]** Mclaw Desktop — 保存后状态页面显示钉钉在线 🟢
...
> 企业微信通过 HTTP 回调接入，**需要公网可访问的 URL**。配置流程需要注意顺序：先配 Mclaw，再创建机器人。
...
> ⚠️ **重要顺序提醒**：企业微信创建机器人时会**立即验证回调 URL**，所以必须**先完成 4.4 节的 Mclaw 配置并启动服务**，确保回调端口可从公网访问，**然后再执行以下步骤**。
...
#### 第二步：配置 Mclaw 并启动（先于创建机器人！）

⚠️ 请先跳到 **4.4 节** 完成 `.env` 配置并启动 Mclaw，确保回调端口已监听。
...
> ⚠️ 点击保存时，企业微信会立即向你的 URL 发送 GET 验证请求。如果 Mclaw 未启动或网络不通，会提示"网络失败"。

保存成功后，**将 Token 和 EncodingAESKey 更新到 `.env` 文件中**，并重启 Mclaw。
...
先安装企业微信依赖（Mclaw Desktop 用户可跳过，依赖会自动安装）：

```bash
pip install mclaw[wework]
```

然后通过以下任一方式填入凭证：

> ⚠️ **注意**：企业微信需要先配好 Mclaw 并启动，再去管理后台创建机器人（创建时会验证回调 URL），所以建议先填好 Corp ID 和端口，启动服务后再补充 Token 和 AES Key。
...
<!-- 📸 配图：Mclaw Desktop 中企业微信配置表单的截图，标注各个输入框 -->
> **[配图位]** Mclaw Desktop — 企业微信配置表单，填入 Corp ID、Token、AES Key

<!-- 📸 配图：Mclaw Desktop 状态页面显示企业微信通道在线状态 -->
> **[配图位]** Mclaw Desktop — 保存后状态页面显示企业微信在线 🟢
...
│                          ↓                                  │
│ 4. 启动 Mclaw，确认回调端口已监听                         │
│                          ↓                                  │
│ 5. 去管理后台创建智能机器人                                   │
│                          ↓                                  │
│ 6. 配置回调 URL，获取 Token 和 EncodingAESKey                │
│                          ↓                                  │
│ 7. 将 Token 和 EncodingAESKey 更新到 .env，重启 Mclaw    │
│                          ↓                                  │
│ 8. 设置机器人可见范围                                        │
│                          ↓                                  │
│ 9. 在企业微信中测试对话                                      │
...
```
用户发消息 → 企业微信推送加密 JSON → Mclaw 回调接收
                                        ↓
                           创建 stream 流式会话
                                        ↓
                           Agent 处理并生成回复
...
| 问题 | 解决方案 |
|------|---------|
| 创建机器人时提示"网络失败" | 回调 URL 不通，确保 Mclaw 已启动，端口已监听，公网可访问 |
...
先安装 QQ 官方机器人依赖（Mclaw Desktop 用户可跳过，依赖会自动安装）：

```bash
pip install mclaw[qqbot]
```
...
<!-- 📸 配图：Mclaw Desktop 中 QQ 官方机器人配置表单的截图 -->
> **[配图位]** Mclaw Desktop — QQ 官方机器人配置表单
...
| 语音发不出去 | 需要 silk 格式 + 公网 URL，目前 Mclaw 会降级为文本提示 |
| 文件发不出去 | QQ 官方 API `file_type=4` 暂未开放，Mclaw 会降级为文本提示 |
...
- 一台部署 OneBot 服务的机器（可以和 Mclaw 在同一台）

### 6.2 部署 OneBot 服务器

Mclaw 通过 OneBot v11 协议的 WebSocket 与中间层通信。你需要先部署一个 OneBot 实现。
...
先安装 OneBot 依赖（Mclaw Desktop 用户可跳过，依赖会自动安装）：

```bash
pip install mclaw[onebot]
```
...
<!-- 📸 配图：Mclaw Desktop 中 OneBot 配置表单的截图 -->
> **[配图位]** Mclaw Desktop — OneBot 配置表单
...
> 如果 OneBot 服务和 Mclaw 不在同一台机器上，请将 `127.0.0.1` 替换为 OneBot 服务器的实际 IP。
...
在 `.env` 中将多个通道的 `*_ENABLED` 设为 `true` 即可。Mclaw 会同时连接所有启用的通道。
...
- **日志**：启动时会打印各通道的连接状态
- **Mclaw Desktop**：在 Mclaw 的 Web 管理界面中查看通道状态和会话列表
...
有。Mclaw 提供了 **Mclaw Desktop** 桌面应用（基于 Tauri），可以通过图形界面完成所有 IM 通道的配置，无需手动编辑 `.env` 文件。详见本教程的 [三种配置方式](#三种配置方式) 章节。

Mclaw Desktop 提供以下 IM 相关能力：
- **配置步骤页**：分通道的表单，开关 + 输入框，保存自动写入 `.env`
- **状态页**：实时显示每个通道的连接状态（🟢 在线 / 🔴 离线 / ⚪ 未启用），支持一键刷新
- **服务管理**：配置修改后可一键重启服务，无需回到命令行
- **IM 会话查看器**：查看各通道的活跃会话和消息历史

<!-- 📸 配图：Mclaw Desktop 完整的 IM 通道管理界面截图（包含配置 + 状态 + 会话） -->
> **[配图位]** Mclaw Desktop 的 IM 通道完整管理界面

### Q8：Mclaw Desktop 和 .env 手动编辑会冲突吗？

不会。Mclaw Desktop 的配置最终也是写入 `.env` 文件，两种方式操作的是同一份文件。你可以先用 Mclaw Desktop 完成基本配置，再手动编辑 `.env` 微调高级参数（如 `TELEGRAM_WEBHOOK_URL`、`WEWORK_CALLBACK_PORT` 等）。修改 `.env` 后重启服务即可生效。
...
# --- 钉钉（需要 mclaw[dingtalk]）---
DINGTALK_ENABLED=false
DINGTALK_CLIENT_ID=
DINGTALK_CLIENT_SECRET=

# --- 企业微信（需要 mclaw[wework]）---
...
# --- QQ 官方机器人（需要 mclaw[qqbot]）---
QQBOT_ENABLED=false
QQBOT_APP_ID=
QQBOT_APP_SECRET=
QQBOT_SANDBOX=false

# --- OneBot（需要 mclaw[onebot] + NapCat/Lagrange）---
