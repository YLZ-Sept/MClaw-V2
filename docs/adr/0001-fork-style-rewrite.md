...
- `src/mclaw/core/agent.py` — 8 433 lines.
- `src/mclaw/core/reasoning_engine.py` — 7 987 lines.

`src/mclaw/orgs/runtime.py` is 5 734 lines and concentrates lifecycle
...
covers `src/mclaw/orgs/` and most of `src/mclaw/core/`, with a fresh
...
1. New code is written under fresh package paths
   (`src/mclaw/runtime/`, `src/mclaw/agent/`) so the legacy paths keep
...
3. **Adopt LangGraph or AutoGen as the engine.** Rejected at this layer: the
   identity / persona / memory / channel surfaces are Mclaw-specific and
...
- Legacy source under audit: [src/mclaw/orgs/runtime.py](../../src/mclaw/orgs/runtime.py),
  [src/mclaw/core/agent.py](../../src/mclaw/core/agent.py),
