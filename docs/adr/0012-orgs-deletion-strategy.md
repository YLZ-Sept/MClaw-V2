...
the legacy ``src/mclaw/orgs/`` package is dead code. P9.9
...
matter if callers still imported ``mclaw.orgs.X``.
...
* P9.8 finishes with ``git grep -nE 'from mclaw\.orgs'
  -- src/mclaw/`` returning 0 hits (every production caller
  has been migrated).
...
* P9.9a (single commit): ``git rm -r src/mclaw/orgs/`` --
...
to v1. The v1 REST file (``src/mclaw/api/routes/orgs.py``,
...
Nothing under v2 calls ``mclaw.orgs.X``, so the shim would
...
* Charter: ``docs/revamp/P-RC-9-CHARTER.md`` -- the
  "git rm -r src/mclaw/orgs/" line this ADR realises.
