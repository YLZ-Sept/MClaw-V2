...
`src/mclaw/core/agent.py` has grown to **8 433 lines**, and
`src/mclaw/core/reasoning_engine.py` to **7 987 lines**. Together they
...
> `mclaw.core.*` re-exports described below have been removed.
> `mclaw.agent.*` is now the only supported public agent import surface.
> Internal execution code may remain under `mclaw.core._*_runtime` while
> it is decomposed, but those underscored modules are not compatibility APIs.

The new agent lives under `src/mclaw/agent/`. Strict
...
For one transitional release, `src/mclaw/core/__init__.py` will
...
`mclaw.agent`. This is the only breaking change for plugin authors.
...
- Plugin authors importing `mclaw.core.Agent` get a deprecation
...
2. **Adopt LangChain `Runnable` chain interface as agent runtime.**
   ...
   identity / permission / audit features that Mclaw already ships.
...
- Legacy under audit: [src/mclaw/core/agent.py](../../src/mclaw/core/agent.py),
  [src/mclaw/core/reasoning_engine.py](../../src/mclaw/core/reasoning_engine.py).
