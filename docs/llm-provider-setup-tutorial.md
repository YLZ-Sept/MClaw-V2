...
> 本教程详细介绍如何为 Mclaw 配置 LLM（大语言模型）服务商，包含各平台的 API Key 申请流程、端点配置方法，以及多端点 Failover 策略。
...
Mclaw 支持多种 LLM 服务商，所有服务商通过统一的端点配置系统管理。你至少需要配置 **1 个 LLM 端点** 才能使用 Mclaw。
...
一个端点 = 一个服务商 + 一个模型 + 一套凭证。Mclaw 通过 `data/llm_endpoints.json` 文件管理所有端点。
...
<!-- 📸 配图：Mclaw Desktop — LLM 端点配置页面全貌 -->
> **[配图位]** Mclaw Desktop — LLM 端点配置页面

<!-- 📸 配图：Mclaw Desktop — 添加端点对话框（选择服务商、填 API Key、选模型） -->
> **[配图位]** Mclaw Desktop — 添加端点对话框

<!-- 📸 配图：Mclaw Desktop — 端点列表，显示已添加的多个端点 -->
> **[配图位]** Mclaw Desktop — 已添加的端点列表

<!-- 📸 配图：Mclaw Desktop — 服务商下拉列表（展示国内+国际全部服务商） -->
> **[配图位]** Mclaw Desktop — 服务商下拉选择

<!-- 📸 配图：Mclaw Desktop — 模型在线拉取列表 -->
> **[配图位]** Mclaw Desktop — 在线拉取可用模型列表

<!-- 📸 配图：Mclaw Desktop — 能力标签勾选（text/tools/thinking/vision） -->
> **[配图位]** Mclaw Desktop — 能力标签勾选

> **提示**：添加端点后可在状态页面进行**健康检查**，验证 API Key 和端点是否正常。

<!-- 📸 配图：Mclaw Desktop — 状态页面的端点健康检查结果 -->
> **[配图位]** Mclaw Desktop — 端点健康检查
...
> **注意**：CLI 向导适合快速配置单个端点。如需配置多端点和 Failover，建议使用 Mclaw Desktop 或手动编辑配置文件。
...
<!-- 📸 配图：Mclaw Desktop — 选择通义千问服务商后的配置界面 -->
> **[配图位]** Mclaw Desktop — 通义千问端点配置
...
<!-- 📸 配图：Mclaw Desktop — DeepSeek 端点配置 -->
> **[配图位]** Mclaw Desktop — DeepSeek 端点配置
...
<!-- 📸 配图：Mclaw Desktop — Kimi 端点配置 -->
> **[配图位]** Mclaw Desktop — Kimi 端点配置
...
<!-- 📸 配图：Mclaw Desktop — 智谱 AI 端点配置 -->
> **[配图位]** Mclaw Desktop — 智谱 AI 端点配置
...
<!-- 📸 配图：Mclaw Desktop — MiniMax 端点配置 -->
> **[配图位]** Mclaw Desktop — MiniMax 端点配置
...
<!-- 📸 配图：Mclaw Desktop — OpenAI 端点配置 -->
> **[配图位]** Mclaw Desktop — OpenAI 端点配置
...
<!-- 📸 配图：Mclaw Desktop — Anthropic 端点配置 -->
> **[配图位]** Mclaw Desktop — Anthropic Claude 端点配置

> **注意**：Anthropic 使用独有的 API 协议，`api_type` 必须选择 `anthropic`（而非 `openai`）。Mclaw Desktop 会自动处理。
...
<!-- 📸 配图：Mclaw Desktop — Google Gemini 端点配置 -->
> **[配图位]** Mclaw Desktop — Google Gemini 端点配置
...
> 以上服务商在 Mclaw Desktop 的服务商下拉列表中均可直接选择。

---

## 十、多端点与 Failover

Mclaw 的核心优势之一是支持**多端点配置与自动故障转移**。强烈建议至少配置 2 个端点。
...
### 10.5 在 Mclaw Desktop 中管理多端点

<!-- 📸 配图：Mclaw Desktop — 多端点列表，显示优先级和状态 -->
> **[配图位]** Mclaw Desktop — 多端点管理，显示优先级排序和健康状态

<!-- 📸 配图：Mclaw Desktop — 编辑端点优先级 -->
> **[配图位]** Mclaw Desktop — 编辑端点优先级

> **提示**：在状态页面点击「健康检查」可批量验证所有端点的连通性。

<!-- 📸 配图：Mclaw Desktop — 批量健康检查结果（全部绿色 / 部分红色） -->
> **[配图位]** Mclaw Desktop — 批量端点健康检查结果

---

## 十一、编译器端点（Prompt Compiler）

Mclaw 内置了 **Prompt Compiler**（提示词编译器），用于对用户指令做预处理（意图识别、指令优化等）。编译器使用快速小模型即可，**不需要思考能力**，可大幅降低整体响应延迟。
...
<!-- 📸 配图：Mclaw Desktop — 编译器端点配置区域 -->
> **[配图位]** Mclaw Desktop — 编译器端点配置
...
- **Mclaw Desktop**：在端点列表中调整优先级，或禁用/启用端点
- **对话中**：使用 `/model` 命令切换
- **手动**：编辑 `llm_endpoints.json` 中的 `priority` 值

### Q5：`api_type` 选 `openai` 还是 `anthropic`？

除了 Anthropic 官方 API 使用 `anthropic` 类型外，几乎所有其他服务商（包括国内服务商）都使用 `openai` 兼容类型。Mclaw Desktop 会自动选择。
...
- **Mclaw Desktop**：在状态页面点击「健康检查」按钮
- **API**：`POST /api/health/check` 可指定端点名称

<!-- 📸 配图：Mclaw Desktop — 单个端点健康检查详情 -->
> **[配图位]** Mclaw Desktop — 端点健康检查详情

### Q8：配置修改后需要重启吗？

- **Mclaw Desktop**：保存后可点击「应用并重启」按钮，或在状态页面重启服务
