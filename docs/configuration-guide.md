...
> 本文档详细介绍 Mclaw 桌面客户端的完整配置流程，包括**快速配置**和**完整配置**两种模式。
...
安装完成后，双击打开 Mclaw 桌面客户端。首次启动时会自动进入配置向导。
...
点击「IM 通道」区域的展开按钮，可以配置即时通讯接入。不配置也可以正常使用 Mclaw（通过桌面聊天窗口交互）。
...
> 各 IM 通道的详细接入教程请参考 [IM 通道接入文档](https://github.com/anthropic-lab/mclaw/blob/main/docs/im-channels.md)。
...
| 步骤 | 说明 | 预计耗时 |
|------|------|----------|
| 1. 创建默认工作区 | 在 `~/.mclaw/workspaces/default` 下创建工作区目录 | < 1 秒 |
| 2. 安装内置 Python | 下载并安装 Python 3.11 嵌入版 | 10~30 秒 |
| 3. 创建虚拟环境并安装依赖 | 创建 venv，pip install mclaw[all] | 1~3 分钟 |
...
你可以选择：
- **「启动服务」**：立即启动 Mclaw 后端服务并跳转到状态面板
- **「跳转状态面板」**：不启动服务，仅查看状态面板

> 恭喜！快速配置已完成，你现在可以开始使用 Mclaw 了。
...
工作区是 Mclaw 的配置隔离单元。每个工作区独立维护以下文件：
- `.env` — 环境变量配置
...
Mclaw 需要 Python 3.11+ 运行环境。你有两种选择：

<!-- 📸 插图：Python 环境配置页面 -->

#### 方式一：使用内置 Python（推荐）

点击 **「安装内置 Python」** 按钮，系统将自动下载并安装 Python 3.11 嵌入版到 `~/.mclaw/runtime/` 目录。
...
LLM 端点是 Mclaw 调用大语言模型的入口。你可以配置多个端点，系统会根据优先级和可用性自动选择。
...
IM 通道让你可以通过即时通讯工具与 Mclaw 对话。所有通道均为**可选**配置。
...
此步骤配置 Mclaw 可以使用的工具和技能扩展。

<!-- 📸 插图：工具与技能配置页面 -->

#### MCP 工具

MCP（Model Context Protocol）允许 Mclaw 通过标准化协议调用外部工具。
...
桌面自动化让 Mclaw 可以操作你的电脑桌面（截图、点击、输入等）。
...
- **启动服务**：点击按钮启动 Mclaw 后端服务
- **卸载 / 清理**（可选）：卸载 mclaw 包或删除运行环境
...
```
~/.mclaw/workspaces/{workspace_id}/.env
```

LLM 端点列表单独存放在：

```
~/.mclaw/workspaces/{workspace_id}/data/llm_endpoints.json
```
