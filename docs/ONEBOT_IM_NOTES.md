...
| 功能 | 关键代码位置 | 说明 |
|------|------------|------|
| WebSocket 反向连接（推荐） | `_run_reverse_server()` | Mclaw 作为 WS 服务端，NapCat/Lagrange 作为客户端连入 |
| WebSocket 正向连接 | `_receive_loop_with_reconnect()` | Mclaw 主动连接 OneBot 实现的 WS 服务器 |
...
|------|--------------------------|--------------------------|
| 默认 | ✅ 默认模式 | — |
| Mclaw 角色 | WS 服务端 (`websockets.serve`) | WS 客户端 (`websockets.connect`) |
...
| 适用场景 | NapCat 和 Mclaw 同机 / 内网 | Mclaw 无法被 NapCat 访问时 |
...
NapCat / Lagrange
        │
    ┌───┴───┐
    │reverse│  (NapCat → Mclaw WS Server)
    │forward│  (Mclaw WS Client → NapCat)
    └───┬───┘
