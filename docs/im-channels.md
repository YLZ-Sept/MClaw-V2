...
Mclaw 支持多个即时通讯平台，每个平台通过独立的适配器 (Adapter) 接入统一的消息网关 (MessageGateway)。
...
| 飞书 | ✅ 稳定 | WebSocket 长连接 | ❌ 不需要 | `pip install mclaw[feishu]` |
| 钉钉 | ✅ 稳定 | Stream 模式 (WebSocket) | ❌ 不需要 | `pip install mclaw[dingtalk]` |
| 企业微信 | ✅ 稳定 | HTTP 回调（智能机器人） | ⚠️ 需要公网 IP | `pip install mclaw[wework]` |
| 微信个人号 | ✅ 实验性 | HTTP 长轮询 (iLink Bot API) | ❌ 不需要 | `pip install mclaw[wechat]` |
| QQ 官方机器人 | ✅ 稳定 | QQ 开放平台 API (WebSocket) | ❌ 不需要 | `pip install mclaw[qqbot]` |
| OneBot | ✅ 稳定 | OneBot v11 (WebSocket) | ❌ 不需要 | `pip install mclaw[onebot]` |
...
1. 启动 Mclaw 后，在 Telegram 中找到你的机器人
...
1. 启动 Mclaw，日志应显示 `Feishu adapter: WebSocket started in background`
2. 在飞书中搜索并打开机器人对话
3. 发送消息，观察日志和机器人回复
...
1. 启动 Mclaw，日志应显示 `DingTalk Stream client starting...`
...
2. Mclaw 增加"主动卡片推送"类业务（如审批结果、告警通知卡片）。

在此之前，钉钉适配器保持现有的"sessionWebhook 文本被动回复 + 长输出自动分段"实现，避免引入未验证收益的代码复杂度。

> 关于 Stream 心跳：`dingtalk-stream` 0.24 自带连接级心跳。Mclaw 在其之上额外做了**线程级 watchdog**（[`dingtalk.py`](../src/mclaw/channels/adapters/dingtalk.py) 的 `_stream_watchdog`），监控线程是否存活并指数退避重启——这是更外一层的兜底，**不是重复造轮子**，保留。
...
> ⚠️ **重要**：创建机器人时企业微信会立即向回调 URL 发送 GET 验证请求，所以必须先在本地配好并启动 Mclaw，确保回调端口可从公网访问，然后再创建机器人。

1. **获取企业 ID (Corp ID)**: 进入 [企业微信管理后台](https://work.weixin.qq.com/) →「我的企业」→「企业信息」页面底部
2. **在 `.env` 中填好配置**（见下方 Mclaw 配置），启动 Mclaw
...
2. Mclaw 被动回复初始 stream（`finish=false`，创建流式会话）
...
1. 在 `.env` 中配好参数，启动 Mclaw，日志应显示 `WeWorkBot adapter started`
...
> **协议跟随**：腾讯官方 OpenClaw 微信通道当前 CLI 已发布 `@tencent-weixin/openclaw-weixin-cli@2.1.x`、插件本体 `@tencent-weixin/openclaw-weixin@2.1.x`（2026-04），但 iLink HTTP 协议本体（`getupdates` / `sendmessage` / `getuploadurl` / AES-128-ECB / CDN）**未变**，Mclaw 适配器无需改动。OpenClaw 用户侧的反推/参考安装命令仍是：
>
> ```bash
> npx -y @tencent-weixin/openclaw-weixin-cli@latest install
> ```
>
> 该命令只用于在 OpenClaw 宿主中安装微信通道插件，与 Mclaw 的接入流程无直接关系——Mclaw 直接调用 iLink 原生 HTTP API，不经过 OpenClaw。
...
3. **CLI 方式**: 运行 `python -m mclaw.setup_center.bridge wechat-onboard-start` 获取二维码

### Mclaw 配置

```bash
# 安装微信依赖
pip install mclaw[wechat]
...
2. 启动 Mclaw，日志应显示 `WeChat adapter: poll loop started`
3. 用另一个微信账号向机器人号发送消息
4. 观察日志和机器人回复
...
Mclaw 的 QQ 适配器**默认走 base64 本地直传**，仅在显式提供 `url` 时走 URL 上传，无需公网 IP/域名。
...
> **语音格式注意**：QQ 现已支持 `silk/wav/mp3/flac` 直接上传，Mclaw `send_voice` 会原格式直传上述四种；其他格式（opus/ogg/amr/m4a/...）会自动调用 `ffmpeg` 转为 mp3 后上传。**不再需要 `pilk` 编码**——pilk 仅用于接收用户语音消息后做 SILK→WAV 解码（配合 Whisper 转写），需要时通过 `pip install mclaw[voice]` 单独安装。

### 验证方法

1. 启动 Mclaw，日志应显示 `QQ Official Bot ready (user: 你的机器人名称)`
...
Mclaw 通过 OneBot v11 协议与中间层通信。你需要先部署一个 OneBot 实现：
...
- **WebSocket 正向连接**: Mclaw 连接到本地 OneBot 服务器，无需公网 IP
- 支持自动断线重连（指数退避策略，初始 1 秒，最大 60 秒）

### 验证方法

1. 先启动 OneBot 服务器（如 NapCat），确认 WebSocket 监听正常
2. 启动 Mclaw，日志应显示 `OneBot adapter connected to ws://127.0.0.1:8080`
...
pip install mclaw[wechat]      # 微信个人号
pip install mclaw[qqbot]       # QQ 官方机器人
pip install mclaw[onebot]      # OneBot（通用协议）

# 组合安装
pip install mclaw[feishu,dingtalk,wechat,qqbot]
```
...
- **ChannelAdapter**: 基类定义在 `src/mclaw/channels/base.py`，各平台实现在 `src/mclaw/channels/adapters/`
- **MessageGateway**: 统一消息路由、会话管理、媒体预处理，定义在 `src/mclaw/channels/gateway.py`
- **deliver_artifacts**: Agent 工具，用于主动发送文件/图片/语音，定义在 `src/mclaw/tools/handlers/im_channel.py`
