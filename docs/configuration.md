...
This document covers all configuration options for Mclaw.

## Environment Variables

Mclaw is configured primarily through environment variables, typically stored in a `.env` file.
...
Mclaw is a self-evolving AI assistant...
...
mclaw --config /path/to/config.yaml

# Override log level
mclaw --log-level DEBUG

# Override model
mclaw --model claude-opus-4-0-20250514

# Disable confirmation prompts
mclaw --auto-confirm

# Run in specific mode
mclaw --mode chat|task|test
```

## POLICIES.yaml Configuration (Policy v2)

Mclaw's security policy lives in `identity/POLICIES.yaml`. Most fields
...
Not recommended for: workstation development (manual `mclaw serve`
restart is fine).
...
```bash
mclaw --auto-confirm                 # interactive CLI
mclaw --auto-confirm run "..."       # one-shot task
mclaw --auto-confirm serve           # API server
```
