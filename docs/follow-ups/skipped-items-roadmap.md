...
(`src/mclaw/plugins/manager.py:300`) and falls back to the
...
| Field | Value |
|-------|-------|
| Status | Planned for Mclaw 2.0 major. |
...
`src/mclaw/plugins/installer.py` exists as the future hook
...
The shim at `src/mclaw/api/routes/_orgs_v2_deprecated_redirects.py`
...
2. After the 30-day Sunset window with `hits == 0` for every path,
   ...
   - Deletes `src/mclaw/api/routes/_orgs_v2_deprecated_redirects.py`
   - Deletes the `app.include_router(_orgs_v2_deprecated_redirects.router)`
     line in `src/mclaw/api/server.py` (see ROADMAP block above
     that line).
   - Drops the `get_deprecated_redirect_stats` import from
     `src/mclaw/api/routes/health.py` and the
...
- `src/mclaw/api/routes/orgs_v2.py::list_templates` (spec, currently envelope).
- `src/mclaw/api/routes/orgs_v2_runtime_orgs.py::list_templates` (runtime, currently bare array — keep).
...
| Sprint | Commit | 变更 | 数据 |
|--------|--------|------|------|
| 14 (v31) | `206c08ab` | IM gateway 并发关闭 + per-adapter wait_for(8s) + os._exit safety net + `mclaw stop` CLI | 首次结构改善 |
...
- `mclaw stop` CLI 正常，代理/死端口两路均处理正确
...
| Trigger merge | `git merge origin/main` (long-overdue upstream sync). Local kept the ADR-0003 split (`mclaw.agent` public package + `core/_agent_runtime.py` private implementation); upstream fixes written against monolithic Agent and ReasoningEngine modules could not be cherry-picked verbatim. |
| Import cutover | Completed 2026-07-23. `mclaw.agent.*` is the only public agent surface; the `mclaw.core.*` re-export modules and package-level agent exports were removed. |
...
| `tests/unit/test_identity.py::TestSyncIdentityFileBundledFallback` + `TestIdentityLoading::test_get_system_prompt_does_not_inject_mclaw_self_identity` / `test_get_system_prompt_replaces_agent_name_placeholder` | C.2 | `agent/identity.py` `agent_voice` + `_resolve_bundled_identity_template` land |
...
- Do not restore `mclaw.core.*` re-export modules. New and existing callers
  must import agent functionality from `mclaw.agent.*`.
...
| `src/mclaw/plugins/manifest.py` or `installer.py` | §A.2 |
| `src/mclaw/api/routes/_orgs_v2_deprecated_redirects.py` or `server.py` include_router for it | §A.3 |
| `src/mclaw/api/routes/orgs_v2.py::list_templates` | §A.4 |
