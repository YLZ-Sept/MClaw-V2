...
默认情况下，Mclaw 的浏览器工具会启动一个新的浏览器实例，没有用户的登录状态、Cookie 或密码管理器。本指南介绍如何让 AI Agent 连接到您**真实的 Chrome 浏览器**，保留所有登录状态。
...
Mclaw 启动后，Chrome DevTools MCP 会自动作为外部 MCP 服务器加载。可以通过以下方式调用：
...
扩展启动后会在 `http://127.0.0.1:12306/mcp` 暴露 MCP 接口。Mclaw 会自动检测。
...
### Q: 两种方案可以同时使用吗？
A: 可以。Chrome DevTools MCP 和 mcp-chrome 扩展互不冲突，Mclaw 会根据可用性自动选择。
