...
corruption around `data/memory/mclaw.db`, for example:

```text
sqlite3.DatabaseError: malformed database schema
```

Close Mclaw before running any command below. Replace `$Workspace` with the
affected workspace/data directory if it is different.

## 1. Locate The Database

```powershell
$Workspace = "D:\mclawdata"
$MemoryDir = Join-Path $Workspace "data\memory"
$Db = Join-Path $MemoryDir "mclaw.db"
...
$Backup = Get-ChildItem $MemoryDir -Force |
  Where-Object {
    $_.Name -like "mclaw.db.bak.*" -or
    $_.Name -like "mclaw.db.snapshot.*"
  } |
...
Start Mclaw again. If it still cannot start, repeat with an older backup or
try Option B.
...
```powershell
$SqliteCandidates = @(
  "$env:LOCALAPPDATA\Programs\Mclaw\sqlite3.exe",
  "$env:ProgramFiles\Mclaw\sqlite3.exe",
  "sqlite3.exe"
)
...
$Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$RecoveredSql = Join-Path $MemoryDir "mclaw.recovered.$Stamp.sql"
$RecoveredDb = Join-Path $MemoryDir "mclaw.recovered.$Stamp.db"
$Quarantine = Join-Path $MemoryDir ".quarantine.$Stamp"
...
Write-Host "Start Mclaw again. It will create a fresh memory database."
```
...
- The last 200 lines of `mclaw-serve.log`.
