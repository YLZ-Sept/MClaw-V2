...
创建文件 `~/.mclaw/feedback.env`：

```ini
OSS_ENDPOINT=https://oss-cn-hangzhou.aliyuncs.com
OSS_BUCKET=mclaw-feedback
OSS_ACCESS_KEY_ID=__填入你的AccessKeyId__
OSS_ACCESS_KEY_SECRET=__填入你的AccessKeySecret__
```

> 凭证来自阿里云 RAM 用户，需拥有 `mclaw-feedback` Bucket 的读写权限。
...
```bash
gh issue view <issue_number> -R mclaw/mclaw
```
...
| System Info | `OS: Windows 10 AMD64 \| Python: 3.11.9 \| Mclaw: 1.26.1` | 用户环境 |
...
1. **`metadata.json`** — 了解用户描述、系统环境、Mclaw 版本
2. **`logs/mclaw.log`** — 搜索 `ERROR`、`CRITICAL`、`Traceback`，关注时间线
...
```bash
# 已修复
gh issue edit <issue_number> -R mclaw/mclaw --remove-label "status:open" --add-label "status:resolved"

# 不会修复
gh issue edit <issue_number> -R mclaw/mclaw --remove-label "status:open" --add-label "status:wontfix"

# 需要更多信息
gh issue edit <issue_number> -R mclaw/mclaw --remove-label "status:open" --add-label "status:need-info"
```

### 2. 添加处理结论评论

```bash
gh issue comment <issue_number> -R mclaw/mclaw --body "修复说明..."
```
...
```bash
gh issue close <issue_number> -R mclaw/mclaw --reason completed

# 如果是 wontfix
gh issue close <issue_number> -R mclaw/mclaw --reason "not planned"
```
...
gh issue list -R mclaw/mclaw --label "status:open" --label "source:feedback"

# 列出需要更多信息的 Issue
gh issue list -R mclaw/mclaw --label "status:need-info"
```
...
1. `gh issue view <number> -R mclaw/mclaw --json title,body,labels` 获取内容
...
1. 更新标签：`gh issue edit <number> -R mclaw/mclaw --remove-label "status:open" --add-label "status:resolved"`
2. 添加结论评论：`gh issue comment <number> -R mclaw/mclaw --body "..."`
3. 关闭 Issue：`gh issue close <number> -R mclaw/mclaw --reason completed`
...
2. `gh issue list -R mclaw/mclaw --label "status:open" --label "source:feedback"` 查看未处理 Issue
