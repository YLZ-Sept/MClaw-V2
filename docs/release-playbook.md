...
| 仓库 | 用途 |
|------|------|
| `mclaw/mclaw` | 主仓库（后端 + Desktop + 移动端） |
| `mclaw/mclaw-web` | 官网（下载页） |
...
- **Python**：系统 `python` 为 2.7，需使用 `r:\Mclaw\.venv\Scripts\python.exe`。
...
STEP 5 — 回填 manifest 使新渠道生效
  gh workflow run backfill-oss.yml --repo mclaw/mclaw --ref main -f upload_binaries=false -f dry_run=false
...
STEP 2 — 使用 version.py 修改版本号
  r:\Mclaw\.venv\Scripts\python.exe scripts/version.py set {BARE_VERSION}
  → 输出会显示所有被修改的文件
...
STEP 5 — 触发构建
  gh workflow run release.yml --repo mclaw/mclaw --ref {BRANCH} -f tag={TAG} -f include_mobile={true|false}
  → release.yml 会先创建 Draft；include_mobile=true 时再调用 mobile.yml

STEP 6 — 等待构建完成（轮询检查，约 15-30 分钟）
  反复执行 gh run list --repo mclaw/mclaw --workflow=release.yml --limit 1
  直到状态为 ✓ completed
  移动端是同一次 release.yml run 的被调用 job，无需单独等待另一个 workflow run

STEP 7 — 触发发布（自动推断渠道）
  gh workflow run publish-release.yml --repo mclaw/mclaw --ref {BRANCH} -f tag={TAG}
  → 等待完成

STEP 8 — 验证
  gh release view {TAG} --repo mclaw/mclaw
...
STEP 3 — 确保版本号一致
  r:\Mclaw\.venv\Scripts\python.exe scripts/version.py check --expected {TAG}
...
STEP 5 — 删除 GitHub 上的旧 Release（如果存在），让构建工作流重新创建 Draft
  gh release delete {TAG} --repo mclaw/mclaw --yes --cleanup-tag=false
  （如果 release 不存在则忽略错误）
...
STEP 8 — 触发发布（强制覆盖 manifest）
  gh workflow run publish-release.yml --repo mclaw/mclaw --ref {BRANCH} -f tag={TAG} -f force_update=true
...
STEP 2 — 监控执行
  ...
  gh run list --repo mclaw/mclaw --workflow=backfill-oss.yml --limit 1
  → 反复轮询直到完成
...
foreach ($ch in $channels) {
  try {
    $r = Invoke-WebRequest -Uri "https://dl-mclaw.fzstack.com/api/$ch.json" -UseBasicParsing -ErrorAction Stop
...
# 检查 versions.json
$r = Invoke-WebRequest -Uri "https://dl-mclaw.fzstack.com/api/versions.json" -UseBasicParsing
$j = $r.Content | ConvertFrom-Json
Write-Host "versions.json: release=$($j.release.Count) pre_release=$($j.pre_release.Count) dev=$($j.dev.Count)"

# 检查 GitHub 最新 Release
gh release list --repo mclaw/mclaw --limit 5
