# Mclaw 桌面终端 - 使用指南与功能介绍

> Mclaw Desktop Terminal —— 集 AI 聊天、IM 通道监控、技能管理、系统状态、全流程配置于一体的桌面客户端。
...
Mclaw 桌面终端基于 **Tauri 2 + React 18** 构建，通过 HTTP SSE 连接后端 `mclaw serve` 服务。支持两种运行模式：

- **本地模式**：桌面端自动启动本地后端服务（需完成配置向导）
- **远程模式**：连接已在运行的 `mclaw serve` 实例（适合开发调试或服务器部署）
...
2. **启动服务**：顶栏点击「启动服务」，或在终端运行 `mclaw serve`
...
技能（Skills）是 Mclaw 的可扩展能力模块，遵循 Agent Skills 标准。
...
1. **远程调试**：在终端运行 `mclaw serve`，桌面端点击「连接服务」输入地址，Python 代码修改即时生效（无需重编译）
...
*本文档基于 Mclaw v1.10.2，最后更新：2026-02-12*
