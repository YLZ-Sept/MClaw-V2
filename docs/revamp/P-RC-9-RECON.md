# P-RC-9 Recon -- ``src/mclaw/orgs/`` integral migration
...
* Legacy package size: **26 files, 18 213 LOC** (``wc -l
  src/mclaw/orgs/*.py`` -> sum).
* v1 REST surface: **89 endpoints, 2 145 LOC** in
  ``src/mclaw/api/routes/orgs.py``
...
* Production caller sites (``git grep -nE 'from mclaw\.orgs'
  -- src/mclaw/`` | ``wc -l``): **86** across 13 unique files;
...
Captured via::
    ...
    Get-ChildItem src/mclaw/orgs/ -File
    git grep -nE '^(class|def|async def) '   -- src/mclaw/orgs/
    git grep -cE '    def |    async def '  -- src/mclaw/orgs/<file>
...
Captured via::
    ...
    git grep -nE 'from mclaw\.orgs' -- src/mclaw/ > recon/callers_src.txt
    git grep -nE 'from mclaw\.orgs' -- tests/        > recon/callers_tests.txt
...
Captured via ``git grep -nE '^@router\.' -- src/mclaw/api/routes/orgs.py``.
...
``mclaw.orgs``, v2 from ``mclaw.runtime.orgs``), fixtures
