...
> Replaces v1 ``mclaw.orgs.blackboard.OrgBlackboard`` (344
...
> Replaces v1 ``mclaw.orgs.project_store.ProjectStore``
...
> Replaces v1 ``mclaw.orgs.node_scheduler.OrgNodeScheduler``
...
> ``mclaw.orgs.command_service.OrgCommandService`` with a
> Protocol-typed, dependency-injected v2 surface under
...
> v1 ``mclaw.orgs.manager.OrgManager`` (683 LOC, 24 public
...
| _this commit_ | P-RC-9 G-RC-9.6 | docs(revamp): G-RC-9.6 P9.6 (OrgRuntime) mini-gate -- PASS (closes P9.6; 22 commits clean; parity 20/20 + contract 25/25; sentinel 6/6 ACTIVE; ADR-0014 budget held 2708 of 3000 LOC; v1 ``src/mclaw/orgs/`` untouched; ACCEPTANCE.md NOT bumped) | +PLACEHOLDER (G-RC-9.6.md NEW 338 LOC + ledger header bump + ledger row +5) | 0 (gate review; cites measured numbers from full pytest 6538p/116s/5xf/13f in 1043.33s + narrowed slice 1457p/12s/5xf in 113.74s) | ADR-0011 (Protocol granularity ceiling held; 4 new public Protocols all <=5 methods); ADR-0014 (OrgRuntime budget revision empirical outcome 2708 LOC of 3000 LOC); ADR-0012 (no-shim invariant held: 22 commits, zero src/mclaw/{orgs,core,channels,api}/ touch); ADR-0013 (2 wall-clock SLA perf_counter contract cases per NIT-I-1 lesson) |
...
| commit hash | phase | title | LOC delta | tests delta | ADR refs |
|---|---|---|---|---|---|
| _this commit_ | P-RC-9 P9.7.charter | docs(revamp): add P9.7 v2 REST endpoint charter (planning round) | +PLACEHOLDER (P-RC-9-P9.7-CHARTER.md NEW ~380 + ledger ~16) | 0 (planning only; ``git diff 89703a28..HEAD -- src/mclaw/ tests/`` empty) | ADR-0011 (no new Protocols per R4); ADR-0012 (v1 delete waits for P9.9 per Q-B 410); ADR-0013 (perf_counter SLA extended to ~5 REST cases); ADR-0014 (budget precedent; ADR-0015 NOT needed this round) |
...
| commit hash | phase | title | LOC delta | tests delta | ADR refs |
|---|---|---|---|---|---|
| _this commit_ | P-RC-9 P9.7a-1 | docs(revamp): P9.7a-1 endpoint inventory + decisions (R1=R3 locked, R2 recon) | +482 LOC (P-RC-9-P9.7-ENDPOINT-INVENTORY.md NEW 254 + P-RC-9-P9.7-DECISIONS.md NEW 220 + ledger +8) | 0 (docs-only; ``git diff 096a5571..HEAD -- src/mclaw/ tests/ apps/`` empty) | ADR-0011 (D-3 layer separation: schemas/orgs_v2/* decoupled from router; D-4 flat helpers per R4 granularity ceiling); ADR-0012 (D-1 R3 308 redirect shim is the only relaxation; physical v1 delete still waits for P9.9); cites P-RC-9-P9.7-CHARTER.md sec 1 + 4 + 8 |
...
> ``0735501e`` created the ``src/mclaw/api/schemas/`` package
> to host ``schemas/orgs_v2/`` but did NOT move the legacy
> ``src/mclaw/api/schemas.py`` contents into the new package
...
``src/mclaw/channels/``, which P9.7's strict-additive
...
> ``src/mclaw/channels/`` 0 HTTP callers confirmed);
> Python ``from mclaw.orgs`` import sweep (90 src + 228
...
> ``src/mclaw/channels/`` confirmed **0** HTTP callers
> (5 in-process ``from mclaw.orgs.command_service``
...
> src/mclaw/ tests/ apps/`` returns empty bytes.
...
> src/mclaw/`` returns empty bytes. P9.8gamma-2
...
> Strict additive verified: ``git diff 754ff465..HEAD -- src/mclaw/``
...
> verified: ``git diff 754ff465..HEAD -- src/mclaw/`` returns
...
> three commits: ``git diff 754ff465..HEAD -- src/mclaw/``
...
> All 3 ride to P9.9 deletion alongside ``src/mclaw/api/routes/orgs.py``.
>
...
> src/mclaw/ apps/setup-center/src/`` returns empty bytes; only
...
> ACTIVE + ``git diff src/mclaw/`` empty bytes); §11 NIT
...
> 4b8a9ad8..HEAD -- src/mclaw/`` returns empty bytes across
...
> src/mclaw/orgs/`` (~18 000 LOC, 6 subsystems) + v1 router
...
> ``from mclaw.orgs.X`` import rewrite sites. P9.9 charter
...
> verification: ``src/mclaw/api/routes/orgs_v2_runtime_orgs.py``
...
> verified: ``git diff 99606d6c..HEAD -- src/mclaw/ apps/
...
> diff d1bec779..HEAD -- src/mclaw/ tests/ apps/`` returns
...
> dominated by ``src/mclaw/runtime/`` (charter 14 -> real 1;
> remaining 13 are pure substring matches on the v2 dotted path
> ``mclaw.runtime.orgs.X``; §6 false-positive forensics in
...
> verbatim 1-to-1 module-prefix swap ``mclaw.orgs.command_service → mclaw.runtime.orgs.command_service``
...
> clean (``ruff check src/mclaw/channels/gateway.py`` — All
> checks passed!). (4) Module-import smoke green
> (``python -c "import mclaw.channels.gateway"`` resolves
...
> src/mclaw/orgs/ src/mclaw/api/ src/mclaw/runtime/ apps/
> tests/`` returns empty bytes — only ``src/mclaw/channels/gateway.py`` (1 source file) + this
> ``docs/revamp/PROGRESS_LEDGER_P9.md`` (+1 row) touched. v1
> ``src/mclaw/orgs/`` byte-untouched (ε will delete; β
...
> (3) Module-import smoke green: ``python -c "import mclaw.api.routes.chat; import mclaw.api.routes.orgs_v2_runtime_orgs; import mclaw.api.server"``
...
> ``ruff format --check src/mclaw/api/server.py`` clean;
> ``ruff format --check src/mclaw/api/routes/chat.py``
...
> src/mclaw/orgs/ src/mclaw/channels/ src/mclaw/runtime/
> src/mclaw/core/ apps/ tests/`` returns empty bytes — only
> ``src/mclaw/api/routes/chat.py`` + ``src/mclaw/api/server.py``
...
| _this commit_ | P-RC-9 P9.9γ-2 | refactor(core): P9.9γ-2 swap core/ import v1→v2 runtime (1 of 2 sites; 1 deferred on v2-internal absorption debt) [P-RC-9 P9.9γ-2] | +~2 net source LOC (``core/_reasoning_engine_legacy.py`` 1 ins / 1 del at L7920 deferred import; ledger +~73 LOC; total ~75 under charter §6 80-LOC γ-2 cap) | 0 (core/-only source touch; ``git diff 09fdb795..HEAD -- src/mclaw/orgs/ src/mclaw/channels/ src/mclaw/runtime/ src/mclaw/api/ apps/ tests/`` returns empty bytes; 8 / 8 P-RC-9 sentinels unchanged) | ADR-0011 (mechanical re-route to v2 ``runtime.orgs.runtime``); ADR-0012 (γ-before-ε: core/ must resolve to v2 BEFORE ε-1 ``git rm`` per R3); canary 3/3 PASS |

> P9.9γ-2 swaps the single cross-tree ``mclaw.orgs.*``
> import in ``src/mclaw/core/`` onto v2 ``runtime.orgs.runtime``
...
>     BEFORE: from mclaw.orgs.runtime import get_runtime
>     AFTER:  from mclaw.runtime.orgs.runtime import get_runtime
...
> ``from mclaw.orgs.models import (NodeSchedule,
...
> anywhere under ``src/mclaw/runtime/``. The α-1 §3
...
> delta. (3) Module-import smoke green: ``import mclaw;
> import mclaw.agent; import mclaw.core._reasoning_engine_legacy;
> import mclaw.runtime.orgs.runtime`` resolves with the
> new L7920 line traversed (direct ``python -c "import
> mclaw.core._reasoning_engine_legacy"`` triggers a
...
> src/mclaw/orgs/ src/mclaw/channels/ src/mclaw/runtime/
> src/mclaw/api/ apps/ tests/`` returns empty bytes —
> only ``src/mclaw/core/_reasoning_engine_legacy.py`` +
> this ledger touched. Cumulative ``git diff 112bc62b..HEAD
> -- src/mclaw/orgs/`` (both γ commits): empty bytes
...
> BEFORE: 12-line ``from mclaw.orgs.models import
...
> AFTER: 10-line ``from mclaw.runtime.orgs.org_models
> import (Organization, OrgEdge, OrgNode, OrgStatus,
> UserPersona, _new_id, _now_iso,
> infer_agent_profile_id_for_node)`` + 1-line
> ``from mclaw.runtime.orgs.scheduler_models import
...
> ``python -c "import mclaw.api.server; import
> mclaw.api.routes.orgs_v2_runtime_orgs; import
> mclaw.runtime.orgs.manager; import
> mclaw.runtime.orgs.org_models"`` returns clean.
> (6) Ruff lint clean on edited files (``ruff check
> src/mclaw/runtime/orgs/org_models.py /
> manager.py / __init__.py`` all checks passed!).
>
> Strict-additive boundary: ``git diff ebd8153d..HEAD
> -- src/mclaw/orgs/`` returns empty bytes -- only
...
> for ``mclaw.orgs.models`` was aspirational on
...
> below):
>
> * From v1 ``mclaw.orgs.templates`` (~1230 LOC):
...
> * From v1 ``mclaw.orgs.plugin_workbench_templates``
...
> * From v1 ``mclaw.orgs.tool_categories`` (~55 LOC
...
>    mclaw.orgs.tool_categories import get_avatar_for_role``
>    line -- the function now lives in the same module,
>    no import needed.
> 2. ``_auto_assign_agent_profiles`` reroutes its v1
>    deferred ``from mclaw.orgs.models import
...
>
> * ``api/server.py:363``: ``from mclaw.orgs.templates
>   import ensure_builtin_templates`` ->
>   ``from mclaw.runtime.orgs._runtime_templates
>   import ensure_builtin_templates``.
> * ``api/routes/orgs_v2_runtime_orgs.py:99``:
>   ``from mclaw.orgs.tool_categories import
>   list_avatar_presets`` ->
>   ``from mclaw.runtime.orgs._runtime_templates
>   import list_avatar_presets``.
> * ``api/routes/orgs_v2_runtime_orgs.py:134``:
>   ``from mclaw.orgs.plugin_workbench_templates
...
> smoke green: ``python -c "import mclaw.api.server;
> import mclaw.api.routes.orgs_v2_runtime_orgs;
> import mclaw.runtime.orgs.manager;
> import mclaw.runtime.orgs.org_models;
> import mclaw.runtime.orgs._runtime_templates"``
> returns clean. (5) Ruff lint clean on all edited files
> (``ruff check src/mclaw/runtime/orgs/_runtime_templates.py
> src/mclaw/runtime/orgs/__init__.py
> src/mclaw/api/server.py
> src/mclaw/api/routes/orgs_v2_runtime_orgs.py``
> all checks passed!).
>
> Strict-additive boundary: ``git diff ef8ebfd7..HEAD
> -- src/mclaw/orgs/`` returns empty bytes; cumulative
> ``git diff ebd8153d..HEAD -- src/mclaw/orgs/`` also
...
> diff 459323d7..HEAD -- src/mclaw/ tests/ apps/``
...
>   drop ``mclaw.orgs.blackboard.OrgBlackboard as
>   V1Blackboard`` + ``mclaw.orgs.models.{MemoryScope,
>   MemoryType}``; keep ``mclaw.runtime.orgs.blackboard``
>   + ``mclaw.runtime.orgs.memory_models`` runner; load
>   golden from ``_golden_blackboard.json`` (164 LOC; 8
>   cases).
> * ``test_command_service_parity.py`` (10 cases / 362 →
>   318 LOC): drop ``mclaw.orgs.command_service``
>   class loader; keep ``mclaw.runtime.orgs.command_models``
>   loader; load golden from ``_golden_command_service.json``
>   (167 LOC; 10 cases).
> * ``test_manager_parity.py`` (12 cases / 319 → 289
>   LOC): drop ``mclaw.orgs.manager.OrgManager``;
>   keep ``mclaw.runtime.orgs.manager.OrgManager``;
...
>   184 LOC): drop ``mclaw.orgs.node_scheduler.OrgNodeScheduler``
>   + ``mclaw.orgs.models.{NodeSchedule, ScheduleType}``;
>   keep ``mclaw.runtime.orgs.node_scheduler.compute_next_fire_time``
...
>   255 LOC): drop ``mclaw.orgs.project_store.ProjectStore``
>   + ``mclaw.orgs.models.{OrgProject, ProjectTask,
>   TaskStatus}``; keep ``mclaw.runtime.orgs.project_models``
>   shard + ``mclaw.runtime.orgs.project_store.JsonProjectStore``;
...
> a3a5fde6..HEAD -- src/mclaw/orgs/`` empty. ``grep
> -rn "mclaw\.orgs\." tests/parity/orgs/`` = 0 hits
...
>   strings → ``mclaw.runtime.orgs.runtime.get_runtime``.
...
>   ``mclaw.agents.profile.AgentProfile`` +
>   ``mclaw.orgs.models.OrgNode`` + ``mclaw.orgs.runtime.OrgRuntime``
...
>   ``mclaw.orgs.models.Organization`` → ``runtime.orgs.org_models``;
>   ``mclaw.orgs.manager.OrgManager`` x2 → ``runtime.orgs.manager``;
>   ``mclaw.orgs.runtime.get_runtime`` patch strings x2 →
>   ``mclaw.runtime.orgs.runtime.get_runtime``.
> * ``test_remaining_qa_fixes.py`` (+13 / -17; 2 sites):
>   ``mclaw.orgs.models.OrgNode`` → 1:1 swap to
>   ``runtime.orgs.org_models``; ``mclaw.orgs.runtime.OrgRuntime``
...
>   2 pre-existing ruff fixes): ``mclaw.orgs.runtime.OrgRuntime``
...
> src/mclaw/orgs/`` returns empty bytes — only ``tests/unit/``
> (8 files) + ``tests/parity/orgs/`` (5 .py + 5 .json from
> δ-2a) + ``docs/revamp/PROGRESS_LEDGER_P9.md`` touched.
> ``grep -rn "mclaw\.orgs\." tests/parity/orgs/ tests/unit/``
...
> "mclaw\.orgs\."`` verification catches but the inventory
...
>   147 / 174 / 198 ``from mclaw.orgs import command_service as
>   cs_module``); ``project_store.ProjectStore`` ``mock.patch``
>   target string → ``mclaw.runtime.orgs.project_store.ProjectStore``
...
>   ``mclaw.orgs.tool_handler`` is still pending decomposition per
...
>   ``"v2 reaches the free function in ``mclaw.orgs.tool_categories``."``
>   → ``"v2 reaches the free function in
>   ``mclaw.runtime.orgs._runtime_templates`` (was v1
...
>   ``grep "mclaw\.orgs\."`` returns zero across tests/api/.
>
...
>   all 5 deferred ``from mclaw.orgs import command_service as
>   cs_module`` imports at lines 113 / 139 / 147 / 174 / 198 →
>   ``from mclaw.runtime.orgs import command_service as cs_module``;
...
> ``Path("src/mclaw/core/reasoning_engine.py")`` which became
...
> closed, every v1 ``mclaw.orgs.*`` reference outside
...
| _this commit_ | P-RC-9 P9.9δ-4-pre | docs(tests/parity/orgs): P9.9δ-4-pre README v1→v2 import example sweep (δ-2a leftover) [P-RC-9 P9.9δ-4-pre] | 1 .md line rewritten (+1 / -1 = net 0 LOC) + ledger this section | post-edit ``git grep -n "mclaw\.orgs\." -- tests/parity/orgs/`` = 0 hits; full-tree ``git grep -ln "mclaw\.orgs\." -- tests/`` = 47 files, **all 47 inside ``tests/orgs/``** (0 outside leakage) -- pre-condition #3 for the upcoming atomic ``git rm -r tests/orgs/`` now PASSES | ADR-0012 (v1 deletion at P9.9 per Q-B ACCEPTED (b)) |

> P9.9δ-4-pre is a single-file documentation sweep absorbing the
> one residual ``mclaw.orgs.`` textual hit that δ-2a's Option-B
...
> v1 ``mclaw.orgs.blackboard.OrgBlackboard``. Swapped 1:1 to
> ``mclaw.runtime.orgs.blackboard.OrgBlackboard`` so the δ-4
> atomic deletion's post-condition ``grep "mclaw\.orgs\."
...
> src/mclaw/orgs/`` = 0 bytes; (3) grep 47 hits all inside
...
> "mclaw\.orgs\." -- tests/`` = **0 files**; narrow slice 585 / 585
...
> -- src/mclaw/orgs/`` still 0 bytes (strict-additive on v1 src
...
> ``src/mclaw/orgs/``), sub-phase breakdown (scheme C 4
...
>   grep ``^(\s+)?(from|import)\s+mclaw\.orgs(\.|$|\s)``
...
> * **§4 308 shim cleanness** -- 101 LOC / **0 ``mclaw.orgs``
...
> ``src/mclaw/`` callers) clean. Scheme C path: ε-1a charter
> ✓ (``0765b3e0``) → ε-1b audit ✓ _this_ → ε-2a v1 router
> + dev-script retire (estimated ~80 ins / -3 100 del) → ε-2b
> atomic ``git rm -r src/mclaw/orgs/`` (estimated ~30 ins /
...
| _this commit_ | P-RC-9 P9.9ε-2a | refactor(api,scripts): P9.9ε-2a retire v1 orgs router + dev scripts + regen OpenAPI snapshot [P-RC-9 P9.9ε-2a] | -3018 del (router 2533 + run_org_live_test.py 277 + test_org_full_task.py 205 + server.py 3 mount/import lines); +N ins (ledger row only); snapshot _openapi_snapshot.json bytewise unchanged (v2-only structurally) | 0 (`git diff a3a5fde6..HEAD -- src/mclaw/orgs/` STILL empty bytes; `git ls-files src/mclaw/orgs/` STILL 26; v1 src physical delete deferred to ε-2b) | ADR-0011 (no-shim invariant on production wiring); ADR-0012 (v1 deletion at P9.9; this commit retires the HTTP surface ahead of the directory delete in ε-2b); ADR-0015 (308 shim byte-untouched: `_orgs_v2_legacy_redirects.py` 0 v1 literals before, 0 after); canary 3/3 PASS within +0.7% of pre-edit 1.857s baseline; sentinel #7 GREEN pre+post |
...
> R-ε-1 conditional callers of v1 `mclaw.orgs.*`:
>
> 1. `src/mclaw/api/routes/orgs.py` -- the v1 REST router
>    itself (2533 LOC / 89 `@router.<method>` decorators / 24
>    `mclaw.orgs.*` import sites enumerated in audit
...
>
> **`src/mclaw/api/server.py` edits (3 lines removed)**:
>
...
>   `mclaw.api.routes.orgs` module and must go together.
...
>
> * Strict import grep
>   `git grep -n -E "^(\s+)?(from|import)\s+mclaw\.orgs(\.|$|\s)"
>   -- src/mclaw/ ":(exclude)src/mclaw/orgs/" apps/
...
>   inside `src/mclaw/runtime/orgs/*` and
>   `src/mclaw/api/schemas/orgs_v2/*` are all
>   docstring/comment back-references ("replaces v1
>   `mclaw.orgs.X`") cataloged in audit section 2.1 as
>   non-imports.
> * v1 src strict-additive on
>   `src/mclaw/orgs/` STILL holds: `git diff
>   a3a5fde6..HEAD -- src/mclaw/orgs/` returns empty
>   bytes; `git ls-files src/mclaw/orgs/` STILL returns
...
>   `src/mclaw/api/routes/_orgs_v2_legacy_redirects.py`
...
> the atomic `git rm -r src/mclaw/orgs/` (26 files /
...
## P9.9ε-2b -- atomic delete src/mclaw/orgs/ (26 files / 20237 LOC; v1 src surface fully closed; R-ε-1..R-ε-4 all RETIRED; HARD STOP)

| _this commit_ | P-RC-9 P9.9ε-2b | chore(src): P9.9ε-2b atomic delete src/mclaw/orgs/ (26 files / 20237 LOC, R-ε-1 RETIRED, v1 src surface fully closed) [P-RC-9 P9.9ε-2b] | -20237 del (26 .py under src/mclaw/orgs/: top-5 runtime 6355 + tool_handler 3474 + templates 1266 + models 1018 + command_service 963 = 13076 LOC / 65%; long-tail 21 files = 7161 LOC; full per-file table in ε-AUDIT §1); +~30 ins (ledger row only); deletion-only commit (largest single deletion of P-RC-9, exceeds δ-4 at -12238) | 0 (tests/ untouched per ε hard rules; tests v1 grep already 0 since δ-4) | ADR-0011 (no-shim invariant; v2 ``runtime/orgs/`` is sole in-tree home of org subsystem post-commit); ADR-0012 (v1 deletion at P9.9 per Q-B ACCEPTED (b); this commit closes ALL v1 src under ``src/mclaw/orgs/``); ADR-0015 (308 shim ``_orgs_v2_legacy_redirects.py`` byte-untouched: 0 v1 literals before and after; ``runtime/orgs/`` byte-untouched); narrow slice 585/585 unchanged; canary 3/3 PASS within ±5% of pre-deletion baseline; sentinel #7 GREEN pre+post (snapshot byte-identical; v2 surface structurally immune per ε-2a evidence) |

> P9.9ε-2b closes the ε phase by atomically deleting the v1 org
> subsystem at ``src/mclaw/orgs/``. After this commit the
> entire v1 org code surface no longer exists in the working
> tree; ``src/mclaw/runtime/orgs/`` (23 files / 10 886 LOC)
...
>    HEAD ``857a5a35``.
> 2. ``git diff a3a5fde6..HEAD -- src/mclaw/orgs/`` = 0
...
>    ``^(\s+)?(from|import)\s+mclaw\.orgs(\.|$|\s)``
>    across ``src/mclaw/`` (excluding ``src/mclaw/orgs/``)
>    + ``apps/`` + ``scripts/`` + ``identity/`` = **0 files /
>    0 sites** (ε-2a closed all 30 sites; none regressed).
> 4. Loose grep ``mclaw\.orgs`` across same scope (also
>    excluding ``src/mclaw/runtime/orgs/`` per ε-AUDIT §2.1
>    docstring-only set) returns **3 files**:
>    ``src/mclaw/api/routes/orgs_v2_runtime_state.py:23``
>    (docstring contrasting ``mclaw.runtime.orgs`` vs v1
>    ``mclaw.orgs``), ``src/mclaw/api/schemas/orgs_v2/nodes.py:3``
>    (module docstring ``Mirrors the wire-stable subset of
>    mclaw.orgs.models.OrgNode``), ``src/mclaw/api/schemas/orgs_v2/orgs.py:3``
>    (module docstring ``Mirrors the wire-stable subset of
>    mclaw.orgs.models.Organization``). All 3 hits are pure
...
> 5. Tests v1 strict grep ``mclaw\.orgs\.`` across
...
> 9. ``git ls-files src/mclaw/orgs/`` = **26** (exact match
>    of ε-AUDIT §1 inventory).
> 10. ``python -c "sum(LOC)"`` over ``src/mclaw/orgs/*.py``
>     = **20 237 LOC** (exact match of ε-AUDIT §1 total).
>
> **The atomic deletion**: single command
> ``git rm -r src/mclaw/orgs/`` removes all 26 tracked
...
>
> * ``ls src/mclaw/orgs/`` -- path does not exist (PowerShell
...
> * ``git ls-files src/mclaw/orgs/`` -- **0 files**.
> * ``git status`` (porcelain) -- 26 staged deletions under
>   ``src/mclaw/orgs/`` + 1 staged modification on
...
> * ``git diff a3a5fde6..HEAD -- src/mclaw/orgs/`` -- now
...
> * Production v1 grep clean: ``git grep -ln "mclaw\.orgs\."
>   -- src/mclaw/ apps/ scripts/ identity/ tests/`` = **0
>   files**. Entire repo is now v1-import-free; only docstring
>   back-references in ``src/mclaw/runtime/orgs/`` and 3
...
>   (src/mclaw/orgs/ non-existence gate + production-grep
...
> the diff ``git diff a3a5fde6..HEAD -- src/mclaw/orgs/``
...
> disappearance: the only edit to ``src/mclaw/orgs/`` over
...
>   src/mclaw/orgs/`` (-20 237 LOC del; 26 files; deletion-
>   only).
>
...
>   ``git ls-files src/mclaw/orgs/`` MUST return 0 (the
>   directory must not be re-created); (ii) production strict-
>   grep ``^(\s+)?(from|import)\s+mclaw\.orgs(\.|$|\s)``
>   across ``src/mclaw/`` + ``apps/`` + ``scripts/`` +
...
>   up gate sealing the P-RC-9 ``src/mclaw/orgs/`` integral
...
> | commit hash | phase | title | LOC delta | tests delta | ADR refs |
> |---|---|---|---|---|---|
> | _this commit_ | P-RC-9 P9.9eta-1a | test(parity/orgs): 9th sentinel (v1 src retired) -- two active invariants for src/mclaw/orgs/ retirement + zero-v1-import production scan | +PLACEHOLDER (sentinel test ~300 + ledger row ~15) | +2 (test_v1_src_directory_retired + test_production_imports_v1_free) | ADR-0011 (no new Protocol; ceiling held); ADR-0012 (v1 deletion at P9.9; sentinel locks in the retirement); ADR-0015 (308 shim NO-OP; sentinel scope is mclaw.orgs.* import literals, not api/routes/) |
>
...
>
> 1. ``test_v1_src_directory_retired`` -- ``src/mclaw/orgs/``
...
>    regex ``^\s*(?:from|import)\s+mclaw\.orgs(?:\.|$|\s)``
>    walks ``*.py`` + ``*.pyi`` under ``src/mclaw/`` + ``apps/``
...
> descent; (ii) bytes-level pre-filter (``b"mclaw.orgs"
...
> mentions ``mclaw.orgs.X`` as the module they replaced)
