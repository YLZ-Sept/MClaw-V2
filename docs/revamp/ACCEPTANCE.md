# Mclaw v2.0.0-rc2 — Acceptance Criteria Verification

This document maps the **five acceptance criteria** from the original
backend revamp plan
(`mclaw_full_backend_revamp_e6d8610d.plan.md` §9) to the
...
`mclaw_revamp_continuation_plan_d6192647.plan.md` §9 says this
...
* `src/mclaw/runtime/supervisor.py` (no `max_task_seconds`
...
* **Wiring proof** — `src/mclaw/channels/gateway.py` per-org
...
* `tests/runtime/test_supervisor.py::test_cancel_writes_final_checkpoint`.
* `src/mclaw/channels/gateway.py` (cancel verb wiring).
* `src/mclaw/runtime/cancel_token.py` (`CancellationToken`).
...
* `src/mclaw/runtime/checkpoint.py` +
...
* `src/mclaw/runtime/nodes/workbench_node.py` +
  `src/mclaw/runtime/nodes/manifest.py`.
...
* `src/mclaw/api/routes/orgs_v2.py` (route definitions).
* `src/mclaw/runtime/templates/registry.py` +
