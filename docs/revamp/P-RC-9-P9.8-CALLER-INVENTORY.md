...
Per charter sec 4 + sec 5 R3: ``src/mclaw/channels/``
...
|---|---:|---|
| ``src/mclaw/channels/gateway.py`` | 3182 | ``from mclaw.orgs.command_service import get_command_service`` |
| ``src/mclaw/channels/gateway.py`` | 3256 | ``from mclaw.orgs.command_service import get_command_service`` |
| ``src/mclaw/channels/gateway.py`` | 3335 | ``from mclaw.orgs.command_service import get_command_service`` |
| ``src/mclaw/channels/gateway.py`` | 3449 | ``from mclaw.orgs.command_service import get_command_service`` |
| ``src/mclaw/channels/gateway.py`` | 3739 | ``from mclaw.orgs.command_service import (...)`` (multi-symbol) |

**Verdict: NO-OP in P9.8.** P9.9 rewrites target from
``mclaw.orgs.command_service`` (v1) to
``mclaw.runtime.orgs.command_service`` (P9.4 vintage)
alongside ``git rm -r src/mclaw/orgs/``.
...
src/mclaw/ tests/ apps/`` returns empty bytes both before
