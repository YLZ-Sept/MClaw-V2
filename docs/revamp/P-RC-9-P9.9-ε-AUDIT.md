...
v1 src: ``git diff a3a5fde6..HEAD -- src/mclaw/orgs/`` =
empty bytes.
...
| tree | files | LOC |
|---|--:|--:|
| ``src/mclaw/orgs/`` (v1; **ε-2b delete target**) | 26 | 20 237 |
| ``src/mclaw/runtime/orgs/`` (v2 destination) | 23 | 10 886 |
| ``src/mclaw/api/routes/orgs.py`` (v1 router; **ε-2a delete target**) | 1 | 2 533 |
| ``src/mclaw/api/routes/_orgs_v2_legacy_redirects.py`` (308 shim; **ADR-0015 NO-OP**) | 1 | 101 |
...
```
git ls-files src/mclaw/orgs/                                              # 26
python -c "from pathlib import Path; print(sum(len(p.read_text(encoding='utf-8',errors='ignore').splitlines()) for p in Path('src/mclaw/orgs').glob('*.py')))"   # 20237
git ls-files src/mclaw/runtime/orgs/                                      # 23
...
Per ``git ls-files src/mclaw/orgs/`` + per-file LF-line
...
``git grep -ln "mclaw\.orgs" -- src/mclaw/
":(exclude)src/mclaw/orgs/" apps/ scripts/ identity/`` returns
**22 files**. The bulk (19 files) are docstring / comment
back-references inside ``src/mclaw/runtime/orgs/*.py`` that
name the v1 module each v2 shard replaces (“replaces v1
``mclaw.orgs.X``”) -- **not** active imports. Per ADR-0011
no-shim invariant ``runtime/orgs/`` ships zero ``mclaw.orgs``
runtime imports.

### 2.2 Strict grep (real import statements only)

``git grep -n -E "^(\s+)?(from|import)\s+mclaw\.orgs(\.|$|\s)"
-- src/mclaw/ ":(exclude)src/mclaw/orgs/" apps/ scripts/
...
| ``src/mclaw/api/routes/orgs.py`` | 24 | v1 router itself (89 endpoints / 2 533 LOC): ``command_service`` (×2), ``models`` (×11), ``manager`` (×4), ``tool_categories`` (×2), ``plugin_workbench_templates`` (×1), ``blackboard`` (×2), ``project_store`` (×1), ``InboxPriority`` (×1) | ``git rm`` in ε-2a |
...
``git grep -n -E "from mclaw\.orgs import|import mclaw\.orgs(\s|$)"
-- src/mclaw/ apps/ scripts/ identity/`` returns **0 hits**.
...
|--:|---|---|---|
| 1 | ``__init__.py`` | ``runtime/orgs/__init__.py`` (v2 re-exports; no live ``from mclaw.orgs import`` consumer per §2.3) | COMPLETE |
...
| 26 | ``tools.py`` | global ``src/mclaw/tools/`` subsystem (out of orgs/; no live v1 caller) | ABSORBED-TRANSITIVELY |
...
``src/mclaw/api/routes/_orgs_v2_legacy_redirects.py``: 101
LOC; ``git grep -n "mclaw\.orgs" --
src/mclaw/api/routes/_orgs_v2_legacy_redirects.py`` returns
**0 hits**.

The shim imports ONLY ``fastapi.{APIRouter, Request, Response}``
and ``__future__.annotations`` -- zero ``mclaw.*`` imports
...
Therefore ε-2b deleting ``src/mclaw/orgs/`` has **zero
...
sites import ``mclaw.orgs.*`` at HEAD ``0765b3e0``:

* ``src/mclaw/api/routes/orgs.py`` -- the v1 router itself
...
4. ε-2b -- atomic ``git rm -r src/mclaw/orgs/`` (mission
...
a3a5fde6..HEAD -- src/mclaw/orgs/`` expected to remain
