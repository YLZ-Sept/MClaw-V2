...
## 1. Cluster A -- `mclaw.orgs.tool_categories` missing module
...
`git grep -nl "tool_categories" -- src/mclaw/` returns
4 stale callers:

* `src/mclaw/agents/factory.py` (charter cites line
  `370`).
* `src/mclaw/agents/profile.py` (**new this recon** --
  not in charter R-11-2 list; verify during P11.1).
* `src/mclaw/orgs/_runtime_templates.py` (charter cites
  `1634` as a comment only; verify before touching).
* `src/mclaw/tools/handlers/org_setup.py` (charter
  cites `129 / 440 / 731`; `731` is the one the G-RC-10
  ledger names as the stale caller).

### 1.3 Root cause hypothesis (verified)

`git log --all --oneline -- src/mclaw/orgs/
tool_categories.py` returns `90a7d77f` as the first commit
(deletion `P9.9 epsilon-2b atomic delete src/mclaw/
...
1. Create `src/mclaw/orgs/_runtime_tool_categories.py`
   with the 149-LOC body restored from `90a7d77f`'s parent
   (`git show 90a7d77f~1:src/mclaw/orgs/
   tool_categories.py`).
...
3. The 4 callers already import the public path
   `mclaw.orgs.tool_categories.<symbol>`; no caller
   edits required.
...
fixture imports `mclaw.core._reasoning_engine_legacy`
(or any sibling that pulls `mclaw.agent` early).
Representative: `tests/runtime/state_graph/guards/
test_tool_filters.py:13..20` -- the `legacy_aliases`
fixture does `from mclaw.core import
...
agent.errors`). Importing a submodule of `mclaw.agent`
...
failures read `src/mclaw/core/agent.py` as a file:
...
src/mclaw/core/` shows `_agent_legacy.py` only.
`git log --all --oneline -- src/mclaw/core/agent.py`
post-rename history:
...
* `tests/runtime/state_graph/guards/test_tool_filters.py`
  ...
  `mclaw.core._reasoning_engine_legacy as re_module`.
* `tests/unit/test_action_claim_guard.py` -- 1 error.
  Top-of-module:
  `from mclaw.core._reasoning_engine_legacy import
