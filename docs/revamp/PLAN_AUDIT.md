# Plan Audit — gap analysis vs `mclaw_full_backend_revamp_e6d8610d.plan.md`
...
**Done**: `src/mclaw/agent/state.py` (minimal `TaskState` /
`AgentState`).
...
- `plugins/manager.py` was supposed to "learn to read the new
  ...
  plain tool providers." A `rg WORKBENCH src/mclaw/plugins/`
...
**These do not exist in legacy code.** `src/mclaw/orgs/templates.py`
...
**Plan inheritance**: `src/mclaw/orgs/plugin_workbench_templates.py`
