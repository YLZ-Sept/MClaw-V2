...
(`mclaw_full_backend_revamp_e6d8610d.plan.md`) and the ADRs under
`docs/adr/`.
...
src/mclaw/runtime/nodes/         Phase 4 — first-class node types
...
src/mclaw/runtime/templates/    Phase 5 — typed templates + registry
  __init__.py                          public exports
...
registry. The Phase 6 facade should call::
    ...
    from mclaw.runtime.templates import discover_builtins, GLOBAL_REGISTRY
    discover_builtins()
...
| Slice | Status |
|---|---|
| `src/mclaw/api/routes/orgs_v2.py` (template facade) | **Done.** ``GET /api/v2/orgs/templates`` (list), ``GET /api/v2/orgs/templates/{id}`` (one), ``POST /api/v2/orgs/templates/{id}/instantiate`` (mint a fresh OrgV2). Gated by ``settings.runtime_v2_enabled``. 15 tests. |
| `runtime/templates/registry.py` survivable factory marker | **Done.** ``@template`` now also sets ``__mclaw_template_factory__`` on the function so :func:`collect_builtin_factories` can repopulate the registry without depending on the lazy ``_PENDING`` queue (which test fixtures sometimes drain). 3 new tests. |
...
| `src/mclaw/channels/gateway.py` per-org v2 route | Pending. |
...
Mechanical `git rm` of `src/mclaw/orgs/` and the rewritten
`src/mclaw/core/*.py` files. One commit per concern so a `git
...
3. **Post-RC mechanical cleanup checklist** (gated by the burn-in
   ...
   * Migrate ~20 production callers (channels gateway, api routes,
     ...
     `mclaw.orgs.*` to `runtime.orgs` + `runtime.state_graph`,
     then `git rm -r src/mclaw/orgs/` and the ~50
     `tests/orgs/*` files.
   * Migrate ~20 callers in `sessions/` / `memory/` / `tools/` /
     `agents/` from `mclaw.core.*` MOVE shims to
     `mclaw.agent.*`, then delete the shims.
...
4. Always:
   ...
   * `python -m ruff check src/mclaw/runtime src/mclaw/agent
     src/mclaw/plugins/manager.py tests/runtime tests/agent
...
from mclaw.runtime.stream import StreamBus
from mclaw.runtime.supervisor import Supervisor, DelegationResult
from mclaw.runtime.messenger import Messenger, InMemoryNodeRegistry
