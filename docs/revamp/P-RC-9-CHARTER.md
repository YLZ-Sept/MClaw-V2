...
`src/mclaw/orgs/` package (~880 KB across 26 files, 86
...
[^loc]: Measured with ``wc -l src/mclaw/api/routes/orgs.py`` on
...
established the blocker: the v2 `src/mclaw/runtime/orgs/`
...
mechanical `git rm -r src/mclaw/orgs/` would break:
...
To delete `src/mclaw/orgs/` cleanly, P-RC-9 must implement
the following six v2 subsystems under `src/mclaw/runtime/orgs/`
...
`src/mclaw/orgs/` surface live. The IM gateway, the `/api/orgs`
...
* New plan file under `c:\Users\Peilong_Hong\.cursor\plans\` —
  e.g. `mclaw_orgs_migration_p_rc_9_<id>.plan.md`.
