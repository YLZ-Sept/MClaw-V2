...
volume: it physically retires the v1 ``mclaw.orgs`` surface
...
* Production source v1-import-free: 0 files across
  ``src/mclaw/`` + ``apps/`` + ``scripts/`` + ``identity/``
...
| ``90a7d77f`` | P9.9eps-2b | +233 / **-20 237** | atomic ``git rm -r src/mclaw/orgs/`` (26 files; R-eps-1 RETIRED) |
...
| ``90a7d77f`` epsilon-2b | atomic ``git rm -r src/mclaw/orgs/`` | **-20 237** |
| -- | -- | **= -35 493** |
...
```
git grep -ln "mclaw\.orgs" -- src/mclaw/ ":(exclude)src/mclaw/runtime/orgs/" apps/ scripts/ identity/ tests/ ":(exclude)tests/runtime/orgs/"
```
...
outputs. The only auditable ``mclaw.orgs`` literal
...
``^\s*(?:from|import)\s+mclaw\.orgs(?:\.|$|\s)`` does
not match those.
...
| **M-3** | G-RC-9.6 | v1 method residue (``_recover_orphan_tasks`` et al.) | **CLOSED** | residue lived under ``src/mclaw/orgs/``; epsilon-2b atomic delete (``90a7d77f``; -20 237 LOC) closed by construction |
...
* **Scheme C YELLOW for epsilon** -- main P9.9 charter sec 5.5
  sketched epsilon as a single atomic ``git rm -r src/mclaw/orgs/``
...
* **9th sentinel scope** -- per main charter sec 7.2
  ...
  deletion) + ADR-0015 (sentinel scope is ``mclaw.orgs``
...
* Strict-additive backend boundary held across the entire
  ...
  ``src/mclaw/orgs/`` -- the only edit was its wholesale
