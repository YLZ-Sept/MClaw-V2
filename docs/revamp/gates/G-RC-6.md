...
* a 27 LOC thin shim at ``core/agent.py`` (lazy ``__getattr__``
  to ``_agent_legacy``);
* a real v2 ``Agent`` at ``mclaw.agent.core`` (336 LOC)
...
|---|---|---|---|
| ``src/mclaw/core/agent.py`` | 9602 | 27 | **-9575 (-99.7%)** |
| ``src/mclaw/agent/core.py``  | 68   | 336 | **+268 (+394%)** |
| ``src/mclaw/core/_agent_legacy.py`` | -- | 9208 | (renamed from agent.py post-extraction) |
| ``src/mclaw/runtime/desktop/attachments.py`` | -- | 245 | new |
| ``src/mclaw/runtime/desktop/__init__.py`` | -- | 38 | new |
| ``src/mclaw/agent/safety/destructive_intent.py`` | -- | 318 | new |
| ``src/mclaw/agent/safety/__init__.py`` | -- | 37 | new |
...
ruff clean over the v2 surface throughout (``src/mclaw/runtime
src/mclaw/agent src/mclaw/plugins/manager.py
src/mclaw/channels/gateway.py src/mclaw/config.py
src/mclaw/api/routes/ tests/runtime tests/agent tests/api
...
- The extracted helpers import from ``mclaw.core.risk_intent``
  and ``mclaw.core.trusted_paths`` (deep classifier + path
...
- The two unit tests ``tests/unit/test_desktop_attachment_*.py``
  ...
  ``mclaw.runtime.desktop.attachments`` import is also
  P-RC-7 work.
...
imports retargeted to ``mclaw.agent.*`` / ``mclaw.runtime.*``;
``git rm -r src/mclaw/orgs/`` once IM canary burn-in clears;
``git rm src/mclaw/core/_*_legacy.py`` once shim callers
