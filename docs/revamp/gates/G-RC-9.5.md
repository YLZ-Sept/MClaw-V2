...
schedules + runtime state. v1 ``src/mclaw/orgs/manager.py``
was 683 LOC / 37 methods; the v2 ``src/mclaw/runtime/orgs/manager.py``
...
The v1 charter subsystem under ``src/mclaw/orgs/`` is
**UNTOUCHED** -- ``git diff 7fc863b8..HEAD -- src/mclaw/orgs/``
...
3. ``git diff 7fc863b8..HEAD -- src/mclaw/orgs/ src/mclaw/core/ src/mclaw/channels/ src/mclaw/api/``
   -> **empty** (strict-additive boundary held; the v1
   subsystem under ``src/mclaw/orgs/`` and all peers
   are untouched).
...
zero src/mclaw/orgs/ touch; ACCEPTANCE.md NOT modified
