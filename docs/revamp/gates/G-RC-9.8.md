...
P9.7. v1 endpoint file ``src/mclaw/api/routes/orgs.py`` (2 533
LOC, 89 endpoints) is **BYTE-LEVEL UNTOUCHED**; v1 subsystem
``src/mclaw/orgs/`` (~18 000 LOC) is **BYTE-LEVEL UNTOUCHED**;
``src/mclaw/channels/`` is **BYTE-LEVEL UNTOUCHED** (5
...
held: ``git diff 4b8a9ad8..HEAD -- src/mclaw/`` returns empty
...
3. ``git diff 4b8a9ad8..HEAD -- src/mclaw/`` -> **empty bytes**
   (BYTE-LEVEL UNTOUCHED across all 8 P9.8 commits; v1 subsystem,
   v1 router, channels, and ``src/mclaw/`` writ large all
...
* ``git rm -r src/mclaw/orgs/`` (~18 000 LOC; 6 charter
  subsystems gone).
* ``src/mclaw/api/routes/orgs.py`` -> 410-Gone shim per Q-B
...
* Mechanical ``from mclaw.orgs.X`` rewrite (~90 src + ~228
  test sites) ; v1-internal imports delete with parent files,
  external-to-v1 imports rewrite to ``mclaw.runtime.orgs.X``.
...
``src/mclaw/orgs/`` deletion inventory, ``tests/orgs/``
