...
``src/mclaw/runtime/orgs/*`` slot back to the conventional
``src/mclaw/orgs/*`` location after P-RC-9 retired the v1
src surface, (ii) closed the 5 nits the G-RC-9.9 final
mini-gate deferred (M-2 / P9.7-B / epsilon-O1 / epsilon-O2 /
GroupC), and (iii) replaced the legacy ``mclaw.runtime.orgs``
...
deleted at P10.6 (``mclaw.runtime.orgs`` is now a
...
|---|---|---|
| (i) | Namespace flatten -- collapse ``src/mclaw/runtime/orgs/*`` (25 .py) back to ``src/mclaw/orgs/*`` so every downstream caller imports ``mclaw.orgs.*`` | **DELIVERED**. P10.1 atomic ``git mv``; P10.3a..f swept 124 sites; P10.6 deleted the shim and tightened sentinel #9 to fail-loud. |
...
|--:|---|---|---|
| 1 | ``src/mclaw/orgs/`` exists with 25+ .py files | **SATISFIED** | ``Get-ChildItem src/mclaw/orgs *.py`` returns 27 files (25 P10.1 originals + the P10.5a M-2 split products ``_runtime_agent_pipeline_executor.py`` + ``_runtime_plugin_assets_outputs.py``). |
| 2 | ``src/mclaw/runtime/orgs/`` gone or shim-only | **SATISFIED** | P10.6 (``cea93777``) ``git rm`` deleted ``__init__.py`` + pruned the empty directory; ``Test-Path src/mclaw/runtime/orgs`` -> False. |
| 3 | ``git grep -l 'mclaw\.runtime\.orgs' -- src/mclaw/ tests/ apps/ scripts/`` = 0 (or shim only pre-P10.6) | **SATISFIED** | At HEAD post-P10.6 the only matches are inside the sentinel file's strict regex source bytes (``tests/parity/orgs/test_v1_src_retired_sentinel.py``); ZERO live import statements. |
...
| ``d8275080`` | P10.2 | feat(orgs): add ``mclaw.runtime.orgs`` deprecation shim re-exporting from new location | +109 |
| ``5ac2c786`` | P10.3a | refactor(src/mclaw): sweep imports to canonical (31 sites / 12 files) | +35 |
| ``eb96fc15`` | P10.4 | test(sentinel-9): reverse Test 2 polarity (ban ``mclaw.runtime.orgs.*`` in src/) | -68 |
...
| ``e1680941`` | P10.3f | docs(src/mclaw): sweep docstring/comment refs (12 sites / 11 files) | +20 |
...
| ``cea93777`` | P10.6 | refactor(orgs): remove ``mclaw.runtime.orgs`` deprecation shim + tighten sentinel #9 whitelist | +103 |
| **TOTAL** | | | **~+1 343** |
...
* **17 cases** ``tests/unit/test_org_setup_tool.py`` --
  ``ModuleNotFoundError: No module named 'mclaw.orgs.tool_categories'``;
...
``import`` lines confirms none touch ``mclaw.orgs.*`` /
``mclaw.runtime.orgs.*`` in a way the P10.x sweeps
...
|---|---|---|---|
| ``mclaw.orgs.tool_categories`` missing module (17 ``test_org_setup_tool.py`` failures) | P9.9 epsilon-2b deletion not migrated; ``tools/handlers/org_setup.py:731`` is a stale caller | MED | P-RC-11 candidate |
