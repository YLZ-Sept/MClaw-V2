...
* **N-G7-1** (P8.0 ``caf5d7f3``): added v2-only smoke
  ...
  that builds ``mclaw.agent.brain.Brain`` against a stub
...
* **N-G7-3** (P8.0 ``caf5d7f3``): cleaned ``CORE_COMPONENTS`` dead
  string prefixes in ``src/mclaw/evolution/log_analyzer.py:75-76``
  from ``mclaw.core.brain`` / ``mclaw.core.agent`` (no
  longer match anything after P7.14 shim deletion) to
  ``mclaw.core._brain_legacy`` / ``mclaw.core._agent_legacy``
...
| Ruff (v2 surface) | ``src/mclaw/{runtime,agent} src/mclaw/plugins/manager.py tests/{runtime,agent,api,parity}`` | clean |
...
* ``src/mclaw/main.py:28`` had a top-level
  ``from .core.agent import Agent`` -- ``mclaw.exe --help``
  exited 1 with ``ModuleNotFoundError: No module named
  'mclaw.core.agent'`` at CLI startup.
* ``src/mclaw/mcp_server.py:75`` had the same import inside
...
2. Commit ``c676b759`` (P8.7-fix): one-line
   ...
   checks (import ``mclaw.main``, import
   ``mclaw.mcp_server``, ``python -m mclaw --help`` exit 0).
...
absolute form ``from mclaw.core.<deleted>``) across the entire
