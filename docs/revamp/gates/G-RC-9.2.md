...
P9.2 replaces v1 ``mclaw.orgs.project_store.ProjectStore``
(281 LOC, 15 public methods, single JSON-file backend) with a
v2 Protocol-typed surface under
``src/mclaw/runtime/orgs/project_store.py`` (~877 LOC
...
is in-house, lifted from existing Mclaw modules
...
* Caller redirection from ``mclaw.orgs.project_store`` to
  ``mclaw.runtime.orgs.project_store`` -- P9.8.
...
* **ADR-0012** (no shim under v1) -- P9.2 lands the v2 surface
  ...
  ``src/mclaw/orgs/`` (sentinel section 10). v1 deletion
  is P9.9.
...
```
git diff 4d2f3078..HEAD -- src/mclaw/orgs/ src/mclaw/core/ src/mclaw/channels/ src/mclaw/api/
