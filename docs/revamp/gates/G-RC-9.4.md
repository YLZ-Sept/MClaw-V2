...
v1 ``mclaw.orgs.command_service`` (963 LOC) with a
...
* Caller redirection ``mclaw.orgs.`` -> ``mclaw.runtime.orgs.`` -- P9.8.
...
* **ADR-0012** (no shim under v1) -- P9.4 lands the v2
  ...
  ``src/mclaw/orgs/`` (sentinel section 10.3). v1
  deletion is P9.9.
...
```
git diff ffb8b908..52d9bbc8 -- src/mclaw/orgs/ src/mclaw/core/ src/mclaw/channels/ src/mclaw/api/
# (empty -- v1 orgs/, core/, channels/, api/ untouched since G-RC-9.3 close)
```

The only ``src/mclaw/`` adjacent change in P9.4 is the
