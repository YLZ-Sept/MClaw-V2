# G-RC-9 -- P-RC-9 (v1 ``mclaw.orgs`` retirement) final roll-up gate
...
P-RC-9 is the v1 ``mclaw.orgs`` retirement epic. Across 10
...
* **Production source v1-import-free**: 0 ``mclaw.orgs`` import
  sites at HEAD ``a0bdb599`` across ``src/mclaw/`` + ``apps/``
...
integral v1 ``mclaw.orgs`` (~18 000 LOC src) -> v2 migration,
...
|---|--:|---|
| ``src/mclaw/runtime/orgs/`` | 23 | v2 OrgRuntime + 5 subsystem shards + absorption shards (``org_models.py``, ``_runtime_templates.py``) + 8 ``_runtime_*`` slices |
| ``src/mclaw/runtime/llm/`` | 5 | LLM gateway shards landed alongside OrgRuntime |
| ``src/mclaw/agent/`` | 42 | v2 Agent stack supporting orgs orchestration (pre-existing + P-RC-9 additions) |
| ``src/mclaw/api/routes/orgs_v2*.py`` | 9 | v2 REST mint (incl. 308 shim ``_orgs_v2_legacy_redirects.py``) |
| ``src/mclaw/api/schemas/orgs_v2/`` | 5 | v2 Pydantic schemas (commands / nodes / orgs / projects + ``__init__``) |
...
| layer | count | commit |
|---|--:|---|
| ``src/mclaw/orgs/`` (Python modules) | 26 | ``90a7d77f`` (P9.9eps-2b) |
| ``tests/orgs/`` (test files) | 48 | ``4b5499a6`` (P9.9delta-4) |
| ``src/mclaw/api/routes/orgs.py`` (v1 router) | 1 | ``857a5a35`` (P9.9eps-2a) |
...
| v1 deleted -- ``src/mclaw/orgs/*`` | 26 | **-20 237** | P9.9eps-2b atomic (``90a7d77f``) -- R-eps-1 RETIRED |
...
2. The 77-file v1 deletion total does **not** include any
   ...
   formerly under ``src/mclaw/orgs/`` + ``tests/orgs/`` +
