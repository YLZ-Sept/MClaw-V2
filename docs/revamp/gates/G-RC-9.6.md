...
``src/mclaw/orgs/runtime.py`` is **6 355 LOC across 132
...
``git diff fec6fed4..HEAD -- src/mclaw/{orgs,core,channels,api}/``
...
* **ADR-0011**: OrgRuntime is subsystem #6; the 8-module /
  ...
  (section 9 below).
* **ADR-0012**: v1 ``src/mclaw/orgs/runtime.py`` is
  UNTOUCHED through all 22 P9.6 commits; v2 sits entirely
  under ``src/mclaw/runtime/orgs/``. Cutover -> P9.7
...
3. ``git diff fec6fed4..HEAD -- src/mclaw/{orgs,core,channels,api}/``
   -> **empty bytes** (strict-additive boundary held; v1
   subsystem under ``src/mclaw/orgs/`` untouched).
...
OrgCommandService instead of v1 ``mclaw.orgs``. It is
...
``src/mclaw/orgs/`` touch across 22 commits;
...
physical deletion of ``src/mclaw/orgs/`` and final
