...
P9.1 replaces v1 ``mclaw.orgs.blackboard.OrgBlackboard``
(344 LOC, 19 methods, single JSONL backend) with a v2
Protocol-typed surface under
``src/mclaw/runtime/orgs/blackboard.py`` (~760 LOC across
...
* No external repo's blackboard / tuple-space patterns were
  ...
  semantics Mclaw needs (three-tier scope hierarchy +
...
* The Protocol + backend-factory pattern (``get_default_
  ...
  ``runtime/ledger.py`` from P-RC-3 -- this is Mclaw's
...
* Caller redirection from ``mclaw.orgs.blackboard`` to
  ``mclaw.runtime.orgs.blackboard`` -- P9.8.
...
* **ADR-0012** (no shim under v1) -- P9.1 lands the v2
  ...
  ``src/mclaw/orgs/``. v1 deletion is P9.9.
...
Root cause: ``src/mclaw/orgs/blackboard.py`` (v1) takes
