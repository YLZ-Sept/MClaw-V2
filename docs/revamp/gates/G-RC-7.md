...
``mclaw.core.{agent,brain,reasoning_engine,tool_executor,
context_manager}.py`` shims to the v2 ``mclaw.agent.*`` /
``mclaw.runtime.*`` public surfaces, then delete the shim
...
basis for the v2 ``mclaw.agent.*`` classes. Their deletion is
...
* Wholesale ``src/mclaw/orgs/`` deletion -- the v1 package is
  ...
  for.
* ``src/mclaw/core/supervisor.py`` deletion -- this is **not**
...
* ``23f8e05b`` -- refactor(api+agent): retarget Agent + smart_truncate imports to mclaw.agent.*
* ``f542e612`` -- refactor(agents/orchestrator): retarget smart_truncate imports to mclaw.agent.tools
* ``95b01f25`` -- refactor(agents/factory): retarget Agent imports to mclaw.agent.core
* ``8af86b08`` -- refactor(tools): retarget smart_truncate imports to mclaw.agent.tools
* ``716f622f`` -- refactor(memory): retarget smart_truncate + Brain imports to mclaw.agent.*
* ``d16a47f8`` -- refactor(sessions): retarget smart_truncate imports to mclaw.agent.tools
...
|---|---|---|---|
| ``src/mclaw/core/agent.py`` | 9602 | 42 | **DELETED** |
| ``src/mclaw/core/brain.py`` | 1370 | 25 | **DELETED** |
| ``src/mclaw/core/context_manager.py`` | 1166 | 36 | **DELETED** |
| ``src/mclaw/core/reasoning_engine.py`` | 8081 | 25 | **DELETED** |
| ``src/mclaw/core/tool_executor.py`` | 2103 | 41 | **DELETED** |
...
``mclaw.core.{agent,brain,reasoning_engine,tool_executor,
context_manager}``. ``mclaw.core`` is now purely a legacy
...
* **N-G6-3** (P7.0a ``eec1b068``): rewrote
  ``src/mclaw/core/agent.py`` docstring from "lazy delegation
  to ``mclaw.agent.core``" (incorrect during the P-RC-6
  shim window) to "lazy fallback to ``_agent_legacy``; v2
  implementations require explicit ``from mclaw.agent.core
...
legacy shim) to ``from mclaw.agent.confirmation import
get_confirmation_store`` (the canonical v2 home). At the point
``_agent_legacy.py`` runs this import, ``mclaw.agent.
...
|---|---|---|---|
| ``src/mclaw/sessions/`` | 2 | 2 ``smart_truncate`` | P7.1 ``d16a47f8`` |
| ``src/mclaw/memory/`` | 5 | 10 (8 ``smart_truncate`` + 2 ``Brain``) | P7.2 ``716f622f`` |
| ``src/mclaw/tools/`` (excl. handlers/) | 1 | 2 ``smart_truncate`` | P7.3 ``8af86b08`` |
| ``src/mclaw/agents/factory.py`` | 1 | 2 ``Agent`` / ``get_primary_agent`` | P7.4 ``95b01f25`` |
| ``src/mclaw/agents/orchestrator.py`` | 1 | 2 ``smart_truncate`` | P7.5 ``f542e612`` |
| ``src/mclaw/api/`` + ``agent/`` residual | 8 | 8 ``Agent`` + ``smart_truncate`` | P7.6 ``23f8e05b`` |
| ``src/mclaw/core/`` internal (legacy bodies) | 8 | 34 ``from .X`` -> ``from ._X_legacy`` | P7.10 ``2f97c2aa`` |
| ``src/mclaw/{channels,evolution,scheduler,skills,tools/handlers}/`` | 38 | 47 (38 ``Agent`` + 6 ``Brain`` + 3 ``save_overflow``) | P7.13 ``54faf9b4`` |
...
### R-RC-7-A: ``src/mclaw/orgs/`` deletion blocked
...
**Why blocked:** The v2 ``src/mclaw/runtime/orgs/`` package
...
### R-RC-7-B: ``src/mclaw/core/supervisor.py`` not deleted
...
``git grep "from mclaw.core.supervisor" src/``: 0 hits). Only
...
* ``git grep -E "from \.\.\.?core\.(agent|brain|reasoning_engine|
  ...
  ``src/mclaw/agent/{core,context,tools}.py`` are
  docstring-only commentary describing the legacy migration path).
* ``git grep "from mclaw.orgs" src/mclaw/`` -> **63 hits**
  (unchanged from P-RC-7 entry; see R-RC-7-A).
* ``ls src/mclaw/core/*.py | grep -v legacy`` -> 18 leaf
...
* R-RC-7-A: ``src/mclaw/orgs/`` 26-file deletion + ``api/routes/orgs.py``
  rewrite (or migration to ``runtime/orgs/`` once feature parity
  lands).
* R-RC-7-B: ``src/mclaw/core/supervisor.py`` 853-LOC deletion
  (via rename + shim + delete three-commit pattern) + its 3 unit
  tests retargeted to ``runtime/supervisor``.
* Final rename: ``src/mclaw/core/_*_legacy.py`` -> chosen
