...
* a real v2 ``ReasoningEngine`` at ``mclaw.agent.reasoning`` driving
...
``ruff check src/mclaw/runtime src/mclaw/agent
src/mclaw/plugins/manager.py src/mclaw/channels/gateway.py
src/mclaw/config.py src/mclaw/api/routes/ tests/runtime
...
for ~28 importers of ``mclaw.core.reasoning_engine``; this is
...
* **N7** (closed in P5.0b, ``26cee0e7``): ``core/brain.py`` shim adopts the ``mclaw.core import _brain_legacy as _legacy`` fallback pattern; the new ``reasoning_engine.py`` shim (P5.9b/P5.10) uses the same shape.
...
``mclaw.core.reasoning_engine.ReasoningEngine`` at the v2 class,
