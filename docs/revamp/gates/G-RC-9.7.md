...
subsystems. v1 ``src/mclaw/api/routes/orgs.py`` (2 533 LOC,
...
``git diff 8510ade6..HEAD -- src/mclaw/orgs/ core/ channels/
...
* **ADR-0012** (no shim under v1; v1 delete waits for P9.9):
  v1 ``src/mclaw/api/routes/orgs.py`` is UNTOUCHED through
...
``src/mclaw/api/schemas.py`` (legacy module, 159 LOC, 8
Pydantic classes) was shadowed by the
``src/mclaw/api/schemas/`` package created by P9.7a-2b
...
``mclaw.api.schemas``). **Folded in by ``P9.7gamma-3a`` (``b9b74df7``)**: merged
...
3. ``git diff 8510ade6..HEAD -- src/mclaw/orgs/ core/
   channels/ orgs.py apps/`` -> **empty bytes** (strict-additive
   boundary held; v1 subsystem under ``src/mclaw/orgs/`` AND
...
radius from P9.7 (touches ``apps/`` + ``src/mclaw/channels/``,
...
(P9.9 ``git rm -r src/mclaw/orgs/`` + 410 shim under
``api/routes/orgs.py``).
