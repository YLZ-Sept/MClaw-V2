# Mclaw v2.0.0-rc2 -- Release notes
...
(`mclaw_revamp_continuation_plan_d6192647.plan.md`) ran from
...
* **Acceptance criteria documented.**
  ...
  ``src/mclaw/orgs/`` deletion that ``docs/revamp/P-RC-9-CHARTER.md``
...
* **Ruff over the v2 surface** (``src/mclaw/runtime``,
...
The wholesale ``src/mclaw/orgs/`` integral migration is **not**
...
``src/mclaw/orgs/`` surface live. The v2 ``runtime/`` +
...
```
git tag -a v2.0.0-rc2 -m "Mclaw v2.0.0-rc2: continuation plan complete (P-RC-0..8); orgs/ migration deferred to P-RC-9"
```
...
``src/mclaw/runtime/`` and ``src/mclaw/agent/`` packages
contain the new orchestration stack; the ``src/mclaw/core/`` and
``src/mclaw/orgs/`` legacy packages remain installed so existing
...
``src/mclaw/orgs/`` and the shimmed ``src/mclaw/core/*``
...
``src/mclaw/core/state.py`` — the audit-confirmed zero-importer
...
1. ``src/mclaw/orgs/`` removal — needs ``channels/gateway.py``
   and ``api/routes/orgs.py`` rewritten to use ``runtime/orgs/`` and
   ``runtime/state_graph``.
2. ``src/mclaw/core/`` shim removal — needs callers in
   ``sessions/``, ``memory/``, ``tools/``, ``agents/`` to switch from
   ``mclaw.core.*`` to ``mclaw.agent.*``.
