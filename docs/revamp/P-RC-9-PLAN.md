# P-RC-9 Execution Plan -- ``src/mclaw/orgs/`` integral migration
...
After P-RC-9 closes, ``src/mclaw/orgs/`` no longer exists in
``revamp/v3-orgs``. Every behaviour it owned today is served by
``src/mclaw/runtime/orgs/`` and a small set of preserved
...
* ``git ls-files src/mclaw/orgs/ | wc -l`` -> **0** (after
  P9.9).
* ``git grep -nE 'from mclaw\.orgs' -- src/mclaw/`` ->
  **0** (after P9.8).
* ``git grep -nE 'from mclaw\.orgs' -- tests/`` -> **0**
...
* **orgs/ package:** 26 files, 18 213 LOC
  (``Get-ChildItem src/mclaw/orgs/ -File`` + sum). Top three
...
* **v1 REST:** 89 endpoints, 2 145 LOC
  (``git grep -cE '^@router\.' -- src/mclaw/api/routes/orgs.py``;
...
``mclaw.orgs`` package. The 32 ``.models`` imports and the
...
from ``mclaw.orgs.X`` to ``mclaw.runtime.orgs.X`` with a
...
"only re-export from mclaw.core.X" anti-pattern. P-RC-9
...
* LOC budget: 350 in ``src/mclaw/runtime/orgs/blackboard.py``
  + 200 in tests.
...
* Deliverables: every ``from mclaw.orgs.X`` in src/ and
  tests/ becomes ``from mclaw.runtime.orgs.X``. Order:
...
* Commits: 3-4. ``refactor: git rm -r src/mclaw/orgs/``;
  ``refactor: git rm src/mclaw/api/routes/orgs.py`` (or
...
* Deliverables: clean tree. ``git ls-files src/mclaw/orgs/``
...
* **Consequence:** the P9.9 deletion is a single ``git rm -r
  src/mclaw/orgs/`` + ``git rm -r tests/orgs/`` operation,
...
* **Options:**
  * (a) Keep as ``src/mclaw/runtime/orgs/`` indefinitely
    (the path the existing 3 storage-only files already use).
  * (b) Rename to ``src/mclaw/orgs_v2/`` after P9.9 so the
    v2 surface has a clean top-level package.
  * (c) Rename to ``src/mclaw/orgs/`` after P9.9 (reclaim
