...
4b8a9ad8..HEAD -- src/mclaw/ tests/ apps/`` is empty bytes
...
* **Touch ``src/mclaw/api/routes/orgs.py``** (v1
...
* **Touch ``src/mclaw/channels/``**. Recon (section
...
* **R3 -- IM gateway blast radius (LOW, confirmed)**.
  ...
  **in-process** (5 ``from mclaw.orgs.command_service``
...
table of the 5 ``from mclaw.orgs.command_service``
...
| ``src/mclaw/channels/`` | **0** | **0** | 0 | no HTTP callers; in-process imports only (5 ``from mclaw.orgs.command_service`` in gateway.py) |
...
* **Backend Python** (``src/mclaw/api/routes/orgs.py``
  v1 + ``src/mclaw/orgs/`` v1 subsystem): BYTE-LEVEL
...
3. v1 endpoint file UNTOUCHED through all P9.8 commits:
   ...
   src/mclaw/api/routes/orgs.py`` returns empty bytes.
4. v1 subsystem UNTOUCHED: ``git diff 4b8a9ad8..HEAD --
   src/mclaw/orgs/`` returns empty bytes.
5. ``src/mclaw/channels/`` UNTOUCHED (R3 + section 5):
   ``git diff 4b8a9ad8..HEAD -- src/mclaw/channels/``
   returns empty bytes.
...
* ``git rm -r src/mclaw/orgs/`` (~18 000 LOC, 6 charter
  subsystems gone).
...
* Mechanical rewrite of every ``from mclaw.orgs.X``
  in ``src/mclaw/`` (90 sites; top: ``orgs/tool_handler.py``
  ...
  ``from mclaw.runtime.orgs.X`` (~5 batches; pytest
  green after each).
...
+ ledger row; ``git diff 4b8a9ad8..HEAD -- src/mclaw/
