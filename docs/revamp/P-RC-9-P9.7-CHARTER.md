...
def test_every_v2_orgs_endpoint_has_a_contract_test() -> None:
    ...
    from mclaw.api.routes import orgs_v2
...
4. v1 endpoint file NOT touched: ``git diff 89703a28..HEAD
   -- src/mclaw/api/routes/orgs.py`` empty bytes.
5. Boundary discipline: changes ONLY under
   ``src/mclaw/api/routes/orgs_v2*.py`` and (any tiny
   tweaks) ``src/mclaw/runtime/orgs/``;
   ``src/mclaw/orgs/`` untouched.
...
* **P9.8 -- caller migration** (~86 src + 216 test import
  sites). ``from mclaw.orgs.X`` mechanically rewritten
...
* **P9.9 -- physical deletion + 410 shim** (ADR-0012 + Q-B).
  ``git rm -r src/mclaw/orgs/``; v1 ``orgs.py`` becomes
...
``src/mclaw/runtime/orgs/{runtime, manager,
...
src/mclaw/ tests/`` continues to return empty bytes.
