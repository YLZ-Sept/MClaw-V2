...
P-RC-9 left half-done (v1 ``src/mclaw/orgs/`` is
gone but the v2 surface still lives under the
transitional ``src/mclaw/runtime/orgs/`` slot), and
...
* **(i) Namespace flatten** -- collapse the
  transitional ``src/mclaw/runtime/orgs/*`` (25 .py
  files) back to the conventional
  ``src/mclaw/orgs/*`` slot now that v1 src is gone
  (P-RC-9 P9.9 epsilon-2b ``90a7d77f``). The import
  surface returns to ``mclaw.orgs.*`` for every
  downstream caller.
...
1. ``src/mclaw/orgs/`` exists with 25+ .py files
   (the flattened v2 surface).
2. ``src/mclaw/runtime/orgs/`` either gone
   (post-shim removal at P10.6) or contains only the
   temporary re-export shim ``__init__.py`` (one-release
   backward compat).
3. ``git grep -l 'mclaw\.runtime\.orgs' -- src/mclaw/ tests/ apps/ scripts/``
...
5. Sentinels ACTIVE; sentinel #9 augmented (per P10.4)
   so new ``mclaw.runtime.orgs.*`` imports are
...
### 1.1 File inventory -- ``src/mclaw/runtime/orgs/`` (25 .py)
...
(Verbatim from ``Get-ChildItem src/mclaw/runtime/orgs -File``.)
...
``git grep -c 'mclaw\.runtime\.orgs' -- src/ apps/ tests/ scripts/``
at HEAD ``acc7241a``:

| directory             | files | typical caller |
|---|--:|---|
| ``src/mclaw/``      | 21    | api/routes/orgs_v2*, channels/gateway, server, runtime/channel_routing, self-refs inside runtime/orgs/ |
...
* **P10.1 -- Atomic ``git mv``.** Physical directory
  rename ``src/mclaw/runtime/orgs ->
  src/mclaw/orgs``. 25 files relocated; re-seed
  ``src/mclaw/orgs/__init__.py`` (verbatim copy of
  runtime/orgs ``__init__.py``); re-seed
  ``src/mclaw/runtime/__init__.py`` to drop the
  ``orgs`` re-export. Net +0 + ~30 LOC __init__
  shuffle. Mini-gate ``G-RC-10.1.md``.
* **P10.2 -- Temporary re-export shim.** Add
  ``src/mclaw/runtime/orgs/__init__.py`` (<= 30
  LOC) doing ``from mclaw.orgs import *`` plus a
...
* **P10.4 -- Sentinel #9 augment.** Extends
  ...
  ``mclaw.runtime.orgs.*`` import sites outside the
...
* **P10.6 -- Remove the section P10.2 temporary shim.**
  ...
  ``git rm src/mclaw/runtime/orgs/__init__.py`` +
...
* **R-10-3 (LOW) -- third-party plugins import
  ``mclaw.runtime.orgs.*``.** Plugin authors may
...
* **R-10-4 (LOW) -- ``tests/runtime/orgs/`` directory
  ...
  ``mclaw.orgs.X`` correctly, but the directory
...
|--:|---|---|
| 1 | ``src/mclaw/orgs/`` exists with 25+ .py files | ``Get-ChildItem`` at P10.1 close |
| 2 | ``src/mclaw/runtime/orgs/`` gone or shim-only | ``Get-ChildItem`` at P10.6 |
| 3 | ``git grep -l 'mclaw\.runtime\.orgs' -- src/mclaw/ tests/ apps/ scripts/`` = 0 (or shim only pre-P10.6) | grep at P10.3 + P10.6 close |
