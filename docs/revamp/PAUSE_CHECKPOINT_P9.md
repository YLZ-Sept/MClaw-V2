...
- **Structure**: **7–8 sibling modules** under `src/mclaw/runtime/orgs/` (not a single monolith).
...
### `src/mclaw/runtime/orgs/` inventory (2026-05-19 pause)
...
| `src/mclaw/runtime/orgs/` tracked WIP | **None** (porcelain clean) |
...
| OS processes | Several **orphaned** `python -m pytest` processes still running under `D:\Mclaw` (leftover from earlier gates); safe to end via Task Manager if CPU/disk unwanted — not started by this verification |
