...
``git diff a3a5fde6..HEAD -- src/mclaw/orgs/`` returns
empty bytes.

## 0. Mission and exit criteria

ε is the **final physical-deletion phase** for the v1 org
subsystem under ``src/mclaw/orgs/``. After P9.9ε all v1
code under that directory is gone from the tree; v2
``src/mclaw/runtime/orgs/`` (23 files / 10 886 LOC at HEAD
``4b5499a6``) becomes the **sole** in-tree home of the org
runtime.

**Exit criteria (all GREEN before G-RC-9.9):**

1. ``git ls-files src/mclaw/orgs/`` = **0**.
2. ``git grep -ln "mclaw\.orgs" -- src/mclaw/
   ":(exclude)src/mclaw/orgs/" apps/ scripts/ identity/`` =
   **0 production files**.
...
* v1 router ``src/mclaw/api/routes/orgs.py`` (2 533 LOC /
...
**ε deletes**: 26 ``.py`` files under ``src/mclaw/orgs/``,
...
``src/mclaw/orgs/`` (audit §2). Scheme C splits ε into 4
...
* **R-ε-2 (MED)** -- 308 shim accidentally imports v1. The
  ...
  confirms shim is 101 LOC / **zero ``mclaw.orgs``
...
* **R-ε-3 (MED)** -- pytest collect-only ImportError inflation
  ...
  ``tests/``); ε-2b removes ``src/mclaw/orgs/`` (49
...
* **R-ε-4 (LOW)** -- ``runtime/orgs`` absorption gap discovered
  ...
  **0 ABSENT**. ``mclaw.orgs.tool_handler`` (3 474 LOC) is
...
2. ``git diff a3a5fde6..HEAD -- src/mclaw/orgs/`` = 0 bytes
   (strict-additive on v1 src still holds going INTO the
   deletion commit).
3. ``git grep -ln "mclaw\.orgs" -- src/mclaw/
   ":(exclude)src/mclaw/orgs/" apps/ scripts/ identity/``:
...
6. ``git ls-files src/mclaw/orgs/`` = 26 before ε-2b,
   **0 after**.
...
### 5.2 ε-2b -- atomic src/mclaw/orgs/ delete

* Subject: ``chore(orgs): P9.9ε-2b atomic delete
  src/mclaw/orgs/ (26 files / 20237 LOC, v1 subsystem
...
|---|---|---|---|
| ``git ls-files src/mclaw/orgs/`` | 26 | 26 | **0** |
...
* Deletion ONLY in ``src/mclaw/orgs/`` (ε-2b) and
  ``src/mclaw/api/routes/orgs.py`` + the 2 dev scripts
  under ``scripts/`` (ε-2a).
* ``src/mclaw/runtime/orgs/`` -- untouched.
* ``src/mclaw/api/routes/_orgs_v2_legacy_redirects.py`` --
...
4. **ε-2b** -- atomic ``git rm -r src/mclaw/orgs/``
   (20 237 LOC).
