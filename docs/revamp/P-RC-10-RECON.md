...
every ``from mclaw.runtime.orgs.X`` /
``import mclaw.runtime.orgs.X`` import remaining at
...
mapped to its post-flatten ``mclaw.orgs.X`` target,
...
-- src/mclaw/ tests/ apps/ scripts/ identity/``
...
| bucket | files | sites |
|---|--:|--:|
| ``src/mclaw/`` (incl. 2 self-refs)  | 14 | 35 |
...
|---|--:|---|
| **M** (mechanical) | **122** | pure ``from mclaw.runtime.orgs.X import Y`` -> ``from mclaw.orgs.X import Y`` prefix swap; no surrounding context needed |
| **PT** (patched-target) | **0** | no ``unittest.mock.patch("mclaw.runtime.orgs.X.Y")`` string literals anywhere in src/tests/apps/scripts (string-literal grep returns zero matches) |
...
``src/mclaw/runtime/orgs/`` to
``src/mclaw/orgs/`` at P10.1 via a single
``git mv src/mclaw/runtime/orgs src/mclaw/orgs``.
...
|---|--:|--:|---|---|
| ``src/mclaw/api/`` | 5 | 23 | orgs_v2_runtime_orgs (7) / orgs_v2_runtime_projects (5) / server (4) / orgs_v2_runtime_state (2) + 1 sibling route | P10.3a |
| ``src/mclaw/channels/`` | 1 | 6 | gateway.py (5 deferred + 1 module-level) | P10.3a |
| ``src/mclaw/runtime/orgs/`` (self) | 2 | 4 | manager.py x3 + _runtime_templates.py x1 -- absolute self-imports | **P10.1** (atomic) |
| ``src/mclaw/`` (other) | 2 | 2 | core/_reasoning_engine_legacy.py (deferred) + runtime/channel_routing.py | P10.3a |
...
| sites | file |
|--:|---|
| 7 | ``src/mclaw/api/routes/orgs_v2_runtime_orgs.py`` |
| 6 | ``tests/parity/orgs/test_node_scheduler_parity.py`` |
| 6 | ``src/mclaw/channels/gateway.py`` |
| 5 | ``src/mclaw/api/routes/orgs_v2_runtime_projects.py`` |
| 5 | ``tests/integration/test_gateway_org_control.py`` |
| 5 | ``tests/parity/orgs/test_runtime_parity.py`` |
| 4 | ``src/mclaw/api/server.py`` |
...
| 3 | ``src/mclaw/runtime/orgs/manager.py`` (self) |
...
|--:|---|---|---|
| N-1 | ``tests/parity/orgs/README.md:58`` | ``from mclaw.runtime.orgs.blackboard import OrgBlackboard`` | mechanical regex swap in P10.3c -- the line lives in a doc, but the ``s/mclaw\.runtime\.orgs/mclaw\.orgs/`` rule applies cleanly; flag in commit body as "README sample import rewrite" so reviewers know it is doc text, not test code |
| N-2 | ``tests/parity/orgs/README.md:63`` | ``from mclaw.runtime.orgs.blackboard import OrgBlackboard`` | same as N-1 (second occurrence in the same code block) |

Zero dynamic-import callers
(``importlib.import_module("mclaw.runtime.orgs...")``
...
``src/mclaw/runtime/orgs/__init__.py`` re-export
...
* Loop over the 24 sibling submodule names (10
  ...
  = __import__(f"mclaw.orgs.{name}", fromlist=...)``
...
|---|---|--:|--:|---|
| P10.3a | ``src/mclaw/`` (api 5 + channels 1 + core 1 + runtime/channel_routing 1; ``runtime/orgs/`` self-refs already swept at P10.1) | 8 | 31 | ~+45 / ~-45 |
...
mclaw\.runtime\.orgs -- src/ tests/ scripts/``
returning exactly the one shim file
(``src/mclaw/runtime/orgs/__init__.py`` landed at
P10.2).
...
* **R-10-4 (LOW) -- ``tests/runtime/orgs/`` move.** 18
  ...
  they still test ``mclaw.orgs.X`` correctly via
...
```
git grep -nP "^(\s*)(from|import)\s+mclaw\.runtime\.orgs" \
...
52f8709a..HEAD -- src/mclaw/ tests/ apps/ scripts/
