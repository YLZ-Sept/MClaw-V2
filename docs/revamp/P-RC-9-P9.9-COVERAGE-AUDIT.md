...
459323d7..HEAD -- src/mclaw/ tests/ apps/`` is empty
at commit close.
...
P9.9ε-1 (``src/mclaw/orgs/``; charter §2.1).
...
| C10 tools / scenarios / LLM / identity / policies / scaler | Parent v1 modules deleted in ε-1; v2 tool layer lives under ``src/mclaw/tools/`` (out of scope of orgs/); identity / policies / LLM are global subsystems unchanged by P-RC-9; org-scenario tests targeted v1 plumbing that disappears | -- | (covered above) | -- | EQUIVALENT (by construction) |
...
lives inside ``src/mclaw/orgs/``. P9.9ε-1 deletes
...
|--:|---|---|---|--:|
| 1 | ``test_blackboard_parity.py`` | ``mclaw.orgs.blackboard.OrgBlackboard`` (as ``V1Blackboard``) | keep ``mclaw.runtime.orgs.blackboard.OrgBlackboard``; ``MemoryScope`` / ``MemoryType`` move to ``runtime.orgs.memory_models`` | 8 → 8 v2-baseline |
| 2 | ``test_command_service_parity.py`` | ``mclaw.orgs.command_service.OrgCommandService`` (oracle) + ``OrgCommandRequest`` / ``Source`` / ``Surface`` / ``OrgCommandError`` from v1 | runtime equivalents under ``mclaw.runtime.orgs.command_service`` | 10 → 10 v2-baseline |
| 3 | ``test_manager_parity.py`` | ``mclaw.orgs.manager.OrgManager`` (oracle) | ``mclaw.runtime.orgs.manager.OrgManager`` (already imported as v2) | 12 → 12 v2-baseline |
| 4 | ``test_node_scheduler_parity.py`` | ``mclaw.orgs.node_scheduler.OrgNodeScheduler`` (oracle) + ``NodeSchedule`` / ``ScheduleType`` from v1 | ``mclaw.runtime.orgs.scheduler_models.NodeSchedule`` / ``ScheduleType`` | 10 → 10 v2-baseline |
| 5 | ``test_project_store_parity.py`` | ``mclaw.orgs.project_store.ProjectStore`` (oracle) + ``OrgProject`` / ``ProjectTask`` / ``TaskStatus`` from v1 | ``mclaw.runtime.orgs.project_models`` equivalents | 8 → 8 v2-baseline |
...
* Strict additive: ``git diff 459323d7..HEAD -- src/mclaw/ tests/ apps/`` empty at audit close.
