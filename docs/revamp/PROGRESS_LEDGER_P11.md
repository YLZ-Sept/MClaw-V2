...
> not speculate):
>
> * Cluster A -- ``mclaw.orgs.tool_categories`` deleted
...
>   direct ``python -c "import mclaw.core.
...
>   ``src/mclaw/core/agent.py`` which was renamed to
...
## P11.1 -- Cluster A landed (`mclaw.orgs.tool_categories` private shard restored)

> **Sub-phase status (2026-05-22, P11.1 LANDED)**: Re-instates
> the v1 ``mclaw.orgs.tool_categories`` module that
> P9.9 epsilon-2b ``90a7d77f`` atomic-deleted alongside the
> rest of ``src/mclaw/orgs/`` (26 files / 20 237 LOC).
...
>
> 1. New private shard
>    ``src/mclaw/orgs/_runtime_tool_categories.py``
>    (188 LOC = 16-line restoration banner + 172-line v1
>    body restored verbatim from
>    ``git show 90a7d77f~1:src/mclaw/orgs/tool_categories.py``).
...
>    ``src/mclaw/orgs/tool_categories.py`` =
...
> * Backend boot smoke:
>   ``python -c "from mclaw.api.server import create_app; create_app()"``
...
> ``src/mclaw/core/`` / ``src/mclaw/agent/`` /
> ``src/mclaw/llm/`` (concurrent Cluster B+G worker
...
>
> **Hard-rule compliance**: only
> ``src/mclaw/orgs/_runtime_tool_categories.py`` (NEW;
> 188 LOC) + ``src/mclaw/orgs/tool_categories.py`` (NEW;
...
> "import mclaw.core._brain_legacy" ` reproduces the failing
...
> 'mclaw.core._brain_legacy'`.  A parallel cycle re-enters
...
>
> 1. `src/mclaw/core/_brain_legacy.py`: the module-level
...
> 2. `src/mclaw/core/errors.py`: the module-level
>    `from mclaw.agent.errors import UserCancelledError` (line 15)
...
> "import mclaw.core._brain_legacy, mclaw.llm.client,",
> `mclaw.core.errors, mclaw.agent, mclaw.agent.brain,`
> `mclaw.core._agent_legacy, mclaw.core._reasoning_engine_legacy`
...
>   'mclaw.core._reasoning_engine_legacy' has no attribute
...
> conftest fixtures and earlier collection load `mclaw.agent`
...
> pulls mclaw.agent early") but the empirical test-collection order
...
> import from `mclaw.core._reasoning_engine_legacy` no longer
...
>
> **What this commit does NOT do (hard stop)**: ZERO touch on
> `src/mclaw/orgs/` (concurrent Cluster A worker territory per
> the parallel-safety brief), ZERO test edits, ZERO sentinel / ADR /
> charter / recon / gate edits, ZERO push, ZERO tag.  Only
> `src/mclaw/core/_brain_legacy.py` (+3 LOC net) and
> `src/mclaw/core/errors.py` (-1 module-level import +
...
>   `from mclaw.core.policy_v2 ...` (absolute, the canonical
>   form) match.
...
>   failure surfaced as `ModuleNotFoundError: mclaw.core.tool_executor`
>   (same rename pattern: `cd69cd60` rename + `8e8e7da7` shim +
>   `a21cdd4b` delete) -- fixed by the same string-edit
>   `mclaw.core.tool_executor` -> `mclaw.core._tool_executor_legacy`.
...
> from `src/mclaw/core/_reasoning_engine_legacy.py` during the
...
> (`mclaw.core._reasoning_engine_legacy._is_recap_context` etc.),
> but the canonical implementations were relocated to
> `mclaw.runtime.state_graph.guards.*` and the corresponding
> re-exports were never added back.
>
> **Canonical homes confirmed** (`git grep -nP "^(def|async def)\s+..."`):
>
> * `is_recap_context` ->
>   `mclaw.runtime.state_graph.guards.recap_context`
> * `get_mode_ruleset` / `filter_tools_by_intent` /
>   `is_shell_write_command` / `CHAT_INTENT_CORE_TOOLS` /
>   `SHELL_WRITE_PATTERNS` ->
>   `mclaw.runtime.state_graph.guards.tool_filters`
> * `successful_tool_names` ->
>   `mclaw.runtime.state_graph.guards.tool_failure_ack`
> * `extract_unbacked_verbs` ->
>   `mclaw.runtime.state_graph.guards.unbacked_action`
> * `CLAIMED_TOOL_TO_FRAGMENTS` / `VERB_TO_TOOL_FRAGMENTS` ->
>   `mclaw.runtime.state_graph.guards._verb_tool_map`
>
...
>   (`git grep -nl "from mclaw.runtime.state_graph.guards.X import"`):
>   `src/mclaw/agent/reasoning.py` untouched; the parity test files
...
>   via a `import mclaw.agent.brain` warm-up. Those two unit
...
> `src/mclaw/orgs/` (Cluster A territory, already landed at P11.1),
> `src/mclaw/agent/` or `src/mclaw/llm/`, ZERO push, ZERO tag.
> Only `src/mclaw/core/_reasoning_engine_legacy.py` (+35 lines net:
...
>    `src/mclaw/core/reasoning_engine.py` no longer exists
...
>    stale-path pattern: `src/mclaw/core/tool_executor.py`
