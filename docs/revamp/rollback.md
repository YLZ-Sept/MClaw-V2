...
This is the operator runbook for rolling the Mclaw backend back
...
The plan ([`mclaw_full_backend_revamp_e6d8610d.plan.md`][plan]
§7) listed this rollback as a required mitigation but did not write
out the SOP. This file is that SOP. It is referenced from
`src/mclaw/config.py` (the `runtime_v2_enabled` field doc)
and from the post-RC continuation plan
(`mclaw_revamp_continuation_plan_d6192647.plan.md`, P-RC-0
commit 2).
...
[plan]: ../../docs/plans/mclaw_full_backend_revamp_e6d8610d.plan.md
...
Open the Mclaw `.env` (project root) and set::
...
* the `/api/v2/orgs/*` route group (registered in
  `src/mclaw/api/server.py` and gated per-request on
  `settings.runtime_v2_enabled`);
* the canary log hook in `src/mclaw/channels/gateway.py`
...
::
    ...
    pkill -f "mclaw serve"      # or stop the systemd unit
    mclaw serve
...
- Original plan §7 (`mclaw_full_backend_revamp_e6d8610d.plan.md`)
  — the rollback mitigation requirement this file fulfils.
- Continuation plan `mclaw_revamp_continuation_plan_d6192647.plan.md`
