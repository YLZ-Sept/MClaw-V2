...
v1 orgs surface end-to-end: deletes ``src/mclaw/orgs/``
(v1 subsystem, **26 files / 20 237 LOC** measured), deletes
``src/mclaw/api/routes/orgs.py`` (v1 router, **2 533 LOC /
89 endpoints** measured), deletes ``tests/orgs/`` (v1-surface
test bank, **48 files / 12 238 LOC** measured), and sweeps
every remaining ``from mclaw.orgs.X`` import onto the v2
``mclaw.runtime.orgs.X`` surface shipped in P9.1-P9.6.
...
``git diff d49388bb..HEAD -- src/mclaw/ tests/ apps/``
...
* Delete v1 subsystem ``src/mclaw/orgs/`` (26 .py /
...
* Mechanical import sweep of every ``from mclaw.orgs.X``
  literal onto ``mclaw.runtime.orgs.X``. Measured at
...
* Touch ``api/routes/_orgs_v2_legacy_redirects.py`` (the
  **308 shim**, 9 routes / 101 LOC). **Per ADR-0015 option
  ...
  task (sec 8).
* Touch the v2 subsystem ``src/mclaw/runtime/orgs/``
...
### 2.1 v1 source modules to delete (``src/mclaw/orgs/``)
...
"mclaw.orgs"`` per-top-dir buckets):

### 3.1 Backend src (31 files; **23 external sweep**)

| tree | files | resolution |
|---|--:|---|
| ``src/mclaw/api/`` | 7 | sweep -- 1-to-1 same-name + absorption (sec 3.3) |
| ``src/mclaw/channels/`` (= gateway.py) | 1 | sweep in **beta** (R3); 5 in-process imports |
| ``src/mclaw/core/`` | 1 | sweep |
| ``src/mclaw/runtime/`` | 14 | sweep -- v2 internals with type / re-export back-refs to v1 |
| ``src/mclaw/orgs/`` | 8 | **delete with parent** (internal) |
...
| ``tests/parity/`` | 6 | sweep (v2 parity tests sometimes import v1 for diff; rewrite to ``mclaw.runtime.orgs``) |
| ``tests/unit/`` | 8 | sweep |
...
|---|---|---|
| ``mclaw.orgs.command_tracker`` | ``mclaw.runtime.orgs._runtime_dispatch`` (+ ``_runtime_watchdog`` half) | P9.4c + P9.6gamma |
| ``mclaw.orgs.failure_diagnoser`` | ``mclaw.runtime.orgs._runtime_watchdog`` | P9.6gamma |
| ``mclaw.orgs.heartbeat`` | ``mclaw.runtime.orgs._runtime_watchdog`` + ``_runtime_event_bus`` | P9.6gamma |
| ``mclaw.orgs.event_router`` + ``mclaw.orgs.messenger`` | ``mclaw.runtime.orgs._runtime_event_bus`` (+ ``command_service`` half) | P9.4c |
| ``mclaw.orgs.identity`` | ``mclaw.runtime.orgs.manager`` (folded into OrgManager) | P9.5c |
| ``mclaw.orgs.templates`` + ``mclaw.orgs.plugin_workbench_templates`` | ``mclaw.runtime.orgs._runtime_plugin_assets`` | P9.2c |
| ``mclaw.orgs.tool_handler`` (3 474 LOC) | absorbed across ``_runtime_agent_pipeline`` + ``runtime/dispatch`` + ``runtime/tools``; per-symbol mapping in alpha-1 | P9.4c + P9.6gamma |
| ``mclaw.orgs.models`` (1 018 LOC) | split into 4 ``runtime/orgs/{command,memory,project,scheduler}_models.py`` shards | P9.1c-P9.4c |
...
* **R3 -- channels gateway swap order (MEDIUM)**.
  ...
  mclaw.channels.gateway import *"`` smoke.
* **R4 -- orphaned re-exports in ``__init__.py`` (MEDIUM)**.
  Top-level ``__init__.py`` files may carry ``from
  mclaw.orgs.X import Y`` re-exports for backward-compat.
...
file:line table of every ``mclaw.orgs.X`` import (31
...
|---|---|--:|--:|
| gamma-1 | ``src/mclaw/api/`` (routes + server) | 7 | ~60 |
| gamma-2 | ``src/mclaw/runtime/`` cross-refs | 14 | ~100 |
| gamma-3 | ``src/mclaw/core/`` + 3 ``__init__.py`` (R4) | 2 + 3 | ~40 |
...
**epsilon-1** (default): ``git rm -r src/mclaw/orgs/``
...
(1) ``not pathlib.Path("src/mclaw/orgs").exists()``;
(2) ``not pathlib.Path("src/mclaw/api/routes/orgs.py").exists()``;
(3) regex scan for zero ``from mclaw.orgs`` or
``import mclaw.orgs`` matches under
``src/mclaw/`` or ``tests/``.
...
import from ``mclaw.orgs`` (it imports from
``mclaw.api.routes.orgs_v2_*``), so assertion 3 is
...
2. ``src/mclaw/orgs/`` directory **absent** post-eta
...
5. Zero ``from mclaw.orgs`` or ``import mclaw.orgs``
   imports under ``src/mclaw/`` or ``tests/`` (9th
...
``s/mclaw.orgs/mclaw.runtime.orgs/g`` sweep + a
...
* **Q-D (CANDIDATE; optional new row)**: governance for
  ...
  ``src/mclaw/orgs/`` + ``api/routes/orgs.py`` absent
  + zero ``mclaw.orgs`` imports), or do the existing 8
...
d49388bb..HEAD -- src/mclaw/ tests/ apps/`` returns empty
bytes.
