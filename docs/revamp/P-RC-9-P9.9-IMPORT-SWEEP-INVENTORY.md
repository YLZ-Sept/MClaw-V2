...
every ``from mclaw.orgs.X`` / ``import mclaw.orgs.X``
import remaining at HEAD ``1071a8b0`` (branch
``revamp/v3-orgs``; P9.9 charter ``d49388bb`` + ``α-1``
opener), mapped to its v2 ``mclaw.runtime.orgs.X`` (or
...
|---|--:|--:|---|---|
| ``src/mclaw/orgs/`` | 6 | 49 | NO (internal) | deleted with parent in **ε-1** |
| ``src/mclaw/api/`` | 4 | 31 | mixed | 3 swept in **γ-1**, 1 (``orgs.py``) deleted in **ε-1** |
| ``src/mclaw/channels/`` | 1 | 5 | YES | swept in **β-1** (R3 invariant) |
| ``src/mclaw/core/`` | 1 | 1 | YES | swept in **γ-2** |
| ``src/mclaw/runtime/`` | 1 | 1 | YES (special) | swept in **γ-2**; v2 manager importing v1 models |
| **src total** | **13** | **87** | -- | -- |
...
commit that deletes ``src/mclaw/orgs/``.

### 1.1 ``src/mclaw/api/`` (4 files / 31 sites)

| # | file | line | v1 import | v2 target | phase | notes |
|--:|---|--:|---|---|---|---|
| 1 | ``api/routes/chat.py`` | 1511 | ``from mclaw.orgs.command_service import (OrgCommandError, OrgCommandRequest, OrgCommandSource, OrgCommandSurface, default_scope_for_surface)`` | ``from mclaw.runtime.orgs.command_service import (...)`` | **γ-1** | deferred import inside SSE handler; 1-to-1 rewrite |
| 2 | ``api/routes/orgs.py`` | 22 | ``from mclaw.orgs.command_service import (...)`` | self-delete | **ε-1** | parent file ``git rm``-ed |
| 3 | ``api/routes/orgs.py`` | 103 | ``from mclaw.orgs.models import OrgStatus`` | self-delete | **ε-1** | -- |
| 4 | ``api/routes/orgs.py`` | 163 | ``from mclaw.orgs.manager import OrgNameConflictError`` | self-delete | **ε-1** | -- |
| 5 | ``api/routes/orgs.py`` | 174 | ``from mclaw.orgs.tool_categories import list_avatar_presets`` | self-delete | **ε-1** | absorbed module |
| 6 | ``api/routes/orgs.py`` | 234 | ``from mclaw.orgs.plugin_workbench_templates import build_workbench_templates`` | self-delete | **ε-1** | -> ``_runtime_plugin_assets`` if revived |
| 7-18 | ``api/routes/orgs.py`` | 257..2279 (17 sites) | mix of ``manager.OrgNameConflictError`` (x4), ``models.{NodeSchedule, NodeStatus, MemoryScope/Type, InboxPriority, OrgProject, ProjectStatus, ProjectType, ProjectTask, TaskStatus}``, ``blackboard.OrgBlackboard`` (x2), ``project_store.ProjectStore``, ``tool_categories.expand_tool_categories``, ``command_service.get_command_service`` (full enumeration: ``tmp_p10/p99/_recon.txt`` § ``src/mclaw/api/``) | self-delete | **ε-1** | all parent-file ``git rm``-ed; per-line catalogue preserved in recon txt; no γ rewrite required |
| 19 | ``api/routes/orgs_v2_runtime_orgs.py`` | 99 | ``from mclaw.orgs.tool_categories import list_avatar_presets`` | -> absorbed (revive as ``runtime/orgs/_runtime_plugin_assets`` helper OR re-implement inline) | **γ-1** | v2 router still leaning on v1 helper; see §3 |
| 20 | ``api/routes/orgs_v2_runtime_orgs.py`` | 134 | ``from mclaw.orgs.plugin_workbench_templates import build_workbench_templates`` | ``from mclaw.runtime.orgs._runtime_plugin_assets import build_workbench_templates`` | **γ-1** | per §3 absorption map |
| 21 | ``api/server.py`` | 363 | ``from mclaw.orgs.manager import OrgManager`` | ``from mclaw.runtime.orgs.manager import OrgManager`` | **γ-1** | 1-to-1 |
| 22 | ``api/server.py`` | 364 | ``from mclaw.orgs.runtime import OrgRuntime`` | ``from mclaw.runtime.orgs.runtime import OrgRuntime`` | **γ-1** | 1-to-1 |
| 23 | ``api/server.py`` | 365 | ``from mclaw.orgs.templates import ensure_builtin_templates`` | ``from mclaw.runtime.orgs._runtime_plugin_assets import ensure_builtin_templates`` | **γ-1** | absorbed per §3 |
| 24 | ``api/server.py`` | 372 | ``from mclaw.orgs.command_service import OrgCommandService, set_command_service`` | ``from mclaw.runtime.orgs.command_service import (...)`` | **γ-1** | 1-to-1 |
...
### 1.2 ``src/mclaw/channels/`` (1 file / 5 sites; β-1)

| # | file | line | v1 import | v2 target | phase | notes |
|--:|---|--:|---|---|---|---|
| 25 | ``channels/gateway.py`` | 3182 | ``from mclaw.orgs.command_service import get_command_service`` | ``from mclaw.runtime.orgs.command_service import get_command_service`` | **β-1** | 1-to-1; deferred import in IM handler |
...
| 29 | ``channels/gateway.py`` | 3739 | ``from mclaw.orgs.command_service import (OrgCommandError, OrgCommandRequest, OrgCommandSource, OrgCommandSurface, default_scope_for_surface)`` | ``from mclaw.runtime.orgs.command_service import (...)`` | **β-1** | 5-name multi-line; same surface used in chat.py:1511 |

**β-1 net edits**: 1 file / 5 sites; ~50 LOC insertions
(symbol-for-symbol substitution).

### 1.3 ``src/mclaw/core/`` (1 file / 1 site; γ-2)

| # | file | line | v1 import | v2 target | phase | notes |
|--:|---|--:|---|---|---|---|
| 30 | ``core/_reasoning_engine_legacy.py`` | 7920 | ``from mclaw.orgs.runtime import get_runtime  # 延迟导入避免环路`` | ``from mclaw.runtime.orgs.runtime import get_runtime`` | **γ-2** | deferred import preserving cycle-break comment |

### 1.4 ``src/mclaw/runtime/`` (1 file / 1 site; γ-2 special)

| # | file | line | v1 import | v2 target | phase | notes |
|--:|---|--:|---|---|---|---|
| 31 | ``runtime/orgs/manager.py`` | 55 | ``from mclaw.orgs.models import (NodeSchedule, Organization, OrgEdge, OrgNode, OrgStatus, ScheduleType, UserPersona, _new_id, _now_iso, infer_agent_profile_id_for_node)`` | split into 4 typed shards (see §3) | **γ-2** | v2 manager still leaning on v1 ``models``; the 10 symbols re-route per-symbol to ``command_models`` / ``memory_models`` / ``project_models`` / ``scheduler_models`` |

**γ-2 net edits**: 2 files / 2 sites (1 cross-tree
deferred import + 1 v2-internal symbol-split).

### 1.5 ``src/mclaw/orgs/`` (6 files / 49 sites; ε-1)
...
``git rm -r src/mclaw/orgs/`` lands in ε-1. **NOT γ
...
| # | file | line | v1 import | v2 target | phase | notes |
|--:|---|--:|---|---|---|---|
| 1 | ``tests/e2e/test_p0_regression.py`` | 180 | ``from mclaw.orgs.command_service import OrgCommandService`` | ``from mclaw.runtime.orgs.command_service import OrgCommandService`` | **δ-3** | 1-to-1 |
| 2 | ``tests/e2e/test_p0_regression.py`` | 241 | ``from mclaw.orgs.tool_handler import OrgToolHandler`` | absorbed: ``from mclaw.runtime.orgs._runtime_agent_pipeline import OrgToolHandler`` (or re-export shim) -- §3 | **δ-3** | per-symbol mapping required; see §3 |

### 2.2 ``tests/integration/`` (1 file / 5 sites; δ-3)

| # | file | line | v1 import | v2 target | phase | notes |
|--:|---|--:|---|---|---|---|
| 3 | ``tests/integration/test_gateway_org_control.py`` | 113, 139, 147, 174, 198 | ``from mclaw.orgs import command_service as cs_module`` (x5) | ``from mclaw.runtime.orgs import command_service as cs_module`` | **δ-3** | 5 deferred imports in IM-canary fixtures; 1-to-1 |
...
|--:|---|--:|---|---|---|
| 4 | ``parity/orgs/test_blackboard_parity.py`` | 30 | ``from mclaw.orgs.blackboard import OrgBlackboard as V1Blackboard`` | drop import + rewrite 8 cases to v2-only smoke | Option B; sentinel #1 |
| 5 | ``parity/orgs/test_blackboard_parity.py`` | 31 | ``from mclaw.orgs.models import MemoryScope, MemoryType`` | swap to ``from mclaw.runtime.orgs.memory_models import MemoryScope, MemoryType`` | -- |
| 6 | ``parity/orgs/test_command_service_parity.py`` | 44 | ``from mclaw.orgs.command_service import (...)`` | drop import + rewrite 10 cases to v2-only smoke | Option B; sentinel #4 |
| 7 | ``parity/orgs/test_manager_parity.py`` | 82 | ``from mclaw.orgs.manager import OrgManager`` | drop import + rewrite 12 cases to v2-only smoke | Option B; sentinel #5 |
| 8 | ``parity/orgs/test_node_scheduler_parity.py`` | 81 | ``from mclaw.orgs.models import NodeSchedule as V1NS`` | swap to ``from mclaw.runtime.orgs.scheduler_models import NodeSchedule as V1NS`` (rename alias to ``NS``) | shared shard import |
| 9 | ``parity/orgs/test_node_scheduler_parity.py`` | 82 | ``from mclaw.orgs.models import ScheduleType as V1ST`` | swap to ``from mclaw.runtime.orgs.scheduler_models import ScheduleType as ST`` | -- |
| 10 | ``parity/orgs/test_node_scheduler_parity.py`` | 140-142 | repeat of 81-82 + ``from mclaw.orgs.node_scheduler import OrgNodeScheduler as V1Sched`` | drop V1Sched import + collapse parity to v2-only smoke | Option B; sentinel #3 |
| 11 | ``parity/orgs/test_project_store_parity.py`` | 102 | ``from mclaw.orgs.models import OrgProject, ProjectTask, TaskStatus`` | swap to ``from mclaw.runtime.orgs.project_models import (...)`` | -- |
| 12 | ``parity/orgs/test_project_store_parity.py`` | 103 | ``from mclaw.orgs.project_store import ProjectStore`` | drop import + rewrite 8 cases to v2-only smoke | Option B; sentinel #2 |
...
| # | file | line | v1 import | v2 target | phase | notes |
|--:|---|--:|---|---|---|---|
| 13 | ``unit/test_c17_second_pass_audit.py`` | 222, 267 | ``from mclaw.orgs.event_store import OrgEventStore`` (x2) | absorbed: ``from mclaw.runtime.orgs._runtime_event_bus import OrgEventStore`` (per §3) | **δ-2** | per §3 absorption |
| 14 | ``unit/test_delegation_preamble.py`` | 138 | ``from mclaw.orgs.identity import OrgIdentity`` | absorbed: ``from mclaw.runtime.orgs.manager import OrgIdentity`` (per §3) | **δ-2** | identity folded into OrgManager (P9.5c) |
| 15 | ``unit/test_delegation_preamble.py`` | 139 | ``from mclaw.orgs.models import Organization, OrgNode`` | per-symbol shard split (§3) | **δ-2** | -- |
| 16 | ``unit/test_failure_diagnoser_tone.py`` | 14 | ``from mclaw.orgs.failure_diagnoser import (...)`` | ``from mclaw.runtime.orgs._runtime_watchdog import (...)`` | **δ-2** | per §3 |
| 17 | ``unit/test_org_delegation_validator.py`` | 11 | ``from mclaw.orgs.failure_diagnoser import summarize`` | ``from mclaw.runtime.orgs._runtime_watchdog import summarize`` | **δ-2** | per §3 |
| 18 | ``unit/test_org_runtime_root_chain_dedup.py`` | 20 | ``from mclaw.orgs.models import OrgNode`` | per-symbol shard split (§3) | **δ-2** | -- |
| 19 | ``unit/test_org_runtime_root_chain_dedup.py`` | 21 | ``from mclaw.orgs.runtime import OrgRuntime`` | ``from mclaw.runtime.orgs.runtime import OrgRuntime`` | **δ-2** | 1-to-1 |
| 20 | ``unit/test_org_setup_tool.py`` | 22 | ``from mclaw.orgs.models import Organization`` | per-symbol shard split (§3) | **δ-2** | -- |
| 21 | ``unit/test_org_setup_tool.py`` | 343, 396 | ``from mclaw.orgs.manager import OrgManager`` (x2) | ``from mclaw.runtime.orgs.manager import OrgManager`` | **δ-2** | 1-to-1 |
| 22 | ``unit/test_remaining_qa_fixes.py`` | 4 | ``from mclaw.orgs.models import OrgNode`` | per-symbol shard split (§3) | **δ-2** | -- |
| 23 | ``unit/test_remaining_qa_fixes.py`` | 5 | ``from mclaw.orgs.runtime import OrgRuntime`` | 1-to-1 v2 | **δ-2** | -- |
| 24 | ``unit/test_web_search_provider_panel.py`` | 414 | ``from mclaw.orgs.runtime import OrgRuntime`` | 1-to-1 v2 | **δ-2** | deferred import |
...
``mclaw.runtime.orgs``. δ-3 has no tests/api/ work.
...
``store`` (each maps verbatim to ``mclaw.runtime.orgs.X``).
...
|---|---|---|---|
| ``mclaw.orgs.command_tracker`` | ``mclaw.runtime.orgs._runtime_dispatch`` (+ ``_runtime_watchdog`` half) | P9.4c + P9.6γ | tests/orgs/* only (vanishes in δ-4) |
| ``mclaw.orgs.failure_diagnoser`` | ``mclaw.runtime.orgs._runtime_watchdog`` | P9.6γ | unit/test_failure_diagnoser_tone.py; unit/test_org_delegation_validator.py |
| ``mclaw.orgs.heartbeat`` | ``mclaw.runtime.orgs._runtime_watchdog`` + ``_runtime_event_bus`` | P9.6γ | tests/orgs/* only |
| ``mclaw.orgs.event_router`` | ``mclaw.runtime.orgs._runtime_event_bus`` | P9.4c | src/mclaw/orgs/runtime.py:5221 (vanishes in ε-1) |
| ``mclaw.orgs.messenger`` | ``mclaw.runtime.orgs._runtime_event_bus`` + ``command_service`` | P9.4c | tests/orgs/* only |
| ``mclaw.orgs.identity`` | ``mclaw.runtime.orgs.manager`` (folded into OrgManager) | P9.5c | unit/test_delegation_preamble.py:138 |
| ``mclaw.orgs.templates`` | ``mclaw.runtime.orgs._runtime_plugin_assets`` | P9.2c | api/server.py:365 |
| ``mclaw.orgs.plugin_workbench_templates`` | ``mclaw.runtime.orgs._runtime_plugin_assets`` | P9.2c | api/orgs_v2_runtime_orgs.py:134 |
| ``mclaw.orgs.tool_categories`` | absorbed inline (revive in ``_runtime_plugin_assets`` OR keep as standalone helper) | P9.2c | api/orgs_v2_runtime_orgs.py:99 |
| ``mclaw.orgs.event_store`` | ``mclaw.runtime.orgs._runtime_event_bus`` (OrgEventStore re-export) | P9.4c | unit/test_c17_second_pass_audit.py:222, 267 |
| ``mclaw.orgs.tool_handler`` (3 474 LOC) | split: ``_runtime_agent_pipeline`` + ``runtime/dispatch`` + ``runtime/tools``; per-symbol | P9.4c + P9.6γ | e2e/test_p0_regression.py:241 (``OrgToolHandler``) |
| ``mclaw.orgs.models`` (1 018 LOC) | split into 4 typed shards: ``command_models`` / ``memory_models`` / ``project_models`` / ``scheduler_models`` under ``runtime/orgs/`` | P9.1c-P9.4c | runtime/orgs/manager.py:55; tests/parity/orgs/*; tests/unit/* |

**Per-symbol map for ``mclaw.orgs.models`` (10
...
|---|---|--:|--:|--:|
| **γ-1** | ``src/mclaw/api/`` swap | 3 (chat / orgs_v2_runtime_orgs / server) | 7 | ~50 LOC |
...
Charter §3.1 reported 14 ``src/mclaw/runtime/`` files
...
``mclaw\.orgs`` matches the v2 dotted path
``mclaw.runtime.orgs.X`` (the substring ``mclaw.
orgs`` literally appears inside ``mclaw.runtime.orgs``
starting at offset 9). With ~280 ``from mclaw.runtime.
orgs.X`` imports across the v2 subsystem, every v2 file
trips the loose grep.

**Strict regex fix**:
``(?<![\w.])mclaw\.orgs(?=[.\s])`` -- the negative
...
loose grep counted are pure ``mclaw.runtime.orgs.X``
import lines (v2 internal cross-refs).

**Net γ scope correction**: charter projected 14
``src/mclaw/runtime/`` γ-2 edits; real count is **1**
...
(``from mclaw.orgs.X import Y``). STRICT grep across
all ``__init__.py`` files under ``src/mclaw/`` and
``tests/`` at HEAD ``1071a8b0``:

**Result: ZERO ``__init__.py`` files re-export from
``mclaw.orgs.*``.** R4 risk **CLEAR**. No package
