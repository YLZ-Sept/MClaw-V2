...
> ``src/mclaw/orgs/`` (commit ``37536a62``); 1:1 rename per
...
> ``mclaw.orgs.X`` imports cleanly via the moved
> ``__init__.py``''s 21 relative re-export blocks.

| commit hash | phase | title | LOC delta | tests delta | ADR refs |
|---|---|---|---|---|---|
| ``37536a62`` | P-RC-10 P10.1 | refactor(orgs): atomic flatten src/mclaw/runtime/orgs/* -> src/mclaw/orgs/* (25 files) [P-RC-10 P10.1] | +4 / -4 (relative-form swaps; 25-file rename = 0 LOC) | 0 (test slice deliberately skipped between P10.1 and P10.2; green again after P10.2) | ADR-0011 (subsystem decomposition; no Protocol change) |

## P10.2 -- Backward-compat shim + sentinel #9 Option-Z relax

> **Sub-phase status (2026-05-21, P10.2 LANDED)**: Single new
> file ``src/mclaw/runtime/orgs/__init__.py`` (~46 LOC)
> re-exports ``mclaw.orgs.*`` via ``from mclaw.orgs
...
> ``mclaw.runtime.orgs.*`` augment rides P10.4 per charter.
...
> trips on the new ``from mclaw.orgs.X`` lines because the
...
| commit hash | phase | title | LOC delta | tests delta | ADR refs |
|---|---|---|---|---|---|
| _this commit_ | P-RC-10 P10.3a | refactor(src/mclaw): P10.3a sweep mclaw.runtime.orgs imports to canonical mclaw.orgs (31 sites / 12 files) [P-RC-10 P10.3a] | +31 / -31 (mechanical prefix swap) + ~5 ledger row | 261 parity+contracts (1 sentinel-#9 Test 2 break expected per inverted regex; P10.4 fix-forward) / 192 runtime-orgs (unchanged) | ADR-0011 (subsystem decomposition; no Protocol change) |
...
> ``^\s*(?:from|import)\s+mclaw\.runtime\.orgs(?:\.|$|\s)``
> across ``src/mclaw/`` (``*.py`` + ``*.pyi``); the prior v1
> ban on ``mclaw.orgs.*`` is dropped because P10.1 made that
> path canonical v2. Single whitelist entry
> ``src/mclaw/runtime/orgs/__init__.py`` (the P10.2
...
> mclaw.runtime.orgs import OrgManager`` into a throwaway
> ``src/mclaw/_p10_4_adversarial_probe.py`` -> sentinel
...
> baseline). No source under ``src/mclaw/`` touched; the
> P10.2 shim ``src/mclaw/runtime/orgs/__init__.py`` keeps
> emitting its DeprecationWarning whenever future code imports
> the legacy path -- 0 such callers remain in ``tests/runtime/``.

| commit hash | phase | title | LOC delta | tests delta | ADR refs |
|---|---|---|---|---|---|
| _this commit_ | P-RC-10 P10.3b | refactor(tests/runtime): P10.3b sweep mclaw.runtime.orgs imports to canonical mclaw.orgs (30 sites / 18 files) [P-RC-10 P10.3b] | +37 / -37 (mechanical prefix swap: 30 import lines + 7 docstring strings) + ~35 ledger row | 262 parity+contracts (unchanged) / 192 runtime-orgs (unchanged) | ADR-0011 (subsystem decomposition; no Protocol change) |

## P10.3c -- Sweep ``tests/api/`` + ``tests/parity/`` import sites to canonical ``mclaw.orgs``

> **Sub-phase status (2026-05-21, P10.3c LANDED)**: Mechanical
> 1:1 prefix swap of every ``from|import mclaw.runtime.orgs``
> reference under ``tests/api/`` and ``tests/parity/`` to the
> canonical ``mclaw.orgs`` path. **43 sites across 18
...
> ``mclaw.runtime.orgs`` as the banned-string needle for
...
| commit hash | phase | title | LOC delta | tests delta | ADR refs |
|---|---|---|---|---|---|
| _this commit_ | P-RC-10 P10.3c | refactor(tests/api,tests/parity): P10.3c sweep mclaw.runtime.orgs imports to canonical mclaw.orgs (37 sites / 18 files) [P-RC-10 P10.3c] | +43 / -43 (mechanical prefix swap: 37 import lines + 6 docstring strings; 1 BOM-bearing file preserved) + ~40 ledger row | 262 parity+contracts (unchanged; 0 warnings -- was 1) / 192 runtime-orgs (unchanged; 1 warning remains from scripts/ -- cleared by P10.3e) | ADR-0011 (subsystem decomposition; no Protocol change) |

## P10.3d -- Sweep ``tests/unit/`` + ``tests/integration/`` + ``tests/e2e/`` import sites to canonical ``mclaw.orgs``

> **Sub-phase status (2026-05-21, P10.3d LANDED)**: Mechanical
> 1:1 prefix swap of every ``from|import mclaw.runtime.orgs``
> reference under ``tests/unit/``, ``tests/integration/`` and
> ``tests/e2e/`` to the canonical ``mclaw.orgs`` path,
...
| commit hash | phase | title | LOC delta | tests delta | ADR refs |
|---|---|---|---|---|---|
| _this commit_ | P-RC-10 P10.3d | refactor(tests/unit,tests/integration,tests/e2e): P10.3d sweep mclaw.runtime.orgs imports to canonical mclaw.orgs (19 sites / 10 files) [P-RC-10 P10.3d] | +22 / -22 (mechanical prefix swap: 19 import lines + 3 mock-target strings) + ~30 ledger row | 262 parity+contracts (unchanged) / 192 runtime-orgs (unchanged) | ADR-0011 (subsystem decomposition; no Protocol change) |

## P10.3e -- Sweep ``scripts/`` import sites to canonical ``mclaw.orgs``

> **Sub-phase status (2026-05-21, P10.3e LANDED)**: Mechanical
> 1:1 prefix swap of every ``from|import mclaw.runtime.orgs``
> reference under ``scripts/`` to the canonical
> ``mclaw.orgs`` path, plus the 2 Sphinx-style module-
...
> (``python -c 'from mclaw.api.server import create_app;
> create_app()'`` with ``-W always::DeprecationWarning``)
> emits 0 lines sourced from ``mclaw.runtime.orgs`` (the
...
> import regex (
> ``^\s*(?:from|import)\s+mclaw\.runtime\.orgs(?:\.|$|\s)``)
> with ``EXTENDED_ROOTS = [src/mclaw, tests, scripts]``
...
>
> **Residual ``mclaw.runtime.orgs`` mentions outside
...
> - 12 in ``src/mclaw/`` source as docstring / Sphinx-
...
> ``mclaw.runtime.orgs`` not v1 ``mclaw.orgs`` -> 
> ``mclaw.orgs`` (canonical v2 runtime, not the legacy v1
...
> ``from mclaw.orgs._runtime_agent_pipeline import ...`` and
> ``from mclaw.orgs._runtime_plugin_assets import ...`` line
> in ``src/`` and ``tests/`` keeps resolving byte-for-byte
> unchanged (verified by ``git grep`` on both module names
> across ``src/mclaw/orgs/__init__.py``,
...
> ``src/mclaw/orgs/manager.py`` I001 import-order issue
...
>
> Next: P10.6 (remove the `src/mclaw/runtime/orgs/__init__.py`
> shim) is unblocked by (a) P10.3 cluster grep-clean baseline
> (only shim + sentinel still mention `mclaw.runtime.orgs`)
...
## P10.6 -- remove mclaw.runtime.orgs deprecation shim + tighten sentinel #9

> **Sub-phase status (2026-05-22, P10.6 LANDED)**: Physical
> removal of the P10.2 deprecation shim. The lone tracked
> file `src/mclaw/runtime/orgs/__init__.py` (-46 LOC) is
...
> `src/mclaw/orgs/` (the v2 canonical location), not the
> retired shim path.
>
...
>   canonical `mclaw.orgs.*` path; no external plugins
...
> first import, re-exported `mclaw.orgs.*` via a wildcard
> `from mclaw.orgs import *`, and installed 24
...
> |---|---|---|
> | `_SHIM_ALLOWLIST` tuple | `("src/mclaw/runtime/orgs/__init__.py",)` (length 1) | **DROPPED** -- constant removed entirely |
...
> | Scan regex | `^\s*(?:from\|import)\s+mclaw\.runtime\.orgs(?:\.\|$\|\s)` | UNCHANGED (carried byte-stable from P10.4) |
> | Scope of scan | `src/mclaw/**/*.{py,pyi}` minus `_SHIM_ALLOWLIST` | `src/mclaw/**/*.{py,pyi}` -- NO exemption |
>
> The sentinel file itself lives at
> `tests/parity/orgs/test_v1_src_retired_sentinel.py` which
> is OUTSIDE `_SRC_ROOT = repo/src/mclaw`, so its own
...
> | # | check | result |
> |---|---|---|
> | 1 | `Test-Path src/mclaw/runtime/orgs/__init__.py` -> False | PASS (`git rm` succeeded; file removed from index) |
> | 1b | `Test-Path src/mclaw/runtime/orgs` -> False | PASS (directory pruned via `Remove-Item -Recurse -Force` after stale `__pycache__` blocked auto-removal) |
> | 2 | repo-wide `git grep mclaw.runtime.orgs` excluding `docs/revamp/` + `tmp_p10/` | PASS -- 9 hits ALL inside `tests/parity/orgs/test_v1_src_retired_sentinel.py` (regex source bytes + 8 docstring/error-message back-references; ZERO actual imports) |
> | 3 | `python -c "from mclaw.api.server import create_app; create_app()"` | PASS -- 417 routes registered; 0 `mclaw.runtime.orgs`-related `DeprecationWarning` (22 other unrelated warnings remain from pre-existing FastAPI `on_event` + sqlalchemy noise -- OUT OF P10.6 SCOPE) |
> | 4 | `python -c "import mclaw.runtime.orgs"` -> `ModuleNotFoundError` | PASS -- raises `ModuleNotFoundError: No module named 'mclaw.runtime.orgs'`; exit 1; this is the INTENDED loud failure |
...
> | 8 | adversarial: inject `from mclaw.runtime.orgs import OrgManager` into `src/mclaw/_adv_p10_6_inject.py`, re-run sentinel | PASS -- Test 2 FAILED loudly (1 hit reported with file:line:text and rewrite instruction); after `Remove-Item` revert, sentinel green again |
>
> Hard-rule compliance: only the shim file (deleted) + the
> sentinel test (tightened) + this ledger row are modified.
> `src/mclaw/orgs/` content untouched. `src/mclaw/runtime/__init__.py`
> inspection (`git grep -nP "orgs" -- src/mclaw/runtime/__init__.py`)
> returned a SINGLE hit on a docstring line ("replaces
> `src/mclaw/orgs/` per ADR-0002"), i.e. NO active
...
> e2e / import-time clean with zero `mclaw.runtime.orgs`
