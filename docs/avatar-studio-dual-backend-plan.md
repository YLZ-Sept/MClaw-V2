...
- **Pixelle-Video**（`D:\Mclaw_AI_Video\refs\Pixelle-Video`）：通过 `comfykit` 调 ComfyUI workflow，双后端（RunningHub 云 / 本地 ComfyUI）自动适配。数字人步骤封装为 3 个 workflow JSON，模型选择外移到 workflow 文件。
- **调研文档**：`D:\Mclaw_AI_Video\findings\comfyui_runninghub_intro.md`（509 行 ComfyUI/RunningHub/comfykit 零基础扫盲）
