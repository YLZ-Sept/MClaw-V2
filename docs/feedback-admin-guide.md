...
- **Worker 地址**: `https://feedback.mclaw.ai`（备用: `https://bug-report-worker.zacon365.workers.dev`）
...
curl -s -H "Authorization: Bearer <ADMIN_API_KEY>" \
  "https://feedback.mclaw.ai/admin/reports" | python -m json.tool
```
...
```bash
curl -s -H "Authorization: Bearer <ADMIN_API_KEY>" \
  "https://feedback.mclaw.ai/admin/reports/<REPORT_ID>" | python -m json.tool
```
...
```bash
curl -s -H "Authorization: Bearer <ADMIN_API_KEY>" \
  "https://feedback.mclaw.ai/admin/reports/<REPORT_ID>/download" \
  -o <REPORT_ID>.zip
```
...
curl -s -X DELETE -H "Authorization: Bearer <ADMIN_API_KEY>" \
  "https://feedback.mclaw.ai/admin/reports/<REPORT_ID>"
```
...
WORKER="https://feedback.mclaw.ai"
OUTDIR="bug-reports"
mkdir -p $OUTDIR
