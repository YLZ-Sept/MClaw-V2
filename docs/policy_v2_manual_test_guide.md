...
Move-Item r:\Mclaw\data r:\Mclaw\data.backup-$(Get-Date -Format yyyyMMdd-HHmm)

# 2. 清空 caches
Remove-Item -Recurse -Force r:\Mclaw\.pytest_cache -ErrorAction SilentlyContinue
Remove-Item -Recurse -Force r:\Mclaw\apps\setup-center\node_modules\.vite -ErrorAction SilentlyContinue
...
mclaw serve --host 127.0.0.1 --port 8001

# 5. 启动桌面 GUI（窗口 B，新开 PowerShell）
cd r:\Mclaw\apps\setup-center
npm run tauri dev
```
...
```powershell
Copy-Item r:\Mclaw\data\audit\policy_decisions.jsonl `
  "r:\Mclaw\data\audit\snapshot-T1-$(Get-Date -Format yyyyMMdd-HHmm).jsonl"
```
...
# 看 audit chain 完整性
python -c "from mclaw.core.policy_v2.audit_chain import verify_chain; from pathlib import Path; r=verify_chain(Path('data/audit/policy_decisions.jsonl')); print(f'OK={r.ok} total={r.total} broken_at={r.broken_at}')"
...
1. **立即截图 + 复制后端日志**最近 100 行（`logs/mclaw.log` 或控制台）
...
- **前置**: §0.3 启动序列
- **步骤**:
  1. 在窗口 A 跑 `mclaw serve --port 8001`
...
- **前置**: MT-001 通过
- **步骤**:
  1. 在桌面 chat 输入 `请用 list_directory 列出 r:\Mclaw\data` （触发 audit 写入）
  2. 等工具执行完，再输入 4-5 个类似只读命令
  3. 窗口 A 看 log，过滤 `audit_writer`
  4. 跑下面这段 Python 看 stats：
     ```python
     # r:\Mclaw\.venv\Scripts\python.exe
     from mclaw.core.policy_v2.audit_writer import get_async_audit_writer
...
- **步骤**:
  1. 桌面 chat：`请用 write_file 在 r:\Mclaw 下写一个 test_t1.txt，内容是 "hello"`
  2. 观察前端
- **期望**:
...
- **步骤**:
  1. 桌面 chat：`请用 write_file 把 r:\Mclaw\identity\SOUL.md 的第一行改成 "TEST"`
  2. 等 SSE event
- **期望**:
...
- **步骤**:
  1. chat：`请用 write_file 在 r:\Mclaw 下写个 test_decision.txt`
...
- **前置**: 任意 mode
- **步骤**:
  1. chat：`请用 write_file 写一个 t1.txt 到 r:\Mclaw`
...
- **步骤**:
  ```powershell
  cd r:\Mclaw
  python -c "from mclaw.core.policy_v2.audit_writer import AsyncBatchAuditWriter; w1=AsyncBatchAuditWriter('data/audit/x.jsonl'); w2=AsyncBatchAuditWriter('data\\audit\\x.jsonl'); print('fwd:', repr(w1._path)); print('back:', repr(w2._path)); assert w1._path==w2._path; print('PASS')"
  ```
- **期望**:
  - 输出两个 `_path` 完全相同
  - 输出 `PASS`
...
- **步骤**:
  1. chat：`请用 write_file 在 r:\Mclaw\test_plan.txt 写 "x"`
  2. 看 SSE
- **期望**:
  - **deny**（不是弹 confirm）
  - reason 含 `session_role` 或 `plan` 字样
  - test_plan.txt 没被创建（`Test-Path r:\Mclaw\test_plan.txt` → False）
...
- **步骤**:
  1. chat：`请用 write_file 在 r:\Mclaw\data\plans\test_plan.md 写 "# plan"`
- **期望**:
...
- **前置**: session_role=`ask`
- **步骤**:
  1. chat：`请用 write_file 写 r:\Mclaw\test_ask.txt`
...
- **步骤**:
  ...
  3. 立即接：`请用 write_file 在 r:\Mclaw 写 switch_t.txt`
  4. **不要重启后端**
- **期望**:
  - 步骤 2 显示 plan
...
- **步骤**:
  ...
  - `r:\Mclaw\data\audit\policy_decisions.jsonl`
  - `r:\Mclaw\data\sessions\xxx.json`
  - `r:\Mclaw\.env`
  - `r:\Mclaw\identity\AGENT.md`
  - `r:\Mclaw\.mclaw\config`
...
- **步骤**:
  1. 用非 owner 的 Telegram 账号给 bot 发：`请删除 r:\Mclaw\test_owner.txt`
- **期望**:
...
- **步骤**:
  1. chat：`请用 write_file 在 r:\Mclaw 写 t30.txt 内容 "1"`
...
- **辅助验证**:
  ```python
  python -c "from mclaw.core.policy_v2.session_role_matrix import get_disabled_tools_for_role; print(sorted(get_disabled_tools_for_role('plan')))"
  ```
...
- **步骤**:
  1. Telegram 发 `请用 write_file 写 r:\Mclaw\im_t.txt`
- **期望**:
...
- **前置**: SchedulerView 创建 task，prompt 含 `请删除 r:\Mclaw\test_scheduled.txt`，strategy=`deny`
- **步骤**:
  1. 手动 trigger 任务
- **期望**:
  - 任务执行
...
- **前置**: 桌面 + Telegram 都连上
- **步骤**:
  1. 桌面 chat：`请 spawn 一个 sub-agent 去删除 r:\Mclaw\test_child.txt`
...
- **前置**: 同上
- **步骤**:
  1. chat：`请用 delegate_parallel 让 3 个 sub-agent 同时 write_file 到同一个路径 r:\Mclaw\test_dedup.txt`
...
- **步骤**:
  ```powershell
  mclaw run "请用 write_file 在 r:\Mclaw 写 cli_t.txt"
  ```
- **期望**:
...
- **步骤**:
  ```powershell
  echo "请用 write_file 在 r:\Mclaw 写 pipe_t.txt" | mclaw run
  ```
- **期望**:
...
- **步骤**:
  ...
  echo "请用 delete_file 删除 x" | mclaw run
  ```
...
- **步骤**:
  1. 装两个 plugin 重启
  2. chat：`请用 write_file 写 r:\Mclaw\original.txt`
...
- **步骤**:
  1. 准备一个测试文件 `r:\Mclaw\cp_test.txt` 内容 "before"
  2. chat：`请用 write_file 把 r:\Mclaw\cp_test.txt 改成 "after"`（mutating_scoped 通常不触发 checkpoint，但用 strict 改成 destructive 类工具）
  3. 用 delete_file 替代：`请用 delete_file 删除 r:\Mclaw\cp_test.txt`，点 allow
...
- **步骤**:
  1. chat：`请用 write_file 写 5 个文件 t1.txt ~ t5.txt 到 r:\Mclaw\data\plans`
...
- **步骤**:
  ```python
  # python REPL
  from mclaw.core.policy_v2.shell_risk import classify_shell_command
  import time
...
- **步骤**:
  ...
  3. 跑 `python -c "from mclaw.core.policy_v2.audit_chain import verify_chain; from pathlib import Path; print(verify_chain(Path('data/audit/policy_decisions.jsonl')))"`
...
- **步骤**:
  ```powershell
  cd r:\Mclaw
  python -c "from mclaw.core.policy_v2 import PolicyEngineV2, ApprovalClass, get_policy_engine_v2; print('OK')"
  python -c "from mclaw.core.policy_v2.classifier import ApprovalClassifier; print('OK')"
  python -c "from mclaw.core.policy_v2.engine import PolicyEngineV2; print('OK')"
  python -c "from mclaw.core.audit_logger import AuditLogger; print('OK')"
  python -c "from mclaw.channels.gateway import Gateway; print('OK')"
  python -c "from mclaw.orgs.runtime import execute_tool_with_policy; print('OK')"
...
- **步骤**:
  ```powershell
  python -c "from mclaw.orgs.runtime import execute_tool_with_policy; import inspect; sig=inspect.signature(execute_tool_with_policy); print(sig); print(list(sig.parameters.keys()))"
...
- **步骤**:
  1. owner 账号发 `请删除 r:\Mclaw\im_telegram.txt` → 收到 InlineKeyboard 卡片
  2. 点 "批准" → 删除执行
...
from mclaw.core.policy_v2.audit_writer import get_async_audit_writer
from mclaw.core.policy_v2.audit_chain import verify_chain
from mclaw.core.policy_v2.global_engine import get_config_v2
