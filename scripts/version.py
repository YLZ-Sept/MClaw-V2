...
- 一键同步到：
  ...
  - apps/setup-center/src-tauri/Cargo.lock (mclaw-setup-center package entry)
...
def _update_cargo_lock(version: str) -> bool:
    ...
    for i, raw in enumerate(lines):
        ...
        if in_pkg and line.startswith("name = "):
            ...
            is_target_pkg = name == "mclaw-setup-center"
            continue
...
def _update_bundled_version(version: str) -> bool:
    ...
    """
    path = ROOT / "src" / "mclaw" / "_bundled_version.txt"
    old = path.read_text(encoding="utf-8").strip() if path.exists() else ""
    if old == version:
        return False
    path.write_text(version, encoding="utf-8", newline="\n")
    print(f"src/mclaw/_bundled_version.txt: {old} -> {version}")
    return True
...
def check(expected: str | None) -> int:
    ...
    # 找到 mclaw-setup-center 这个 package 的 version
    lm = re.search(
        r'(?ms)^\[\[package\]\]\s*\nname\s*=\s*"mclaw-setup-center"\s*\nversion\s*=\s*"([^"]+)"\s*$',
        cargo_lock,
    )
    ...
    sdk_pyproject = ROOT / "mclaw-plugin-sdk" / "pyproject.toml"
    sdk_version_py = ROOT / "mclaw-plugin-sdk" / "src" / "mclaw_plugin_sdk" / "version.py"
