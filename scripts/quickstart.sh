...
MCLAW_ROOT_DEFAULT="${MCLAW_ROOT:-$HOME/.mclaw}"
OPENAKITA_APP_DIR_DEFAULT="${OPENAKITA_APP_DIR:-$MCLAW_ROOT_DEFAULT/app}"
OPENAKITA_VENV_DIR_DEFAULT="${OPENAKITA_VENV_DIR:-$MCLAW_ROOT_DEFAULT/venv}"
...
Options:
  --dir <path>            App working directory (default: ~/.mclaw/app)
  --venv <path>           Virtualenv directory (default: ~/.mclaw/venv)
  ...
  --no-init               Skip running `mclaw init`
  --no-wrapper            Skip creating ~/.local/bin/mclaw wrapper
  --force-wrapper         Overwrite existing wrapper if present
  -y, --yes               Non-interactive defaults
  -h, --help              Show this help

Environment variables (optional):
  MCLAW_ROOT, OPENAKITA_APP_DIR, OPENAKITA_VENV_DIR, OPENAKITA_EXTRAS,
...
if [[ "$RUN_INIT" == "1" ]]; then
  echo -e "${YELLOW}Running setup wizard (mclaw init)...${NC}"
  ...
  if [[ -t 0 ]]; then
    ...
    if [[ -e /dev/tty ]]; then
      ...
      echo "  cd \"$APP_DIR\" && source \"$VENV_DIR/bin/activate\" && mclaw init"
    fi
  fi
  popd >/dev/null
fi
...
if [[ "$INSTALL_WRAPPER" == "1" ]]; then
  ...
  WRAPPER_PATH="$WRAPPER_DIR/mclaw"
  mkdir -p "$WRAPPER_DIR"
