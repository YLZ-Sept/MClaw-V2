...
Why this lives in ``scripts/`` and not in ``src/mclaw/``:

  * It is a developer / maintainer tool, not used at runtime.
  * ``pyproject.toml`` only packages ``src/mclaw`` into the wheel, so
...
# ``src/mclaw/core/policy_v2/enums.py``. Imported lazily-by-string so
# the script can run without the mclaw venv (handy for one-off
...
def _classify(tool: ToolEntry) -> Suggestion:
    ...
    # Mclaw plugin tools follow ``<scope>_<scope>..._<verb>`` shape
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--repo-root",
        default=".",
        help="path to the Mclaw repo root (default: CWD)",
    )
    return p
