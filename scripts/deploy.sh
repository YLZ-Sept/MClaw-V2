...
EOF
    ...
    print_info "提示: 通过 Setup Center 或 mclaw llm-config 添加 LLM 端点"
...
ExecStart=$(pwd)/venv/bin/mclaw serve
Restart=always
RestartSec=10
...
WantedBy=multi-user.target"
    ...
    local service_file="mclaw.service"
    ...
    echo "  sudo systemctl enable mclaw"
    echo "  sudo systemctl start mclaw"
}
...
show_completion() {
    ...
    echo -e "     ${CYAN}mclaw llm-config${NC}  # 交互式配置向导"
    ...
    echo ""
    echo -e "  4. 启动 Agent:"
    echo -e "     ${CYAN}mclaw${NC}        # 交互模式"
    echo -e "     ${CYAN}mclaw serve${NC}  # 服务模式 (Telegram/IM)"
    echo ""
    echo -e "${BLUE}新特性:${NC}"
