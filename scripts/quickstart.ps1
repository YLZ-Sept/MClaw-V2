...
.DESCRIPTION
  - 创建独立的 venv（默认在 %USERPROFILE%\.mclaw\venv）
  - 安装 mclaw（可选 extras、可选镜像）
  - 可选安装 Playwright 浏览器
  - 可选运行 mclaw init（在 AppDir 目录生成 .env / data / identity）
...
[CmdletBinding()]
param(
  [string]$AppDir = "$env:USERPROFILE\.mclaw\app",
  [string]$VenvDir = "$env:USERPROFILE\.mclaw\venv",
  [string]$Extras = "",
...
# Activate venv (for `mclaw` entrypoint)
...
if (-not $NoInit) {
  Write-Color "Running setup wizard (mclaw init)..." Cyan
  Push-Location $AppDir
  try { mclaw init } finally { Pop-Location }
}

if (-not $NoWrapper) {
  $binDir = "$env:USERPROFILE\.mclaw\bin"
  New-Item -ItemType Directory -Path $binDir -Force | Out-Null
  $cmdPath = Join-Path $binDir "mclaw.cmd"
