"""LOC invariant audit for the Mclaw revamp.

Why this script exists
----------------------
The post-RC continuation plan
(`mclaw_revamp_continuation_plan_d6192647.plan.md` §0.1)
makes a single hard rule the entire phase enforcement hangs on:
the active runtime giants under `src/mclaw/core/` and
`src/mclaw/orgs/` may **never** grow line-wise, and the v2
facades / real implementations under `src/mclaw/agent/`
...
TRACKED_FILES: list[str] = [
    ...
    # public API lives under mclaw.agent; these files are implementation
    # details until their responsibilities are split into smaller modules.
    "src/mclaw/core/_brain_runtime.py",
    "src/mclaw/core/_tool_runtime.py",
    "src/mclaw/core/_context_runtime.py",
    "src/mclaw/core/_reasoning_runtime.py",
    "src/mclaw/core/_agent_runtime.py",
    "src/mclaw/core/_supervisor_runtime.py",
    "src/mclaw/orgs/runtime.py",
    "src/mclaw/orgs/tool_handler.py",
    "src/mclaw/orgs/templates.py",
    "src/mclaw/orgs/messenger.py",
    "src/mclaw/agent/core.py",
    "src/mclaw/agent/reasoning.py",
    "src/mclaw/agent/brain.py",
    "src/mclaw/agent/tools.py",
    "src/mclaw/agent/context.py",
]
...
def _is_agent(path: str) -> bool:
    return path.startswith("src/mclaw/agent/")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="LOC invariant audit for the Mclaw revamp.")
    parser.add_argument(
        "--init",
        action="store_true",
