...
Credentials are read from environment variables or ~/.mclaw/feedback.env:
...
def _load_env() -> None:
    """Load credentials from ~/.mclaw/feedback.env if it exists."""
    env_file = Path.home() / ".mclaw" / "feedback.env"
...
def _get_bucket() -> oss2.Bucket:
    ...
    if missing:
        ...
        print("Set them or create ~/.mclaw/feedback.env", file=sys.stderr)
        sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Manage Mclaw feedback reports stored in Alibaba Cloud OSS",
    )
