...
将飞书官方 CLI 工具 ([larksuite/cli](https://github.com/larksuite/cli)) 接入 Mclaw 插件系统，让 AI Agent 可以通过自然语言操作飞书开放平台的 11 大业务域。
...
本插件与 Mclaw 内置的飞书 IM 通道（`channels/adapters/feishu.py`）是**互补关系**：
...
将 `lark-cli-tool/` 目录复制到 Mclaw 的 `data/plugins/` 下，或通过设置中心安装。
...
Integrates the official Lark/Feishu CLI tool ([larksuite/cli](https://github.com/larksuite/cli)) into the Mclaw plugin system, enabling AI Agents to operate 11 core business domains on the Feishu/Lark Open Platform via natural language.
