...
We take security seriously. If you discover a security vulnerability within Mclaw, please follow these steps:
...
Mclaw has file system access. To limit exposure:

```bash
# Run in a dedicated directory
mkdir ~/mclaw-workspace
cd ~/mclaw-workspace

# Consider using Docker for isolation
docker run --rm -it mclaw
```
...
Mclaw can execute shell commands. Recommendations:
...
As with any LLM-based system, Mclaw may be susceptible to prompt injection. Mitigations:
...
For general questions, use [GitHub Discussions](https://github.com/mclaw/mclaw/discussions).

---

Thank you for helping keep Mclaw and its users safe!
