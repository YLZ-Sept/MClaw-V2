...
这是 Mclaw 的可视化安装与配置中心（跨平台：Windows / macOS / Linux）。

### 目标（分阶段）

- **v0（已打通主链路）**：多工作区、创建/选择工作区、生成工作区文件（`.env`/`data/llm_endpoints.json`/`identity/SOUL.md`）、检测内置 Python、创建 venv、pip 安装 mclaw、按 OpenAI/Anthropic 协议拉取模型列表、写入端点配置、写入 IM/代理等 env。
...
如果后端已通过 `mclaw serve` 单独启动，优先使用外部后端开发模式：
