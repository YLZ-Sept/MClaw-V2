...
const _HASH_TO_VIEW: Record<string, ViewId> = {
  ...
  "scheduler": "scheduler", "memory": "memory", "knowledge": "knowledge", "status": "status",
...
function UserDocsFrame({
  ...
  return (
    <div className="card" style={{ margin: 16, padding: 32, textAlign: "center" }}>
      ...
      <button onClick={() => window.open("https://mclaw.ai", "_blank", "noopener,noreferrer")}>
        打开在线文档
      </button>
      {available === "checking" && (
...
function MainApp() {
    ...
    invoke<any>("take_startup_recovery_notice")
      .then((payload) => {
        ...
        const message = reason === "native_frontend_crash"
          ? "Mclaw 刚刚从前端异常退出中恢复。后台服务和数据仍然保留，可以继续使用。"
          : "Mclaw 刚刚从桌面端崩溃中自动恢复。后台服务和数据仍然保留，可以继续使用。";
  ...
  useEffect(() => {
    try {
      ...
      window.dispatchEvent(new Event("mclaw_app_ready"));
    } catch {
      // ignore
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const onMessage = (e: MessageEvent) => {
      if (e.data?.type === "mclaw-navigate" && typeof e.data.hash === "string") {
  ...
  const [apiBaseUrl, setApiBaseUrl] = useState(() =>
    ...
    : IS_WEB ? ""
    : (localStorage.getItem("mclaw_apiBaseUrl") || DEFAULT_LOCAL_API_BASE),
  );
  ...
  const [mclawInstalled, setOpenakitaInstalled] = useState(false);
  ...
  const notifyPluginAppsReady = useCallback(() => {
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("mclaw:plugin-apps-changed", {
  ...
  async function refreshAll() {
    if (IS_TAURI) {
      ...
      try {
        ...
        setInfo({ os: "web", arch: "", homeDir: "", mclawRootDir: infoPath });
      } catch {
        // Backend not reachable yet (auth pending, first load) — use safe defaults
        setInfo({ os: "web", arch: "", homeDir: "", mclawRootDir: "" });
      ...
      try {
        ...
        if (!cancelled) {
          try {
            ...
            const vd = joinPath(plat.mclawRootDir, "venv");
            const v = await invoke<string>("mclaw_version", { venvDir: vd });
            if (!cancelled && v) {
          ...
          } catch { /* venv not found or mclaw not installed */ }
  ...
  const handleAppResumed = useCallback(() => {
    ...
    lastResumeRef.current = now;
        ...
        setTimeout(() => { visibilityGraceRef.current = false; }, 10000);
        reconnectWsNow();
        window.dispatchEvent(new Event("mclaw_app_resumed"));
  ...
  useEffect(() => {
    ...
    const timer = setInterval(async () => {
      ...
      try {
        ...
        if (IS_TAURI && dataMode !== "remote") {
          try {
            const alive = await invoke<boolean>("mclaw_check_pid_alive", { workspaceId: currentWorkspaceId });
            if (alive) {
  ...
  const venvDir = useMemo(() => {
    if (!info) return "";
    return joinPath(info.mclawRootDir, "venv");
  }, [info]);
  ...
  useEffect(() => {
    ...
    const unsub = onWsEvent((event, data) => {
      ...
      if (event === "skills:changed") {
        try {
          window.dispatchEvent(new CustomEvent("mclaw:skills-changed", {
        ...
        try {
          ...
          try {
            const ss = await invoke<ServiceStatus>(
              "mclaw_service_start", { venvDir, workspaceId: opts.targetId },
            );
            setServiceStatus(ss);
          } catch (e) {
            setRestartOverlay({ phase: "fail" });
  ...
  async function doLoadProviders() {
    ...
    try {
      ...
      if (shouldUseHttpApi()) {
        ...
        try {
          const raw = await invoke<string>("mclaw_list_providers", { venvDir });
      ...
      if (!shouldUseHttpApi()) {
        try {
          const v = await invoke<string>("mclaw_version", { venvDir });
          setOpenakitaVersion(v || "");
        } catch {
          setOpenakitaVersion("");
        }
  ...
  async function restartService(): Promise<void> {
    ...
    try {
      ...
      if (IS_TAURI && venvDir && currentWorkspaceId) {
        try {
          await invoke("mclaw_ensure_channel_deps", {
            venvDir,
            workspaceId: currentWorkspaceId,
          });
        } catch { /* 非关键步骤，失败不影响流程 */ }
      ...
      if (IS_TAURI && wsId && dataMode === "local") {
        try {
          const ss = await invoke<ServiceStatus>("mclaw_service_status", { workspaceId: wsId });
      ...
      // and let mclaw serve restart itself via /api/config/restart.
      ...
      if (IS_TAURI && wsId && venvDir && dataMode === "local" && tauriManagedBackend) {
        ...
        try {
          await invoke("mclaw_service_stop", { workspaceId: wsId });
        } catch { /* PID 文件可能不存在 */ }
        ...
        try {
          const ss = await invoke<ServiceStatus>(
            "mclaw_service_start", { venvDir, workspaceId: wsId },
          );
          setServiceStatus(ss);
        } catch (e) {
          setRestartOverlay({ phase: "fail" });
  ...
  async function refreshStatus(overrideDataMode?: "local" | "remote", overrideApiBaseUrl?: string, forceAliveCheck?: boolean) {
    ...
    try {
      ...
      if (useHttpApi) {
        ...
        try {
          ...
          if (effectiveDataMode !== "remote" && currentWorkspaceId) {
            try {
              const skillsRaw = await invoke<string>("mclaw_list_skills", { venvDir, workspaceId: currentWorkspaceId });
        ...
        if (effectiveDataMode !== "remote" && currentWorkspaceId) {
          try {
            const ss = await invoke<ServiceStatus>("mclaw_service_status", { workspaceId: currentWorkspaceId });
            setServiceStatus((prev) => ({
      ...
      // skills (requires mclaw installed in venv)
      try {
        const skillsRaw = await invoke<string>("mclaw_list_skills", { venvDir, workspaceId: currentWorkspaceId });
        const skillsParsed = JSON.parse(skillsRaw) as { count: number; skills: any[] };
        const skills = Array.isArray(skillsParsed.skills) ? skillsParsed.skills : [];
      ...
      if (effectiveDataMode !== "remote") {
        try {
          const ss = await invoke<ServiceStatus>("mclaw_service_status", {
            workspaceId: currentWorkspaceId,
          });
          setServiceStatus(ss);
      ...
      if (effectiveDataMode !== "remote") {
        try {
          const procs = await invoke<Array<{ pid: number; cmd: string }>>("mclaw_list_processes");
  ...
  async function doStartLocalService(effectiveWsId: string) {
    ...
    try {
      ...
      const ss = await invoke<ServiceStatus>("mclaw_service_start", {
        venvDir,
        workspaceId: effectiveWsId,
      });
      setServiceStatus(ss);
      const ready = await waitForServiceReady("http://127.0.0.1:18900", LOCAL_SERVICE_READY_TIMEOUT_MS);
      const real = await invoke<ServiceStatus>("mclaw_service_status", {
        workspaceId: effectiveWsId,
      });
  ...
  async function doStopService(wsId?: string | null) {
    ...
    try {
      const ss = await invoke<ServiceStatus>("mclaw_service_stop", { workspaceId: id });
      setServiceStatus(ss);
    ...
    // Final status
    try {
      const final_ss = await invoke<ServiceStatus>("mclaw_service_status", { workspaceId: id });
  ...
  async function refreshServiceLog(workspaceId: string) {
    try {
      ...
      if (shouldUseHttpApi()) {
        ...
        chunk = await invoke<{ path: string; content: string; truncated: boolean }>("mclaw_service_log", {
          workspaceId,
          tailBytes: 60000,
        });
      }
  ...
  useEffect(() => {
    ...
    if (!mclawInstalled && dataMode !== "remote") return;
    void doRefreshSkills();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [view, stepId, currentWorkspaceId, mclawInstalled, skillsDetail, dataMode]);
    ...
    try {
      ...
      if (!shouldUseHttpApi() && skillsList.length === 0 && currentWorkspaceId) {
        try {
          ...
          logger.warn("App", "mclaw_list_skills via Tauri failed", { error: String(e) });
        }
      }
  ...
  async function obRunSetup() {
    ...
    try {
      ...
      try {
        const effectiveVenv = venvDir || (info ? joinPath(info.mclawRootDir, "venv") : "");
        ...
        if (!backendInfo.bundled && !backendInfo.venvReady) {
          log("未找到可用后端，尝试自动创建 venv 并安装 mclaw...");
          ...
          if (pythonCandidate) {
            ...
            updateTask("backend-check", { detail: "安装 mclaw..." });
            logTask("检查后端环境", "running", "安装 mclaw...");
            setInstallProgress({ stage: "安装 mclaw...", percent: 0 });
            try {
      ...
      try {
        ...
        if (earlyProbe) {
          ...
          if (backendStartInFlight) {
            ...
            const ss = await invoke<ServiceStatus>("mclaw_service_start", { venvDir: effectiveVenv, workspaceId: activeWsId });
            setServiceStatus(ss);
        ...
        return (
          <div className="obPage">
            <div className="flex flex-col items-center text-center max-w-[520px] gap-5">
              <img src={logoUrl} alt="Mclaw" className="w-20 h-20 rounded-2xl shadow-lg mb-1" />
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              // joinPath(info.mclawRootDir, "venv") / "runtime"
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              // PlatformInfo，否则 info.mclawRootDir 仍是旧 root。
                              try {
                                const plat = await invoke<PlatformInfo>("get_platform_info");
                                setInfo(plat);
                              } catch { /* ignore */ }
                  ...
                  if (IS_TAURI && earlyStartWsId) {
                    ...
                    (async () => {
                      try {
                        ...
                        setObBackendStartupPhase("starting", t("onboarding.backendStartup.starting"));
                        const ss = await invoke<ServiceStatus>("mclaw_service_start", { venvDir: effectiveVenv, workspaceId: wsId });
                        setServiceStatus(ss);
                        setObBackendStartupPhase("waiting", t("onboarding.backendStartup.waiting"));
  ...
  function renderStepContent() {
    ...
    if (view === "knowledge") {
      return <KnowledgeBaseView serviceRunning={serviceStatus?.running ?? false} apiBaseUrl={apiBaseUrl} />;
    }
    if (view === "identity") {
      return (
        ...
        {showPwBanner && (
          <div style={{
            ...
            <button className="btnSmall" style={{ whiteSpace: "nowrap", fontWeight: 500, fontSize: isMobile ? 11 : undefined, padding: isMobile ? "2px 8px" : undefined }} onClick={() => {
              ...
              localStorage.setItem("mclaw_pw_banner_dismissed", "1");
                ...
                <button className="btnPrimary" disabled={!!busy} onClick={async () => {
                  ...
                  try {
                    ...
                    if (data.status === "ok") {
                      ...
                      if (!authOk) {
                        setApiBaseUrl(url);
                        localStorage.setItem("mclaw_apiBaseUrl", url);
