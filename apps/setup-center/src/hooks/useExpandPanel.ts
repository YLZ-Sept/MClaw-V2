// useExpandPanel — listen for `mclaw:expand-panel` CustomEvents and bring
...
//   2. ConfigHintCard dispatches `mclaw:navigate-view` to switch to the
//      settings view, then `mclaw:expand-panel` after a short delay so the
 ...
 * ``mclaw:expand-panel`` event fires with ``detail.anchor === targetAnchor``,
...
export function useExpandPanel(targetAnchor: string) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mclaw:expand-panel", handler as EventListener);
    return () => {
      window.removeEventListener("mclaw:expand-panel", handler as EventListener);
    };
  }, [targetAnchor]);
...
export function dispatchExpandPanel(anchor: string, scrollTo?: string): void {
  ...
  window.dispatchEvent(
    new CustomEvent<ExpandPanelDetail>("mclaw:expand-panel", { detail }),
  );
}
