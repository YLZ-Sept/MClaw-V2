...
// `mclaw_backend_ready` 事件。一旦 ready 就一直为 true（不会 false 回滚），
...
export function useBackendReady(): boolean {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mclaw_backend_ready", handler);
    if (window.__OPENAKITA_BACKEND_READY) setReady(true);
    return () => window.removeEventListener("mclaw_backend_ready", handler);
  }, [ready]);
