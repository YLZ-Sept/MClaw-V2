 ...
 *     restart via ``/api/shutdown``) and standalone ``mclaw serve``
...
export const DEGRADED_REFRESH_EVENT = "mclaw.degraded.refresh";
...
async function getDesktopToken(): Promise<string | null> {
  ...
  try {
    return await invoke<string>("mclaw_desktop_session_token");
  } catch {
    return null;
  }
}
...
interface Props {
  apiBase: string;
  /**
   * Optional override — when running standalone ``mclaw serve`` from
...
const DegradedRepairDialog: React.FC<DialogProps> = ({ apiBase, canAutoRestart, onClose }) => {
  ...
  const quarantine = async (subsystem: string) => {
    ...
    try {
      ...
      if (desktopToken) {
        headers["X-Mclaw-Desktop-Token"] = desktopToken;
      }
      ...
      >
        ...
        <p style={{ fontSize: 13, color: "#374151", margin: "8px 0 16px" }}>
          {t("degraded.dialogDescription", {
            defaultValue:
              "Mclaw detected one or more subsystems unable to open their database. You can quarantine each one (rename the file, recreate empty on next boot) — no other data is touched.",
          })}
        </p>
            ...
            <div style={{ marginTop: 6 }}>
              {canAutoRestart
                ? t("degraded.restartTauri", {
                    defaultValue:
                      "Mclaw will stop the backend; the desktop app will respawn it automatically.",
                  })
                : t("degraded.restartStandalone", {
                    defaultValue:
                      "Stop the current backend with Ctrl+C in your terminal and run `mclaw serve` again.",
                  })}
            </div>
            {canAutoRestart && (
              <button
