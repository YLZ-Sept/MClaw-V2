...
export function MemoryView({ serviceRunning, apiBaseUrl = "" }: Props) {
  ...
  const [sessionLegacyDismissed, setSessionLegacyDismissed] = useState<boolean>(() => {
    try {
      return window.sessionStorage.getItem("mclaw.legacy_banner_snoozed") === "1";
    } catch {
      return false;
    }
  });
  ...
  const handleClaimLegacy = async () => {
    ...
    try {
      ...
      // 这里把前端 snooze 一起对齐。
      try {
        window.sessionStorage.removeItem("mclaw.legacy_banner_snoozed");
      } catch {
        /* ignore */
      }
  ...
  const handleSnoozeLegacy = () => {
    ...
    try {
      window.sessionStorage.setItem("mclaw.legacy_banner_snoozed", "1");
    } catch {
      /* ignore */
    }
