...
export function ChatView({
  ...
  const STORAGE_KEY_DATA_EPOCH = `mclaw_data_epoch_${wsTag}`;
  ...
  const [sidebarPinned, setSidebarPinned] = useState(() => {
    try { return localStorage.getItem("mclaw_convSidebarPinned") === "true"; } catch { return false; }
  });
  ...
  useEffect(() => {
    ...
    window.addEventListener("mclaw_activate_org", handler);
    return () => window.removeEventListener("mclaw_activate_org", handler);
  }, []);
  ...
  const clientIdRef = useRef(() => {
    let id = sessionStorage.getItem("mclaw_client_id");
    if (!id) {
      id = typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : genId();
      sessionStorage.setItem("mclaw_client_id", id);
    }
    return id;
  });
  ...
  useEffect(() => {
    ...
    window.addEventListener("mclaw_app_resumed", handler);
    return () => window.removeEventListener("mclaw_app_resumed", handler);
  }, []);
  ...
  useEffect(() => {
    ...
    void restoreSessionsUntilReady<Record<string, unknown>>({
      ...
      reconcile: async (data) => {
        ...
        try { localStorage.removeItem("mclaw_data_epoch"); } catch { /* ignore */ }
  ...
  const quickStartItems = useMemo<Array<{
    ...
    { id: "research", icon: <IconBarChart size={20} />, text: t("chat.quickStart.research", "帮我做一份 Mclaw 竞品分析") },
    { id: "ppt", icon: <IconPlan size={20} />, text: t("chat.quickStart.ppt", "帮我生成一份项目汇报 PPT 大纲") },
    { id: "search", icon: <IconGlobe size={20} />, text: t("chat.quickStart.search", "帮我搜索 Mclaw 的最新动态") },
    ...
    { id: "summary", icon: <IconClipboard size={20} />, text: t("chat.quickStart.summary", "帮我总结一下今天的工作内容") },
    {
      ...
      text: t("chat.quickStart.diagnose", "帮我检查 Mclaw 运行日志并诊断问题"),
      mode: "ask",
        ...
        <div ref={scrollContainerRef} role="log" aria-live="polite" aria-label={t("chat.messageList", "消息列表")} className="flex min-h-0 flex-1 flex-col overflow-hidden px-5 py-4">
          ...
          <MessageList
            ...
            onCompletionAction={(msg, action) => {
              ...
              onOpenFeedback?.({
                mode: "bug",
                title: t("chat.diagnosticFeedbackPrefillTitle", "Mclaw 运行日志诊断反馈"),
                description: t(
            ...
            <div className="convSidebarTopRow">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  try { localStorage.setItem("mclaw_convSidebarPinned", String(next)); } catch {}
                }}
