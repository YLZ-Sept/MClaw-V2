 ...
 *   1) bridge:render-ready  -- plugin called window.Mclaw.ready() (best;
 ...
 *      for plugins that did NOT call Mclaw.ready())
...
export default function PluginAppHost({ pluginId, apiBase, onViewChange }: PluginAppHostProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mclaw:plugin-reloaded", onPluginReloaded);
    return () => window.removeEventListener("mclaw:plugin-reloaded", onPluginReloaded);
  }, [pluginId]);
  ...
  const handleIframeLoad = useCallback(() => {
    ...
    // handshake / Mclaw.ready() will normally fire well within it; if
