...
// Mirrors PluginErrorTracker.health_snapshot() in src/mclaw/plugins/sandbox.py.
...
export default function PluginManagerView({ visible, httpApiBase }: Props) {
  ...
  const notifyAppsChanged = () => {
    try {
      window.dispatchEvent(new CustomEvent("mclaw:plugin-apps-changed"));
    } catch { /* ignore */ }
  };
      ...
      if (action === "reload") {
        ...
        try {
          window.dispatchEvent(
            new CustomEvent("mclaw:plugin-reloaded", { detail: { pluginId: id } }),
          );
        } catch { /* ignore */ }
