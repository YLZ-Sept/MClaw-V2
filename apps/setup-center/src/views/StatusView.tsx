  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        logger.warn("mclaw_runtime_last_error failed", String(e));
      }
    })();
    return () => {
      cancelled = true;
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-5 px-6 py-5">
      ...
      {/* Banner: RUNTIME_PERMISSION_DENIED — 企业 AD / 杀软"勒索软件防护"拦截
          ...
          mclaw_open_runtime_root 命令——它在目录尚未创建时会自动回退到
        ...
        {IS_TAURI && detectedProcesses.length > 1 && (
          <div className="statusPanelAlert">
            ...
            <Button size="sm" variant="destructive" style={{ marginLeft: "auto" }} onClick={async () => {
              ...
              try {
                const stopped = await invoke<number[]>("mclaw_stop_all_processes");
...
function MemoryRepairDialog({
  ...
  const executeAction = async () => {
    ...
    try {
      ...
      if (IS_TAURI) {
        headers["X-Mclaw-Desktop-Token"] =
          await invoke<string>("mclaw_desktop_session_token");
      }
      const body =
        action === "restore"
