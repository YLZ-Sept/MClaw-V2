...
export function AdvancedView(props: AdvancedViewProps) {
    ...
    if (apiUrl) {
      ...
      safeFetch(`${apiUrl}/api/system-info`, { signal: AbortSignal.timeout(8_000) })
        ...
        .then((data) => {
          ...
          if (data.mclaw_version) sysInfo["Backend"] = data.mclaw_version;
      ...
      safeFetch(`${apiUrl}/api/config/env`, { signal: AbortSignal.timeout(5_000) })
        ...
        .then((data) => {
          ...
          if (data.env?.HUB_ENABLED != null) {
            ...
            localStorage.setItem("mclaw_storeVisible", String(enabled));
          }
        })
        .catch(() => {});
    }
  ...
  async function fetchSystemInfo() {
    ...
    try {
      ...
      if (data.mclaw_version) sysInfo[t("adv.sysVersion")] = data.mclaw_version;
      setAdvSysInfo(sysInfo);
  ...
  async function opsHandleBundleExport() {
    ...
    try {
      ...
      const filename = `mclaw-diagnostic-${ts}.zip`;
        ...
        <Section title={t("adv.hubTitle")}
          toggle={
            <Switch
              ...
              onCheckedChange={(v) => {
                setStoreVisible(v);
                localStorage.setItem("mclaw_storeVisible", String(v));
                if (shouldUseHttpApi()) {
