...
function _getApiBase(): string {
  ...
  try {
    const raw = localStorage.getItem("mclaw_servers");
    const activeId = localStorage.getItem("mclaw_active_server");
    if (raw && activeId) {
...
function buildExportText(
  ...
  sections.push("====== Mclaw Log Export ======");
...
async function exportLogs(): Promise<ExportResult> {
  ...
  const filename = `mclaw-logs-${dateLabel()}.log`;
