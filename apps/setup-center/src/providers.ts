 ...
 * 与 Python 端 mclaw.llm.capabilities.infer_capabilities 的关键词规则保持一致。
 *
 * ⚠ 维护提示：如果 Python 端的推断规则有修改，需要同步更新此函数。
 * 参见: src/mclaw/llm/capabilities.py → infer_capabilities()
 */
    ...
    if (body?.error === "setup_required") {
      try {
        window.dispatchEvent(
          new CustomEvent("mclaw:setup-required", { detail: body }),
        );
