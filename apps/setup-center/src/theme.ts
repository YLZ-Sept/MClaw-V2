...
export function initTheme() {
  const pref = (localStorage.getItem("mclaw-theme-pref") as Theme) || "system";
  applyTheme(pref);

  const mediaQuery = window.matchMedia("(prefers-color-scheme: dark)");
  const listener = (e: MediaQueryListEvent) => {
    const currentPref = localStorage.getItem("mclaw-theme-pref") as Theme | null;
...
export const THEME_CHANGE_EVENT = "mclaw-theme-change";

let _previewBackup: Theme | null = null;

export function setThemePref(theme: Theme) {
  _previewBackup = null;
  localStorage.setItem("mclaw-theme-pref", theme);
  applyTheme(theme);
  window.dispatchEvent(new CustomEvent(THEME_CHANGE_EVENT, { detail: theme }));
}

export function getThemePref(): Theme {
  return (localStorage.getItem("mclaw-theme-pref") as Theme) || "system";
}
