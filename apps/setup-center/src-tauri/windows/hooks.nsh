; Mclaw Setup Center - NSIS Hooks
; 目标：
; - 卸载时强制杀掉残留进程（Setup Center 本体 + Mclaw 后台服务）
; - 勾选"清理用户数据"时，删除用户目录下的 ~/.mclaw
...
!macro _Mclaw_ResolveRoot
  ExpandEnvStrings $R9 "%USERPROFILE%\.mclaw"
...
!macro _Mclaw_WriteCleanupScript
  InitPluginsDir
  ...
  FileWrite $R8 "$$defaultRoot = Join-Path $$env:USERPROFILE '.mclaw'$\r$\n"
  ...
  FileWrite $R8 "    return (Test-Path -LiteralPath (Join-Path $$full '.mclaw-root'))$\r$\n"
  FileWrite $R8 "}$\r$\n"
  ; ── Clean each root ──
...
;   2. Kills by PID files (reads mclaw-*.pid from data dirs)
...
;      timestamped copy under %USERPROFILE%\.mclaw\logs\, but ALWAYS exits
...
!macro _Mclaw_WriteKillScript
  ...
  FileWrite $R9 "            $$alive = @(Get-Process -Name mclaw-server -EA $$EA)$\r$\n"
  FileWrite $R9 "            if ($$alive.Count -eq 0) { return }$\r$\n"
  FileWrite $R9 "        }$\r$\n"
  FileWrite $R9 "    } catch {}$\r$\n"
  FileWrite $R9 "}$\r$\n"
  ; ── function: Kill all Mclaw processes ──
  FileWrite $R9 "function Kill-OA {$\r$\n"
  FileWrite $R9 "    Stop-OAGracefully$\r$\n"
  FileWrite $R9 "    Get-Process -Name mclaw-desktop,mclaw-setup-center,mclaw-server -EA $$EA |$\r$\n"
  FileWrite $R9 "        Stop-Process -Force -EA $$EA$\r$\n"
  FileWrite $R9 "    & cmd /c 'taskkill /IM mclaw-desktop.exe /T /F >nul 2>&1'$\r$\n"
  FileWrite $R9 "    & cmd /c 'taskkill /IM mclaw-setup-center.exe /T /F >nul 2>&1'$\r$\n"
  FileWrite $R9 "    & cmd /c 'taskkill /IM mclaw-server.exe /T /F >nul 2>&1'$\r$\n"
  ; Kill by PID files (resolve custom data root first)
  FileWrite $R9 "    $$root = Join-Path $$env:USERPROFILE '.mclaw'$\r$\n"
  ...
  FileWrite $R9 "        Get-ChildItem (Join-Path $$runDir 'mclaw-*.pid') -EA $$EA | ForEach-Object {$\r$\n"
  ...
  FileWrite $R9 "        } | Stop-Process -Force -EA $$EA$\r$\n"
  FileWrite $R9 "    }$\r$\n"
  ; Kill Mclaw-owned Python processes (embedded bootstrap interpreter + uv
  ...
  FileWrite $R9 "            }$\r$\n"
  FileWrite $R9 "            if (-not $$owned -and $$cl -and ($$cl -match 'mclaw\.main' -or $$cl -match 'mclaw-server' -or $$cl -match 'resources\\bootstrap' -or $$cl -match '\.mclaw\\')) { $$owned = $$true }$\r$\n"
  ...
  ; -LiteralPath: $InstDir defaults to $LOCALAPPDATA\Mclaw; tolerate Chinese
  ...
  FileWrite $R9 "try {$\r$\n"
  FileWrite $R9 "    $$logDir = Join-Path $$env:USERPROFILE '.mclaw\logs'$\r$\n"
...
!macro NSIS_HOOK_PREINSTALL
  ...
  ; copied to %USERPROFILE%\.mclaw\logs\install_locked_*.log for support.
  ;
  ...
  ${If} ${FileExists} "$R0\*"
    ...
    !insertmacro _Mclaw_WriteCleanupScript
...
; Generates a PowerShell script that resolves BOTH data roots and removes only Mclaw-owned entries.
; Used by NSIS_HOOK_POSTUNINSTALL — same self-resolving pattern as _oa_cleanup.ps1.
!macro _Mclaw_WriteUninstDataScript
  InitPluginsDir
  FileOpen $R8 "$PLUGINSDIR\_oa_uninst_data.ps1" w
  FileWrite $R8 "$$EA = 'SilentlyContinue'$\r$\n"
  FileWrite $R8 "$$def = Join-Path $$env:USERPROFILE '.mclaw'$\r$\n"
  ...
  FileWrite $R8 "    }$\r$\n"
  FileWrite $R8 "    foreach ($$f in @('state.json','config.json','.env','cli.json','root_config.json','custom_root.txt','.mclaw-root')) {$\r$\n"
...
!macro NSIS_HOOK_POSTUNINSTALL
  ...
  ; before this hook runs, so we only handle ~/.mclaw + custom root here.
  ${If} $DeleteAppDataCheckboxState = 1
  ${AndIf} $UpdateMode <> 1
    !insertmacro _Mclaw_WriteUninstDataScript
