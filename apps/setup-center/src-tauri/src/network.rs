...
pub(crate) fn export_env_backup(
    ...
    let dest = if let Some(p) = dest_path {
        ...
        let ts = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_secs();
        downloads_dir.join(format!("mclaw-env-backup-{ts}.env"))
    };
...
mod tests {
    ...
    fn pypi_json_urls_keep_requested_source_then_known_fallbacks() {
        assert_eq!(
            pypi_json_urls("mclaw", Some("https://mirrors.aliyun.com/pypi/simple/")),
            vec![
                "https://mirrors.aliyun.com/pypi/mclaw/json",
                "https://pypi.tuna.tsinghua.edu.cn/pypi/mclaw/json",
                "https://pypi.org/pypi/mclaw/json",
            ]
        );
    }
    ...
    fn pypi_json_urls_do_not_duplicate_an_official_source() {
        assert_eq!(
            pypi_json_urls("mclaw", Some("https://pypi.org/simple/")),
            vec![
                "https://pypi.org/pypi/mclaw/json",
                "https://pypi.tuna.tsinghua.edu.cn/pypi/mclaw/json",
            ]
