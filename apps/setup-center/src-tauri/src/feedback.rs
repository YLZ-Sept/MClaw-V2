...
pub(crate) fn export_diagnostic_bundle(
    ...
    let dest = if let Some(p) = dest_path {
        ...
        let ts = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_secs();
        downloads_dir.join(format!("mclaw-diagnostic-{ts}.zip"))
    };
...
pub(crate) const DEFAULT_FEEDBACK_ENDPOINT: &str = "https://feedback-mclaw.fzstack.com";
...
pub(crate) fn pending_feedback_path() -> PathBuf {
    mclaw_root_dir().join("pending_feedback.json")
}
...
pub(crate) fn build_feedback_zip(
    ...
    let ws_dir = workspace_dir(&workspace_id);
    let temp_dir = mclaw_root_dir().join("temp-feedback");
    ...
    // ~5 MB mini dumps to ~/.mclaw/crashdumps/mclaw-*.dmp.
...
pub(crate) fn collect_windows_crash_artifacts(
    ...
    // `AppCrash_mclaw-setup-center.exe_<hash>`, others are just
    ...
    let ps_cmd = format!(
        "$ev = Get-WinEvent -LogName Application -MaxEvents 200 -FilterXPath \"{}\" \
         ...
         Where-Object {{ $_.Message -match 'mclaw' }} | \
