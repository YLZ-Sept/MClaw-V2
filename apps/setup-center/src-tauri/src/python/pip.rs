...
pub(crate) fn install_bundled_python_sync(
    ...
    let py = managed_python_seed_path().ok_or_else(|| {
        "安装包内置 Python 不可用。请重新安装 Mclaw 以恢复 resources/bootstrap/python"
            .to_string()
    })?;
...
/// 解析可用的 Python 解释器路径。
/// 只使用 Mclaw 管理的环境：venv → bootstrap Python seed。
pub(crate) fn resolve_python(venv_dir: &str) -> Result<(PathBuf, Option<String>), String> {
    ...
    let py = find_pip_python().ok_or_else(|| {
        "未找到可用 Python 解释器（venv/bootstrap）。请重新安装 Mclaw 以恢复内置 Python。"
            .to_string()
    })?;
    Ok((py, None))
}
...
pub(crate) async fn pip_install(
    ...
    spawn_blocking_result(move || {
        let install_id = install_id.unwrap_or_else(|| PIP_INSTALL_DEFAULT_ID.to_string());
        let install_id_ref = install_id.as_str();
        pip_install_set_stage(install_id_ref, "安装 mclaw（pip）", 30);
        ...
        verify.args([
            "-c",
            "import mclaw; import mclaw.setup_center.bridge; print(getattr(mclaw,'__version__',''))",
        ]);
        let v = verify.output().map_err(|e| format!("verify mclaw failed: {e}"))?;
        if !v.status.success() {
            let stdout = String::from_utf8_lossy(&v.stdout).to_string();
            let stderr = String::from_utf8_lossy(&v.stderr).to_string();
            pip_install_finish_progress(install_id_ref, true);
            return Err(format!(
                "mclaw 已安装，但缺少 Setup Center 所需模块（mclaw.setup_center.bridge）。\n这通常意味着你安装的 mclaw 版本过旧或来源不包含该模块。\nstdout:\n{}\nstderr:\n{}",
                stdout, stderr
            ));
        }
        ...
        log.push_str("=== verify ===\n");
        log.push_str("import mclaw.setup_center.bridge: OK\n");
        emit_line("import mclaw.setup_center.bridge: OK\n");
        if !ver.is_empty() {
            log.push_str(&format!("mclaw version: {ver}\n"));
            emit_line(&format!("mclaw version: {ver}\n"));
        }
        emit_stage("完成", 100);
...
mod tests {
    ...
    fn test_venv_python_path_platform_layout() {
        let dir = if cfg!(windows) {
            r"C:\Users\test\.mclaw\venv"
        } else {
            "/home/test/.mclaw/venv"
        };
    ...
    fn test_ensure_pip_available_seeds_uv_venv_without_pip() {
        let temp =
            std::env::temp_dir().join(format!("mclaw-pip-seed-test-{}", std::process::id()));
        if temp.exists() {
