...
pub(crate) const CANONICAL_BACKEND_ENTRYPOINT: &str =
    "from mclaw.main import app as mclaw_app; mclaw_app()";
...
pub(crate) fn ensure_runtime_layout() -> Result<(), String> {
    ...
    for dir in [
        ...
        if let Err(e) = fs::create_dir_all(&dir) {
            ...
            // `%LOCALAPPDATA%\MclawDesktop\` 设为受保护目录，此时
...
/// Centralized runtime environment builder for Mclaw-managed subprocesses.
///
/// Core/bootstrap subprocesses must not inherit user Python, Conda, pip, or SSL
/// state. Both paths receive explicit Mclaw runtime locations and secret
/// scrubbing markers.
...
pub(crate) fn apply_runtime_bootstrap_env(cmd: &mut Command, pip_index: Option<&RuntimePipIndex>) {
    ...
    //     Mclaw 自管目录而不是 `%LOCALAPPDATA%\uv\python`，便于卸载、
    //     便于"修复运行环境"按钮一刀清理。
...
///
/// 解决的边角：用户把 Mclaw 安装到 `C:\anaconda3\Mclaw\` 这种路径，
/// 我们的 seed `pyvenv.cfg::home = C:\anaconda3\Mclaw\resources\bootstrap\python`
...
/// "用户装在 C:\anaconda3\Mclaw\ → seed 路径含 anaconda → 自拒死循环"）。
...
pub(crate) fn app_runtime_health_code(venv_dir: &Path) -> String {
    ...
    // 自己 seed 路径的场景（用户装到 C:\anaconda3\Mclaw\）也能放行。
...
pub(crate) fn ensure_app_venv(
    ...
    cmd.arg("certifi");
    cmd.args(["--reinstall-package", "mclaw"]);
    ...
    if health_check_python(
        ...
        Err(format!(
            "app venv health check failed after Mclaw install: python={}, log={}",
            app_py.display(),
            log_path.display()
        ))
    }
...
pub(crate) fn get_backend_executable(venv_dir: &str) -> (PathBuf, Vec<String>) {
    ...
    // have ~/.mclaw/venv. New installers do not create this environment.
    eprintln!(
...
#[tauri::command]
pub(crate) fn mclaw_runtime_last_error() -> RuntimeLastError {
...
/// MclawDesktop\` 还在，但 `runtime\` 子目录因为 AD 策略建不出来）。
///
...
#[tauri::command]
pub(crate) fn mclaw_open_runtime_root() -> Result<OpenedRuntimePath, String> {
...
mod tests {
    ...
    fn backend_entrypoint_imports_one_canonical_main_module() {
        ...
        assert_eq!(args[0], "-u");
        assert_eq!(args[1], "-c");
        assert!(args[2].contains("from mclaw.main import app"));
    ...
    fn test_runtime_command_timeout_terminates_the_child() {
        ...
        let log_path = std::env::temp_dir().join(format!(
            "mclaw-runtime-timeout-test-{}-{}.log",
            std::process::id(),
            now_ms()
        ));
        let started = Instant::now();
