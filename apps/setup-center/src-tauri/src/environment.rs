...
const OPTIONAL_MODULES_MANIFEST: &str =
    include_str!("../../../../src/mclaw/optional_modules.json");
...
pub(crate) fn get_root_dir_info() -> RootDirInfo {
    RootDirInfo {
        ...
        current_root: mclaw_root_dir().to_string_lossy().to_string(),
...
pub(crate) fn set_custom_root_dir(
    ...
    if let Some(ref p) = clean_path {
        ...
        if !target.is_absolute() {
            return Err("请使用绝对路径（如 D:\\MyData\\.mclaw 或 /data/mclaw）".into());
        }
        ensure_safe_mclaw_data_root(&target)?;
    ...
    if let Some(ref old_root) = migrate_old_root {
        if is_safe_mclaw_data_root(old_root) {
            let dir_names = [
                "workspaces",
                "venv",
                "runtime",
...
pub(crate) fn check_environment() -> EnvironmentCheck {
    ...
    // Check running processes (extract workspace_id from filename: mclaw-{ws_id}.pid)
        ...
        for entry in entries.flatten() {
            ...
            if path.extension().and_then(|e| e.to_str()) == Some("pid") {
                let ws_id = path
                    .file_stem()
                    .and_then(|s| s.to_str())
                    .and_then(|s| s.strip_prefix("mclaw-"))
                    .unwrap_or("unknown");
    ...
    EnvironmentCheck {
        mclaw_root: root.to_string_lossy().to_string(),
        has_old_venv,
        has_old_runtime,
        has_old_workspaces,
        old_version,
...
pub(crate) fn check_backend_availability(venv_dir: String) -> BackendAvailability {
    ...
    let venv_ready = legacy_venv_has_mclaw_backend(&venv_dir);
...
pub(crate) fn legacy_venv_has_mclaw_backend(venv_dir: &str) -> bool {
    ...
    command.args([
        "-c",
        "import mclaw; import mclaw.main; import mclaw.setup_center.bridge",
    ]);
    command
        .output()
...
/// Reset the entire Mclaw installation to factory state.
...
pub(crate) fn factory_reset() -> Result<String, String> {
    ...
    let stopped = mclaw_stop_all_processes();
...
mod tests {
    ...
    fn test_check_backend_availability_rejects_empty_venv() {
        let temp =
            std::env::temp_dir().join(format!("mclaw-empty-venv-test-{}", std::process::id()));
        if temp.exists() {
    ...
    #[test]
    fn test_mclaw_root_dir_is_valid() {
        let root = mclaw_root_dir();
        assert!(!root.to_string_lossy().is_empty());
        // Should contain .mclaw unless overridden by MCLAW_ROOT
        let root_str = root.to_string_lossy();
        assert!(
            root_str.contains(".mclaw") || std::env::var("MCLAW_ROOT").is_ok(),
            "root dir should contain '.mclaw' or MCLAW_ROOT should be set: {}",
            root_str
        );
    }
    ...
    fn test_data_root_rejects_drive_or_filesystem_root() {
        ...
        assert!(!is_safe_mclaw_data_root(&root));
        assert!(ensure_safe_mclaw_data_root(&root).is_err());
    }

    #[test]
    fn test_data_root_rejects_home_directory() {
        if let Some(home) = home_dir() {
            assert!(!is_safe_mclaw_data_root(&home));
            assert!(ensure_safe_mclaw_data_root(&home).is_err());
        }
    }
    ...
    fn test_data_root_allows_dedicated_directory() {
        let dedicated = if cfg!(windows) {
            PathBuf::from(r"D:\MclawData\.mclaw")
        } else {
            PathBuf::from("/tmp/mclaw-data/.mclaw")
        };
        assert!(is_safe_mclaw_data_root(&dedicated));
        assert!(ensure_safe_mclaw_data_root(&dedicated).is_ok());
    }
}
