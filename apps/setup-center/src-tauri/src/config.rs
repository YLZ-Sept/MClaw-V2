...
pub(crate) struct PlatformInfo {
    ...
    arch: String,
    home_dir: String,
    mclaw_root_dir: String,
}

/// 计算"未配置 custom_root 时的"默认 Mclaw 数据目录字符串。
///
/// 注意：日常显示请用 [`mclaw_root_dir`] 取真实 root，否则会和后端
/// 实际写入位置不一致；此函数仅作为兜底/迁移场景的"默认值"语义保留。
#[allow(dead_code)]
pub(crate) fn default_mclaw_root() -> String {
    let home = home_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
    home.join(".mclaw").to_string_lossy().to_string()
}

#[tauri::command]
pub(crate) fn get_platform_info() -> PlatformInfo {
    let home = home_dir().unwrap_or_else(|| std::path::PathBuf::from("."));
    // 用 mclaw_root_dir() 而不是 default_mclaw_root()，确保前端
    // 显示的 root（以及拼出的 runtime / venv / logs hint）与后端 Rust /
    // Python 真正使用的位置完全一致。否则在用户配置了 custom_root 或
    // 设置了 MCLAW_ROOT 环境变量时，面板会指向 ~/.mclaw 而真实
    // runtime 落在另一个磁盘，让人误以为"runtime 没建出来"。
    PlatformInfo {
        os: std::env::consts::OS.to_string(),
        arch: std::env::consts::ARCH.to_string(),
        home_dir: home.to_string_lossy().to_string(),
        mclaw_root_dir: mclaw_root_dir().to_string_lossy().to_string(),
    }
}
...
pub(crate) fn is_safe_mclaw_data_root(path: &Path) -> bool {
...
pub(crate) fn ensure_safe_mclaw_data_root(path: &Path) -> Result<(), String> {
    if is_safe_mclaw_data_root(path) {
        Ok(())
    } else {
        Err("数据目录不能设置为磁盘根目录、用户主目录或系统常用目录。请使用专用目录，例如 D:\\MclawData\\.mclaw".into())
    }
}
...
pub(crate) fn write_root_marker(root: &Path) -> Result<(), String> {
    ...
    fs::write(
        root.join(MCLAW_ROOT_MARKER),
        b"Mclaw data root\nDo not delete this file unless you no longer use this directory for Mclaw.\n",
    )
...
pub(crate) fn mclaw_root_dir() -> PathBuf {
    if let Ok(val) = std::env::var("MCLAW_ROOT") {
        if !val.is_empty() {
...
pub(crate) fn run_dir() -> PathBuf {
    mclaw_root_dir().join("run")
}

/// 安装配置日志目录：~/.mclaw/logs/
pub(crate) fn setup_logs_dir() -> PathBuf {
    mclaw_root_dir().join("logs")
}

/// 进程内 minidump 落地目录：~/.mclaw/crashdumps/
...
pub(crate) fn crashdumps_dir() -> PathBuf {
    mclaw_root_dir().join("crashdumps")
}
...
/// Append a diagnostic line to `~/.mclaw/logs/autostart.log`.
...
#[tauri::command]
pub(crate) fn mclaw_desktop_session_token() -> String {
...
pub(crate) fn start_onboarding_log(date_label: String) -> Result<String, String> {
    ...
    let header = format!("Mclaw 安装配置日志 开始于 {}\n", date_label);
...
/// 前端 JS 日志批量追加到 ~/.mclaw/logs/frontend.log。
#[tauri::command]
...
pub(crate) fn save_log_export(filename: String, content: String) -> Result<String, String> {
    let downloads = dirs_next::download_dir()
        ...
        .unwrap_or_else(|| mclaw_root_dir().join("logs"));
...
pub(crate) fn bundled_resource_dir(resource_name: &str) -> PathBuf {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let static_names: &[&str] = &[
            "MclawDesktop",       // tauri.conf.json productName used by deb resource dir
            "Mclaw Desktop",      // legacy productName with a space
            "mclaw-setup-center", // Cargo.toml package name (Tauri 2.x default)
            "mclaw-desktop",      // legacy / mainBinaryName override
            "open-akita-desktop",
        ];
