...
//!   (~1-5 MB) dump into `~/.mclaw/crashdumps/`.
...
//!   `\mclaw-{pid}-{ticks}.dmp`, opens the file via
...
//! looks for `~/.mclaw/crashdumps/*.dmp` regardless of platform, so
...
mod imp {
    ...
    /// Pre-built wide-char prefix `<dump_dir>\mclaw-`. We append the
    ...
    pub fn install(dump_dir: &Path) {
        ...
        // need to touch formatting APIs. Layout: "<dump_dir>\mclaw-".
        let prefix: Vec<u16> = OsStr::new(&format!("{}\\mclaw-", dump_dir.display()))
            .encode_wide()
            .collect();
    ...
    unsafe extern "system" fn crash_filter(info: *const EXCEPTION_POINTERS) -> i32 {
        ...
        // Build "<dir>\mclaw-<pid>-<tick>.dmp\0" in a fixed stack
