...
pub(crate) fn healthy_backend_pid(port: u16) -> Option<u32> {
    ...
    if json.get("service").and_then(|v| v.as_str()) != Some("mclaw") {
        return None;
    }
    json.get("pid")
        .and_then(|v| v.as_u64())
...
///
/// macOS 上通过 DMG 拖拽覆盖安装后，旧的 mclaw-server 进程可能仍在端口上
...
/// Append a crash entry to `~/.mclaw/logs/crash.log`.
///
...
pub(crate) fn write_crash_log(message: &str, show_dialog: bool) -> PathBuf {
    ...
    if show_dialog {
        #[cfg(windows)]
        {
            ...
            let body = format!(
                "Mclaw Desktop 启动失败 (startup failed)\n\n\
                 {message}\n\n\
                 崩溃日志已写入 (crash log): {}\n\
                 请将此日志发送给开发者以帮助诊断问题。",
                crash_path.display()
            );
            let caption = "Mclaw – Crash";
            let wb = to_wide(&body);
