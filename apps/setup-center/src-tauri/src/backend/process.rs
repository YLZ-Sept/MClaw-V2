...
pub(crate) fn state_file_path() -> PathBuf {
    mclaw_root_dir().join("state.json")
}

pub(crate) fn workspaces_dir() -> PathBuf {
    mclaw_root_dir().join("workspaces")
}
...
pub(crate) fn service_pid_file(workspace_id: &str) -> PathBuf {
    run_dir().join(format!("mclaw-{}.pid", workspace_id))
}
...
pub(crate) fn can_auto_stop_backend(workspace_id: &str, pid: u32) -> bool {
    ...
    // `python -m mclaw serve`, so keep it alive.
...
pub(crate) fn list_service_pids() -> Vec<ServicePidEntry> {
    ...
    for e in rd.flatten() {
        ...
        if !name.starts_with("mclaw-") || !name.ends_with(".pid") {
            continue;
        }
        let ws = name
            .trim_start_matches("mclaw-")
...
pub(crate) fn service_lock_file(workspace_id: &str) -> PathBuf {
    run_dir().join(format!("mclaw-{}.lock", workspace_id))
}
...
pub(crate) fn is_pid_file_valid(data: &PidFileData) -> bool {
    ...
    if data.started_at == 0 {
        return is_mclaw_process(data.pid);
    }
...
/// 检查指定 PID 是否属于 Mclaw 后端进程（python/mclaw-server）。
/// 用于判断 PID 文件是否有效——避免 Windows PID 复用导致的误判。
pub(crate) fn is_mclaw_process(pid: u32) -> bool {
    ...
    #[cfg(windows)]
    {
        ...
        // 进程名包含 python 或 mclaw-server → 可能是后端
        if exe_name.contains("mclaw-server") {
            return true;
        }
        if !exe_name.contains("python") {
            return false; // 既不是 python 也不是 mclaw-server，肯定不是后端
        }

        // Step 2: python 进程需进一步检查命令行是否包含 mclaw
    ...
    #[cfg(target_os = "linux")]
    {
        if let Ok(cmdline) = fs::read_to_string(format!("/proc/{}/cmdline", pid)) {
            return cmdline.to_lowercase().contains("mclaw");
        }
        let output = Command::new("ps")
...
/// 扫描并杀死所有进程名为 python/pythonw 且命令行包含 "mclaw" 和 "serve" 的进程。
/// 用于托盘退出时兜底清理孤儿进程（PID 文件可能已被删除但进程仍存活）。
/// 返回被杀掉的 PID 列表。
pub(crate) fn kill_mclaw_orphans() -> Vec<u32> {
    ...
    #[cfg(windows)]
    {
        ...
        if unsafe { win::Process32FirstW(snap, &mut pe) } != 0 {
            loop {
                ...
                // PyInstaller 打包后端进程名为 mclaw-server.exe
                if name_lower.contains("mclaw-server") {
        ...
        // Step 1.5: kill orphaned mclaw-server.exe (PyInstaller bundled
        // backend). The original code killed every process named like that on
        // sight, which is unsafe when the user has another Mclaw install
        ...
        // Step 2: 对每个 python 进程查命令行，判断是否是 mclaw serve 进程
        ...
        for ppid in python_pids {
            ...
            if let Ok(out) = c.output() {
                let s = String::from_utf8_lossy(&out.stdout).to_lowercase();
                // 精确匹配模块调用签名
                if s.contains("mclaw.main") && (s.contains(" serve") || s.ends_with("serve")) {
                    if is_pid_running(ppid) {
                        let _ = kill_pid(ppid);
                        killed.push(ppid);
                    }
    ...
    #[cfg(not(windows))]
    {
        // 搜索 mclaw.main serve (venv 模式) 和 mclaw-server (PyInstaller 模式)
        let patterns = [
...
/// 扫描所有进程名含 python 且命令行包含 "mclaw" 和 "serve" 的进程。
/// 返回 MclawProcess 列表，供前端多进程检测使用。
#[derive(Debug, Serialize, Deserialize, Clone)]
#[serde(rename_all = "camelCase")]
pub(crate) struct MclawProcess {
    pid: u32,
    cmd: String,
}

#[tauri::command]
pub(crate) fn mclaw_list_processes() -> Vec<MclawProcess> {
    let mut out = Vec::new();
    #[cfg(windows)]
    {
        ...
        for (ppid, parent_pid) in python_pids {
            ...
            if let Ok(cmd_out) = c.output() {
                ...
                // 精确匹配模块调用签名，避免 venv 路径中 .mclaw 误报
                if s_lower.contains("mclaw.main")
    ...
    #[cfg(not(windows))]
    {
        // ps aux | grep mclaw.main.*serve  —— 精确匹配模块调用
...
/// 停止所有检测到的 Mclaw serve 进程。
/// 返回被停止的 PID 列表。
#[tauri::command]
pub(crate) fn mclaw_stop_all_processes() -> Vec<u32> {
    ...
    // 第 2 层：兜底扫描所有命令行含 mclaw serve 的 python 进程并杀掉
    let orphans = kill_mclaw_orphans();
    for pid in orphans {
