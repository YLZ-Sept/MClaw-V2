...
#[tauri::command]
pub(crate) fn mclaw_service_status(workspace_id: String) -> Result<ServiceStatus, String> {
...
#[tauri::command]
pub(crate) fn mclaw_check_pid_alive(workspace_id: String) -> Result<bool, String> {
    ...
    if let Some(data) = read_pid_file(&workspace_id) {
        ...
        // PID 存活，但需验证是否真的是 Mclaw 进程
        if !is_mclaw_process(data.pid) {
...
pub(crate) fn strip_harmful_toolchain_env(cmd: &mut Command) {
    ...
    // when Mclaw is creating or repairing its own toolchain.
...
#[tauri::command]
pub(crate) async fn mclaw_service_start(
    venv_dir: String,
    workspace_id: String,
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        mclaw_service_start_impl(venv_dir, workspace_id)
    })
    .await
...
pub(crate) fn mclaw_service_start_impl(
    venv_dir: String,
    workspace_id: String,
) -> Result<ServiceStatus, String> {
    ...
    let log_path = log_dir.join("mclaw-serve.log");
    ...
    cmd.env(
        "MCLAW_ROOT",
        mclaw_root_dir().to_string_lossy().to_string(),
    );
    ...
    let child = cmd.spawn().map_err(|e| {
        let msg = format!("spawn mclaw serve failed: {e}");
...
#[tauri::command]
pub(crate) fn mclaw_service_stop(workspace_id: String) -> Result<ServiceStatus, String> {
    let _lifecycle_guard = BACKEND_LIFECYCLE_LOCK.lock().unwrap();
    set_backend_manually_stopped(&workspace_id, true)?;
    let pid_file = service_pid_file(&workspace_id);
    let port = read_workspace_api_port(&workspace_id);
...
#[tauri::command]
pub(crate) fn mclaw_service_log(
    workspace_id: String,
    tail_bytes: Option<u64>,
) -> Result<ServiceLogChunk, String> {
    let ws_dir = workspace_dir(&workspace_id);
    let log_path = ws_dir.join("logs").join("mclaw-serve.log");
...
///
/// 老的"重试启动/修复"只是再次调 `mclaw_service_start`，但 `ensure_venv`
...
pub(crate) fn repair_runtime_env() -> Result<String, String> {
    ...
    if let Some(ws_id) = state.current_workspace_id.clone() {
        match mclaw_service_stop(ws_id.clone()) {
...
pub(crate) fn set_tray_backend_status(
    ...
    let base = match status.as_str() {
        "alive" => "Mclaw - Running",
        "degraded" => "Mclaw - Backend Unresponsive",
        "dead" => "Mclaw - Backend Stopped",
        _ => "Mclaw",
    };
    ...
    if status == "dead" {
        #[cfg(windows)]
        {
            ...
            cmd.args([
                ...
                "try { \
                    $aumid = 'com.mclaw.setupcenter'; \
                    $rp = \"HKCU:\\SOFTWARE\\Classes\\AppUserModelId\\$aumid\"; \
                    if (!(Test-Path $rp)) { New-Item $rp -Force | Out-Null; Set-ItemProperty $rp -Name DisplayName -Value 'Mclaw Desktop' }; \
                    [Windows.UI.Notifications.ToastNotificationManager, Windows.UI.Notifications, ContentType = WindowsRuntime] | Out-Null; \
                    $xml = [Windows.UI.Notifications.ToastNotificationManager]::GetTemplateContent([Windows.UI.Notifications.ToastTemplateType]::ToastText02); \
                    $t = $xml.GetElementsByTagName('text'); \
                    $t[0].AppendChild($xml.CreateTextNode('Mclaw')) | Out-Null; \
        ...
        #[cfg(not(windows))]
        {
            ...
            let _ = Command::new("osascript")
                .args([
                    "-e",
                    "display notification \"Backend service has stopped\" with title \"Mclaw\"",
                ])
                .spawn();
        }
    }
...
pub(crate) fn scan_mclaw_orphans_with_timing(
    context: &str,
    total_started: Instant,
) -> Vec<u32> {
    let scan_started = Instant::now();
    let killed = kill_mclaw_orphans();
    log_to_file(&format!(
...
pub(crate) fn run_tray_quit_cleanup(app: tauri::AppHandle) {
    ...
    scan_mclaw_orphans_with_timing("tray-cleanup", quit_started);
    ...
    let still_pid = list_service_pids()
        ...
        .collect::<Vec<_>>();
    let still_orphans = scan_mclaw_orphans_with_timing("tray-verify", quit_started);
...
pub(crate) fn cleanup_backends_on_run_event_exit() {
    ...
    scan_mclaw_orphans_with_timing("run-event-exit", cleanup_started);
}
...
mod tests {
    ...
    fn playwright_browser_env_is_applied_only_for_an_installed_browser() {
        let modules_root = std::env::temp_dir().join(format!(
            "mclaw-playwright-env-test-{}-{}",
            std::process::id(),
            now_ms()
        ));
        let browsers_dir = modules_root.join("browser").join("browsers");

        let mut missing = Command::new("mclaw-test");
        ...
        let mut installed = Command::new("mclaw-test");
