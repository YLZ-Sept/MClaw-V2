...
pub fn run() {
    ...
    // heap corruption / illegal instruction) to ~/.mclaw/crashdumps/
                    ...
                    if need_start {
                        ...
                        std::thread::spawn(move || {
                            ...
                            if backend_was_manually_stopped(&ws_clone) {
                                ...
                                match mclaw_service_start_impl(venv_dir, ws_clone) {
                                    Ok(status) => {
                                        log_to_file(&format!(
            ...
            // 60+ 分钟才能在 autostart.log 里看到下一次探测。
            {
                ...
                std::thread::spawn(move || {
                    ...
                    loop {
                        ...
                        let venv_dir = mclaw_root_dir().join("venv");
                        let venv_dir_str = venv_dir.to_string_lossy().to_string();
                        if managed_python_seed_path().is_none()
                            && !legacy_venv_has_mclaw_backend(&venv_dir_str)
                        {
                            consecutive_failures = 0;
                            continue;
                        }
