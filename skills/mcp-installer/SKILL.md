---
name: mclaw/skills@mcp-installer
description: Install, configure, and add MCP servers to the Mclaw system. Use when the user needs to install MCP packages (npm/pip/uvx), connect remote HTTP/SSE MCP services, set up custom local MCP servers, or manage MCP server configuration and lifecycle.
license: MIT
metadata:
  author: mclaw
  version: "1.0.0"
...
Mclaw 使用目录结构管理 MCP 服务器。每个 MCP 服务器是一个独立目录，包含配置和工具定义：
