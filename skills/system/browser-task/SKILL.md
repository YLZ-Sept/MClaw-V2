...
- 通过 CDP (Chrome DevTools Protocol) 复用 Mclaw 已启动的浏览器
- 自动继承 Mclaw 系统配置的 LLM（来自 llm_endpoints.json）
- 基于 [browser-use](https://github.com/browser-use/browser-use) 开源项目

## 高级：操作用户已打开的 Chrome

如果想让 Mclaw 操作你已打开的 Chrome 页面，需要以调试模式启动 Chrome：
...
启动后，Mclaw 会自动检测并连接，可以操作你已打开的标签页。
