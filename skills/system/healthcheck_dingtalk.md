...
## 背景
- 报错：`mclaw.channels.adapters.dingtalk` 发送图片失败，服务端返回“robot 不存在”。
...
⚠️ 说明：Mclaw 当前钉钉适配器将 `robotCode` 直接使用 `app_key`（即 DINGTALK_CLIENT_ID）。若你的机器人 `robotCode` 并非该值，则会导致 “robot 不存在”。
