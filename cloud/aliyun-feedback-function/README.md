...
2. 创建 Bucket（例如 `mclaw-feedback`），地域选 **华东1（杭州）**
...
2. 创建新 Token：
   - 名称：`mclaw-feedback-bot`
   - 仓库范围：**Only select repositories** → `mclaw/mclaw`
...
5. **配置环境变量**：
   ...
   | `OSS_BUCKET` | `mclaw-feedback` | 第 1 步 |
   ...
   | `PUBLIC_URL` | `https://feedback-mclaw.fzstack.com` | 可选，FC 公网地址（用于生成退订链接）。注意：FC 保留 `FC_` 前缀，不能用 `FC_PUBLIC_URL` |
...
7. 记下 **公网访问地址**，格式为：
   ...
   ```
   也可以在 FC 控制台或 DNS 中绑定自定义域名（例如 `feedback-mclaw.fzstack.com`）。
...
```json
{
  ...
  "resolved_at": null,
  "github_issue_url": "https://github.com/mclaw/mclaw/issues/42",
  "size_bytes": 123456
}
```
...
```json
{
  ...
  "type": "bug",
  "summary": "点击设置后白屏...",
  "system_info": "OS: Windows 10 | Python: 3.11 | Mclaw: 1.25.9",
  "contact_email": "user@example.com",
...
```json
{
  "upload_url": "https://mclaw-feedback.oss-cn-hangzhou.aliyuncs.com/feedback/2026-03-09/abc123/report.zip?Expires=...&Signature=...",
  "report_id": "abc123def456",
...
```json
{
  ...
  "report_id": "abc123def456",
  "feedback_token": "aBcDeFgHiJkL_1234567890ab",
  "issue_url": "https://github.com/mclaw/mclaw/issues/42"
}
```
...
{
  "status": "ok",
  "comment_url": "https://github.com/mclaw/mclaw/issues/42#issuecomment-xxx"
}
```
...
```bash
# 设置凭证（或创建 ~/.mclaw/feedback.env）
export OSS_ENDPOINT=https://oss-cn-hangzhou.aliyuncs.com
export OSS_BUCKET=mclaw-feedback
