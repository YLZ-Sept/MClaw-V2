...
<p align="center">
  <a href="https://mclaw.ai"><img src="https://img.shields.io/badge/🌐_官网-mclaw.ai-orange?style=for-the-badge" alt="Official Website" height="28" /></a>
  &nbsp;
  <a href="https://mclaw.ai/download"><img src="https://img.shields.io/badge/📥_下载-Desktop_App-blue?style=for-the-badge" alt="Download" height="28" /></a>
  &nbsp;
...
<p align="center">
  ...
  <img src="https://img.shields.io/github/v/release/mclaw/mclaw?style=flat-square" alt="Release" height="20" />
...
**别人的 AI 只能聊天，Mclaw 帮你做事。**

Mclaw 是一款开源全能 AI 助手——多个 AI Agent 协作分工、组建 AI 公司自主运营、自动上网搜索、操作你的电脑、管理文件、定时执行任务，还能接入飞书/钉钉/企微/Telegram/QQ，**扫码即绑定，随时响应**。它会记住你的偏好，会自己学新技能，遇到问题永不放弃。通过**插件系统**可以无限扩展能力，**六层沙箱安全**保护你的系统安全。

**全图形化配置，5 分钟上手，全程不用敲一行命令。**

<p align="center">
  🌐 <a href="https://mclaw.ai"><b>官网 mclaw.ai</b></a> &nbsp;|&nbsp;
  📥 <a href="https://mclaw.ai/download"><b>下载桌面端</b></a> &nbsp;|&nbsp;
  📖 <a href="https://mclaw.ai/docs"><b>在线文档</b></a> &nbsp;|&nbsp;
...
**全图形化，不用敲命令行**——这是 Mclaw 和同类开源产品最大的区别：

<p align="center">
  <img src="docs/assets/desktop_quick_config.gif" alt="Mclaw 快速配置" width="800" />
</p>
...
> **下载**：[GitHub Releases](https://mclaw.ai/download) — Windows (.exe) / macOS (.dmg) / Linux (.deb)
>
> 更多信息请访问官网 **[mclaw.ai](https://mclaw.ai)**

### 方式二：pip 安装

```bash
pip install mclaw[all]    # 安装全部可选功能（IM 通道 + 桌面自动化）
mclaw init                # 运行配置向导
mclaw                     # 启动交互式 CLI
```
...
```bash
mclaw                          # 交互式聊天
mclaw run "帮我写个计算器"       # 执行单个任务
mclaw serve                    # 服务模式（接入 IM）
mclaw serve --dev              # 开发模式，热加载
mclaw daemon start             # 后台守护进程
mclaw status                   # 查看运行状态
```
...
Mclaw 支持**桌面端、Web 端、移动端**——随时随地，多端协同：
...
<p align="center">
  <img src="docs/assets/desktop_terminal.png" alt="Mclaw 桌面应用" width="800" />
</p>
...
<p align="center">
  ...
  <br/>
  <sub>▶ 点击观看：在 Mclaw 上建立了一个公司，它还自主运营了（B站）</sub>
</p>

不只是多个 Agent 协作，而是搭建一家 **AI 公司**。Mclaw 内置完整的组织编排引擎（AgentOrg），支持在图形界面中可视化搭建公司架构，让 AI 像真实公司一样分工协作、自主运营：
...
Mclaw 内置完整的多 Agent 协作系统——不是只有一个 AI，而是一个 **AI 团队**：
...
<p align="center">
  ...
  <br/>
  <sub>▶ 点击观看：Mclaw 扫码绑定聊天软件，微信、飞书、企业微信（B站）</sub>
</p>
...
Mclaw 提供完整的插件架构，通过 `plugin.json` 清单声明能力，三级权限模型保障安全，10 个生命周期钩子深度集成：
...
Mclaw 实现了**六层纵深防御**安全模型，从路径管理到 OS 级隔离，层层把关：
...
Mclaw 内置完整的 [MCP（Model Context Protocol）](https://modelcontextprotocol.io/) 客户端，让 AI 可以连接任意外部服务：
...
> 了解更多请访问 **[mclaw.ai](https://mclaw.ai)**
...
Mclaw 源代码采用 GNU Affero General Public License v3.0 only（AGPL-3.0-only）授权，详见 [LICENSE](LICENSE)。

`Mclaw` 名称、Logo、图标、截图和其他品牌资产不在 AGPL-3.0-only 授权范围内。品牌使用规则详见 [TRADEMARK.md](TRADEMARK.md)。
...
<p align="center">
  <strong>Mclaw — 开源多 Agent AI 助手，不只是聊天，是帮你做事的 AI 团队</strong><br/>
