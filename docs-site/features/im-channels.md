...
消息通道让你通过**日常使用的聊天软件**与 Mclaw 对话——一个 Mclaw 实例可以同时连接多个平台。
...
3. 在 Mclaw 中填入 Token：[配置 Telegram](/web/#/config/im)
4. （可选）如在中国大陆使用，配置代理地址

**配对码安全机制**：首次连接时，Mclaw 会生成一个一次性配对码。在 Telegram 对话中发送该配对码，完成身份绑定后才能正常使用，防止未授权访问。
...
4. 在 Mclaw 中填入凭证：[配置钉钉](/web/#/config/im)
...
3. 根据网络环境选择通信模式
4. 在 Mclaw 中完成配置：[配置企业微信](/web/#/config/im)
...
3. 在 Mclaw 中填入 WebSocket URL：`ws://127.0.0.1:端口`
