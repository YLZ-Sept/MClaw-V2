...
Mclaw 提供 `mclaw` 命令行工具，是与系统交互的另一种方式。适合无桌面环境的服务器部署、脚本自动化，或者你就是喜欢终端。

::: tip 如何获得 `mclaw` / `oa` 命令
这些命令由 Python 包提供，需先通过 `pip install mclaw` 安装（详见[安装指南](/guide/installation)）。安装后 `mclaw` 可执行文件会随 pip 自动加入 PATH。

桌面版安装包**不会**向系统 PATH 注册 `mclaw` / `oa` 命令——桌面用户请直接使用图形界面；如需终端命令，按上面的方式用 pip 安装即可。
:::
...
| 命令 | 说明 | 示例 |
|------|------|------|
| `mclaw` | 启动交互式 CLI + IM 通道 | `mclaw` |
| `mclaw init [dir]` | 初始化配置向导 | `mclaw init ~/my-workspace` |
| `mclaw run "task"` | 执行单个任务后退出 | `mclaw run "总结今天的邮件"` |
| `mclaw selfcheck` | 运行系统自检 | `mclaw selfcheck` |
| `mclaw status` | 查看 Agent 运行状态 | `mclaw status` |
| `mclaw compile` | 编译 Identity 文件 | `mclaw compile` |
| `mclaw prompt-debug` | 输出当前组装的完整 Prompt | `mclaw prompt-debug` |
| `mclaw serve` | 仅启动 API 服务（IM + HTTP） | `mclaw serve` |
...
```bash
mclaw run "帮我把 report.md 翻译成英文"
mclaw run "分析 data.csv 中的销售趋势"
```
