...
本页帮你在 3 分钟内跑起 Mclaw，完成第一次对话并接入 IM 通道。
...
```
你 > 你好，介绍一下你自己
Mclaw > 你好！我是 Mclaw，一个多 Agent AI 助手。我可以帮你处理各种任务……
```
...
1. 在 Mclaw 中打开 [消息通道配置](/web/#/im)
2. 选择"飞书"，按提示扫码授权
3. 在飞书中找到 Mclaw 机器人，发送消息测试
...
脚本将自动检查 Python 环境、安装 Mclaw 并运行 `mclaw init`。
...
| 方式 | 命令 | 说明 |
|------|------|------|
| 交互式 CLI | `mclaw` | 默认模式，在终端对话 |
| 单次任务 | `mclaw run "任务描述"` | 执行一个任务后退出 |
| API 服务 | `mclaw serve` | 启动 Web 服务 + API |
