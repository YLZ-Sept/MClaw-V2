...
本页列出 Mclaw 的所有安装方式、可选依赖和首次运行流程。选择最适合你的方式开始。
...
```bash
curl -fsSL https://get.mclaw.com | bash
```

**Windows（PowerShell，以管理员身份运行）：**

```powershell
irm https://get.mclaw.com/install.ps1 | iex
```

脚本自动完成：检查 Python 版本 → 创建虚拟环境 → 安装 Mclaw → 运行初始化向导。

### 方式四：桌面应用（Tauri）

前往 [GitHub Releases](https://github.com/mclaw/mclaw/releases) 下载对应平台的安装包：
...
| 安装标记 | 说明 |
|----------|------|
| `mclaw[feishu]` | 飞书 / Lark 通道 |
| `mclaw[dingtalk]` | 钉钉通道 |
| `mclaw[wework]` | 企业微信通道 |
| `mclaw[onebot]` | OneBot 协议（可接各类 QQ 框架） |
| `mclaw[qqbot]` | QQ 官方机器人 |
| `mclaw[windows]` | Windows 桌面自动化 |
| `mclaw[all]` | 以上全部 |

可组合使用：

```bash
pip install "mclaw[feishu,dingtalk]"
```
...
| 模式 | 命令 | 说明 |
|------|------|------|
| 交互式 CLI | `mclaw` | 在终端中持续对话，适合日常使用 |
| 单次任务 | `mclaw run "任务描述"` | 执行一个任务后自动退出 |
| API 服务 | `mclaw serve` | 启动 FastAPI 服务，提供 Web 界面和 REST API |
...
**Q：需要 GPU 吗？**
不需要。Mclaw 调用云端 LLM API，本地只需普通 CPU 即可运行。
