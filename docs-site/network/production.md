...
本页面介绍如何在服务器上正式部署 Mclaw。适用于需要长期运行、多人访问、高可用的场景。

::: tip 快速选择
- **个人本地使用** → 无需看本页，直接桌面应用或 `mclaw` 启动即可
...
services:
  mclaw:
    image: mclaw/mclaw:latest
    container_name: mclaw
...
Mclaw 暴露 `/metrics` 端点，兼容 Prometheus 格式。
...
| 指标 | 说明 |
|------|------|
| `mclaw_requests_total` | API 请求总数 |
| `mclaw_llm_tokens_total` | LLM Token 消耗总量 |
| `mclaw_llm_latency_seconds` | LLM 请求延迟 |
| `mclaw_active_sessions` | 当前活跃会话数 |
| `mclaw_tool_calls_total` | 工具调用总数 |
...
```bash
#!/bin/bash
BACKUP_DIR="/backups/mclaw"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)

mkdir -p "$BACKUP_DIR"
tar czf "$BACKUP_DIR/mclaw_$TIMESTAMP.tar.gz" \
...
```bash
# 每天凌晨 3 点自动备份
0 3 * * * /opt/mclaw/backup.sh
```

### 恢复

```bash
tar xzf mclaw_20260318_030000.tar.gz -C /opt/mclaw/
...
- [CLI 命令参考](/advanced/cli) — `mclaw serve` 等服务端命令
