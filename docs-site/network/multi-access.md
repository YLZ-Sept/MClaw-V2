...
Mclaw 支持多种方式访问。根据你的网络环境选择合适的方案：
...
::: tip 关于本机豁免
你在运行 Mclaw 的同一台电脑上打开浏览器，仍然走 `127.0.0.1` 直连，不会触发 Setup 流程；Setup 流程只对**其他设备**生效。
:::
...
从任意网络位置访问你的 Mclaw 实例。

### 方案 A：云服务器部署

1. 在云服务器（阿里云 / 腾讯云 / AWS 等）上部署 Mclaw
...
工作原理：IM 平台（Telegram 服务器、飞书服务器等）负责消息的收发，你的 Mclaw 实例通过 **长连接或轮询** 与平台通信，无需暴露端口。
