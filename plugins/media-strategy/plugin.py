...
def _get_active_scheduler() -> Any:
    try:
        from mclaw.scheduler import get_active_scheduler  # type: ignore
    except ImportError:
        return None
    try:
...
class Plugin(PluginBase):
    ...
    def _build_router(self) -> APIRouter:
        ...
        async def _parse_radar_url(body: RadarUrlBody) -> tuple[str, Any]:
            ...
            user_agent = str(settings.get("user_agent") or "Mclaw-MediaStrategy/0.1")
            try:
        ...
        async def save_report(report_id: str, fmt: str) -> dict[str, Any]:
            ...
            safe_report_id = "".join(ch for ch in report_id if ch.isalnum() or ch in "-_")[:12]
            target_dir = _downloads_dir() / "Mclaw" / "media-strategy"
            target_dir.mkdir(parents=True, exist_ok=True)
        ...
        async def create_schedule(payload: dict[str, Any] = Body(...)) -> dict[str, Any]:
            ...
            try:
                from mclaw.scheduler.task import ScheduledTask  # type: ignore
            except ImportError as exc:
                raise HTTPException(
        ...
        async def scheduler_channels() -> dict[str, Any]:
            ...
            try:
                from mclaw.api.routes.scheduler import (
