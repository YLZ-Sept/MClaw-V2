...
bootstrap: probe plugin-local ``deps/`` first, then a private Mclaw
...
def _mclaw_root() -> Path:
    raw = os.environ.get("MCLAW_ROOT", "").strip()
    return Path(raw) if raw else Path.home() / ".mclaw"
...
def _candidate_dirs(plugin_dir: Path | None) -> list[Path]:
    ...
    dirs.append(_mclaw_root() / "modules" / PLUGIN_ID / "site-packages")
