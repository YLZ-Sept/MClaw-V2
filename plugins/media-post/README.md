...
Mclaw first-class plugin that turns an edited video into platform-ready
...
for vision and the selected Mclaw endpoint for SEO text, all driven from a single React UI that
...
| `seo_pack` | 5-platform SEO bundle (TikTok / Bilibili / WeChat / Xiaohongshu / YouTube) | Mclaw text model | — |
...
- **Mclaw** ≥ 1.27.0 with SDK `>=0.7.0,<0.8.0`
...
# 3. Open Mclaw → sidebar → Media Post → Settings → paste API Key → Save
