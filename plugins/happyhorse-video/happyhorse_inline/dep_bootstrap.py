...
1. **Probe** the three places host-managed deps could already live and
   ...
   - ``~/.mclaw/modules/happyhorse-video/site-packages/`` — host's
   ...
   - ``<sys.executable>/_internal/Lib/site-packages/`` — bundled
     ...
     occasionally seeded by older Mclaw versions).
...
2. **Import-or-install**: if the import still fails, run ``pip install``
   ...
   ``~/.mclaw/modules/happyhorse-video/site-packages/`` so the install
...
def _patch_simplejson_jsondecodeerror() -> bool:
    ...
    Some PyInstaller Mclaw builds expose a ``simplejson`` module that
...
def _mclaw_root() -> Path:
    """Mirror ``runtime_env._get_mclaw_root`` without importing it.

    The plugin must NOT depend on ``mclaw.*`` internals (those move
    between releases) so we re-derive the same value: ``$MCLAW_ROOT``
    if set, else ``~/.mclaw``.
    """
...
def _candidate_site_dirs(plugin_dir: Path | None) -> list[Path]:
    ...
    # while the Mclaw process still had a .pyd file locked (seen with
    ...
    cands.append(_mclaw_root() / "modules" / "happyhorse-video" / "site-packages")
...
def _stale_module_prefixes(import_name: str) -> tuple[str, ...]:
    ...
    if import_name == "oss2":
        return (
            ...
            # Mclaw Desktop builds already import a bundled Crypto package
...
def _resolve_python_executable() -> str:
    ...
    ``mclaw-server.exe`` which intercepts CLI args and refuses
...
def ensure_importable(
    ...
    *,
    ...
    # environments and fixes Mclaw Desktop builds where ``requests``
