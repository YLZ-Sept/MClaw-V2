...
Plugins can point at a relay registered in Mclaw's shared relay
...
4. When ``mclaw.relay`` cannot be imported (degraded environment)
...
# ─── Fake mclaw.relay module so tests don't depend on PYTHONPATH ──
...
def _install_stub_relay_module(monkeypatch, *, refs_by_name=None, raises=None):
    """Inject a fake ``mclaw.relay`` module into sys.modules.
    ...
    monkeypatch.setitem(sys.modules, "mclaw", SimpleNamespace(relay=fake_pkg))
    monkeypatch.setitem(sys.modules, "mclaw.relay", fake_pkg)
...
def test_plugin_local_relay_url_overrides_without_host_registry(monkeypatch):
    ...
    must not require mclaw.relay or a global LLM relay entry."""
    monkeypatch.setitem(sys.modules, "mclaw.relay", None)
    c = HappyhorseDashScopeClient(
        _read_settings_factory(
            api_key="sk-direct",
...
def test_mclaw_relay_import_failure_falls_back_gracefully(monkeypatch):
    """When the bundled / installed plugin runs in an environment
    that does not expose mclaw.relay, the client must NOT crash —
