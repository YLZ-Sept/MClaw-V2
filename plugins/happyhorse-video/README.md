...
happyhorse-video 跟随 Mclaw 主仓发布，不需要额外 `pip install`。开发态：
...
打开 Mclaw → 插件 → HappyHorse Studio → 进入插件 UI → **Settings** Tab：
...
主仓的默认组织模板 [`aigc-video-studio`](../../src/mclaw/orgs/templates.py)（**AIGC 视频创作工作室**）已重做为基于本插件的 7 节点两级工作室：
...
Mclaw source code is licensed under AGPL-3.0-only. The plugin includes `Mclaw` as `author` in `plugin.json` and `LICENSE` notices follow the upstream project. See top-level [`LICENSE`](../../LICENSE) and [`TRADEMARK.md`](../../TRADEMARK.md).
