...
def make_default_settings() -> dict[str, Any]:
    return {
        ...
        # mclaw.relay registry. New UI no longer exposes it, but keeping it
    ...
    def _settings(self) -> dict[str, Any]:
        ...
        if relay_name and use_relay and not relay_base_url:
            try:
                ...
                # where the mclaw package is not on sys.path (e.g.
                # bundled exe variants that ship plugins separately).
                from mclaw.relay import RelayNotFound, resolve_relay_endpoint

                ref = resolve_relay_endpoint(relay_name, required_capability="video")
            except (ImportError, ModuleNotFoundError) as exc:
                logger.info(
                    "happyhorse-video: mclaw.relay not importable (%s); "
