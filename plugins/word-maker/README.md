...
`brain.access`, while `host_brain_unavailable` means Mclaw has no usable
text endpoint. Text calls use `api.get_llm()` only; the settings selector can
follow the current Mclaw model or strictly require one configured endpoint.
...
- If Brain is unavailable, check plugin `brain.access` permission first, then
  the Mclaw LLM endpoint configuration. Manual project creation and template
