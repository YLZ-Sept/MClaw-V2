...
_BRAIN_SYSTEM_PROMPT = "You are assisting word-maker, an Mclaw plugin for guided Word document generation."
...
class WordBrainHelper:
    ...
    def _brain_state(self) -> tuple[dict[str, Any], Any | None]:
        status: dict[str, Any] = {
            ...
            "message": "Mclaw Brain is not available",
        }
        ...
        if not callable(get_llm):
            ...
            status["message"] = "Mclaw host does not expose get_llm"
            return status, None
        try:
            brain = get_llm()
        except Exception as exc:  # noqa: BLE001 - status should explain host failures.
            status["reason"] = "host_brain_error"
            status["message"] = f"Mclaw Brain lookup failed: {exc}"
            return status, None
        if brain is None:
            status["reason"] = "host_brain_unavailable"
            status["message"] = "Mclaw Brain is not available"
            return status, None
        ...
        status.update(
            {
                ...
                "brain_injected": True,
                "reason": "available",
                "message": "Mclaw Brain is available",
            }
        )
        return status, brain
