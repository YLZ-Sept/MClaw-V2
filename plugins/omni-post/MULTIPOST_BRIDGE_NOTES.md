...
- 最稳定的调用入口是顶层网页，不是 Mclaw 插件 iframe。
- Mclaw 桌面端 Tauri/WebView 不能直接运行 Chrome/Edge 扩展。
...
- 当前页面在 Mclaw 插件 iframe 内，content script 没有稳定注入该 frame。
- 当前运行环境是 Mclaw 桌面端/Tauri WebView，不是 Chrome/Edge。
