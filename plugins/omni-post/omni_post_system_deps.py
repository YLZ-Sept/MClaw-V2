...
def _mclaw_root() -> Path:
    ...
    return Path(env_root) if env_root else Path.home() / ".mclaw"
...
class OmniPostSystemDeps:
    ...
    async def _run_install(self, dep_id: str, command: tuple[str, ...]) -> None:
        ...
        if dep_id == "playwright-chromium":
            env.setdefault("PLAYWRIGHT_BROWSERS_PATH", str(_mclaw_root() / "ms-playwright"))
        install_ok = False
        try:
            ...
            if dep_id == "playwright-chromium" and install_ok:
                os.environ.setdefault(
                    "PLAYWRIGHT_BROWSERS_PATH", str(_mclaw_root() / "ms-playwright")
                )
    ...
    def _detect_playwright(self, spec: DepSpec) -> dict[str, Any]:
        ...
        plugin_root = _mclaw_root() / "ms-playwright"
        roots.extend(
            [
                plugin_root,
