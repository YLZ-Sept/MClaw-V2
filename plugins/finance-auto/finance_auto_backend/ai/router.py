...
Mclaw's already-instantiated ``LLMClient`` (whichever the host has
...
2. classify each one as local (Ollama / LM Studio / vLLM / LocalAI /
   ...
   ``mclaw.llm.types.is_local_endpoint_config`` (battle-tested);
...
The plugin does not currently re-implement Mclaw's failover /
...
class LLMResponder(Protocol):
    ...
    Mclaw's ``LLMClient`` already exposes ``async def chat(...)``;
...
# ---------------------------------------------------------------------------
# Host-brain responder — routes completions through Mclaw's own Brain
# ---------------------------------------------------------------------------
...
class HostBrainResponder:
    ...
    This is how the plugin reuses Mclaw's already-configured LLM
    ...
    async def complete(
        ...
        *,
        ...
        system = (
            ...
            "Mclaw finance-auto plugin. Follow the task instructions "
            "in the user message exactly. When asked for JSON, return "
            "strict JSON with no commentary."
        )
        from mclaw.plugins.llm_support import complete_text
...
def _is_local_endpoint(provider: str, base_url: str) -> bool:
    """Best-effort: prefer Mclaw's helper if importable; otherwise
    ...
    try:
        from mclaw.llm.types import is_local_endpoint_config
...
def collect_endpoints_from_host_client(host_client: Any) -> list[EndpointDescriptor]:
    """Inspect an Mclaw ``LLMClient`` and return every endpoint it
