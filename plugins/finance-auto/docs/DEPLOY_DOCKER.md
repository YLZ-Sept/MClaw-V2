...
> **Audience**: operators running Mclaw + finance-auto in a Linux
...
echo "$PASSPHRASE" > /etc/mclaw/finance-auto.passphrase
chmod 600 /etc/mclaw/finance-auto.passphrase

# 2. Build the image (from the Mclaw repo root).
#    The default Dockerfile installs Mclaw core; finance-auto
#    plugin code ships under plugins/finance-auto/ but its runtime
#    deps require an extra install step (see §3).
docker build -t mclaw:1.0.0-rc1 .
...
| `-p 18900:18900` | Mclaw HTTP API (FastAPI) | **YES** — the Setup Center desktop client connects here |
...
services:
  mclaw:
    build: .
    container_name: mclaw-finance-auto
    restart: unless-stopped
    ports:
      - "18900:18900"
    volumes:
...
The repo-root `Dockerfile` installs Mclaw core via
...
RUN pip install --no-cache-dir ".[finance-auto]" \
    && pip install --no-cache-dir ./mclaw-plugin-sdk
```
...
```bash
docker exec mclaw env | grep OPENAKITA_FINANCE_AUTO_PASSPHRASE
```
...
```bash
docker build --no-cache -t mclaw:1.0.0-rc1 .
```
...
- [ ] Encrypted backups (`POST /api/plugins/finance-auto/backups`)
      scheduled via the Mclaw scheduler or external cron.
