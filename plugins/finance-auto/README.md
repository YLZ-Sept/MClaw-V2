# finance-auto · Mclaw 财务自动化插件

> **版本**：v1.0.0-rc1 (Release Candidate 1)
> **协议**：AGPL-3.0-only（随 Mclaw 主体）
...
**finance-auto** 是 Mclaw 平台的"小企业 + 一般纳税人 + 多审计师协作"
...
| 桌面端 GUI | Mclaw Setup Center (Tauri 2.x) | 同左 |

**插件级 Python 依赖**（不随 Mclaw 主体安装，需单独 `pip install`）：
...
（见 `src/mclaw/plugins/manifest.py` 的 `ALL_PERMISSIONS`）。本插件实际
...
桌面端打开 Mclaw Setup Center → 侧边栏 **应用**（apps）分组下点
**财务自动化**。
...
- **算法**：AES-256-GCM（cipher）+ PBKDF2-HMAC-SHA256（KDF，
  ...
  `8a628d1f`）。
- **AAD**：固定 `mclaw-finance-v1`，防止跨场景密文复用。
...
- 未注册用户 → 整路由 `403 rbac_denied`（`X-Mclaw-User-Id` header 或 `?user_id=` query 均可标识；缺省 `local` 走单机管理员旁路）。
...
Mclaw Setup Center 启动后，finance-auto 自动加载。密钥种子走系统
...
- **代码 License**：AGPL-3.0-only（随 Mclaw 主体；见仓库根
  [`LICENSE`](../../LICENSE)）。
- **商标**：`Mclaw` 名称与 logo 受
...
**最后更新**：2026-05-24 · 维护：Mclaw team · 反馈：
GitHub Issues / Mclaw 仓库根 `_finance_plugin_*` 审查报告。
