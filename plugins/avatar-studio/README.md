...
avatar-studio 跟随 Mclaw 主仓发布，不需要额外 `pip install`。开发态：
...
目标 Mclaw 实例的 `plugins/` 目录即可。所有依赖（vendored helpers、UI
Kit）都在插件内部，不依赖 `mclaw_plugin_sdk.contrib`、不依赖
`/api/plugins/_sdk/*`。
...
打开 Mclaw → 插件 → Avatar Studio → 进入插件 UI → **Settings** Tab：
...
Pixelle A10）。这一点也是 avatar-studio 区别于早期 Mclaw 插件最重要的
工程改进 —— 用户不必为了改一个字符配置而重启整个 Mclaw 主进程。
...
```bash
# 0. 确保 Mclaw 已启动并加载了 avatar-studio
