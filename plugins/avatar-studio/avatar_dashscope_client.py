...
def make_default_settings() -> dict[str, Any]:
    return {
        ...
        "timeout": 60.0,
        # Shared relay registry (see mclaw.relay). Empty by default so
    ...
    def _settings(self) -> dict[str, Any]:
        ...
        if str(merged.get("relay_endpoint") or "").strip():
            try:
                ...
                logger.info(
                    "avatar-studio: mclaw.relay not importable (%s); "
        ...
        *,
        ...
        try:
            ...
            raise VendorError(
                ...
                "请在 Mclaw 设置中心的插件/运行时修复入口重新安装或修复该插件依赖，"
                "不要直接污染宿主 Python。\n"
