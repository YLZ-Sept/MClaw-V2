...
"""
        self.get_llm()
        system = (
            "You are the Mclaw ppt-maker per-slide content engine. "
    ...
    async def compose_additional_context(
        ...
        *,
        ...
        # Optional web search using Mclaw's built-in handler
...
"""
    ...
    async def _call_json(
        ...
        *,
        ...
        self.get_llm()
        system = (
            "You are the Mclaw ppt-maker planning engine. "
    ...
    async def _web_search_context(*, title: str, prompt: str, max_results: int) -> str:
        ...
        if not query:
            return ""
        try:
            from mclaw.tools.handlers.web_search import WebSearchHandler  # type: ignore
        except Exception:
