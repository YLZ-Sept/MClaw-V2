...
from mclaw.plugins.llm_support import llm_catalog_payload, validate_llm_endpoint
...
class Plugin(PluginBase):
    """Mclaw plugin entry for guided PPT generation."""
...
def _default_settings() -> dict[str, str]:
    return {
        ...
        # official DashScope endpoint. See src/mclaw/relay/.
...
def _apply_relay_overrides(settings: dict[str, str]) -> dict[str, str]:
    ...
    Failure modes:
      ...
      * mclaw.relay missing       -> noop, settings keep per-plugin
    ...
    if not relay_name:
        return settings
    try:
        from mclaw.relay import apply_relay_override
