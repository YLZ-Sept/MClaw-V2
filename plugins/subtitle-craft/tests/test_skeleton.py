...
- Red-line grep guards (matches Gate 0 in ``docs/subtitle-craft-plan.md``):
  ...
  ``plugins-archive`` / ``_shared`` / ``sdk.contrib`` / ``mclaw-plugin-sdk.contrib``,
...
def test_plugin_skeleton_import():
    try:
        ...
        pytest.skip("mclaw SDK not available in test env")
...
_FORBIDDEN_IMPORT_PATTERNS = [
    ...
    re.compile(r"^\s*from\s+mclaw-plugin-sdk\.contrib\b", re.MULTILINE),
