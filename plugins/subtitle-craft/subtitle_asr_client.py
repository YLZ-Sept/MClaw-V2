"""Subtitle Craft ASR plus Mclaw-hosted text processing.
...
class SubtitleAsrClient(BaseVendorClient):
    ...
    async def _translate_one(self, text: str, *, src: str, tgt: str, model: str) -> str:
        try:
            ...
            raise AsrError(f"Mclaw translation failed: {exc}", kind="unknown") from exc
    ...
    async def call_qwen_plus(
        ...
        *,
        ...
        Routes the prompt through Mclaw's Plugin LLM facade. Returns the
        ...
        try:
            ...
            raise AsrError(f"Mclaw text generation failed: {exc}", kind="unknown") from exc
