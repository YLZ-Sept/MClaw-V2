...
can be ``mclaw-server.exe`` instead of a real Python interpreter. This
...
def module_site_packages() -> Path:
    ...
    return _mclaw_root() / "modules" / PLUGIN_ID / "site-packages"
...
def _iter_app_venv_dirs() -> list[Path]:
    """Return candidate Mclaw app-venv directories, nearest usable first."""
    ...
    add(Path.home() / ".mclaw" / "runtime" / "app-venv")
    return dirs
...
def run_ytdlp_subprocess(
    ...
    *,
    ...
    """Run yt-dlp with Mclaw-friendly env; use shell wrapper in frozen Windows."""
