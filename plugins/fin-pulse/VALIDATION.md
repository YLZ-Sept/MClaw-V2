...
| 8 | Ollama `127.0.0.1` 在 Docker 里失效 | 自托管 LLM 常见陷阱 | fin-pulse 不直接管理 LLM endpoint —— 调用透传到宿主 `LLMClient`；Settings 的「AI Brain」卡片明示「在 Mclaw 主 Settings → Models 修改 provider / base_url」 | `finpulse_ai/filter.py` (全部 `await api.get_brain().chat(...)`) + README §4.4 |
