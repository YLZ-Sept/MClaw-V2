...
> Finance news radar for Mclaw. Three canonical modes —
...
```bash
cd D:/Mclaw/plugins/fin-pulse
# the 5-asset UI bundle ships vendored; no build step required
```

Restart the Mclaw host and confirm in Plugin Manager that
...
4. **Mclaw text model** — choose “follow current” or strictly lock one
...
- **Ask tab** — 7 agent tool cards with JSON samples and
  "copy natural-language prompt" buttons. Paste into the Mclaw
...
- Rows also show up in the main Mclaw **Scheduler** panel, but you
...
8. In Mclaw main chat, ask *"今天美股有什么大事"* → Brain
...
Same as Mclaw — see `D:/Mclaw/LICENSE`.
