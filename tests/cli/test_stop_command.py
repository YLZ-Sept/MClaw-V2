...
``mclaw stop`` CLI subcommand must POST ``/api/shutdown`` reliably
...
* v29 graceful-restart audit attempted ``python -m mclaw.api.cli stop``
  and got ``No module named mclaw.api.cli`` (Phase B = fail). The CLI
