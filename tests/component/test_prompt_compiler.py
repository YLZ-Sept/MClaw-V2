...
from mclaw.prompt.budget import BudgetConfig
...
class TestPromptCompileFunctions:
    ...
    def test_identity_core_owns_identity_not_platform_safety(self):
        from mclaw.prompt.budget import estimate_tokens
        from mclaw.prompt.compiler import _COMPILE_PROMPTS, _compile_with_rules
...
"""
    ...
    def test_agent_behavior_owns_only_mclaw_specific_deltas(self):
        from mclaw.prompt.budget import estimate_tokens
        from mclaw.prompt.compiler import _COMPILE_PROMPTS, _compile_with_rules
...
class TestCompileAll:
    def test_compile_all_with_identity_dir(self, tmp_path):
        from mclaw.prompt.compiler import compile_all
...
class TestBuildSystemPrompt:
    def test_build_returns_string(self, tmp_path):
        from mclaw.prompt.builder import build_system_prompt

        identity_dir = tmp_path / "identity"
        identity_dir.mkdir()
        (identity_dir / "SOUL.md").write_text("# Soul\nI am Mclaw.", encoding="utf-8")
    ...
    def test_build_includes_identity(self, tmp_path):
        ...
        (identity_dir / "SOUL.md").write_text(
            "# Soul\nI am Mclaw, the loyal dog.", encoding="utf-8"
        )

        prompt = build_system_prompt(identity_dir=identity_dir, tools_enabled=False)
        assert "Mclaw" in prompt or "loyal" in prompt or len(prompt) > 50
    ...
    def test_agent_voice_empty_falls_back_to_mclaw(self, tmp_path):
    ...
    def test_agent_voice_replaces_in_none_mode(self, tmp_path):
        ...
        from mclaw.prompt.builder import PromptMode, build_system_prompt
        ...
        # "你是 Mclaw" wording (other unrelated rules sections may still mention
        # the Mclaw project by name; we only guard the identity self-reference).
        assert "你是 码哥，一个 AI 助手。" in prompt
        assert "你是 Mclaw，一个 AI 助手。" not in prompt

    def test_agent_voice_replaces_per_model_base_prompt(self, tmp_path):
        """Per-model 基础提示词不能把自定义 Agent 重新钉死为 Mclaw。"""
        from mclaw.prompt.builder import PromptMode, PromptProfile, build_system_prompt
        ...
        assert "你是 Mclaw，一个帮助用户完成各类任务的 AI 助手。" not in prompt
        assert "# Agent Identity" in prompt
        assert "# Mclaw System" not in prompt
        assert "Mclaw 仅指运行平台或上游开源项目，不是当前 Agent 的自称" in prompt
...
class TestAgentResolveVoice:
    ...
    def test_resolve_voice_falls_back_to_settings_when_all_empty(self):
        ...
        agent._agent_profile = None
        agent.name = ""
        # settings.agent_name defaults to "Mclaw"
