...
from mclaw.evolution.log_analyzer import ErrorPattern, LogEntry
from mclaw.evolution.self_check import SelfChecker, SelfCheckPromptUnavailableError
...
class TestSelfCheckResilience:
    def test_workspace_prompt_takes_precedence(self, mock_brain, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("mclaw.config.settings.project_root", tmp_path)
    ...
    def test_bundled_prompt_is_used_without_workspace_override(
        ...
        package_root = tmp_path / "package" / "mclaw"
        ...
        monkeypatch.setattr("mclaw.config.settings.project_root", workspace_root)
        monkeypatch.setattr(
            "mclaw.evolution.self_check.resources.files",
            ...
            "selfcheck_timeout": ErrorPattern(
                ...
                samples=[
                    LogEntry(
                        timestamp=now,
                        level="ERROR",
                        logger_name="mclaw.scheduler.executor",
            ...
            "real_error": ErrorPattern(
                ...
                samples=[
                    LogEntry(
                        timestamp=now,
                        level="ERROR",
                        logger_name="mclaw.tools.browser",
    ...
    async def test_run_daily_check_saves_partial_report_on_time_budget(
        ...
        (log_dir / "error.log").write_text(
            "2026-03-31 00:00:00,000 - mclaw.tools.browser - ERROR - Browser failed\n",
            encoding="utf-8",
        )
        ...
        ticks = iter([0.0, 2.0])
        monkeypatch.setattr(
            "mclaw.evolution.self_check.time.monotonic",
    ...
    async def test_run_daily_check_skips_llm_and_fix_when_prompt_is_unavailable(
        ...
        package_root = tmp_path / "empty-package" / "mclaw"
        package_root.mkdir(parents=True)
        monkeypatch.setattr("mclaw.config.settings.project_root", tmp_path)
        monkeypatch.setattr(
            "mclaw.evolution.self_check.resources.files",
