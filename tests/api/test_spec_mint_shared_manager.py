...
:class:`mclaw.orgs.store.JsonOrgStore` a manager-backed read
shim. The shim docstring + ``orgs_v2.py::_resolve_manager_for_writes``
both assume that ``api/server.py`` calls
:func:`mclaw.orgs.store.set_default_org_manager` right after
...
def wired_app(
    ...
    Using ``create_app()`` (mirroring ``mclaw serve`` minus the IM
    ...
    under ``tmp_path/data/orgs/`` instead of the real ``D:/Mclaw/data``
    tree (303 live orgs there at the time of writing). The TestClient
    presents ``client.host == "testclient"`` and so fails the
    ``_is_local_request`` bypass in :mod:`mclaw.api.auth`; we mint
