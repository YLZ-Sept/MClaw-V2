...
async def test_stop_im_channels_bounded_under_3s(_restore_main_globals, monkeypatch):
    ...
    monkeypatch.setattr(
        "mclaw.config.settings.lifespan_stage_timeout_s", 1, raising=False
    )
...
async def test_stop_im_channels_gateway_and_desktop_pool_run_in_parallel(
    ...
    """
    monkeypatch.setattr(
        "mclaw.config.settings.lifespan_stage_timeout_s", 5, raising=False
    )
...
async def test_stop_im_channels_stage_timeout_does_not_kill_others(
    ...
    monkeypatch.setattr(
        "mclaw.config.settings.lifespan_stage_timeout_s", 0.4, raising=False
    )
