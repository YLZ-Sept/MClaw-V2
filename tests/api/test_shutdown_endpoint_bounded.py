...
def test_shutdown_endpoint_returns_immediately(
    ...
    monkeypatch.setattr("mclaw.config.settings.shutdown_force_exit_grace_s", 30, raising=False)
...
def test_shutdown_endpoint_disabled_when_grace_zero(
    ...
    """
    monkeypatch.setattr("mclaw.config.settings.shutdown_force_exit_grace_s", 0, raising=False)
