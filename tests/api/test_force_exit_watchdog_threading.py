...
def _patch_os_exit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    truncated. Patching ``mclaw.api.server.os._exit`` at fixture
    scope makes the misfire harmless.
    """
    monkeypatch.setattr("mclaw.api.server.os._exit", lambda code=0: None)
...
def test_threading_watchdog_registers_named_daemon_timer(
    ...
    """
    monkeypatch.setattr(
        "mclaw.config.settings.shutdown_force_exit_grace_s", 30, raising=False
    )
    ...
    try:
        ...
        assert timer.name == .mclaw-force-exit-watchdog"
...
def test_threading_watchdog_fires_os_exit_after_grace(
    ...
    fires; the patch target is ``mclaw.api.server.os._exit`` because
    ``_do_force_exit`` resolves ``os`` through the module-level import.
    """
    monkeypatch.setattr(
        "mclaw.config.settings.shutdown_force_exit_grace_s", 1, raising=False
    )
    ...
    app = _fresh_fake_app()
    try:
        with patch("mclaw.api.server.os._exit", side_effect=_fake_exit):
            _arm_force_exit_watchdog_sync(app)
            timer = app.state._force_exit_task  # type: ignore[attr-defined]
            assert isinstance(timer, threading.Timer)
...
def test_threading_watchdog_disabled_when_grace_zero(
    ...
    """
    monkeypatch.setattr(
        "mclaw.config.settings.shutdown_force_exit_grace_s", 0, raising=False
    )
...
def test_dispatcher_routes_to_threading_by_default(
    ...
    monkeypatch.setattr(
        "mclaw.config.settings.shutdown_force_exit_use_threading",
        True,
        raising=False,
    )
