...
The audit lib lives at repo root (``D:/Mclaw/_audit_lib``) so
v20+ scripts can ``from _audit_lib import ...`` without depending on
the mclaw package; pytest finds it via the repo-root entry that
...
# audit lib is intentionally NOT shipped inside ``src/mclaw/`` to
...
def test_parse_line_timestamp_python_logger_format() -> None:
    ...
    parser must accept it -- it is the Mclaw default.
    """
...
def test_grep_logs_since_skips_files_with_old_mtime(tmp_path: Path) -> None:
    ...
    Mclaw logger always touches the file when appending a new
    line.
    """
...
def test_parse_line_timestamp_handles_edge_years(raw: str, expected_year: int) -> None:
    ...
    that is fine for Mclaw's logs which only span 2024+.
    """
