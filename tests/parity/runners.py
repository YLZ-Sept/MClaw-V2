...
def _permission_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.permission import check_mode_permission
...
def _token_budget_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.token_budget import TokenBudget
...
def _working_facts_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.working_facts import (
...
def _loop_budget_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.loop_budget import LoopBudgetGuard
...
def _trusted_paths_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.trusted_paths import is_trusted_workspace_path
...
def _primary_agent_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.core import get_primary_agent, set_primary_agent
...
def _reasoning_decision_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.reasoning import Decision, DecisionType
...
def _brain_response_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.brain import Response
...
def _smart_truncate_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.tools import smart_truncate
...
def _confirm_normalize_v1(case: ParityCase) -> ParityResult:
    from mclaw.core.confirmation_state import normalize_confirmation_answer

    return _confirm_normalize_eval(normalize_confirmation_answer, case)


def _confirm_normalize_v2(case: ParityCase) -> ParityResult:
    from mclaw.agent.confirmation import normalize_confirmation_answer
...
def _domain_allowlist_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.domain_allowlist import DomainAllowlist
...
def _user_profile_resolve_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.user_profile import resolve_profile_key
...
def _capability_id_v1(case: ParityCase) -> ParityResult:
    from mclaw.agent.capabilities import build_capability_id, build_namespace, normalize_slug
...
KIND_MODULES: dict[str, tuple[str, str]] = {
    "permission_mode": ("mclaw.agent.permission", "mclaw.agent.permission"),
    "token_budget": ("mclaw.agent.token_budget", "mclaw.agent.token_budget"),
    "working_facts": ("mclaw.agent.working_facts", "mclaw.agent.working_facts"),
    "loop_budget": ("mclaw.agent.loop_budget", "mclaw.agent.loop_budget"),
    "trusted_paths": ("mclaw.agent.trusted_paths", "mclaw.agent.trusted_paths"),
    "smart_truncate": ("mclaw.core._tool_runtime", "mclaw.agent.tools"),
    "context_estimate_tokens": ("mclaw.core.context_utils", "mclaw.agent.context"),
    "brain_response": ("mclaw.core._brain_runtime", "mclaw.agent.brain"),
    "reasoning_decision": ("mclaw.core._reasoning_runtime", "mclaw.agent.reasoning"),
    "primary_agent": ("mclaw.core._agent_runtime", "mclaw.agent.core"),
    "confirm_normalize": ("mclaw.core.confirmation_state", "mclaw.agent.confirmation"),
    "domain_allowlist": ("mclaw.agent.domain_allowlist", "mclaw.agent.domain_allowlist"),
    "user_profile_resolve": ("mclaw.agent.user_profile", "mclaw.agent.user_profile"),
    "capability_id": ("mclaw.agent.capabilities", "mclaw.agent.capabilities"),
}
