...
* P9.9epsilon-2a (``857a5a35``) -- ``git rm`` on
  ``src/mclaw/api/routes/orgs.py`` (-2 533 LOC; 89 v1
  endpoints) + 2 dev scripts + OpenAPI snapshot regen.
* P9.9epsilon-2b (``90a7d77f``) -- atomic
  ``git rm -r src/mclaw/orgs/`` (-20 237 LOC; 26 files);
...
1. **v1 src directory retired (Option-Z augmented at P10.2)** --
   ``src/mclaw/orgs/`` is either gone (original P-RC-9
...
_SHIM_DIR = _REPO / "src" / "mclaw" / "runtime" / "orgs"
...
def test_v1_src_directory_retired() -> None:
    """``src/mclaw/orgs/`` is either gone or the post-P10.1 v2 surface.
    ...
    ``src/mclaw/runtime/orgs/``; 25 files; no semantic v1 regrowth).
    ...
    assert not missing, (
        "``src/mclaw/orgs/`` exists but lacks the P-RC-10 P10.1 v2 "
...
def test_production_imports_v1_free() -> None:
    """Zero ``mclaw.runtime.orgs.*`` imports under ``src/mclaw/``.

    **Polarity reversed in P-RC-10 P10.4 + tightened in P10.6**
    (post-flatten + post-shim-removal reality):

    * P-RC-9 era this test banned ``mclaw.orgs.*`` (then the
      retired v1 surface) while ``mclaw.runtime.orgs.*`` was
    ...
    * **P10.6 (this commit) ``git rm``ed the shim file and
      ...
      the directory ``src/mclaw/runtime/orgs/`` itself
      MUST NOT exist.

    Net effect: production code under ``src/mclaw/`` MUST
    NOT regrow imports of the deprecated
    ``mclaw.runtime.orgs.*`` path under ANY circumstance.
    ...
    assert not _SHIM_DIR.exists(), (
        "``src/mclaw/runtime/orgs/`` still exists post-P-RC-10 P10.6 -- "
        "the deprecation shim should have been removed via "
        "``git rm src/mclaw/runtime/orgs/__init__.py`` AND the "
        "directory pruned (no stale ``__pycache__``). Listing: "
        + str(sorted(p.name for p in _SHIM_DIR.iterdir()))
        + "\n\nFix: ``Remove-Item -Path src/mclaw/runtime/orgs/ "
        "-Recurse -Force`` (Windows) or ``rm -rf "
        "src/mclaw/runtime/orgs/`` (POSIX); if the dir was "
        "regenerated by a phantom import, audit ``src/`` for any "
        "``mclaw.runtime.orgs`` reference Test 2's import scan "
    ...
    assert not hits, (
        "Stale ``mclaw.runtime.orgs`` import statement(s) found in "
        "production source under ``src/mclaw/`` -- this is the "
        ...
        "code MUST import from ``mclaw.orgs.*`` directly:\n"
        + "\n".join(f"  {rel}:{ln}: {line}" for rel, ln, line in hits)
        + "\n\nFix: rewrite ``mclaw.runtime.orgs.X`` -> "
        "``mclaw.orgs.X`` (1:1 prefix swap; see "
