...
async def test_memory_delete_by_query_reads_turn_scoped_authorization():
    ...
    from mclaw.agent import ToolExecutor
    from mclaw.core.risk_intent import TurnRiskAuthorization
    from mclaw.core.tool_execution_context import ToolExecutionContext
    from mclaw.memory.types import MemoryType
    ...
    memory = SimpleNamespace(
        ...
        source="manual",
        content="MCLAW_RISKGATE_689_REPRO_TEST — 用于 RiskGate 复现的测试记忆标识。",
    )
    ...
    turn_auth = TurnRiskAuthorization(
        original_message="请删除长期记忆中所有包含 MCLAW_RISKGATE_689_REPRO_TEST 的记忆。",
        ...
        authorized_intent={
            ...
            "scope": {
                ...
                "raw": "请删除长期记忆中所有包含 MCLAW_RISKGATE_689_REPRO_TEST 的记忆。",
            },
        },
    )
...
async def test_session_history_survives_restart(tmp_path: Path):
    ...
    from mclaw.sessions.manager import SessionManager
...
async def test_dashscope_recovered_response_no_cooldown():
    ...
    from mclaw.llm.types import LLMResponse, StopReason, TextBlock, Usage
...
async def test_plugin_failure_jsonl_appends(tmp_path: Path):
    ...
    from mclaw.plugins.manager import PluginManager
