...
from mclaw.orgs._runtime_agent_host import NodeToolHost, build_node_tool_host
from mclaw.orgs._runtime_delegation import (
    current_delegation_assignment_var,
    current_delegation_output_slot_var,
)
from mclaw.orgs._runtime_event_store import OrgEventStore
...
class _FakeAgent:
    ...
    the full ``mclaw.core.agent.Agent`` import graph (browser /
        ...
        plugin_tools=[
            {
                ...
                "x.mclaw-execution": {"kind": "external_task", "timeout_s": 900},
                "x.mclaw-idempotency-param": "client_request_id",
            }
        ],
    )
...
async def test_node_tool_host_unifies_video_resolution_and_normalizes_duration(
    ...
    from mclaw.orgs._runtime_media_contract import media_contract_ledger
...
def test_node_tool_host_dispose_breaks_further_dispatch(tmp_path: Path) -> None:
    ...
    from mclaw.orgs._runtime_agent_host import ToolNotAvailable
