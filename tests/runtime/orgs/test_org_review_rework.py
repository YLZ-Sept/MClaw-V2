...
* 核心2 (重做闭环): on 退回 the child is RE-DISPATCHED with the parent's
  ...
  ``agent_run_started``), bounded by ``MCLAW_ORG_REWORK_MAX``; on
...
* 核心3 (超时隔离): a single node activation that blocks past
  ``MCLAW_ORG_NODE_TIMEOUT_S`` is failed-and-reported
...
from mclaw.orgs._default_agent_builder import (
    DefaultAgentBuilder,
...
from mclaw.orgs._runtime_artifact_flow import artifact_ledger
from mclaw.orgs._runtime_delegation import (
    DelegationExecutionResult,
    current_delegation_assignment_var,
    current_delegation_output_slot_var,
)
from mclaw.orgs._runtime_delivery_manifest import (
    DeliveryManifest,
    delivery_manifest_ledger,
)
from mclaw.orgs._runtime_media_quality import (
    current_media_quality_failures,
    current_media_quality_failures_var,
)
from mclaw.runtime.execution_context import UpstreamContext
...
def test_parent_review_accepts_first_pass(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MCLAW_ORG_REVIEW_ENABLED", "1")
    monkeypatch.setenv("MCLAW_ORG_NODE_TIMEOUT_S", "0")
    bus = _RecordingBus()
...
def test_structured_review_is_not_overridden_by_prose_tokens(
    ...
    monkeypatch.setenv("MCLAW_ORG_REVIEW_ENABLED", "1")
    monkeypatch.setenv("MCLAW_ORG_REWORK_MAX", "1")
    monkeypatch.setenv("MCLAW_ORG_NODE_TIMEOUT_S", "0")
    bus = _RecordingBus()
    lookup = _Lookup(["n0", "n1"])
...
def test_hard_gate_can_be_disabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    — proves the override is gated by MCLAW_ORG_REVIEW_HARD_GATE."""
    monkeypatch.setenv("MCLAW_ORG_REVIEW_ENABLED", "1")
    monkeypatch.setenv("MCLAW_ORG_REWORK_MAX", "1")
    monkeypatch.setenv("MCLAW_ORG_NODE_TIMEOUT_S", "0")
    monkeypatch.setenv("MCLAW_ORG_REVIEW_HARD_GATE", "0")
    bus = _RecordingBus()
    lookup = _Lookup(["n0", "n1"])
...
def test_reject_triggers_rework_then_accept(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MCLAW_ORG_REVIEW_ENABLED", "1")
    monkeypatch.setenv("MCLAW_ORG_REWORK_MAX", "2")
    monkeypatch.setenv("MCLAW_ORG_NODE_TIMEOUT_S", "0")
    bus = _RecordingBus()
    lookup = _Lookup(["n0", "n1"])
...
def test_deterministic_media_failure_reworks_even_when_llm_review_is_disabled(
    ...
    monkeypatch.setenv("MCLAW_ORG_REVIEW_ENABLED", "0")
    monkeypatch.setenv("MCLAW_ORG_REWORK_MAX", "1")
    bus = _RecordingBus()
...
def test_exhausted_media_failure_propagates_to_parent_run(
    ...
    monkeypatch.setenv("MCLAW_ORG_REVIEW_ENABLED", "0")
    monkeypatch.setenv("MCLAW_ORG_REWORK_MAX", "0")
    bus = _RecordingBus()
    executor = _make_executor(
        bus=bus,
        lookup=_Lookup(["n0", "n1"]),
...
def test_coordinator_preserves_child_output_and_exposes_child_manifest(
    ...
    from mclaw.orgs._runtime_node_tools import record_node_file_output
...
def test_node_timeout_isolation(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MCLAW_ORG_REVIEW_ENABLED", "0")
    monkeypatch.setenv("MCLAW_ORG_NODE_TIMEOUT_S", "1")
    bus = _RecordingBus()
    lookup = _Lookup(["n0"])
