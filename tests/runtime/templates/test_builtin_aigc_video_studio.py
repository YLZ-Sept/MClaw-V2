...
from mclaw.runtime.models import EdgeKind, NodeType
from mclaw.runtime.templates import (
    TemplateRegistry,
    TemplateSpec,
    discover_builtins,
)
from mclaw.runtime.templates.builtin.aigc_video_studio import (
    PLUGIN_ID,
    aigc_video_studio,
)
...
def test_registry_bootstrap_picks_up_template_via_discover_builtins(
    ...
    """
    from mclaw.runtime.templates import registry as registry_mod

    monkeypatch.setattr(registry_mod, "_PENDING", [], raising=False)
    for mod in list(sys.modules):
        if mod.startswith("mclaw.runtime.templates.builtin"):
            sys.modules.pop(mod)
...
def test_module_reexports_factory_for_external_use() -> None:
    ...
    mod = importlib.import_module("mclaw.runtime.templates.builtin.aigc_video_studio")
