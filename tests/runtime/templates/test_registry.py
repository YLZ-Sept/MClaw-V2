...
from mclaw.runtime.models import EdgeKind, NodeStatus, NodeType, OrgStatus
...
from mclaw.runtime.templates.registry import _PENDING
...
def test_discover_builtins_returns_zero_for_missing_package() -> None:
    from mclaw.runtime.templates.registry import discover_builtins

    n = discover_builtins(package="mclaw.runtime.templates.builtin_does_not_exist")
    assert n == 0
...
def test_template_decorator_attaches_survivable_factory_mark(
    ...
    monkeypatch.setattr(
        "mclaw.runtime.templates.registry._PENDING", [], raising=False
    )
...
def test_collect_builtin_factories_returns_empty_for_missing_package() -> None:
    factories = collect_builtin_factories(
        package="mclaw.runtime.templates.builtin_does_not_exist"
    )
    assert factories == []
