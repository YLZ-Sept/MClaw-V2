...
from mclaw.runtime.desktop import attachments
...
def test_maybe_inline_local_image_returns_data_url(tmp_path) -> None:
    ...
    with patch(
        "mclaw.api.routes.upload.get_upload_dir",
...
def test_save_data_uri_attachment_writes_file(tmp_path) -> None:
    ...
    with (
        ...
        patch("mclaw.api.routes.upload.MAX_UPLOAD_SIZE", 1024),
        patch("mclaw.api.routes.upload.BLOCKED_EXTENSIONS", {".exe"}),
    ):
        out = attachments.save_data_uri_attachment(
...
def test_module_exports_are_stable() -> None:
    ...
    from mclaw.runtime.desktop import __all__ as pkg_all
