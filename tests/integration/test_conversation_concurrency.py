...
from mclaw import config as config_mod
from mclaw.agent import Agent
from mclaw.api.routes.conversation_lifecycle import (
    BusyInfo,
    ConversationLifecycleManager,
    StartResult,
)
from mclaw.api.routes.double_texting import DoubleTextingPolicy, resolve_policy
from mclaw.api.routes.turn_registry import TurnRegistry
from mclaw.core import conversation_metrics as metrics
from mclaw.core.agent_state import AgentState, TaskState, TaskStatus
from mclaw.sessions.session import SessionContext
...
class TestConversationLifecycleRefresh:
    ...
    async def test_refresh_extends_busy_lease_without_changing_since(self, monkeypatch) -> None:
        from mclaw.api.routes import conversation_lifecycle as lifecycle_mod
...
class TestAppendMarkerPersistence:
    ...
    async def test_append_marker_calls_persistence(self, monkeypatch) -> None:
        from mclaw.sessions.session import Session, SessionConfig
...
class TestSSEDeltaCoalescer:
    ...
    def test_size_threshold_flushes_immediately(self) -> None:
        from mclaw.core.sse_throttle import DeltaCoalescer
    ...
    def test_chain_text_is_coalesced(self) -> None:
        from mclaw.core.sse_throttle import COALESCED_TYPES, DeltaCoalescer
...
class TestShellSoftKill:
    ...
    async def test_cancelled_error_carries_partial_payload_shape(
        ...
        from mclaw.tools.shell import ShellTool
