...
under the installed ``mclaw`` package directory. Identity templates use
...
1. Walk up from the running ``mclaw`` package directory looking for a
   sibling ``identity/`` (the repo-checkout / ``pip install -e .`` path).
2. Fall back to ``.mclaw-package-root>/identity/<file>`` (the
...
class TestIdentityTemplatesAreShipped:
    ...
    @pytest.mark.parametrize("source_path", REQUIRED_IDENTITY_SOURCES)
    def test_identity_template_target_under_mclaw_identity(
        self, force_include: dict[str, str], source_path: str
    ) -> None:
        """Wheel-side target must live under ``mclaw/identity/`` so the
        ...
        assert target.startswith("mclaw/identity/"), (
            f"{source_path!r} → {target!r}: target must be under "
            f"'mclaw/identity/' so it ends up next to the "
            f"installed mclaw package directory."
        )
        relative = source_path[len("identity/") :]
        assert target == f"mclaw/identity/{relative}", (
            f"{source_path!r} → {target!r}: target should mirror the "
            f"source layout exactly (expected 'mclaw/identity/{relative}')."
        )


class TestPackagedPromptsAreShipped:
    SOURCE_DIR = "src/mclaw/prompts"
    REQUIRED_ASSETS = [
        "core/always_on.md",
        "core/extended.md",
        "core/safety.md",
    ...
    def test_prompt_is_in_source_distribution(self, sdist_include: list[str]) -> None:
        assert "src/mclaw" in sdist_include, (
...
class TestWizardCanResolveBundledTemplate:
    ...
    def test_resolves_soul_md_example_in_repo(self) -> None:
        from mclaw.setup.wizard import _resolve_identity_template
