"""Isolation guard: org nodes must not write into the Mclaw source tree.

Regression for the "stray ``src/mclaw/orgs/tool_handler.py``" pollution
...
from mclaw.orgs._runtime_node_tools import _guarded_write_violation


def test_blocks_write_into_package_source_tree() -> None:
    assert _guarded_write_violation("write_file", {"path": "src/mclaw/orgs/x.py"})
    assert _guarded_write_violation("edit_file", {"path": "src/mclaw/main.py"})
...
def test_blocks_move_destination_into_source_tree() -> None:
    assert _guarded_write_violation(
        "move_file", {"src": "data/orgs/o1/out.md", "dst": "src/mclaw/leak.py"}
    )
...
def test_non_write_tools_are_not_guarded() -> None:
    ...
    # are guarded.
    assert _guarded_write_violation("read_file", {"path": "src/mclaw/main.py"}) is None
