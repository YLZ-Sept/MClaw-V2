...
历史背景：
- C6 之前 mock 点是 ``mclaw.core.policy.get_policy_engine``，决策走 v1 PolicyEngine。
- C6 之后 mock 点切换到 ``mclaw.core.policy_v2.global_engine.get_engine_v2``
...
from mclaw.agent.permission import check_permission
from mclaw.agent.tools import ToolExecutor
from mclaw.core import _reasoning_runtime as reasoning_engine_module
...
def _patch_engine_to_raise(monkeypatch: pytest.MonkeyPatch, exc: Exception) -> None:
    ...
    monkeypatch.setattr(
        "mclaw.core.policy_v2.adapter._get_engine",
        _boom,
    )
...
class _RiskGateCommitRegistry:
    def __init__(self) -> None:
        from mclaw.core.policy_v2 import ToolPolicy
    ...
    ctx = ToolExecutionContext(
        risk_authorization=TurnRiskAuthorization(
            ...
            authorized_intent={
                ...
                "scope": {"query": "MCLAW_RISKGATE_689_REPRO_TEST"},
