...
from mclaw.api.routes import bug_report
from mclaw.api.routes import config as config_routes
from mclaw.llm.capabilities import infer_capabilities, is_image_generation_model
...
async def test_security_write_reports_saved_runtime_refresh_failure(monkeypatch):
    ...
    monkeypatch.setattr(
        "mclaw.core.policy_v2.global_engine.reset_policy_v2_layer",
        _fail_reset,
    )
...
async def test_security_write_uses_request_scoped_runtime_coordinator(monkeypatch):
    from mclaw.runtime_config_coordinator import RuntimeApplyResult
...
def test_apply_llm_runtime_config_refreshes_all_runtime_components(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mclaw.llm.config.load_endpoints_config",
...
def test_collect_endpoint_summary_redacts_keys_and_keeps_diagnostic_fields(tmp_path, monkeypatch):
    ...
    import mclaw.llm.config as llm_config
