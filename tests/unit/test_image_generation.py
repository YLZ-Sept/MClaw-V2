...
from mclaw.llm.image_generation import (
...
from mclaw.tools.handlers.system import SystemHandler
...
async def test_generate_image_falls_back_and_saves_base64_result(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "mclaw.llm.image_generation.load_image_endpoints",
        lambda _workspace: [primary, backup],
    )
    monkeypatch.setattr("mclaw.llm.image_generation.request_image", fake_request)
