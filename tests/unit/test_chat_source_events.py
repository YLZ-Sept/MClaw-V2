...
from mclaw.api.routes.chat import (
    _extract_mcp_call,
...
def test_extract_source_used_from_mclaw_marker():
    event = {
        "type": "tool_call_end",
        "tool": "web_fetch",
        "id": "abc",
        "result": (
            '[MCLAW_SOURCE] {"requested_url":"https://example.com/a",'
...
def test_extract_mcp_call_returns_structured_payload():
    event = {
        ...
        "result": (
            ...
            '[MCLAW_MCP] {"status":"ok","server":"github","tool":"list_repos",'
...
def test_extract_mcp_call_ignores_other_tools():
    assert (
        _extract_mcp_call(
            {"type": "tool_call_end", "tool": "web_fetch", "result": "[MCLAW_MCP] {}"}
        )
        is None
    )
    assert (
    ...
    assert (
        _extract_mcp_call(
            {"type": "tool_call_end", "tool": "call_mcp_tool", "result": "[MCLAW_MCP] not-json"}
        )
        is None
    )
...
def test_org_file_attachments_convert_to_chat_attachments():
    attachments = _org_file_attachments_to_chat_attachments(
        [
            {
                "filename": "方案.md",
                "file_path": r"D:\Mclaw\workspace\方案.md",
                "file_size": 128,
            },
            {
                "filename": "方案.md",
    ...
    assert attachments == [
        {
            "type": "document",
            "name": "方案.md",
            "localPath": r"D:\Mclaw\workspace\方案.md",
            "size": 128,
