...
from mclaw.core.risk_scope import authorization_covers_tool_call, extract_tool_scope
...
def test_authorization_scope_covers_matching_text_and_exact_fields():
    ...
    assert authorization_covers_tool_call(
        ...
        tool_input={"query": "MCLAW_RISKGATE_689_REPRO_TEST", "source": "profile"},
        policy=policy,
    )
    ...
    assert not authorization_covers_tool_call(
        auth,
        tool_name="memory_delete_by_query",
        tool_input={"query": "MCLAW_RISKGATE_689_REPRO_TEST", "source": "other"},
        policy=policy,
    )
    assert not authorization_covers_tool_call(
        auth,
...
def test_authorization_scope_can_match_query_against_raw_text():
    ...
    auth = TurnRiskAuthorization(
        ...
        authorized_intent={
            "operation": "memory_delete",
            "scope": {"raw": "请删除所有包含 MCLAW_RISKGATE_689_REPRO_TEST 的记忆"},
        },
    )
    ...
    assert authorization_covers_tool_call(
        auth,
        tool_name="memory_delete_by_query",
        tool_input={"query": "MCLAW_RISKGATE_689_REPRO_TEST"},
        policy=policy,
    )
