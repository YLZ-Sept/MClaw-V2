"""Unit tests for :func:`mclaw.plugins.reseed.warn_on_drift`.
...
# isort: off  -- workaround for pre-existing import cycle in mclaw.plugins
#                (``core.capabilities`` <-> ``agent`` <-> ``skills.registry``).
# Importing ``mclaw.agent`` first forces ``core.capabilities`` to finish
...
import mclaw.agent  # noqa: F401  pylint: disable=unused-import

from mclaw.plugins.reseed import (
    EXCLUDED_DIR_NAMES,
    MTIME_TOLERANCE_SEC,
    STATUS_SOURCE_NEWER,
    compute_drift,
...
def test_warn_emitted_when_source_newer(drift_tree: tuple[Path, Path], caplog):
    ...
    assert "mclaw plugins reseed --apply" in msg
...
def test_silenced_when_flag_disabled(drift_tree: tuple[Path, Path], monkeypatch, caplog):
    ...
    from mclaw.plugins import reseed as _reseed
