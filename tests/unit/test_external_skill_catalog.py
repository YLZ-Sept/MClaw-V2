...
from mclaw.skills.loader import DEFAULT_DISABLED_SKILLS, SkillLoader
...
def test_system_presets_do_not_reference_removed_external_skills() -> None:
    ...
    external_references = {
        ...
        if skill.startswith(("mclaw/skills@", "obra/superpowers@"))
    }
...
def test_namespaced_preset_reference_keeps_registry_skill(tmp_path: Path) -> None:
    ...
    loader.prune_external_by_allowlist(
        set(), agent_referenced_skills={"mclaw/skills@demo-skill"}
    )
