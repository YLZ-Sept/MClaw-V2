...
Mirrors how `web_fetch` emits `[MCLAW_SOURCE]`: the chat route can
...
from mclaw.tools.handlers.mcp import _build_mcp_envelope
...
def test_envelope_marks_success_with_server_and_tool():
    ...
    assert line.startswith("[MCLAW_MCP] ")
    payload = json.loads(line[len("[MCLAW_MCP] ") :])
    assert payload == {
        "status": "ok",
        "server": "github",
        "tool": "list_repos",
