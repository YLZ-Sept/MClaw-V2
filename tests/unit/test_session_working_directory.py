...
from mclaw.api.routes.files import router as files_router
from mclaw.api.routes.sessions import router as sessions_router
from mclaw.api.schemas import AttachmentInfo
from mclaw.api.working_directories import (
    authorize_working_directory,
    resolve_chat_attachments,
    resolve_session_file,
)
from mclaw.core.policy_v2 import PolicyContext, reset_current_context, set_current_context
from mclaw.core.working_directory import (
...
from mclaw.sessions import Session, SessionManager


def test_legacy_session_defaults_to_configuration_workspace(tmp_path, monkeypatch):
    monkeypatch.setattr(
        "mclaw.core.working_directory.config_workspace",
...
def test_disabled_feature_forces_configuration_workspace(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mclaw.core.working_directory.working_directory_feature_enabled",
        lambda: False,
    )
...
def test_session_create_rejects_working_directory_change(tmp_path, monkeypatch):
    ...
    second.mkdir()
    monkeypatch.setattr(
        "mclaw.api.working_directories.authorize_working_directory",
...
def test_web_directory_browser_is_limited_to_configured_roots(tmp_path, monkeypatch):
    ...
    outside.mkdir()
    monkeypatch.setattr(
        "mclaw.api.working_directories.configured_working_roots",
        lambda: (root.resolve(),),
    )
    app = FastAPI()
    app.include_router(sessions_router)
...
def test_uploaded_attachment_resolves_server_path_without_client_local_path(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mclaw.api.routes.upload.UPLOAD_DIR", upload_dir)
    session = Session.create(
...
def test_terminal_sessions_are_namespaced_by_conversation(tmp_path):
    from mclaw.tools.terminal import TerminalSessionManager
