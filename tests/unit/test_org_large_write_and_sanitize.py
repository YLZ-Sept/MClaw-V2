...
from mclaw.orgs._runtime_node_tools import (
...
def fs_handler(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    from mclaw.tools.file import FileTool
    ...
    monkeypatch.setattr("mclaw.core.policy_v2.get_config_v2", lambda: _Config())
    agent = MagicMock()
...
async def test_execute_node_tool_appends_into_per_command_workspace(
    ...
    import mclaw.orgs._runtime_node_artifacts as _artifacts
    from mclaw.orgs._runtime_agent_host import NodeToolHost
...
async def test_execute_node_tool_fast_fails_on_truncated_write_args(
    ...
    import mclaw.orgs._runtime_node_tools as nt
...
def test_is_content_moderation_error_detects_dashscope_rejection() -> None:
    from mclaw.orgs._default_agent_builder import _is_content_moderation_error
...
def test_node_degrades_instead_of_failing_on_moderation_rejection() -> None:
    ...
    from mclaw.orgs._runtime_agent_pipeline import AgentSpec
