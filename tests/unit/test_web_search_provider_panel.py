...
from mclaw.tools.tool_hints import ConfigHint, ToolConfigError
from mclaw.tools.web_search import (
    AuthFailedError,
    ContentFilterError,
...
from mclaw.tools.web_search.registry import register
...
class TestConfigHint:
    def test_to_llm_text_has_no_markers(self) -> None:
        ...
        for marker in ("<", ">", "{", "}", "config_hint", ".mclaw-"):
...
class TestWebSearchHandlerErrorMapping:
    ...
    def _isolate_registry(self, monkeypatch: pytest.MonkeyPatch):
        ...
        from mclaw.tools.web_search import registry as reg

        monkeypatch.setattr(reg, "_PROVIDERS", {}, raising=False)
        monkeypatch.setattr(reg, "_LOADED", True, raising=False)
        monkeypatch.setattr(
            "mclaw.tools.handlers.web_search.settings.web_search_provider",
            "",
        )
        yield

    async def _run_handler(self) -> tuple[str | None, ConfigHint | None]:
        from mclaw.tools.handlers.web_search import WebSearchHandler
...
class TestToolExecutorPropagation:
    async def test_execute_tool_returns_tuple(self) -> None:
        from mclaw.agent.permission import PermissionDecision
...
class TestReasoningEngineHelper:
    def test_builds_two_events_when_hint_present(self) -> None:
        from mclaw.core._reasoning_runtime import _build_tool_end_events
...
def test_llm_converter_drops_hint_field() -> None:
    ...
    from mclaw.llm.converters.tools import convert_tool_result_to_openai
...
def test_providers_endpoint_lists_all() -> None:
    ...
    from mclaw.api.routes.web_search import router
