...
from mclaw.storage.degraded import DegradedRegistry, registry
from mclaw.storage.safe_sqlite import (
    SQLiteUnavailable,
    safe_open_async,
    safe_open_async_ctx,
    safe_open_sync,
...
def test_safe_open_sync_rejects_sync_folder_path(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("MCLAW_ALLOW_SYNC_FOLDER_DB", raising=False)
    with pytest.raises(SQLiteUnavailable) as ei:
...
def test_safe_open_sync_sync_folder_override(tmp_path, monkeypatch):
    ...
    one_drive_dir.mkdir()
    target = one_drive_dir / "x.db"
    monkeypatch.setenv("MCLAW_ALLOW_SYNC_FOLDER_DB", "1")
...
def test_module_level_registry_importable_from_subthread():
    from mclaw.storage.degraded import registry as _r
...
def test_env_var_does_not_leak(monkeypatch, tmp_path):
    assert os.environ.get("MCLAW_ALLOW_SYNC_FOLDER_DB") is None
