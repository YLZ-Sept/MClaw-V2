...
from mclaw.api.routes import health as health_module
from mclaw.api.routes import memory_repair as repair_module
from mclaw.storage.degraded import registry as degraded_registry
...
def _reset_state(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.delenv("MCLAW_DESKTOP_SESSION_TOKEN", raising=False)
...
class TestDesktopTokenGate:
    """When ``MCLAW_DESKTOP_SESSION_TOKEN`` is set, the quarantine
    ...
    def test_quarantine_rejects_missing_desktop_token(
        ...
        monkeypatch.setenv("MCLAW_DESKTOP_SESSION_TOKEN", "desktop-secret")
        token = client.get("/api/memory/repair/degraded").json()["confirmation_token"]
        r = client.post(
            "/api/memory/repair/quarantine",
            json={"subsystem": "feedback", "confirmation_token": token},
    ...
    def test_quarantine_accepts_matching_desktop_token(
        ...
        r = client.post(
            "/api/memory/repair/quarantine",
            json={"subsystem": "feedback", "confirmation_token": token},
            headers={"X-Mclaw-Desktop-Token": "desktop-secret"},
        )
        assert r.status_code == 200, r.text
...
class TestTokenStatsRegistersUnderTokenTrackingKey:
    ...
    def test_register_helper_uses_shared_key(self) -> None:
        from mclaw.api.routes import token_stats as ts
