...
from mclaw.runtime_channel_deps import (
...
def test_channel_deps_no_enabled_channels_is_ok(monkeypatch):
    monkeypatch.setattr("mclaw.runtime_channel_deps.inject_module_paths_runtime", lambda: None)
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.patch_simplejson_jsondecodeerror", lambda logger=None: False
    )
...
def test_channel_deps_packaged_mode_rejects_frozen_sys_executable(monkeypatch):
    ...
    monkeypatch.setattr("mclaw.runtime_channel_deps.IS_FROZEN", True)
    monkeypatch.setattr("mclaw.runtime_channel_deps.get_app_python_executable", lambda: None)
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.get_python_executable", lambda: sys.executable
    )
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.CHANNEL_DEPS",
        {"feishu": [("definitely_missing_mclaw_dep", "definitely-missing.mclaw-dep")]},
    )
    ...
    assert result["missing"] == ["definitely-missing.mclaw-dep"]
...
def test_pip_raw_progress_exposes_current_download_eta(monkeypatch):
    ...
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.time.monotonic", lambda: next(clock)
    )
...
def test_ensure_channel_deps_returns_install_errors(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mclaw.runtime_channel_deps.inject_module_paths_runtime", lambda: None)
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.patch_simplejson_jsondecodeerror",
        lambda logger=None: False,
    )
    monkeypatch.setattr("mclaw.runtime_channel_deps.IS_FROZEN", False)
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.get_app_python_executable", lambda: sys.executable
    )
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.get_python_executable", lambda: sys.executable
    )
    monkeypatch.setattr("mclaw.runtime_channel_deps.get_channel_deps_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.apply_runtime_pip_environment",
        lambda **_: {},
    )
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.resolve_pip_index",
        lambda: {"url": "https://example.invalid/simple/", "trusted_host": "example.invalid"},
    )
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps._probe_python_runtime",
        lambda *a, **kw: (True, ""),
    )
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.CHANNEL_DEPS",
        {
            "feishu": [
                (
                    "definitely_missing_mclaw_dep_for_test",
                    "definitely-missing.mclaw-dep-for-test",
                )
            ]
        },
    )
    ...
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.subprocess.run",
        lambda *a, **kw: _Result(),
    )
    # 让 TimeoutExpired 仍可被 except 捕获
    monkeypatch.setattr(
        "mclaw.runtime_channel_deps.subprocess.TimeoutExpired", _sp.TimeoutExpired
    )

    result = ensure_channel_dependencies(workspace_env={"FEISHU_ENABLED": "true"})

    assert result["status"] == "error"
    assert result["missing"] == ["definitely-missing.mclaw-dep-for-test"]
    assert "errors" in result
    assert "definitely-missing.mclaw-dep-for-test" in result["errors"]
    assert (
        "Could not find a version" in result["errors"]["definitely-missing.mclaw-dep-for-test"]
    )
