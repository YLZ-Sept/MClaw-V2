from mclaw.core.stream_accumulator import (
    StreamAccumulator,
    StreamingInternalTraceScrubber,
    post_process_streamed_decision,
)
from mclaw.llm.providers.openai import OpenAIProvider
from mclaw.llm.providers.openai_responses import OpenAIResponsesProvider
...
def test_trace_scrubber_does_not_swallow_real_inline_tool_call_outside_trace():
    ...
    意图（Mclaw dot-style 工具调用要求前导 ``.`` 且工具名已注册）；
    不应被 trace scrubber 吞掉，post_process 应能提取出 web_search 调用。
    """
    from mclaw.llm.converters import tools as tools_mod
