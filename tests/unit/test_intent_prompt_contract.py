...
from mclaw.core._brain_runtime import (
    Brain,
    Response,
...
def test_compiler_configuration_reason_distinguishes_missing_and_disabled(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "mclaw.core._brain_runtime.get_default_config_path", lambda: config_path
    )
...
def test_stable_main_chat_core_has_8_to_15_tools_in_fixed_order():
    from mclaw.tools.defer_config import STABLE_MAIN_CHAT_CORE_TOOLS
...
suggest_plan: false
""",
        "https://github.com/mclaw/mclaw/issues/532 帮我分析这个 issue 当前是否仍存在",
    )
