...
from mclaw.api.routes.memory import router as memory_router
...
def test_v3_migration_backs_up_and_quarantines_legacy_desktop_memory(tmp_path):
    ...
    from mclaw.memory.storage import _SCHEMA_VERSION

    assert list(db_path.parent.glob(f"mclaw.db.bak.v2_to_v{_SCHEMA_VERSION}.*"))
