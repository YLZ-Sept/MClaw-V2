...
from mclaw.scheduler.executor import TaskExecutor
...
from mclaw.scheduler.triggers import CronTrigger, IntervalTrigger, OnceTrigger, Trigger
...
class TestSystemTaskTimeouts:
    ...
    async def test_system_task_sets_and_resets_background_token_budget(self, monkeypatch):
        from mclaw.core.token_tracking import get_token_budget, record_usage
        ...
        monkeypatch.setattr(
            "mclaw.config.settings.scheduler_background_token_budget",
            20,
        )
...
class TestTaskAgentProfiles:
    async def test_executor_creates_selected_agent_profile(self, monkeypatch):
        ...
        monkeypatch.setattr("mclaw.agents.factory.AgentFactory.create", fake_create)
    ...
    async def test_chat_created_task_inherits_current_agent_profile(self):
        from mclaw.tools.handlers.scheduled import ScheduledHandler
...
class TestSystemTaskRegistration:
    async def test_memory_task_keeps_user_custom_trigger(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "mclaw.scheduler.consolidation_tracker.ConsolidationTracker",
            FakeTracker,
        )
