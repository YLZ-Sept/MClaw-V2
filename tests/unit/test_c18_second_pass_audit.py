...
2. **BUG-C1**: ``audit_logger._global_audit`` is a process-wide
   ...
   Hot-reload or ``MCLAW_AUDIT_LOG_PATH`` could change
...
from mclaw.core.policy_v2 import audit_chain, global_engine, hot_reload
from mclaw.core.policy_v2.hot_reload import PolicyHotReloader
...
class TestBugC1AuditLoggerSingletonRefresh:
    ...
    def test_env_override_audit_path_takes_effect_on_rebuild(
        ...
        """End-to-end: ``MCLAW_AUDIT_LOG_PATH`` set on a rebuild
        ...
        monkeypatch.setenv("MCLAW_AUDIT_LOG_PATH", str(second_log))
        global_engine.rebuild_engine_v2(yaml_path=policies)
