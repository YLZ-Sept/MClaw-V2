...
from mclaw.core.policy_v2.entry_point import (
    IM_WEBHOOK_CHANNELS,
...
def test_force_unattended_overrides_cli_tty():
    """``mclaw run`` is non-interactive even when stdin is a TTY."""
...
def test_stream_renderer_security_confirm_skips_on_no_tty(monkeypatch):
    ...
    from mclaw.cli import stream_renderer as sr
    ...
    with patch(
        "mclaw.core.security_confirmation.resolve_security_confirmation",
...
def test_build_policy_context_accepts_unattended_strategy():
    """Caller (mclaw run / mcp_server) feeds classifier.default_strategy
    into build_policy_context so the strategy is on the ctx without needing
    a Session round-trip."""
    from mclaw.core.policy_v2.adapter import build_policy_context
...
# --- D6: MCP server installs unattended PolicyContext for mclaw_chat ---


def test_mcp_server_classifier_for_stdio_tool():
    """The MCP server runs over stdio (no TTY, no SSE); ``mclaw_chat``
...
def test_mcp_server_imports_classifier():
    ...
    import mclaw.mcp_server as mcp_mod
