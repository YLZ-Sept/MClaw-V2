...
def test_organization_watchdog_disabled_by_default():
    from mclaw.orgs.org_models import Organization
...
class TestGetResources:
    ...
    def test_returns_templates(self, handler, tmp_data_dir):
        with patch("mclaw.config.settings") as mock_settings:
...
class TestCreate:
    ...
    async def test_create_success(self, handler, tmp_data_dir):
        ...
        assert "[MCLAW_ORG]" in result
        marker = result.split("[MCLAW_ORG]", 1)[1].strip().splitlines()[0]
...
class TestToolRegistration:
    ...
    def test_exported_from_definitions(self):
        from mclaw.tools.definitions import ORG_SETUP_TOOLS

        assert len(ORG_SETUP_TOOLS) > 0

    def test_not_in_base_tools(self):
        from mclaw.tools.definitions import BASE_TOOLS, ORG_SETUP_TOOLS
    ...
    def test_handler_has_create_handler(self):
        from mclaw.tools.handlers.org_setup import create_handler
...
class TestListOrgs:
    ...
    def test_list_empty(self, handler, tmp_data_dir):
        with patch("mclaw.config.settings") as ms:
...
class TestDeleteOrg:
    ...
    async def test_delete_nonexistent(self, handler, tmp_data_dir):
        with (
            patch("mclaw.config.settings") as ms,
            patch("mclaw.orgs.runtime.get_runtime", return_value=None),
        ):
            ms.data_dir = tmp_data_dir
