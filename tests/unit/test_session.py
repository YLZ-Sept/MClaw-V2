...
from mclaw.sessions.session import (
    Session,
    SessionConfig,
    SessionContext,
    SessionState,
...
class TestSessionCreation:
    ...
    def test_context_focus_terms_are_session_scoped_and_serialized(self):
        s = Session(id="s1", channel="cli", chat_id="c1", user_id="u1")
        s.add_message("user", "继续修改 src/mclaw/memory/retrieval.py 的 retrieval gate")

        assert "src/mclaw/memory/retrieval.py" in s.context.focus_terms
...
class TestSessionPersistence:
    ...
    def test_catalog_build_failure_keeps_cold_sessions_available(self, tmp_path, monkeypatch):
        from mclaw.sessions.catalog import SessionCatalogStore
