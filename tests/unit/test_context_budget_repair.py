...
from mclaw.agent.brain import Brain
from mclaw.agent.context import ContextManager
from mclaw.agent.loop_budget import LoopBudgetGuard
from mclaw.agent.reasoning import ReasoningEngine
from mclaw.core.microcompact import microcompact
from mclaw.prompt.builder import _build_catalogs_section
...
def test_token_anomaly_compaction_uses_configured_summary_chars(monkeypatch):
    monkeypatch.setattr(
        "mclaw.core._reasoning_runtime.settings.context_token_anomaly_threshold", 100
    )
    monkeypatch.setattr(
        "mclaw.core._reasoning_runtime.settings.context_cached_summary_chars", 100
    )
