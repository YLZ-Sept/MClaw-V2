...
from mclaw.core.risk_intent import TurnRiskAuthorization

_TEST_QUERY = "MCLAW_RISKGATE_689_REPRO_TEST"
...
def _open_tool_confirmation(
    *,
    ...
    from mclaw.core.risk_gate_workflow import get_risk_gate_workflow
...
def test_confirmation_store_tracks_multiple_pending_in_one_conversation():
    from mclaw.agent.ui_confirm_bus import reset_ui_confirm_bus
    from mclaw.core.confirmation_state import get_confirmation_store
    from mclaw.core.risk_gate_workflow import get_risk_gate_workflow
...
def test_confirmation_store_times_out_pending_records_then_evicts_them(monkeypatch):
    from mclaw.core import confirmation_state as state_mod
...
async def test_risk_gate_workflow_owns_tool_call_state_and_waiter():
    from mclaw.agent.ui_confirm_bus import get_ui_confirm_bus, reset_ui_confirm_bus
    from mclaw.core.risk_gate_workflow import get_risk_gate_workflow
...
async def test_security_confirm_endpoint_resolves_riskgate_tool_call():
    from mclaw.agent.ui_confirm_bus import get_ui_confirm_bus, reset_ui_confirm_bus
    from mclaw.api.routes import config as config_mod
    from mclaw.core.confirmation_state import get_confirmation_store
...
async def test_riskgate_tool_wait_timeout_marks_store_terminal():
    from mclaw.agent.ui_confirm_bus import reset_ui_confirm_bus
    from mclaw.core.confirmation_state import get_confirmation_store
    from mclaw.core.risk_gate_tools import resolve_riskgate_tool_decision
...
async def test_riskgate_tool_prompt_result_centralizes_confirmed_execution():
    from mclaw.agent.reasoning import (
...
def test_ask_user_reply_request_is_structured():
    from mclaw.api.routes.chat import _ask_user_reply_context
    from mclaw.api.schemas import ChatRequest
