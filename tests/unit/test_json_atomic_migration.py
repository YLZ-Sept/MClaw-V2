...
from mclaw.utils.atomic_io import atomic_json_write, read_json_safe


def _hold_transaction_lock(path: str, entered, release) -> None:
    from mclaw.utils.atomic_io import path_transaction_lock
...
def test_atomic_json_write_can_fail_without_direct_overwrite(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("mclaw.utils.atomic_io.time.sleep", lambda _seconds: None)
...
def test_orchestrator_persist_uses_atomic_json_write(tmp_path, monkeypatch):
    ...
    from mclaw.utils import atomic_io as _io
    ...
    monkeypatch.setattr("mclaw.agents.orchestrator.atomic_json_write", spy, raising=False)
    ...
    from mclaw.agents import orchestrator as _o
...
@pytest.mark.parametrize(
    ...
    [
        ("mclaw.core.proactive", ["_load", "_save"]),
        # ADR-0003: the user-profile manager moved to ``mclaw.agent.user_profile``;
        # ``mclaw.agent.user_profile`` is now a thin re-export shim (no source-level
        # ``def _load_state(``), so point the source-scan at the canonical home.
        ("mclaw.agent.user_profile", ["_load_state", "_save_state"]),
        ("mclaw.sessions.user", ["_load_users", "_save_users"]),
        ("mclaw.channels.adapters.telegram", ["_load_paired_users", "_save_paired_users"]),
        ("mclaw.channels.media.storage", ["_load_index", "_save_index"]),
        ("mclaw.llm.registries", ["load_custom_providers", "save_custom_providers"]),
        ("mclaw.workspace.backup", ["read_backup_settings", "write_backup_settings"]),
        ("mclaw.hub.device", ["get_or_create_device_id"]),
        ("mclaw.agent.identity", ["_load_hashes", "_save_hashes"]),
        ("mclaw.orgs.manager", ["load_state", "save_state"]),
    ],
)
...
def test_backup_settings_roundtrip(tmp_path):
    from mclaw.workspace.backup import read_backup_settings, write_backup_settings
...
def test_identity_hashes_roundtrip(tmp_path):
    from mclaw.agent.identity import _load_hashes, _save_hashes
...
def test_identity_hashes_recover_from_corruption(tmp_path):
    from mclaw.agent.identity import _HASH_FILE, _load_hashes, _save_hashes
