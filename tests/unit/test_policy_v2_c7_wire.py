...
from mclaw.core.policy_v2.context import (
    PolicyContext,
    ToolPolicy,
    TrustedPathOverride,
)
from mclaw.core.policy_v2.global_engine import (
    rebuild_engine_v2,
    reset_engine_v2,
)
...
class TestBuildPolicyContext:
    ...
    def test_explicit_turn_replay_does_not_relax_riskgate_commit_policy(self):
        ...
        from mclaw.core.policy_v2 import evaluate_via_v2

        message = "请删除长期记忆中所有包含 MCLAW_RISKGATE_689_REPRO_TEST 的记忆。"
        ctx = build_policy_context(
            session=None,
            mode="agent",
        ...
        try:
            decision = evaluate_via_v2(
                ...
                {"query": "MCLAW_RISKGATE_689_REPRO_TEST", "dry_run": False},
            )
        finally:
    ...
    def test_memory_delete_dry_run_preview_does_not_need_tool_confirmation(self):
        ...
        ctx = build_policy_context(
            session=None,
            mode="agent",
            user_message="请删除长期记忆中所有包含 MCLAW_RISKGATE_689_REPRO_TEST 的记忆。",
            tool_policies=_MEMORY_DELETE_POLICY,
        )
        token = set_current_context(ctx)
        try:
            decision = evaluate_via_v2(
                "memory_delete_by_query",
                {"query": "MCLAW_RISKGATE_689_REPRO_TEST", "dry_run": True},
            )
        finally:
            reset_current_context(token)
    ...
    def test_memory_delete_real_delete_without_riskgate_requests_tool_riskgate(self):
        ...
        try:
            decision = evaluate_via_v2(
                "memory_delete_by_query",
                {
                    "query": "MCLAW_RISKGATE_689_REPRO_TEST",
                    "dry_run": False,
                    "confirm_token": "token",
                },
            )
        ...
        assert decision.metadata["riskgate_scope"]["query"] == "MCLAW_RISKGATE_689_REPRO_TEST"
...
class TestEvaluateMessageIntentViaV2:
    ...
    def test_engine_failure_returns_confirm(self, monkeypatch):
        ...
        import mclaw.core.policy_v2.adapter as adapter_mod
...
class TestExplicitLookupWiring:
    ...
    def test_filesystem_write_file_explicit(self, monkeypatch):
        ...
        from mclaw.tools.handlers import SystemHandlerRegistry
        from mclaw.tools.handlers.filesystem import FilesystemHandler
        ...
        from mclaw.core.policy_v2 import DecisionSource
...
class TestContextVarLifecycle:
    def test_set_and_reset_context(self):
        """set/reset 配对正确归位。"""
        from mclaw.core.policy_v2 import get_current_context
