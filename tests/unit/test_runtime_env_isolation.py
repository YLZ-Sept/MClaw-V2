...
from mclaw.api.routes.agents import ProfileCreateRequest, create_agent_profile
from mclaw.experience import ToolExperienceTracker
from mclaw.runtime_envs import (
    apply_execution_environment,
    resolve_agent_env,
    resolve_scratch_env,
    resolve_skill_env,
)
from mclaw.skills.parser import parse_skill
from mclaw.skills.runtime_registry import (
    mark_skill_loaded,
...
def test_agent_profile_create_api_persists_runtime_policy(monkeypatch, tmp_path):
    from mclaw.agents import profile as profile_module
...
def test_execution_env_resolvers_are_scoped(monkeypatch, tmp_path):
    monkeypatch.setattr("mclaw.runtime_envs.get_runtime_root", lambda: tmp_path / "runtime")
...
def test_apply_execution_environment_prefers_managed_python(monkeypatch, tmp_path):
    monkeypatch.setattr("mclaw.runtime_envs.get_runtime_root", lambda: tmp_path / "runtime")
    monkeypatch.setattr(
        "mclaw.runtime_envs.resolve_pip_index",
    ...
    assert env["MCLAW_EXECUTION_ENV_SCOPE"] == "agent"
    assert env["MCLAW_AGENT_PYTHON"] == str(spec.python_path)
    assert env["MCLAW_EXECUTION_DEPS_HASH"] == spec.deps_hash
...
def test_skill_runtime_registry_tracks_loaded_and_pending_update(monkeypatch, tmp_path):
    monkeypatch.setattr("mclaw.skills.runtime_registry._get_mclaw_root", lambda: tmp_path)
