...
class TestReadPermissionModeLabel:
    ...
    def test_fallback_when_v2_unavailable(self, monkeypatch) -> None:
        ...
        from mclaw.core.policy_v2 import confirmation_mode as cm

        def _boom():
            raise RuntimeError("v2 not initialized")

        monkeypatch.setattr("mclaw.core.policy_v2.global_engine.get_config_v2", _boom)
...
class TestPolicyEngineFieldsDeleted:
    ...
    def _read_executable_v2_sources(cls) -> str:
        ...
        src_root = Path(__file__).resolve().parents[2] / "src" / "mclaw"
        chunks: list[str] = []
