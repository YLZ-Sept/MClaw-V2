...
Walks ``src/mclaw/tools/definitions/*.py`` plus the desktop handler's
...
def _definitions_dir() -> Path:
    return _project_root() / "src" / "mclaw" / "tools" / "definitions"


def _desktop_handler_path() -> Path:
    return _project_root() / "src" / "mclaw" / "tools" / "handlers" / "desktop.py"
