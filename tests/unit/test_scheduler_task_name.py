...
def test_scheduler_add_task_rejects_invalid_name():
    ...
    from mclaw.scheduler.scheduler import TaskScheduler
    from mclaw.scheduler.task import ScheduledTask
    ...
    bad = ScheduledTask.create(
        name="../escape",
        task_type=__import__("mclaw.scheduler.task", fromlist=["TaskType"]).TaskType.REMINDER,
        trigger_type=__import__(
            "mclaw.scheduler.task", fromlist=["TriggerType"]
        ).TriggerType.ONCE,
