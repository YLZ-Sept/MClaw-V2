...
from mclaw.api.routes import health, memory_repair
from mclaw.api.routes.memory_repair import _file_repair_lock, _memory_dir, _move_triplet
from mclaw.memory.exceptions import MemoryStorageUnavailable
from mclaw.memory.manager import MemoryManager
from mclaw.memory.noop_store import NoopUnifiedStore
...
def test_memory_manager_enters_degraded_mode_for_corrupt_db(tmp_path: Path):
    ...
    _write_corrupt_db(data_dir / "mclaw.db")
...
def test_last_shutdown_marker_from_previous_clean_stop_is_clean(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("mclaw.config.settings", Settings())
    ...
    monkeypatch.setenv("MCLAW_SPAWN_STARTED_AT_MS", "2000")
