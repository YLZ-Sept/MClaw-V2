...
Mclaw 的内置工具持续增加 (当前 38 handler / ~125+ tool). 每个新工具
...
1. **AST 层**: 静态扫所有 ``src/mclaw/tools/handlers/*.py``, 找
...
HANDLERS_DIR = REPO_ROOT / "src" / "mclaw" / "tools" / "handlers"
...
def test_registry_get_tool_class_returns_nonnull_for_every_registered_tool():
    ...
    for handler_name, factory_name in handler_specs:
        try:
            module = __import__(
                f"mclaw.tools.handlers.{handler_name}",
...
def test_register_logs_when_tool_lacks_explicit_approval_class(caplog):
    ...
    with caplog.at_level(
        logging.DEBUG, logger="mclaw.tools.handlers"
    ):
...
def test_register_does_not_warn_when_all_tools_have_classes():
    ...
    from mclaw.core.policy_v2 import ApprovalClass
    from mclaw.tools.handlers import SystemHandlerRegistry
    ...
    logger_obj = logging.getLogger("mclaw.tools.handlers")
