...
def test_prompt_compiler_import_surface() -> None:
    """``mclaw.prompt.compiler.check_compiled_outdated`` must import.
...
def test_agent_reasoning_engine_export() -> None:
    """``from mclaw.agent import ReasoningEngine`` must succeed.

    The reasoning engine is exported only from the canonical agent package.
    """
    from mclaw.agent import ReasoningEngine
...
def test_removed_core_agent_modules_do_not_resolve() -> None:
    removed = (
        "mclaw.core.agent",
        "mclaw.core.reasoning_engine",
        "mclaw.core.tool_executor",
        "mclaw.core.identity",
        "mclaw.core.permission",
    )
...
def test_removed_llm_adapter_does_not_resolve() -> None:
    assert importlib.util.find_spec("mclaw.llm.adapter") is None


def test_core_package_does_not_export_agent_types() -> None:
    import mclaw.core as core
