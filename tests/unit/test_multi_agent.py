...
from mclaw.agents.fallback import _AUTO_DEGRADE_THRESHOLD, FallbackResolver
from mclaw.agents.orchestrator import (
...
from mclaw.agents.profile import (
    AgentProfile,
    AgentType,
    ProfileStore,
    SkillsMode,
)
from mclaw.sessions.session import Session, SessionConfig, SessionContext
...
class TestProfileStore:
    ...
    def test_load_does_not_heal_custom_profile_with_diverged_name_i18n_zh(
        ...
        with caplog.at_level(logging.WARNING, logger="mclaw.agents.profile"):
            store = ProfileStore(tmp_path / "agents")
...
class TestAgentFactory:
    ...
    async def test_create_sets_preferred_endpoint(self):
        ...
        with patch("mclaw.core._agent_runtime.Agent") as MockAgent:
...
class TestAgentInstancePool:
    ...
    def pool(self, mock_factory):
        from mclaw.agents.factory import AgentInstancePool
    ...
    def test_get_existing(self, pool):
        from mclaw.agents.factory import _PoolEntry
...
class TestAgentOrchestrator:
    ...
    async def test_sub_agent_stream_events_are_broadcast_to_websocket(
        ...
        monkeypatch.setattr("mclaw.api.routes.websocket.broadcast_event", fake_broadcast)
        mock_agent = mock_pool.get_or_create.return_value
        mock_agent.chat_with_session_stream = fake_stream
...
class TestAgentToolHandler:
    ...
    def handler(self, mock_agent):
        from mclaw.tools.handlers.agent import AgentToolHandler
...
class TestEdgeCasesAndBugs:
    ...
    def test_health_entry_first_failure_is_counted(self):
        ...
        """
        from mclaw.agents.fallback import _HealthEntry
