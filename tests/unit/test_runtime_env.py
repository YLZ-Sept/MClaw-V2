...
Tests the helper functions in mclaw.runtime_env that locate Python executables
...
from mclaw.runtime_manager import get_runtime_environment_report as manager_runtime_report
...
def test_runtime_pip_environment_scrubs_python_conda_and_secret_env(monkeypatch):
    ...
    assert env["MCLAW_SUBPROCESS_SECRET_SCRUB"] == "1"
    ...
    env = apply_subprocess_secret_scrub(
        {
            ...
            "MCLAW_FORCE_ANTHROPIC_API_KEY": "allowed",
            "PATH": "base",
        }
    )
...
def test_user_subprocess_environment_empty_overrides_inherit_host(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ANTHROPIC_API_KEY", "secret")
    monkeypatch.setattr("mclaw.runtime_env.get_agent_python_executable", lambda: None)
    monkeypatch.setattr("mclaw.runtime_env.get_app_python_executable", lambda: None)
    monkeypatch.setattr("mclaw.runtime_env.get_managed_node_seed", lambda: None)
    monkeypatch.setattr(
        "mclaw.runtime_env.get_workspace_dependency_cache_root", lambda: tmp_path / "cache"
    )
    ...
    assert "ANTHROPIC_API_KEY" not in env
    assert env["MCLAW_ENV_TRUST_SOURCE"] == "user-subprocess"
...
def test_managed_toolchain_seed_resolvers(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("mclaw.runtime_env.get_bootstrap_manifest_path", lambda: manifest)
    monkeypatch.setattr(
        "mclaw.runtime_env.verify_python_executable", lambda path: path == str(py)
    )
...
def test_bootstrap_manifest_path_resolves_dual_venv_home(monkeypatch, tmp_path):
    ...
    monkeypatch.delenv("MCLAW_BOOTSTRAP_DIR", raising=False)
...
def test_managed_node_environment_uses_runtime_cache(monkeypatch, tmp_path):
    ...
    node.parent.mkdir()
    node.write_text("", encoding="utf-8")
    monkeypatch.setattr("mclaw.runtime_env.get_managed_node_seed", lambda: str(node))
    monkeypatch.setattr(
        "mclaw.runtime_env.get_workspace_dependency_cache_root", lambda: tmp_path / "cache"
    )
    ...
    assert env["MCLAW_NODE"] == str(node)
    assert env["MCLAW_NODE_BIN"] == str(node.parent)
...
def test_readonly_seed_roots_and_node_command(monkeypatch, tmp_path):
    ...
    node.write_text("", encoding="utf-8")
    monkeypatch.setenv("MCLAW_READONLY_SEED_DIRS", str(seed))
    monkeypatch.setattr("mclaw.runtime_env.get_bootstrap_manifest_path", lambda: None)
    monkeypatch.setattr("mclaw.runtime_env.get_managed_node_seed", lambda: "")
...
def test_mcp_command_resolution_uses_managed_npx(monkeypatch, tmp_path):
    from mclaw.tools.mcp import MCPClient, MCPServerConfig
...
def test_mcp_command_resolution_falls_back_to_host_path(monkeypatch, tmp_path):
    ...
    package manager (apt, brew, nvm, scoop, etc.) and do not have Mclaw-managed Node.
    """
    from mclaw.tools.mcp import MCPClient, MCPServerConfig
    ...
    monkeypatch.setattr("mclaw.runtime_env.get_managed_node_seed", lambda: None)
    monkeypatch.setattr("mclaw.runtime_env.get_bootstrap_manifest_path", lambda: None)
    monkeypatch.setattr("mclaw.runtime_env.get_readonly_seed_roots", list)
...
class TestInjectModulePaths:
    def test_inject_adds_existing_site_packages(self, tmp_path):
        ...
        try:
            with patch("mclaw.runtime_env._get_mclaw_root", return_value=tmp_path):
...
class TestRegisterDllDirectories:
    ...
    def test_registers_torch_lib(self, tmp_path):
        ...
        try:
            ...
            from mclaw.runtime_env import _register_dll_directories
