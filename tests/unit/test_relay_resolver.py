"""mclaw.relay.resolver — plugin-facing relay endpoint lookup.
...
from mclaw.relay import RelayNotFound, list_relay_endpoints, resolve_relay_endpoint
...
def test_workspace_resolution_falls_back_to_env(workspace_with_relays, monkeypatch):
    """MCLAW_WORKSPACE is set by mclaw serve; honour it when
    the caller doesn't pass an explicit workspace_dir."""
    monkeypatch.setenv("MCLAW_WORKSPACE", str(workspace_with_relays))
