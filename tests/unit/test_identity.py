...
# ``Identity`` is canonically defined in ``mclaw.agent.identity`` (ADR-0003
...
import mclaw.agent.identity as identity_mod
from mclaw.agent.identity import (
    Identity,
    _file_hash,
    _save_hashes,
)
...
def identity_dir(tmp_path):
    ...
    (d / "SOUL.md").write_text(
        "# Soul\n\n你是 Mclaw，一只忠诚的秋田犬AI助手。", encoding="utf-8"
    )
...
class TestIdentityLoading:
    def test_load_all_documents(self, identity_dir):
        ...
        identity.load()
        assert "Mclaw" in identity.soul or "秋田犬" in identity.soul
    ...
    def test_get_system_prompt_does_not_inject_mclaw_self_identity(self, tmp_path):
        ...
        assert "你是 Mclaw，一个全能自进化AI助手。" not in prompt
