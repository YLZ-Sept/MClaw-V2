...
from mclaw.memory.lifecycle import LifecycleManager
from mclaw.memory.storage import _SCHEMA_VERSION, MemoryStorage
from mclaw.memory.types import MemoryPriority, MemoryType, SemanticMemory
from mclaw.memory.unified_store import UnifiedStore
...
def test_workspace_resolver_default_behavior(monkeypatch):
    ...
    from mclaw.memory.workspace_resolver import (
        LEGACY_DEFAULT_WORKSPACE_ID,
        resolve_memory_workspace_id,
    )

    monkeypatch.delenv("MCLAW_DESKTOP_PROJECT_WORKSPACE", raising=False)
...
def test_workspace_resolver_opt_in_project_mode(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("MCLAW_DESKTOP_PROJECT_WORKSPACE", "1")
...
def test_search_memories_workspace_fallback(tmp_path):
    ...
    from mclaw.memory.manager import MemoryManager
...
def test_backfill_sentinel_archives_legacy_json_once(tmp_path: Path):
    ...
    from mclaw.memory.storage import _instance_registry
...
def test_isolated_memory_md_seed_phase_2b3():
    ...
    from mclaw.agents.factory import AgentFactory
    from mclaw.agents.profile import AgentProfile, AgentType
    ...
    # 不应该硬编码全局 Mclaw 身份标记
    assert "Mclaw 的全局记忆" not in seed
...
def test_apply_memory_isolation_seeds_md_when_missing(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("mclaw.agents.profile.get_profile_store", lambda: fake_store)
    ...
    monkeypatch.setattr(
        "mclaw.memory.manager.MemoryManager",
...
def test_iter_owned_session_ids_phase_2b5_audit(tmp_path: Path):
    ...
    storage = MemoryStorage(tmp_path / "mclaw.db")
...
def test_trace_memory_blocks_cross_owner_access(tmp_path: Path):
    ...
    from mclaw.memory.manager import MemoryManager
    from mclaw.tools.handlers.memory import MemoryHandler
...
async def test_daily_consolidator_dedup_does_not_cross_tenants(tmp_path: Path):
    ...
    """
    from mclaw.memory.daily_consolidator import DailyConsolidator
    from mclaw.memory.manager import MemoryManager
...
async def test_global_store_source_blocks_cross_user():
    from mclaw.agents.factory import _GlobalStoreSource
