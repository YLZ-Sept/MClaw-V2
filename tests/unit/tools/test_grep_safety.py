...
@pytest.mark.parametrize(
    "dangerous_path",
    [
        Path.home() / ".mclaw" / "runtime" / "sessions",
        Path.home() / ".mclaw" / "workspaces" / "proj",
        Path("/tmp/.mclaw/runtime_stub") if Path("/tmp").exists() else None,
    ],
)
def test_grep_path_forbidden_blocks_mclaw_data_plane(dangerous_path):
    """含 ``.mclaw/runtime`` / ``workspaces`` 的路径必须被拒。"""
