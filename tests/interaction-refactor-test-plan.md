...
- **形态 A（CLI 安装态）**: 通过 `pipx/pip` 安装并运行 `mclaw`，重点验证核心后端能力、CLI 交互、SSE 对话质量。
...
**代码审查验证**:
- `src/mclaw/events.py` 的 `StreamEventType` 枚举包含所有 22 种事件
...
**代码审查验证**:
- `src/mclaw/utils/errors.py` 存在且导出 `classify_error()` 和 `format_user_friendly_error()`
...
**代码审查验证**:
- `src/mclaw/core/policy.py` 中 `_pending_ui_confirms` 有 TTL 清理机制
