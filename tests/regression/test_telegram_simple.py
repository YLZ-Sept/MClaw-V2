...
# MCLAW_TEST_TELEGRAM_TOKEN opt-in env var so CI / dev
# environments with a stale or placeholder TELEGRAM_BOT_TOKEN
# do not trip telegram.InvalidToken at fixture time.
if not os.environ.get("MCLAW_TEST_TELEGRAM_TOKEN"):
    pytest.skip(
        "requires MCLAW_TEST_TELEGRAM_TOKEN env var (real Telegram bot token)",
        allow_module_level=True,
    )

BOT_TOKEN = os.environ["MCLAW_TEST_TELEGRAM_TOKEN"]
...
async def test_adapter_message_conversion():
    ...
    try:
        from mclaw.channels import MessageContent, UnifiedMessage, MessageType, MediaFile
