...
after the cutoff, but the Mclaw logger uses a daily-rotated
...
This module parses each line's leading timestamp (the Mclaw
...
without pulling the mclaw package.
"""
...
# Mclaw logger format (see ``logging.Formatter('%(asctime)s ...')``):
...
DEFAULT_LOG_DIRS: tuple[Path, ...] = (
    Path("D:/Mclaw/logs"),
    Path("D:/Mclaw/data/logs"),
)
DEFAULT_LOG_GLOB = "*.log*"


def parse_line_timestamp(line: str) -> float | None:
    """Best-effort parse of the Mclaw logger timestamp prefix.
